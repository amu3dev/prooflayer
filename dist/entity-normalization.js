...
export function preferredProjectName(identity, candidates) {
    if (identity === "sb (signalboard)")
        return "SB (SignalBoard)";
    return [...candidates]
        .sort((a, b) => projectNameScore(b) - projectNameScore(a) || a.localeCompare(b))[0] ?? identity;
}
function projectNameScore(value) {
    let score = 0;
    if (/^[A-Z][A-Za-z0-9]*(?:\s+[A-Z][A-Za-z0-9]*)*$/.test(value))
        score += 2;
    if (/\(.+\)/.test(value))
        score += 2;
    if (!/[.!?]$/.test(value))
