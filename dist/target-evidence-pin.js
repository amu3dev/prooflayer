...
import { TargetEvidencePinManifestSchema, TargetEvidencePinSchema, } from "./evidence-pin-schemas.js";
import { loadEvidenceSnapshot, } from "./evidence-snapshots.js";
...
export async function pinTargetEvidenceSnapshot(workspace, targetId, snapshotId, options = {}) {
    ...
    if (await pathExists(paths.pinPath) || await pathExists(paths.manifestPath)) {
        ...
        if (status.status === "current" &&
            ...
            return resultFromLoaded(await loadTargetEvidencePin(workspace, targetId), "already-current");
        }
        if (status.status === "current") {
            throw new Error(`Target ${targetId} is already pinned to ${status.snapshotId}. Use evidence-upgrade to change the pin explicitly.`);
        }
        throw new Error(`Stored evidence pin for ${targetId} is ${status.status} and was not overwritten. ${status.reasons.join(" ")}`);
    }
    return writePin(workspace, target, snapshotId, "pin", options.now);
}
export async function upgradeTargetEvidenceSnapshot(workspace, targetId, snapshotId, options = {}) {
    const target = await showTarget(workspace, targetId);
    const paths = targetEvidencePinPaths(workspace, target);
    if (!(await pathExists(paths.pinPath)) && !(await pathExists(paths.manifestPath))) {
        throw new Error(`Target ${targetId} has no evidence pin to upgrade. Use evidence-pin first.`);
    }
    const status = await getTargetEvidencePinStatus(workspace, targetId);
    if (status.status !== "current") {
        throw new Error(`Stored evidence pin for ${targetId} is ${status.status}; invalid or stale pins are never replaced silently. ${status.reasons.join(" ")}`);
    }
...
export async function getTargetEvidencePinStatus(workspace, targetId) {
    let target;
    try {
    ...
    if (!pinExists && !manifestExists) {
        return {
            ...emptyStatus(targetId, target.type, "missing", ["Target has no explicitly pinned Evidence Snapshot."]),
            ...base,
        };
    }
    if (!pinExists || !manifestExists) {
        return {
            ...emptyStatus(targetId, target.type, "invalid", ["Evidence pin artifact set is incomplete."]),
            ...base,
        };
    }
    let pin;
    let manifest;
    try {
        pin = TargetEvidencePinSchema.parse(await readJson(paths.pinPath, null));
        manifest = TargetEvidencePinManifestSchema.parse(await readJson(paths.manifestPath, null));
    }
    catch (error) {
        return {
            ...emptyStatus(targetId, target.type, "invalid", [`Evidence pin contract is invalid: ${errorMessage(error)}`]),
            ...base,
        };
    }
    const targetSha256 = await hashFile(paths.targetPath);
    const pinSha256 = await hashFile(paths.pinPath);
    const targetHashMatches = pin.target.sha256 === targetSha256 &&
        manifest.targetSha256 === targetSha256;
    const pinHashMatches = manifest.pinSha256 === pinSha256;
    const identityMatches = pin.id === pinIdFor(targetId, target.type, targetSha256, pin.snapshot) &&
    ...
    let snapshotContentHashMatches = null;
    let snapshotManifestHashMatches = null;
...
export async function loadTargetEvidencePin(workspace, targetId) {
    const status = await getTargetEvidencePinStatus(workspace, targetId);
    if (status.status !== "current") {
        throw new Error(`Target Evidence Snapshot pin for ${targetId} is ${status.status}: ${status.reasons.join(" ")}`);
    }
    const target = await showTarget(workspace, targetId);
    const paths = targetEvidencePinPaths(workspace, target);
    const pin = TargetEvidencePinSchema.parse(await readJson(paths.pinPath, null));
    const manifest = TargetEvidencePinManifestSchema.parse(await readJson(paths.manifestPath, null));
    return {
...
export function targetEvidencePinPaths(workspace, target) {
    ...
    const targetRelativePath = `targets/${targetDirectory}/${target.id}/target.json`;
    return {
        rootRelativePath,
...
export function formatTargetEvidencePinResult(result) {
    return [
...
export function formatTargetEvidencePinStatus(status) {
...
async function writePin(workspace, target, snapshotId, operation, nowProvider) {
    ...
    if (status.status !== "current") {
        throw new Error(`Evidence pin failed validation: ${status.status}. ${status.reasons.join(" ")}`);
    }
    return resultFromLoaded(await loadTargetEvidencePin(workspace, target.id), operation === "pin" ? "pinned" : "upgraded");
}
function resultFromLoaded(loaded, result) {
    return {
        ...
        eligibleRoleEvidenceCount: loaded.snapshot.snapshot.completeness.eligibleRoleEvidenceCount,
        eligibleJobEvidenceCount: loaded.snapshot.snapshot.completeness.eligibleJobEvidenceCount,
    };
}
function pinIdFor(targetId, targetType, targetSha256, snapshot) {
...
function emptyStatus(targetId, targetType, status, reasons) {
    return {
        targetId,
        targetType,
        pinExists: false,
