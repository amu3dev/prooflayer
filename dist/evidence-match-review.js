...
import { EditedEvidenceMatchSchema, EvidenceMatchProposalManifestSchema, EvidenceMatchProposalReviewSchema, EvidenceMatchReviewManifestSchema, } from "./schemas.js";
import { getEvidenceMatchProposalStatus, showEvidenceMatchProposal, } from "./evidence-match-proposal.js";
...
export async function initializeEvidenceMatchReview(workspace, proposalId, options = {}) {
...
export async function setEvidenceMatchReviewDecision(workspace, proposalId, proposedMatchId, input, options = {}) {
    ...
    if (!existing)
        throw new Error(`Review is missing proposed match ID: ${proposedMatchId}`);
    if (existing.decision !== "pending")
        throw new Error(`A decision already exists for proposed match: ${proposedMatchId}`);
    if (input.decision === "edit") {
        if (!input.editedMatch)
            throw new Error("Edit requires an edited match.");
...
export async function setEvidenceCoverageReviewDecision(workspace, proposalId, proposedCoverageId, input, options = {}) {
    await assertProposalReviewable(workspace, proposalId);
    const proposal = await showEvidenceMatchProposal(workspace, proposalId);
    ...
    if (!existing)
        throw new Error(`Review is missing proposed coverage ID: ${proposedCoverageId}`);
    if (existing.decision !== "pending")
        throw new Error(`A decision already exists for proposed coverage: ${proposedCoverageId}`);
    if (input.decision === "edit" && !input.editedStatus)
        throw new Error("Coverage edit requires an edited status.");
    if (input.decision !== "edit" && input.editedStatus)
        throw new Error("Only a coverage edit may include edited status.");
    const now = (options.now ?? (() => new Date()))().toISOString();
    const updated = EvidenceMatchProposalReviewSchema.parse({
        ...review,
...
export async function completeEvidenceMatchReview(workspace, proposalId, options = {}) {
    await assertProposalReviewable(workspace, proposalId);
    const proposal = await showEvidenceMatchProposal(workspace, proposalId);
    const review = await showEvidenceMatchReview(workspace, proposalId);
...
export async function showEvidenceMatchReview(workspace, proposalId) {
    const proposal = await showEvidenceMatchProposal(workspace, proposalId);
    const paths = pathsForProposal(workspace, proposal);
    if (!(await pathExists(paths.reviewPath)))
        throw new Error(`Match review not found for proposal: ${proposalId}`);
    return EvidenceMatchProposalReviewSchema.parse(await readJson(paths.reviewPath, null));
}
export async function getEvidenceMatchReviewStatus(workspace, proposalId) {
    const proposal = await showEvidenceMatchProposal(workspace, proposalId);
    const paths = pathsForProposal(workspace, proposal);
    ...
    try {
        const review = EvidenceMatchProposalReviewSchema.parse(await readJson(paths.reviewPath, null));
        const manifest = EvidenceMatchReviewManifestSchema.parse(await readJson(paths.manifestPath, null));
        const proposalManifest = EvidenceMatchProposalManifestSchema.parse(await readJson(proposalManifestPath(workspace, proposal.targetType, proposal.targetId, proposalId), null));
...
export async function readEditedEvidenceMatchFile(filePath) {
    try {
...
export function formatEvidenceMatchReviewStatus(status) {
    return [
...
async function assertProposalReviewable(workspace, proposalId) {
    ...
    if (!status.readyForReview)
        throw new Error(`Only a current ready-for-review match proposal may be reviewed. Current status: ${status.status}`);
}
async function validateEditedMatch(workspace, targetId, edited) {
    ...
    if (new Set(edited.evidenceIds).size !== edited.evidenceIds.length)
        throw new Error("Edited match contains duplicate evidence IDs.");
    const invalid = edited.evidenceIds.filter((id) => !eligible.has(id));
    if (invalid.length)
        throw new Error(`Edited match references ineligible evidence: ${invalid.join(", ")}`);
    if (edited.matchType === "contradictory" && edited.coverage !== "conflicting")
        throw new Error("Contradictory edited matches require conflicting coverage.");
    if (edited.matchType !== "contradictory" && edited.coverage === "conflicting")
        throw new Error("Conflicting edited coverage requires a contradictory match.");
    if (["supporting", "partial", "contradictory"].includes(edited.matchType) && edited.limitations.length === 0)
        throw new Error(`${edited.matchType} edited matches require limitations.`);
}
function assertMutableReview(review) {
    if (review.status === "completed")
        throw new Error("Completed match reviews cannot be modified.");
}
function assertExactReviewCoverage(review, matchIds, coverageIds) {
    ...
    if (!sameUniqueSet(reviewedMatches, matchIds))
        throw new Error("Review match decisions do not exactly cover proposed matches.");
    if (!sameUniqueSet(reviewedCoverage, coverageIds))
        throw new Error("Review coverage decisions do not exactly cover proposed coverage records.");
}
function validateCompletedDecisionSemantics(review, proposal) {
    ...
    const acceptedByExpectation = new Map();
    for (const decision of review.matchDecisions) {
        ...
        if (!proposed)
            throw new Error(`Unknown reviewed proposed match: ${decision.proposedMatchId}`);
        const item = decision.decision === "edit" ? decision.editedMatch : proposed;
        if (!item)
            throw new Error(`Edited match content is missing: ${decision.proposedMatchId}`);
    ...
    for (const decision of review.coverageDecisions) {
        ...
        if (!proposed)
            throw new Error(`Unknown reviewed proposed coverage: ${decision.proposedCoverageId}`);
        const status = decision.decision === "edit" ? decision.editedStatus : proposed.status;
        if (!status)
            throw new Error(`Edited coverage status is missing: ${decision.proposedCoverageId}`);
        ...
        if (status === "matched" && !hasDirectFull)
            throw new Error("Matched coverage requires an accepted direct full-coverage match.");
        if (status === "partially-matched" && (accepted.length === 0 || hasDirectFull))
            throw new Error("Partially matched coverage requires accepted partial/supporting evidence and no direct full match.");
        if (status === "conflicting" && !hasContradiction)
            throw new Error("Conflicting coverage requires an accepted contradictory evidence match.");
    }
}
...
async function persistReview(workspace, review, paths) {
    ...
    const manifest = EvidenceMatchProposalManifestSchema.parse(await readJson(proposalManifestPath(workspace, proposal.targetType, proposal.targetId, proposal.id), null));
...
function resolveWithin(workspace, relativePath) {
    ...
    if (relation === ".." || relation.startsWith(`..${path.sep}`) || path.isAbsolute(relation))
        throw new Error(`Match review path escapes workspace: ${relativePath}`);
    return resolved;
}
