import { z } from "zod";
import { ModelGenerationSettingsSchema, TargetExpectationImportanceSchema, TargetExpectationKindSchema, TargetExpectationNecessitySchema, TargetInterpretationSourceReferenceSchema, } from "./schemas.js";
...
export const TargetFitAssessmentSchema = z.discriminatedUnion("targetType", [
    ...
    if (new Set(ids).size !== ids.length)
        context.addIssue({ code: z.ZodIssueCode.custom, message: "Assessment IDs must be unique" });
    if (new Set(expectationIds).size !== expectationIds.length)
        context.addIssue({ code: z.ZodIssueCode.custom, message: "Expectation assessments must be unique" });
});
...
export const FitAssessmentReviewDecisionSchema = z.object({
    ...
    if (value.decision === "edit" && !value.editedAssessment)
        context.addIssue({ code: z.ZodIssueCode.custom, message: "Edit requires editedAssessment" });
    if (value.decision !== "edit" && value.editedAssessment)
        context.addIssue({ code: z.ZodIssueCode.custom, message: "Only edit may contain editedAssessment" });
});
export const FitAssessmentSummaryReviewDecisionSchema = z.object({
    ...
    if (value.decision === "edit" && !value.editedSummary)
        context.addIssue({ code: z.ZodIssueCode.custom, message: "Summary edit requires editedSummary" });
    if (value.decision !== "edit" && value.editedSummary)
        context.addIssue({ code: z.ZodIssueCode.custom, message: "Only summary edit may contain editedSummary" });
});
