...
import { PublicProfileSchema } from "./schemas.js";
export const PUBLIC_PROFILE_FILE = "config/public-profile.json";
export async function initializePublicProfile(workspace, initial = {}) {
    const profilePath = path.join(workspace, PUBLIC_PROFILE_FILE);
    if (await pathExists(profilePath)) {
        return {
            created: false,
            path: PUBLIC_PROFILE_FILE,
            profile: PublicProfileSchema.parse(await readJson(profilePath, {}))
        };
    }
...
export async function loadPublicProfile(workspace) {
    const profilePath = path.join(workspace, PUBLIC_PROFILE_FILE);
    if (!(await pathExists(profilePath)))
        return null;
    return PublicProfileSchema.parse(await readJson(profilePath, {}));
}
export function formatPublicProfileSummary(profile) {
    if (!profile) {
        return [
