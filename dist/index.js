...
import { buildJobResumeDraftScaffold, formatBuildJobResumeDraftScaffoldResult, formatJobResumeDraftScaffoldStatus, getJobResumeDraftScaffoldStatus, showJobResumeDraftScaffold, } from "./job-resume-drafting.js";
...
job
    ...
    .action(async (options) => {
    console.log(formatGuidedJobCreation(await createGuidedJob(getWorkspace(), {
        file: options.file,
        title: options.title,
...
job
    ...
    console.log(formatJobWorkflowRunResult(await runJobWorkflow(getWorkspace(), targetId, parseGuidedJobRunOptions(options))));
});
job
    ...
    .action(async (targetId, options) => {
    console.log(formatJobWorkflowRunResult(await continueJobWorkflow(getWorkspace(), targetId, parseGuidedJobRunOptions(options))));
});
job
...
job
    ...
    .action(async (targetId, options) => {
    console.log(formatFinalizeJobWorkflowResult(await finalizeJobWorkflow(getWorkspace(), targetId, {
...
jobRequirements
    ...
    .action(async (targetId, options) => {
    console.log(formatBuildJobRequirementsResult(await buildJobRequirements(getWorkspace(), targetId, options)));
});
jobRequirements
    .command("show <target-id>")
    .description("Print the deterministic Job Requirement Model as stable JSON.")
    .action(async (targetId) => {
    process.stdout.write(`${JSON.stringify(await showJobRequirementModel(getWorkspace(), targetId), null, 2)}\n`);
});
jobRequirements
    .command("status <target-id>")
    .description("Inspect deterministic Job Requirement Model lifecycle and dependencies.")
    .action(async (targetId) => {
    console.log(formatJobRequirementModelStatus(await getJobRequirementModelStatus(getWorkspace(), targetId)));
});
jobRequirements
    ...
    .action(async (targetId, options) => {
    console.log(formatApproveJobRequirementsResult(await approveJobRequirements(getWorkspace(), targetId, {
...
jobRequirements
    ...
    .action(async (targetId) => {
    process.stdout.write(`${JSON.stringify(await showApprovedJobRequirements(getWorkspace(), targetId), null, 2)}\n`);
});
jobRequirements
...
jobAssessment
    ...
    .action(async (targetId, options) => {
    console.log(formatBuildJobFitProofAssessmentResult(await buildJobFitProofAssessment(getWorkspace(), targetId, options)));
});
jobAssessment
    .command("show <target-id>")
    .description("Print deterministic Job fit and proof assessment as stable JSON.")
    .action(async (targetId) => {
    process.stdout.write(`${JSON.stringify(await showJobFitProofAssessment(getWorkspace(), targetId), null, 2)}\n`);
});
jobAssessment
    .command("status <target-id>")
...
jobResumeDraft
    ...
    .action(async (targetId, options) => {
    console.log(formatBuildJobResumeDraftScaffoldResult(await buildJobResumeDraftScaffold(getWorkspace(), targetId, options)));
});
jobResumeDraft
    .command("scaffold-show <target-id>")
    .description("Print the deterministic Job resume draft scaffold as stable JSON.")
    .action(async (targetId) => {
    process.stdout.write(`${JSON.stringify(await showJobResumeDraftScaffold(getWorkspace(), targetId), null, 2)}\n`);
});
jobResumeDraft
    .command("scaffold-status <target-id>")
    .description("Inspect Job scaffold integrity and dependency freshness.")
    .action(async (targetId) => {
    console.log(formatJobResumeDraftScaffoldStatus(await getJobResumeDraftScaffoldStatus(getWorkspace(), targetId)));
});
jobResumeDraft
    .command("approve <target-id>")
    ...
    .action(async (targetId, options) => {
    console.log(formatApproveJobResumeDraftResult(await approveJobResumeDraft(getWorkspace(), targetId, options)));
});
jobResumeDraft
    .command("approved-show <target-id>")
    .description("Print the current approved structured Job Resume Draft as stable JSON.")
    .action(async (targetId) => {
    process.stdout.write(`${JSON.stringify(await showApprovedJobResumeDraft(getWorkspace(), targetId), null, 2)}\n`);
});
jobResumeDraft
    .command("approved-status <target-id>")
    .description("Inspect approved Job draft integrity, provenance, and lifecycle.")
    .action(async (targetId) => {
    console.log(formatApprovedJobResumeDraftStatus(await getApprovedJobResumeDraftStatus(getWorkspace(), targetId)));
});
...
jobResumeDraftProposal
    ...
    .action(async (targetId, options) => {
    console.log(formatJobResumeDraftProposalResult(await generateJobResumeDraftProposal(getWorkspace(), targetId, options)));
});
jobResumeDraftProposal
    .command("list <target-id>")
    .description("List stored structured Job Resume Draft proposals.")
    .action(async (targetId) => {
    console.log(formatJobResumeDraftProposalList(await listJobResumeDraftProposals(getWorkspace(), targetId)));
});
jobResumeDraftProposal
    ...
    process.stdout.write(`${JSON.stringify(await showJobResumeDraftProposal(getWorkspace(), proposalId), null, 2)}\n`);
});
jobResumeDraftProposal
    .command("status <proposal-id>")
    .description("Inspect Job draft proposal integrity, dependencies, and review eligibility.")
    .action(async (proposalId) => {
    console.log(formatJobResumeDraftProposalStatus(await getJobResumeDraftProposalStatus(getWorkspace(), proposalId)));
});
jobResumeDraftProposal
...
jobResumeDraftProposal
    ...
    console.log(formatJobResumeDraftReviewStatus(await getJobResumeDraftReviewStatus(getWorkspace(), review.id)));
});
jobResumeDraftProposal
    .command("review-show <review-id>")
    .description("Print one complete Job Resume Draft review as stable JSON.")
    .action(async (reviewId) => {
    process.stdout.write(`${JSON.stringify(await showJobResumeDraftReview(getWorkspace(), reviewId), null, 2)}\n`);
});
jobResumeDraftProposal
    .command("review-status <review-id>")
    .description("Inspect Job draft decision counts and manifest integrity.")
    .action(async (reviewId) => {
    console.log(formatJobResumeDraftReviewStatus(await getJobResumeDraftReviewStatus(getWorkspace(), reviewId)));
});
for (const itemType of ["section", "draft-item", "claim-ledger", "section-order", "ambiguity"]) {
    jobResumeDraftProposal
...
jobRequirementProposal
    ...
    .action(async (targetId, options) => {
    console.log(formatJobRequirementProposalResult(await generateJobRequirementProposal(getWorkspace(), targetId, options)));
});
jobRequirementProposal
    .command("list <target-id>")
    .description("List stored Job Requirement Model proposals.")
    .action(async (targetId) => {
    console.log(formatJobRequirementProposalList(await listJobRequirementProposals(getWorkspace(), targetId)));
});
jobRequirementProposal
    .command("show <proposal-id>")
    .description("Print one normalized Job Requirement Model proposal as stable JSON.")
    .action(async (proposalId) => {
    process.stdout.write(`${JSON.stringify(await showJobRequirementProposal(getWorkspace(), proposalId), null, 2)}\n`);
});
jobRequirementProposal
    .command("status <proposal-id>")
    .description("Inspect Job Requirement Model proposal lifecycle and review eligibility.")
    .action(async (proposalId) => {
    console.log(formatJobRequirementProposalStatus(await getJobRequirementProposalStatus(getWorkspace(), proposalId)));
});
jobRequirementProposal
    .command("replay <proposal-id>")
...
jobRequirementProposal
    ...
    .action(async (proposalId, options) => {
    console.log(formatJobRequirementReviewStatus(await initializeJobRequirementReview(getWorkspace(), proposalId, {
        reviewerName: options.reviewer,
    })));
});
jobRequirementProposal
    ...
    .action(async (proposalId) => {
    process.stdout.write(`${JSON.stringify(await showJobRequirementReview(getWorkspace(), proposalId), null, 2)}\n`);
});
jobRequirementProposal
    .command("review-status <proposal-id>")
    .description("Inspect Job Requirement Model review decisions and integrity.")
    .action(async (proposalId) => {
    console.log(formatJobRequirementReviewStatus(await getJobRequirementReviewStatus(getWorkspace(), proposalId)));
});
jobRequirementProposal
    ...
    .action(async (proposalId, requirementId, options) => {
    const choices = Number(Boolean(options.accept)) +
        Number(Boolean(options.reject)) +
        Number(Boolean(options.editFile));
    if (choices !== 1) {
    ...
    const editedRequirement = options.editFile
        ...
        : undefined;
    console.log(formatJobRequirementReviewStatus(await setJobRequirementReviewDecision(getWorkspace(), proposalId, requirementId, {
        decision: options.accept ? "accept" : options.reject ? "reject" : "edit",
        editedRequirement,
        reviewNote: options.note,
    })));
...
jobRequirementProposal
    ...
    .action(async (proposalId) => {
    console.log(formatJobRequirementReviewStatus(await completeJobRequirementReview(getWorkspace(), proposalId)));
});
...
function parseGuidedJobRunOptions(options) {
    return {
        requirementSource: JobRequirementInputTypeSchema.parse(options.requirementsSource ?? "deterministic"),
...
function parseGuidedExportFormats(value) {
    const formats = value.split(",").map((entry) => entry.trim()).filter(Boolean);
    if (formats.length === 0)
        throw new Error("--formats must include at least one format.");
