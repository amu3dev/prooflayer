...
import { RoleProfileSchema } from "./schemas.js";
import { approveInterpretationProposal, getApprovedInterpretationStatus, } from "./approved-interpretation.js";
...
import { completeProposalReview, initializeProposalReview, } from "./target-proposal-review.js";
...
import { getRoleResumeDraftReviewStatus, } from "./role-resume-draft-review.js";
...
const REQUIRED_ROLE_RESUME_EXPORTS = ["markdown", "html", "docx"];
...
export async function inspectRoleWorkflow(workspace, targetId) {
    ...
    const exportsReady = REQUIRED_ROLE_RESUME_EXPORTS.every((format) => exports.some((entry) => entry.format === format && entry.status === "current"));
    const stages = [
                ...
                : proposalReady || draftReview?.status === "in-progress" ? "ready-for-review"
                    : understandingLifecycle.status === "invalid" ? "invalid"
                        : understanding ? "paused"
                            : "not-started";
    const currentStage = exportsReady ? "export"
        : rendered ? "rendering"
...
export async function confirmGeneratedRoleDirection(workspace, targetId, options = {}) {
    const target = await requireRoleTarget(workspace, targetId);
    ...
    const profileIsCurrent = await pathExists(profilePath)
        && await readJson(profilePath, null).then((stored) => stableJson(stored) === stableJson(profile)).catch(() => false);
    if (!profileIsCurrent)
        await writeBufferAtomic(profilePath, Buffer.from(serializedProfile, "utf8"));
...
async function latestRoleDraftProposal(workspace, targetId) {
    ...
    for (const proposal of proposals.slice().reverse()) {
        ...
        if (status.status !== "missing")
            return { id: proposal.id, status: status.status, readyForReview: status.readyForReview };
    }
    return undefined;
}
async function currentRoleExports(workspace, targetId) {
...
function canonicalExpectationKind(kind) {
    return {
...
function canonicalExpectationGroup(group) {
    return {
