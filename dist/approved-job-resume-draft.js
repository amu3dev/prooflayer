...
import { ApprovedJobResumeDraftManifestSchema, ApprovedJobResumeDraftSchema, JobResumeDraftProposalManifestSchema, JobResumeDraftReviewManifestSchema, } from "./job-resume-draft-schemas.js";
import { buildJobResumeDraftClaimLedger, buildJobResumeDraftEvidenceUsage, getJobResumeDraftProposalStatus, jobResumeDraftProposalPaths, listJobResumeDraftProposals, validateJobResumeDraftPayload, } from "./job-resume-draft-proposal.js";
import { getJobResumeDraftReviewStatus, jobResumeDraftReviewPaths, mergeReviewedJobResumeDraft, showJobResumeDraftReview, } from "./job-resume-draft-review.js";
...
export async function approveJobResumeDraft(workspace, targetId, options = {}) {
    ...
    const proposalManifest = JobResumeDraftProposalManifestSchema.parse(await readJson(proposalPaths.manifestPath, null));
    const reviewManifest = JobResumeDraftReviewManifestSchema.parse(await readJson(reviewPaths.manifestPath, null));
    ...
    if (existingStatus.status === "current" && await pathExists(paths.manifestPath)) {
        ...
        if (manifest.proposalId === proposal.id &&
            ...
            return approvalResult(await showApprovedJobResumeDraft(workspace, targetId), review, proposal.id, paths, "already-current");
        }
    }
    ...
    }, proposal.id, scaffold, context, scaffoldSha256);
    ...
    const completeness = deriveJobResumeDraftCompleteness(scaffold, sections, claimLedger, evidenceUsage, risks, validated.ambiguities);
    const now = (options.now ?? (() => new Date()))().toISOString();
    let createdAt = now;
    if (await pathExists(paths.draftPath)) {
        try {
            createdAt = ApprovedJobResumeDraftSchema.parse(await readJson(paths.draftPath, null)).createdAt;
        }
        catch {
    ...
    return approvalResult(approved, review, proposal.id, paths, existingStatus.status === "missing" ? "created" : "rebuilt");
}
export async function showApprovedJobResumeDraft(workspace, targetId) {
    const paths = approvedJobResumeDraftPaths(workspace, targetId);
    if (!(await pathExists(paths.draftPath))) {
        throw new Error(`Approved Job resume draft not found: ${targetId}`);
    }
    return ApprovedJobResumeDraftSchema.parse(await readJson(paths.draftPath, null));
}
export async function getApprovedJobResumeDraftStatus(workspace, targetId) {
    const paths = approvedJobResumeDraftPaths(workspace, targetId);
    ...
    let manifest;
    try {
        draft = ApprovedJobResumeDraftSchema.parse(await readJson(paths.draftPath, null));
        manifest = ApprovedJobResumeDraftManifestSchema.parse(await readJson(paths.manifestPath, null));
...
export function deriveJobResumeDraftCompleteness(scaffold, sections, claimLedger, evidenceUsage, risks, ambiguities) {
    ...
    const evidenceUsageComplete = [...selectedEvidence].every((id) => evidenceUsage.some((entry) => entry.evidenceId === id));
    const provenanceComplete = items.every((entry) => entry.requirementIds.length > 0 &&
    ...
    const unresolvedCriticalIssueCount = risks.filter((entry) => entry.severity === "critical" || entry.severity === "high").length;
...
export function assertApprovedJobResumeDraft(draft) {
    ...
    if (draft.draftingPolicy.name !== JOB_RESUME_DRAFTING_POLICY_NAME ||
    ...
    if (items.some((entry) => !entry.requirementIds.length ||
    ...
    if (draft.claimLedger.length !== items.length ||
...
export function approvedJobResumeDraftPaths(workspace, targetId) {
...
export function formatApproveJobResumeDraftResult(result) {
    return [
...
export function formatApprovedJobResumeDraftStatus(status) {
    return [
...
function applyReviewTrust(item, proposal, review) {
    ...
    return {
        ...item,
        trustState: edited.decision === "edit" ? "human-edited" : "human-approved",
        provenance: {
            ...item.provenance,
...
function deriveApprovedRisks(sections, validationIssues, evidenceUsage) {
