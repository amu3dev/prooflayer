...
import { InterpretationProposalManifestSchema, ModelInterpretationPayloadSchema, TargetInterpretationProposalSchema, } from "./schemas.js";
...
import { getTargetInterpretationStatus, showTargetInterpretation, } from "./target-interpretation.js";
...
export async function generateInterpretationProposal(workspace, targetId, options = {}) {
    ...
    return resultFromProposal(normalized, location, normalized.status === "ready-for-review" ? "created" : "validation-failed");
}
export async function createDeterministicRoleConfirmationProposal(workspace, targetId, options = {}) {
    const dependencies = await loadCurrentDependencies(workspace, targetId);
    if (dependencies.target.type !== "role" || !dependencies.roleProfileSha256) {
...
export async function replayInterpretationProposal(workspace, proposalId) {
    ...
    const manifest = InterpretationProposalManifestSchema.parse(await readJson(located.location.manifestPath, null));
...
export async function showInterpretationProposal(workspace, proposalId) {
    const located = await locateProposal(workspace, proposalId);
    return TargetInterpretationProposalSchema.parse(await readJson(located.location.proposalPath, null));
}
export async function listInterpretationProposals(workspace, targetId) {
    ...
    for (const file of files) {
        const proposal = TargetInterpretationProposalSchema.parse(await readJson(file, null));
...
export async function getInterpretationProposalStatus(workspace, proposalId) {
    const located = await locateProposal(workspace, proposalId);
    ...
    let manifest;
    try {
        proposal = TargetInterpretationProposalSchema.parse(await readJson(location.proposalPath, null));
        manifest = InterpretationProposalManifestSchema.parse(await readJson(location.manifestPath, null));
    }
    catch (error) {
    ...
    if (!proposalHashMatches)
        reasons.push("Proposal SHA-256 does not match its manifest.");
    if (!rawResponseHashMatches)
        reasons.push("Raw response SHA-256 does not match its manifest.");
    ...
    if (reasons.length > 0) {
        return invalidStatus(base, proposalExists, manifestExists, rawResponseExists, reasons, proposalHashMatches, rawResponseHashMatches);
    }
    let dependencies;
    try {
    ...
    const deterministicInterpretationHashMatches = dependencies.interpretationSha256 === manifest.deterministicInterpretationSha256;
...
export function formatProposalGenerationResult(result) {
    return [
...
export function formatProposalList(entries) {
    if (entries.length === 0)
...
export function renderProposalPrompt(normalizedInput, templateVersion = PROPOSAL_PROMPT_TEMPLATE_VERSION, policyVersion = PROPOSAL_POLICY_VERSION) {
    return [
...
function normalizedModelInput(dependencies) {
    return {
...
function normalizeRawProposal(input) {
    ...
    if (payload)
        issues.push(...validatePayload(payload, input.dependencies));
    const proposedExpectations = payload && issues.length === 0
        ...
        : [];
    const proposedGroups = payload && issues.length === 0
        ? payload.proposedGroups.map((group, index) => ({
            id: `proposed-group_${hashText(`${input.proposalId}\u0000${index}\u0000${stableJson(group)}`).slice(0, 12)}`,
            kind: group.kind,
            title: group.title,
            expectationIds: group.expectationIndexes.map((entry) => proposedExpectations[entry]?.id),
            sourceReferences: group.sourceReferences,
        }))
        : [];
    const proposedAmbiguities = payload && issues.length === 0
...
function validatePayload(payload, dependencies) {
    ...
    payload.proposedExpectations.forEach((expectation, index) => {
        for (const id of expectation.sourceAnalysisItemIds) {
            if (!analysisItems.has(id))
                issues.push(issue("UNSUPPORTED_SOURCE_ID", `Unknown source analysis item: ${id}`, `proposedExpectations.${index}`));
        }
        for (const id of expectation.sourceExpectationIds) {
            if (!expectations.has(id))
                issues.push(issue("UNSUPPORTED_SOURCE_ID", `Unknown source expectation: ${id}`, `proposedExpectations.${index}`));
        }
        ...
        if (forbidden)
            issues.push(issue("FORBIDDEN_CONTENT", forbidden, `proposedExpectations.${index}`));
    });
    ...
    for (const [collectionName, entries] of [
        ...
        entries.forEach((entry, index) => {
            for (const id of entry.sourceAnalysisItemIds) {
                if (!analysisItems.has(id))
                    issues.push(issue("UNSUPPORTED_SOURCE_ID", `Unknown source analysis item: ${id}`, `${collectionName}.${index}`));
            }
            ...
            if (forbidden)
                issues.push(issue("FORBIDDEN_CONTENT", forbidden, `${collectionName}.${index}`));
        });
    }
...
async function findCachedProposal(workspace, targetId, fingerprint) {
    ...
    for (const entry of entries) {
        if (entry.status !== "current" || entry.proposalStatus !== "ready-for-review")
            continue;
...
async function locateProposal(workspace, proposalId) {
    if (!/^proposal_[a-f0-9]+$/.test(proposalId))
    ...
    if (files.length > 1)
        throw new Error(`Proposal ID is ambiguous: ${proposalId}`);
    const proposal = TargetInterpretationProposalSchema.parse(await readJson(files[0], null));
...
async function countTargetProposals(workspace, target) {
...
function invalidStatus(base, proposalExists, manifestExists, rawResponseExists, reasons, proposalHashMatches = null, rawResponseHashMatches = null) {
    return {
        ...base,
        proposalExists,
        manifestExists,
...
export async function proposalFileTimestamps(workspace, proposalId) {
    const located = await locateProposal(workspace, proposalId);
