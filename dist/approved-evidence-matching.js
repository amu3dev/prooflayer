...
import { hashFile, hashText, pathExists, readJson, } from "./fs-utils.js";
import { EvidenceMatchProposalManifestSchema, EvidenceMatchReviewManifestSchema, ManualEvidenceMatchingSchema, } from "./schemas.js";
import { EVIDENCE_MATCHER_NAME, EVIDENCE_MATCHER_VERSION, EVIDENCE_MATCHING_POLICY_VERSION, expectationProvenance, getApprovedEvidenceMatchingStatus, loadMatchingContext, writeApprovedMatching, } from "./evidence-matching.js";
import { getEvidenceMatchProposalStatus, showEvidenceMatchProposal, } from "./evidence-match-proposal.js";
import { getEvidenceMatchReviewStatus, showEvidenceMatchReview, } from "./evidence-match-review.js";
import { showTarget } from "./targets.js";
export async function approveEvidenceMatchProposal(workspace, proposalId, options = {}) {
    ...
    const proposalManifest = EvidenceMatchProposalManifestSchema.parse(await readJson(proposalManifestPath, null));
    const reviewManifest = EvidenceMatchReviewManifestSchema.parse(await readJson(reviewManifestPath, null));
    if (reviewManifest.proposalSha256 !== proposalManifest.proposalSha256)
        throw new Error("Match review does not reference the current proposal hash.");
    ...
    for (const proposed of proposal.proposedMatches) {
        ...
        if (!expectation)
            throw new Error(`Approved expectation is no longer eligible: ${proposed.expectationId}`);
        const edited = decision.decision === "edit" ? decision.editedMatch : undefined;
        if (decision.decision === "edit" && !edited)
            throw new Error(`Edited match content is missing: ${proposed.id}`);
        ...
        if (evidenceEntries.some((entry) => !entry))
            throw new Error(`Reviewed match references evidence that is no longer eligible: ${proposed.id}`);
    ...
    for (const decision of review.coverageDecisions) {
        ...
        if (!proposed)
            throw new Error(`Unknown reviewed coverage record: ${decision.proposedCoverageId}`);
        if (!expectationById.has(proposed.expectationId))
            throw new Error(`Coverage references an ineligible expectation: ${proposed.expectationId}`);
    ...
    const written = await writeApprovedMatching(workspace, context, [...manual.matches, ...reviewedMatches], explicitCoverage, {
        proposalId,
...
export function formatApproveEvidenceMatchingResult(result) {
    return [
...
async function loadManualMatches(workspace, targetType, targetId, approvedInterpretationSha256, eligibleEvidenceSetSha256) {
...
export async function approvedMatchingLifecycle(workspace, targetId) {
...
function resolveWithin(workspace, relativePath) {
    ...
    if (relation === ".." || relation.startsWith(`..${path.sep}`) || path.isAbsolute(relation))
        throw new Error(`Approved matching path escapes workspace: ${relativePath}`);
    return resolved;
}
