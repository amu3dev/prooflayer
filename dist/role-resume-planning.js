...
import { FitAssessmentManifestSchema, } from "./fit-assessment-schemas.js";
import { showTarget } from "./targets.js";
import { getFitAssessmentStatus, loadAssessmentContext, showFitAssessment, } from "./fit-assessment.js";
import { loadEffectiveEvidenceClaimReviews } from "./evidence-claim-review.js";
import { RoleResumeContentPlanSchema, RoleResumePlanManifestSchema, } from "./role-resume-plan-schemas.js";
...
export async function loadRoleResumePlanningContext(workspace, targetId, options = {}) {
    const target = await showTarget(workspace, targetId);
    if (target.type !== "role")
        throw new Error("Role Resume Content Planning accepts Role Targets only; Job Targets are rejected.");
    ...
    if (status.status !== "current")
        throw new Error(`Approved role assessment must be current before planning. Current status: ${status.status}`);
    ...
    if (assessment.targetType !== "role" || assessment.mode !== "role-positioning")
        throw new Error("Approved assessment is not a role-positioning assessment.");
    ...
    if (assessment.approvedInterpretation.sha256 !== base.approvedInterpretationSha256 ||
    ...
    const assessmentManifest = FitAssessmentManifestSchema.parse(await readJson(resolveWithin(workspace, status.manifestPath), null));
    ...
    const effectiveReviewedMetrics = [...effectiveReviews.values()]
        .filter(({ review }) => review.metricReview.state === "verified"
    ...
    const legacyReviewedMetrics = claims
        .filter((claim) => !effectivelyReviewedClaimIds.has(claim.id)
...
export async function buildRoleResumePlan(workspace, targetId, options = {}) {
    ...
    if (status.status === "current")
        return planResult(await showRoleResumePlan(workspace, targetId), paths, "already-current");
...
export async function promoteDeterministicRoleResumePlan(workspace, targetId, options = {}) {
...
export async function showRoleResumePlan(workspace, targetId, artifactType = "deterministic") {
    const paths = roleResumePlanPaths(workspace, targetId, artifactType);
    if (!(await pathExists(paths.planPath)))
        throw new Error(`${artifactType} role resume plan not found: ${targetId}`);
    return RoleResumeContentPlanSchema.parse(await readJson(paths.planPath, null));
}
export async function getRoleResumePlanStatus(workspace, targetId, artifactType = "deterministic") {
    const paths = roleResumePlanPaths(workspace, targetId, artifactType);
    const planExists = await pathExists(paths.planPath);
    const manifestExists = await pathExists(paths.manifestPath);
    const base = { targetId, artifactType, planExists, manifestExists, planPath: paths.planRelativePath, manifestPath: paths.manifestRelativePath };
    if (!planExists && !manifestExists)
        return emptyStatus(base, "missing", [`No ${artifactType} role resume plan exists.`]);
    if (!planExists || !manifestExists)
        return emptyStatus(base, "invalid", ["Role resume plan artifact set is incomplete."]);
    let plan;
    let manifest;
    try {
        plan = RoleResumeContentPlanSchema.parse(await readJson(paths.planPath, null));
        manifest = RoleResumePlanManifestSchema.parse(await readJson(paths.manifestPath, null));
    }
    catch (error) {
    ...
    let context;
    try {
        context = await loadRoleResumePlanningContext(workspace, targetId, { allowPartial: true });
    }
    catch (error) {
        return { ...emptyStatus(base, "stale", [`Current planning dependencies are unavailable: ${errorMessage(error)}`]), planHashMatches };
    }
    try {
        assertRoleResumePlanConsistency(plan, context);
    }
    catch (error) {
        return { ...emptyStatus(base, "invalid", [`Stored plan is inconsistent: ${errorMessage(error)}`]), planHashMatches };
    }
...
export function deriveRoleResumeContentPlan(context, createdAt, updatedAt, allowPartial = false) {
...
export function deriveExpectationSelection(context, planId, entry) {
    let decision;
    if (["unsupported", "conflicting"].includes(entry.supportStatus) || entry.defensibility === "none")
        decision = "exclude";
    else if (entry.supportStatus === "not-assessed")
        decision = "defer";
    else if (entry.supportStatus === "partially-supported" || entry.defensibility === "low")
        decision = "defer";
    else if (["critical", "high"].includes(entry.materiality) && ["high", "medium"].includes(entry.defensibility))
        decision = "primary";
    else if (entry.materiality === "medium")
        decision = "secondary";
    else
        decision = "supporting";
    const restrictions = [
...
export function derivePositioningScope(summary) {
    if (summary.mode !== "role-positioning")
        throw new Error("Role positioning requires a role assessment summary.");
    if (summary.overallPositioning === "well-supported")
        return "direct-role-positioning";
    if (summary.overallPositioning === "supported-with-gaps")
        return "adjacent-role-positioning";
    if (["partially-supported", "conflicting"].includes(summary.overallPositioning))
        return "stretch-positioning";
    return "insufficient-evidence";
}
export function assertRoleResumePlanConsistency(plan, context) {
    if (plan.targetType !== "role" || plan.mode !== "market-positioning" || plan.targetId !== context.target.id)
        throw new Error("Plan target identity or mode is invalid.");
    if (plan.roleTitle !== context.target.title || plan.positioning.targetRoleTitle !== context.target.title)
        throw new Error("Target role title changed or was treated as another identity.");
    if (plan.planningPolicy.name !== ROLE_RESUME_PLANNING_POLICY_NAME || plan.planningPolicy.version !== ROLE_RESUME_PLANNING_POLICY_VERSION)
        throw new Error("Planning policy identity is invalid.");
    ...
    if (plan.expectationSelections.length !== expectations.size)
        throw new Error("Plan must resolve every approved role expectation.");
    if (new Set(plan.expectationSelections.map((entry) => entry.expectationId)).size !== plan.expectationSelections.length)
        throw new Error("Expectation selections must be unique.");
    const assertReferences = (label, references) => {
        if (references.expectationIds.some((id) => !expectations.has(id)))
            throw new Error(`${label} references an unknown expectation.`);
        if (references.assessmentIds.some((id) => !assessments.has(id)))
            throw new Error(`${label} references an unknown assessment.`);
        if (references.approvedMatchIds.some((id) => !matches.has(id)))
            throw new Error(`${label} references an unknown approved match.`);
        if (references.evidenceIds.some((id) => !evidence.has(id)))
            throw new Error(`${label} references unknown reviewed evidence.`);
    };
    const assertProvenance = (label, provenance) => {
        if (provenance.targetId !== context.target.id ||
    ...
    for (const selection of plan.expectationSelections) {
        ...
        if (!assessment || assessment.id !== selection.assessmentId || !assessments.has(selection.assessmentId))
            throw new Error(`Unknown expectation or assessment selection: ${selection.id}`);
        if (selection.approvedMatchIds.some((id) => !matches.has(id)) || selection.evidenceIds.some((id) => !evidence.has(id)))
            throw new Error(`Selection has unknown match or evidence: ${selection.id}`);
        ...
        if (selection.supportStatus === "conflicting" && selection.decision !== "exclude")
            throw new Error("Conflicting expectations must be excluded.");
        assertProvenance(`Expectation selection ${selection.id}`, selection.provenance);
    }
    if (new Set(plan.evidenceSelections.map((entry) => entry.evidenceId)).size !== plan.evidenceSelections.length)
        throw new Error("Evidence selections must be unique.");
    for (const selected of plan.evidenceSelections) {
        if (!evidence.has(selected.evidenceId))
            throw new Error(`Unknown evidence selection: ${selected.evidenceId}`);
    ...
    for (const boundary of plan.claimBoundaries) {
        if (boundary.expectationId && !expectations.has(boundary.expectationId))
            throw new Error(`Unknown claim-boundary expectation: ${boundary.id}`);
        if (boundary.evidenceIds.some((id) => !evidence.has(id)))
            throw new Error(`Unknown claim-boundary evidence: ${boundary.id}`);
        const selection = plan.expectationSelections.find((entry) => entry.expectationId === boundary.expectationId);
        if (selection?.decision === "exclude" && boundary.boundaryType !== "prohibited")
            throw new Error("Excluded expectations require prohibited claim boundaries.");
    ...
    if (included.some((section, index) => section.order !== index))
        throw new Error("Included section order must be unique and contiguous from zero.");
    if (new Set(plan.sections.map((entry) => entry.type)).size !== plan.sections.length)
        throw new Error("Duplicate resume sections are forbidden.");
    if (new Set(plan.sections.map((entry) => entry.id)).size !== plan.sections.length)
    ...
    if (new Set(themes.map((entry) => normalizeThemeLabel(entry.label))).size !== themes.length)
        throw new Error("Duplicate positioning themes are forbidden.");
    for (const theme of themes) {
        if (!theme.sourceExpectationIds.length || !theme.sourceAssessmentIds.length)
            throw new Error("Positioning themes require approved expectation and assessment provenance.");
    ...
    if (containsFinishedResumeProse(plan))
        throw new Error("Role Resume Content Plans must not contain finished resume prose.");
    if (/\b\d+(?:\.\d+)?%\s*(?:fit|match)\b/i.test(stableJson(plan)))
        throw new Error("Hidden fit scores are forbidden.");
}
export function createRoleResumePlanManifest(plan, context, planPath, planSha256, artifactType, createdAt, updatedAt, review = {}) {
...
export function deterministicRoleResumePlanId(context) {
...
export function roleResumePlanPaths(workspace, targetId, artifactType) {
...
export function formatBuildRoleResumePlanResult(result) {
    return [
...
export function formatRoleResumePlanStatus(status) {
...
function deriveEvidenceSelections(context, planId, selections) {
    const byEvidence = new Map();
    for (const match of context.approvedMatching.matches) {
        for (const evidenceId of match.evidenceIds)
            byEvidence.set(evidenceId, [...(byEvidence.get(evidenceId) ?? []), match]);
    }
    return context.approvedMatching.evidenceSnapshot.eligibleEvidenceIds.map((evidenceId) => {
        ...
        if (contradictory)
            decision = "exclude";
        else if (strong && !historical)
            decision = "preferred";
        else if (selectedMatches.length && historical)
        ...
        const permittedUses = uniqueSorted(assessmentEntries.flatMap((entry) => contentTypesFor(entry)));
        return {
            ...
            prohibitedUses: decision === "exclude" ? [...ResumeContentTypes] : [
                ...(!context.reviewedMetricEvidenceIds.has(evidenceId) ? ["quantified-outcome"] : []),
            ],
...
function deriveClaimBoundary(context, planId, selection) {
    let boundaryType;
    if (selection.decision === "exclude")
        boundaryType = "prohibited";
    else if (selection.decision === "defer")
        boundaryType = "requires-review";
    else if (selection.supportStatus === "partially-supported" || selection.defensibility === "low")
        boundaryType = "allowed-with-caution";
    else
...
function derivePositioning(context, planId, selections) {
    ...
    const themeRank = { primary: 0, secondary: 1, supporting: 2 };
...
function deriveSections(context, planId, selections, evidence, boundaries, positioning) {
    ...
    return specs.map(([type, status, allowed, objective, maximumItemCount]) => {
        const relevant = status === "exclude"
            ...
            : type === "selected-impact"
                ? selected.filter((entry) => boundaries.some((boundary) => boundary.expectationId === entry.expectationId &&
                    boundary.allowedClaimTypes.some((claimType) => ["achievement", "delivery-outcome", "product-outcome", "business-outcome", "quantified-outcome"].includes(claimType))))
...
function deriveExclusions(context, planId, selections, evidence) {
    const exclusions = [];
    ...
    if (![...context.reviewedMetricEvidenceIds].length)
        exclusions.push({
...
export function derivePlanCompleteness(selections, sections, positioning, boundaries, assessmentStatus, allowPartial) {
    const selected = selections.filter((entry) => ["primary", "secondary", "supporting"].includes(entry.decision));
...
export function derivePlanRisks(targetId, selections, evidence, completeness) {
    const risks = [];
    const add = (code, severity, message, expectationIds = [], evidenceIds = []) => risks.push({
    ...
    if (!selections.some((entry) => entry.decision === "primary"))
        add("NO_PRIMARY_POSITIONING_THEME", "critical", "No defensible primary positioning theme is available.");
    const criticalExcluded = selections.filter((entry) => ["critical", "high"].includes(entry.materiality) && entry.decision === "exclude");
    if (criticalExcluded.length)
        add("CRITICAL_EXPECTATION_EXCLUDED", "high", "Material role expectations are excluded from future drafting.", criticalExcluded.map((entry) => entry.expectationId));
    ...
    if (primaryWithLimitedEvidence.length)
        add("PRIMARY_THEME_USES_LIMITED_EVIDENCE", "high", "A primary positioning theme relies on limited-use evidence.", primaryWithLimitedEvidence.map((entry) => entry.expectationId), uniqueSorted(primaryWithLimitedEvidence.flatMap((entry) => entry.evidenceIds)));
    const historical = evidence.filter((entry) => entry.decision !== "exclude" && entry.freshnessRisk === "high");
    if (historical.length)
        add("HISTORICAL_EVIDENCE_OVERRELIANCE", "medium", "Historical evidence must not imply current hands-on depth.", [], historical.map((entry) => entry.evidenceId));
    const contradictory = selections.filter((entry) => entry.supportStatus === "conflicting");
    if (contradictory.length)
        add("CONTRADICTORY_EVIDENCE_PRESENT", "high", "Contradictory reviewed evidence remains explicitly excluded.", contradictory.map((entry) => entry.expectationId), uniqueSorted(contradictory.flatMap((entry) => entry.evidenceIds)));
    ...
    for (const selection of selections.filter((entry) => !["exclude", "defer"].includes(entry.decision))) {
        for (const evidenceId of selection.evidenceIds)
            reuseCounts.set(evidenceId, [...(reuseCounts.get(evidenceId) ?? []), selection.expectationId]);
    }
    const overused = [...reuseCounts.entries()].filter(([, expectationIds]) => expectationIds.length > 3);
    if (overused.length)
        add("EVIDENCE_REUSED_EXCESSIVELY", "medium", "One or more evidence items support too many selected expectations and require explicit drafting restraint.", uniqueSorted(overused.flatMap(([, ids]) => ids)), overused.map(([id]) => id));
    if (!completeness.usableForResumeDrafting)
        add("PLAN_INCOMPLETE", "critical", "Plan is not structurally usable for future resume drafting.");
    return risks;
}
export function derivePlanWarnings(targetId, selections, evidence, positioning) {
    const warnings = [];
    const add = (code, message) => warnings.push({
        id: `planning-warning_${hashText([targetId, code].join("\0")).slice(0, 12)}`,
        code, message, expectationIds: [], approvedMatchIds: [], evidenceIds: [],
    });
    if (!selections.some((entry) => entry.supportStatus === "strongly-supported"))
        add("NO_STRONGLY_SUPPORTED_EXPECTATIONS", "No expectation has strong direct current proof.");
    if (!selections.some((entry) => entry.decision === "primary"))
        add("NO_PRIMARY_EXPECTATIONS_SELECTED", "No primary expectation is safe to select.");
    const usableEvidence = evidence.filter((entry) => entry.decision !== "exclude");
    if (usableEvidence.length > 0 && usableEvidence.every((entry) => entry.freshnessRisk === "high"))
        add("ONLY_HISTORICAL_EVIDENCE_AVAILABLE", "Only historical evidence is available.");
    if (usableEvidence.length > 0 && usableEvidence.every((entry) => entry.decision === "limited-use"))
        add("ONLY_SUPPORTING_EVIDENCE_AVAILABLE", "Only limited-use evidence is available for future drafting.");
    if (!evidence.some((entry) => entry.permittedUses.includes("quantified-outcome")))
        add("NO_QUANTIFIED_OUTCOMES_AVAILABLE", "No reviewed metric is available for quantified outcomes.");
    if (positioning.positioningScope !== "direct-role-positioning")
        add("ROLE_POSITIONING_REQUIRES_CAUTION", "Role positioning requires explicit evidence-boundary caution.");
...
export function derivePlanAmbiguities(targetId, selections, evidence) {
    const ambiguities = [];
    const add = (code, message, expectationIds = [], evidenceIds = []) => ambiguities.push({
    ...
    if (deferred.length)
        add("PRIMARY_VS_SECONDARY_THEME_UNCLEAR", "Deferred expectations require evidence review before positioning.", deferred.map((entry) => entry.expectationId));
    const historical = evidence.filter((entry) => entry.freshnessRisk === "high");
    if (historical.length)
        add("CURRENT_VS_HISTORICAL_CAPABILITY_UNCLEAR", "Historical evidence requires temporal qualification.", [], historical.map((entry) => entry.evidenceId));
    const reuseCounts = new Map();
    for (const selection of selections.filter((entry) => !["exclude", "defer"].includes(entry.decision))) {
        for (const evidenceId of selection.evidenceIds)
            reuseCounts.set(evidenceId, (reuseCounts.get(evidenceId) ?? 0) + 1);
    }
    const reused = [...reuseCounts.entries()].filter(([, count]) => count > 1).map(([id]) => id);
    if (reused.length)
        add("EVIDENCE_REUSE_BOUNDARY_UNCLEAR", "Evidence reused across multiple selected expectations must not become duplicate resume claims.", [], reused);
    return ambiguities;
}
...
function contentTypesFor(entry) {
    const byKind = {
...
function allowedSectionsFor(entry, decision) {
    if (["exclude", "defer"].includes(decision))
        return [];
    const sections = ["professional-summary", "core-capabilities", "professional-experience"];
    if (entry.expectation.type === "technical-skill")
        sections.push("technical-capabilities", "selected-projects");
    if (entry.expectation.type === "leadership")
        sections.push("leadership-capabilities");
    if (entry.expectation.type === "success-outcome")
        sections.push("selected-impact");
    if (entry.expectation.type === "qualification")
        sections.push("education", "certifications");
    return uniqueSorted(sections);
}
function elementProvenance(context, input) {
...
function planProvenance(context) {
    return {
...
function dependency(pathValue, sha256, manifestPath, manifestSha256) {
...
function selectionRationale(decision) {
    return ({
...
function evidenceRationale(decision) {
    return ({
...
function boundaryRationale(type) {
    return ({
...
function humanize(value) { return value.replace(/[-_]+/g, " ").replace(/\b\w/g, (letter) => letter.toUpperCase()); }
function normalizeThemeLabel(value) { return value.toLowerCase().replace(/[^a-z0-9]+/g, " ").trim(); }
function containsFinishedResumeProse(plan) {
...
function planResult(plan, paths, result) {
    return {
        targetId: plan.targetId,
        result,
        planId: plan.id,
...
async function dependencyHashMatches(workspace, targetId, category, id, file, expected) {
