...
import { JobFitProofAssessmentManifestSchema, JobFitProofAssessmentSchema, JobRequirementFitProofAssessmentSchema, } from "./job-fit-proof-assessment-schemas.js";
...
export const JOB_FIT_PROOF_ASSESSMENT_ANALYZER_NAME = "job-fit-proof-assessment";
export const JOB_FIT_PROOF_ASSESSMENT_ANALYZER_VERSION = "1";
export const JOB_FIT_PROOF_ASSESSMENT_POLICY_NAME = "job-fit-proof-assessment-policy";
...
export function jobFitProofAssessmentPaths(workspace, targetId) {
...
export async function buildJobFitProofAssessment(workspace, targetId, options = {}) {
    ...
    if (status.status === "current") {
        return resultFromAssessment(await showJobFitProofAssessment(workspace, targetId), paths, "already-current");
    }
    if ((status.status === "stale" || status.status === "invalid") &&
        !options.rebuild) {
        throw new Error(`Stored Job Fit and Proof Assessment is ${status.status} and was not overwritten. Review dependencies, then use --rebuild. ${status.reasons.join(" ")}`);
    }
    ...
    const assessment = JobFitProofAssessmentSchema.parse({
        schemaVersion: 1,
        id: `job-fit-proof-assessment_${hashText(`${targetId}\u0000${dependencies.normalizedInputSha256}\u0000${JOB_FIT_PROOF_ASSESSMENT_POLICY_VERSION}`).slice(0, 14)}`,
        targetId,
        targetType: "job",
        analyzer: {
    ...
    return resultFromAssessment(assessment, paths, status.status === "missing" ? "created" : "rebuilt");
}
export async function showJobFitProofAssessment(workspace, targetId) {
    ...
    return JobFitProofAssessmentSchema.parse(await readJson(paths.assessmentPath, null));
}
export async function getJobFitProofAssessmentStatus(workspace, targetId) {
    await requireJobTarget(workspace, targetId);
    const paths = jobFitProofAssessmentPaths(workspace, targetId);
    ...
    let manifest;
    try {
        assessment = JobFitProofAssessmentSchema.parse(await readJson(paths.assessmentPath, null));
        manifest = JobFitProofAssessmentManifestSchema.parse(await readJson(paths.manifestPath, null));
    }
    catch (error) {
        return emptyStatus(base, "invalid", [
    ...
    if (!assessmentHashMatches) {
        identityReasons.push("Job Fit and Proof Assessment SHA-256 does not match its manifest.");
    }
    ...
    if (staleReasons.length > 0) {
        return statusWithChecks(base, assessmentHashMatches, checks, "stale", staleReasons);
    }
    const expected = analyzeAssessment(dependencies);
    if (stableJson(assessmentSemantics(assessment)) !== stableJson(expected)) {
        return statusWithChecks(base, assessmentHashMatches, checks, "invalid", [
            "Stored assessment content does not match deterministic analysis of the current Job Requirement Coverage.",
        ]);
    }
    return statusWithChecks(base, assessmentHashMatches, checks, "current", []);
}
export function classifyJobRequirementAssessment(coverage) {
    const assessmentState = coverage.state === "supported"
        ...
        : coverage.state === "partially-supported"
            ...
            : coverage.state === "unsupported"
                ...
                : coverage.state === "contradicted"
                    ? "contradiction"
                    : "indeterminate";
    const proofStrength = assessmentState === "contradiction"
        ? "conflicting"
        : coverage.evidenceQuality === "strong"
            ? "strong"
            : coverage.evidenceQuality === "adequate"
                ...
                : coverage.evidenceQuality === "unavailable"
                    ? "unavailable"
                    : "limited";
    const materiality = coverage.necessity === "ambiguous"
        ? "unknown"
...
export function classifyOverallJobAssessment(assessments) {
    ...
    const supported = mandatory.filter((entry) => entry.assessmentState === "strength" ||
        entry.assessmentState === "supported");
    const gaps = mandatory.filter((entry) => entry.assessmentState === "gap");
    const partial = mandatory.filter((entry) => entry.assessmentState === "partial");
    if (gaps.length === mandatory.length || gaps.length >= 2) {
        return "insufficient";
    }
    if (supported.length === 0 && partial.length > 0)
        return "limited";
    if (gaps.length === 1 || partial.length >= 2)
        return "mixed";
    if (assessments.some((entry) => entry.assessmentState === "contradiction")) {
        return "mixed";
    }
    const allMandatorySupported = mandatory.every((entry) => entry.assessmentState === "strength" ||
        entry.assessmentState === "supported");
    const strongProof = mandatory.every((entry) => entry.proofStrength === "strong" ||
...
export function formatBuildJobFitProofAssessmentResult(result) {
    return [
...
export function formatJobFitProofAssessmentStatus(status) {
...
function analyzeAssessment(dependencies) {
    const requirements = new Map(dependencies.requirementInput.model.requirements.map((entry) => [
        entry.id,
        entry,
    ]));
    const risks = [];
    ...
    const requirementAssessments = dependencies.coverage.requirements
        ...
        if (!requirement) {
            throw new Error(`Coverage references unknown requirement: ${coverage.requirementId}`);
        }
        ...
        const ambiguityIds = addRequirementAmbiguities(ambiguities, coverage, decision);
        return JobRequirementFitProofAssessmentSchema.parse({
            id: `job-requirement-assessment_${hashText(`${dependencies.target.id}\u0000${requirement.id}\u0000${JOB_FIT_PROOF_ASSESSMENT_POLICY_VERSION}`).slice(0, 14)}`,
    ...
    const idsFor = (state) => requirementAssessments
    ...
    return {
        ...
        ambiguities: ambiguities.sort((left, right) => left.id.localeCompare(right.id)),
        completeness: {
            status: requirementIds.length === 0 ? "empty" : "complete",
            requirementIds,
            assessedRequirementIds: [...requirementIds],
            readyForDownstreamPlanning: requirementIds.length > 0 && overallState !== "indeterminate",
            blockingReasons,
        },
    };
}
function gapTypeFor(coverage, state) {
    if (state === "strength" || state === "supported")
        return undefined;
    ...
    if (/\brecen(?:t|cy)\b/.test(warningText))
        return "recency-gap";
    if (/\bdepth\b/.test(warningText))
        return "depth-gap";
    if (/\bscope\b/.test(warningText))
        return "scope-gap";
    const categoryGap = {
    ...
    return (categoryGap[coverage.category] ??
        (state === "partial" ? "partial-proof" : "missing-proof"));
}
function assessmentStatement(state) {
    const statements = {
        strength: "Reviewed evidence provides strong direct support for this requirement.",
        supported: "Reviewed evidence provides defensible support for this requirement.",
        partial: "Reviewed evidence provides partial or indirect support for this requirement.",
        ...
        contradiction: "An explicit approved contradiction conflicts with this requirement.",
        indeterminate: "Current coverage and provenance cannot resolve this requirement defensibly.",
    };
    return statements[state];
}
function overallStatement(state) {
    const statements = {
        strong: "Mandatory requirements have strong or adequate reviewed proof, including at least one explicit strength.",
        credible: "Mandatory requirements have defensible reviewed proof with no critical unresolved gap.",
        mixed: "Reviewed proof is meaningful, but material partial support, a gap, or a non-mandatory contradiction remains.",
        limited: "Reviewed proof is limited across material mandatory requirements.",
        insufficient: "Critical mandatory proof gaps or contradictions prevent a defensible positive assessment.",
        indeterminate: "The available requirement coverage is not sufficient to determine a defensible overall assessment.",
    };
    return statements[state];
}
...
function addRequirementRisks(risks, coverage, decision) {
    const candidates = [];
    if (coverage.necessity === "mandatory" &&
        decision.assessmentState === "gap") {
        candidates.push({
            code: "CRITICAL_MANDATORY_REQUIREMENT_UNSUPPORTED",
            severity: "critical",
            message: "A mandatory requirement has no proof in the current reviewed evidence map.",
        });
    }
    if (coverage.necessity === "mandatory" &&
        ...
        candidates.push({
            ...
            severity: "high",
            message: "A mandatory requirement has only partial or indirect reviewed support.",
        });
    }
    if (decision.assessmentState === "contradiction") {
        candidates.push({
            ...
            message: "An explicit approved contradiction is linked to this requirement.",
        });
    }
    if (decision.proofStrength === "limited") {
        candidates.push({
            ...
            message: "Mapped proof quality is limited and is not upgraded by link quantity.",
        });
    }
    if (decision.materiality === "unknown") {
        candidates.push({
            ...
            severity: "medium",
            message: "Requirement materiality remains unknown because necessity is ambiguous.",
        });
    }
    const warningText = coverage.warnings.join(" ").toLowerCase();
    ...
    return candidates
        .map((candidate) => {
        const risk = {
            id: findingId("risk", candidate.code, coverage.requirementId, candidate.message),
            ...candidate,
...
function addRequirementWarnings(warnings, coverage, decision) {
    const candidates = [];
    if (coverage.necessity === "ambiguous") {
        candidates.push({
            code: "AMBIGUOUS_REQUIREMENT_REMAINS",
            message: "Requirement necessity remains ambiguous and is not silently hardened.",
        });
    }
    if (coverage.necessity === "preferred" &&
        decision.assessmentState === "gap") {
        candidates.push({
            code: "PREFERRED_REQUIREMENT_UNSUPPORTED",
            message: "A preferred requirement has no proof in the current reviewed evidence map.",
        });
    }
    if (coverage.necessity === "contextual" &&
        decision.assessmentState === "gap") {
        candidates.push({
            code: "CONTEXTUAL_REQUIREMENT_UNSUPPORTED",
            message: "A contextual requirement has no proof in the current reviewed evidence map.",
        });
    }
    return candidates
        .map((candidate) => {
        const warning = {
            id: findingId("warning", candidate.code, coverage.requirementId, candidate.message),
            ...candidate,
            requirementId: coverage.requirementId,
            coverageEntryId: coverage.id,
            evidenceLinkIds: coverage.mappedLinkIds,
...
function addRequirementAmbiguities(ambiguities, coverage, decision) {
    const candidates = coverage.ambiguities.map((message) => ({
        code: "REQUIREMENT_AMBIGUITY_PRESERVED",
        message,
    }));
    if (decision.assessmentState === "indeterminate") {
        candidates.push({
            code: "COVERAGE_INDETERMINATE",
            message: "Current deterministic coverage cannot resolve this requirement.",
        });
    }
    if (decision.assessmentState === "gap") {
        candidates.push({
            code: "GAP_DOES_NOT_PROVE_CAPABILITY_ABSENT",
            message: "Absence of reviewed proof does not establish absence of capability.",
        });
    }
    return candidates
        .map((candidate) => {
        const ambiguity = {
            id: findingId("ambiguity", candidate.code, coverage.requirementId, candidate.message),
            ...candidate,
            requirementId: coverage.requirementId,
            coverageEntryId: coverage.id,
            evidenceLinkIds: coverage.mappedLinkIds,
...
function findingId(kind, code, context, message) {
    return `job-assessment-${kind}_${hashText(`${code}\u0000${context}\u0000${message}`).slice(0, 14)}`;
}
async function loadCurrentDependencies(workspace, targetId) {
    const target = await requireJobTarget(workspace, targetId);
    const coverageStatus = await getJobCoverageStatus(workspace, targetId);
    if (coverageStatus.status !== "current") {
        throw new Error(`Job Fit and Proof Assessment requires current Job Requirement Coverage. Current status: ${coverageStatus.status}. ${coverageStatus.reasons.join(" ")}`);
    }
    const coveragePaths = jobCoveragePaths(workspace, targetId);
    const coverage = await showJobCoverage(workspace, targetId);
    const coverageManifest = JobRequirementCoverageManifestSchema.parse(await readJson(coveragePaths.manifestPath, null));
    if (!coverage.completeness.readyForDownstreamAssessment) {
        throw new Error("Job Requirement Coverage is current but is not ready for downstream assessment.");
    }
    const requirementInput = await loadRequirementInput(workspace, targetId, coverage.input.requirementModelType);
    ...
    if (dependencyProblems.length > 0) {
        throw new Error(`Assessment dependency provenance is invalid. ${dependencyProblems.join(" ")}`);
    }
...
function dependencyInput(dependencies) {
    return {
        target: {
...
function dependencyMatches(manifest, dependencies) {
    return {
        ...
        requirementModelHashMatches: manifest.requirementModelType === dependencies.requirementInput.type &&
        ...
        evidenceMapHashMatches: manifest.evidenceMapSha256 === dependencies.evidenceMapSha256,
        evidenceMapManifestHashMatches: manifest.evidenceMapManifestSha256 ===
            dependencies.evidenceMapManifestSha256,
        coverageHashMatches: manifest.coverageSha256 === dependencies.coverageSha256,
        coverageManifestHashMatches: manifest.coverageManifestSha256 ===
            dependencies.coverageManifestSha256,
        analyzerMatches: manifest.analyzerName === JOB_FIT_PROOF_ASSESSMENT_ANALYZER_NAME &&
            manifest.analyzerVersion === JOB_FIT_PROOF_ASSESSMENT_ANALYZER_VERSION,
        policyMatches: manifest.policyName === JOB_FIT_PROOF_ASSESSMENT_POLICY_NAME &&
            manifest.policyVersion === JOB_FIT_PROOF_ASSESSMENT_POLICY_VERSION,
        normalizedInputHashMatches: manifest.normalizedInputSha256 === dependencies.normalizedInputSha256,
    };
}
...
function validateStoredIdentity(assessment, manifest, paths) {
    const reasons = [];
    if (assessment.id !== manifest.assessmentId ||
        ...
        reasons.push("Job Fit and Proof Assessment identity or persistence path is invalid.");
    }
    if (assessment.input.target.sha256 !== manifest.targetSha256 ||
        ...
        reasons.push("Job Fit and Proof Assessment and manifest disagree on dependency hashes.");
    }
    return reasons;
}
function assessmentSemantics(assessment) {
    return {
...
function resultFromAssessment(assessment, paths, result) {
    return {
        ...
        readyForDownstreamPlanning: assessment.completeness.readyForDownstreamPlanning,
    };
}
...
function statusWithChecks(base, assessmentHashMatches, checks, status, reasons) {
    return {
        ...base,
        assessmentHashMatches,
...
async function requireJobTarget(workspace, targetId) {
    ...
    if (target.type !== "job") {
        throw new Error(`Job Fit and Proof Assessment rejects Role Target: ${targetId}`);
    }
    return target;
}
function resolveWithin(workspace, relativePath) {
    ...
    if (relation === ".." ||
        ...
        throw new Error(`Job Fit and Proof Assessment path escapes the workspace: ${relativePath}`);
    }
    return resolved;
}
