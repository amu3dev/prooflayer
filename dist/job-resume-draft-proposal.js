...
import { hashFile, hashText, pathExists, readJson, uniqueSorted, writeBufferAtomic, writeJsonAtomic, } from "./fs-utils.js";
import { ModelJobResumeDraftPayloadSchema, JobResumeDraftProposalManifestSchema, JobResumeDraftProposalSchema, } from "./job-resume-draft-schemas.js";
...
export async function generateJobResumeDraftProposal(workspace, targetId, options = {}) {
    ...
    const proposalId = await nextProposalId(workspace, targetId, requestFingerprint, rawResponseSha256);
    const paths = jobResumeDraftProposalPaths(workspace, targetId, proposalId);
    const normalized = normalizeJobResumeDraftResponse(response.rawText, proposalId, scaffold, context, scaffoldSha256);
    const blocking = normalized.validationIssues.some((entry) => entry.severity === "critical" || entry.severity === "high");
...
export async function showJobResumeDraftProposal(workspace, proposalId) {
    const location = await locateProposal(workspace, proposalId);
    if (!location)
        throw new Error(`Job resume draft proposal not found: ${proposalId}`);
    return JobResumeDraftProposalSchema.parse(await readJson(location.proposalPath, null));
}
export async function listJobResumeDraftProposals(workspace, targetId) {
    ...
    const proposals = await Promise.all(entries.filter((entry) => entry.isDirectory()).map(async (entry) => {
        try {
            return JobResumeDraftProposalSchema.parse(await readJson(path.join(root, entry.name, "proposal.json"), null));
        }
        catch {
            return null;
        }
...
export async function getJobResumeDraftProposalStatus(workspace, proposalId) {
    const location = await locateProposal(workspace, proposalId);
    if (!location)
        return emptyProposalStatus(proposalId, "unknown", "missing", ["Proposal not found."]);
    ...
    let manifest;
    try {
        proposal = JobResumeDraftProposalSchema.parse(await readJson(location.proposalPath, null));
        manifest = JobResumeDraftProposalManifestSchema.parse(await readJson(location.manifestPath, null));
    }
    catch (error) {
...
export async function replayJobResumeDraftProposal(workspace, proposalId) {
    ...
    const normalized = normalizeJobResumeDraftResponse(raw, proposal.id, scaffold, context, proposal.input.scaffoldSha256);
...
export function renderJobResumeDraftPrompt(input) {
    return [
...
export function createJobResumeDraftModelInput(context, scaffold) {
...
export function validateJobResumeDraftPayload(payload, proposalId, scaffold, context, scaffoldSha256) {
    const issues = [];
    ...
    const normalizedSections = payload.sections.map((section) => {
        ...
        const sectionIssues = [];
        if (section.planSectionId !== guard.planSectionId ||
        ...
        const normalizedItems = section.items.map((item) => {
            ...
            const status = itemIssues.some((entry) => entry.severity === "critical" || entry.severity === "high") ? "invalid" : itemIssues.length ? "requires-review" : "valid";
...
export function buildJobResumeDraftClaimLedger(sections, context) {
    const emphasisByRequirement = new Map(context.contentPlan.requirementEmphasis.map((entry) => [entry.requirementId, entry]));
...
export function buildJobResumeDraftEvidenceUsage(sections, scaffold, context) {
    const selectionByEvidence = new Map(context.contentPlan.evidenceSelections.map((entry) => [entry.evidenceId, entry]));
...
export function formatJobResumeDraftProposalResult(result) {
    return [
...
export function formatJobResumeDraftProposalList(proposals) {
...
export function formatJobResumeDraftProposalStatus(status) {
    return [
...
function normalizeJobResumeDraftResponse(raw, proposalId, scaffold, context, scaffoldSha256) {
    let payload;
    try {
...
function validateReferenceExistence(item, lookups, issues) {
    const checks = [
...
function validateLinkConsistency(item, linkById, issues) {
    for (const linkId of item.evidenceMapLinkIds) {
        const link = linkById.get(linkId);
        if (!link)
            continue;
        if (!item.requirementIds.includes(link.requirementId) ||
...
function validateClaimEligibility(item, claimById, evidenceById, issues) {
...
function validateClaimBoundaries(item, boundaryById, issues) {
    ...
    for (const boundaryId of item.claimBoundaryIds) {
        ...
        if (item.claimTypes.some((type) => type !== "quantified-outcome" || !metricAuthorized
...
function validateMetrics(item, metricById, evidenceById, issues) {
...
function validateLanguage(item, scaffold, context, evidenceById, claimById, issues) {
    ...
    const evidenceText = evidence.map((entry) => `${entry.text} ${entry.normalizedSummary} ${entry.dateRange ?? ""} ${entry.company ?? ""} ${entry.project ?? ""}`).join(" ");
    ...
    if (item.itemType === "experience-role" &&
    ...
    if (["experience-role", "experience-bullet"].includes(item.itemType) &&
        evidence.length > 0 &&
    ...
    if (item.claimTypes.some((type) => ["achievement", "delivery-outcome", "product-outcome", "business-outcome"].includes(type)) &&
        evidence.length > 0 &&
    ...
    if (item.itemType === "technology") {
        ...
        if (technologies.length > 0 && !technologies.some((technology) => normalizeText(item.text).includes(normalizeText(technology)))) {
    ...
    const planSection = context.contentPlan.sections.find((entry) => entry.id === item.provenance.planSectionId);
    ...
    const unsupportedNamedTechnologies = context.requirementInput.model.namedTechnologies
        ...
        .filter((technology) => !allowedTerms.some((term) => normalizeText(term) === normalizeText(technology)) &&
        !normalizedApprovedText.includes(normalizeText(technology)));
    if (unsupportedNamedTechnologies.length > 0) {
        issues.push(issue("UNSUPPORTED_JOB_DESCRIPTION_TERMINOLOGY", "Draft introduces named Job Description technology without planned evidence support.", "high", refs(item)));
    }
...
function duplicateAndOveruseIssues(sections) {
...
function itemTypesForSection(type) {
    return ({
        headline: ["headline"],
...
function validateSubset(label, actual, allowed, item, issues) {
    const unknown = actual.filter((entry) => !allowed.includes(entry));
    if (unknown.length > 0) {
        issues.push(issue(`UNKNOWN_${label.toUpperCase().replace(/\s+/g, "_")}`, `Draft item references unplanned ${label} IDs: ${unknown.join(", ")}.`, "critical", refs(item)));
    }
}
function refs(item) {
    return {
...
function issue(code, message, severity, references = {}) {
    return { code, message, severity, ...emptyReferences(), ...references };
}
function warning(proposalId, code, message) {
    return {
...
async function nextProposalId(workspace, targetId, requestFingerprint, rawResponseSha256) {
...
export function jobResumeDraftProposalPaths(workspace, targetId, proposalId, relativeOnly = false) {
...
function emptyProposalStatus(proposalId, targetId, status, reasons, existence = {}) {
    return {
        proposalId,
        targetId,
