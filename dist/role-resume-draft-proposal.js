...
import { ModelRoleResumeDraftPayloadSchema, RoleResumeDraftProposalManifestSchema, RoleResumeDraftProposalSchema, } from "./role-resume-draft-schemas.js";
...
export async function generateRoleResumeDraftProposal(workspace, targetId, options = {}) {
    ...
    return proposalResult(proposal, proposal.status === "validation-failed" ? "validation-failed" : "created");
}
export async function showRoleResumeDraftProposal(workspace, proposalId) {
    const location = await locateProposal(workspace, proposalId);
    if (!location)
        throw new Error(`Role resume draft proposal not found: ${proposalId}`);
    return RoleResumeDraftProposalSchema.parse(await readJson(location.proposalPath, null));
}
export async function listRoleResumeDraftProposals(workspace, targetId) {
    ...
    const proposals = await Promise.all(entries.filter((entry) => entry.isDirectory()).map(async (entry) => {
        try {
            return RoleResumeDraftProposalSchema.parse(await readJson(path.join(root, entry.name, "proposal.json"), null));
        }
        catch {
            return null;
        }
...
export async function getRoleResumeDraftProposalStatus(workspace, proposalId) {
    const location = await locateProposal(workspace, proposalId);
    if (!location)
    ...
    let manifest;
    try {
        proposal = RoleResumeDraftProposalSchema.parse(await readJson(location.proposalPath, null));
        manifest = RoleResumeDraftProposalManifestSchema.parse(await readJson(location.manifestPath, null));
    }
    catch (error) {
...
export async function replayRoleResumeDraftProposal(workspace, proposalId) {
    ...
    const normalized = normalizeRoleResumeDraftResponse(raw, proposal.id, scaffold, context, proposal.input.draftScaffoldSha256);
...
export function renderRoleResumeDraftPrompt(input) {
    return [
...
export function createRoleResumeDraftModelInput(context, scaffold) {
...
export function validateRoleResumeDraftPayload(payload, proposalId, scaffold, context, scaffoldSha256) {
    const issues = [];
    ...
    const normalizedSections = payload.sections.map((section) => {
        ...
        const normalizedItems = section.items.map((item) => {
            ...
            if (!compositionSlot || !guard.compositionSlotIds.includes(item.compositionSlotId)) {
                ...
                if (!sameSet(item.sourceExpectationIds, compositionSlot.sourceExpectationIds)
            ...
            for (const id of item.sourceAssessmentIds)
                if (!assessmentById.has(id))
                    itemIssues.push(issue("UNKNOWN_ASSESSMENT", `Unknown assessment: ${id}`, "critical", refs(item)));
            for (const id of item.approvedMatchIds)
                if (!matchById.has(id))
                    itemIssues.push(issue("UNKNOWN_MATCH", `Unknown approved match: ${id}`, "critical", refs(item)));
            for (const id of item.evidenceIds)
                if (!evidenceById.has(id))
                    itemIssues.push(issue("UNKNOWN_EVIDENCE", `Unknown evidence: ${id}`, "critical", refs(item)));
            for (const id of item.claimBoundaryIds)
                if (!boundaryById.has(id))
                    itemIssues.push(issue("UNKNOWN_CLAIM_BOUNDARY", `Unknown claim boundary: ${id}`, "critical", refs(item)));
            ...
            const validationStatus = itemIssues.some((entry) => entry.severity === "critical" || entry.severity === "high")
                ? "invalid"
...
export function buildClaimLedger(sections, context) {
    const assessmentById = new Map(context.approvedAssessment.expectationAssessments.map((entry) => [entry.id, entry]));
...
export function buildEvidenceUsage(sections, scaffold) {
...
export function formatRoleResumeDraftProposalResult(result) {
    return [
...
export function formatRoleResumeDraftProposalList(proposals) {
...
export function formatRoleResumeDraftProposalStatus(status) {
    return [
...
function normalizeRoleResumeDraftResponse(raw, proposalId, scaffold, context, scaffoldSha256) {
    let payload;
    try {
...
function validateMetrics(item, permission, context, evidenceById, issues) {
...
function validateLanguage(item, scaffold, context, evidenceById, issues) {
    ...
    for (const [pattern, code, message] of forbidden) {
        if (pattern.test(text) && !pattern.test(approvedText))
    ...
    if (item.claimTypes.some((entry) => ["achievement", "delivery-outcome", "product-outcome", "business-outcome"].includes(entry)) &&
        evidence.length &&
    ...
    if (/\b(?:adopted by|commercial adoption|customer adoption|market adoption)\b/i.test(text) &&
    ...
    if (/\b(?:hiring authority|budget (?:ownership|control)|performance reviews?|executive reporting|direct reports?)\b/i.test(text) &&
...
function duplicateIssues(sections) {
...
function validateReferenceSubset(label, actual, allowed, issues, item) {
    const unknown = actual.filter((entry) => !allowed.includes(entry));
    if (unknown.length)
        issues.push(issue(`UNKNOWN_${label.toUpperCase().replace(/\s+/g, "_")}`, `Draft item references unapproved ${label} IDs: ${unknown.join(", ")}.`, "critical", refs(item)));
}
function refs(item) {
    return {
