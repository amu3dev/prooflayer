...
import { EVIDENCE_CLAIM_REVIEW_POLICY_NAME, EVIDENCE_CLAIM_REVIEW_POLICY_VERSION, EVIDENCE_CLAIM_REVIEW_SCHEMA_VERSION, EvidenceClaimReviewInputSchema, EvidenceClaimReviewManifestSchema, EvidenceClaimReviewSchema, } from "./evidence-claim-review-schemas.js";
...
export async function readEvidenceClaimReviewInputFile(filePath) {
    return EvidenceClaimReviewInputSchema.parse(await readJson(path.resolve(filePath), null));
}
export async function createEvidenceClaimReview(workspace, claimId, rawInput, options = {}) {
    ...
    if (!claim)
        throw new Error(`Unknown Evidence Foundation claim ID: ${claimId}`);
    ...
    if (effective && input.supersedesReviewId !== effective.review.id) {
        throw new Error(`Claim ${claimId} already has effective review ${effective.review.id}; ` +
    ...
    if (await pathExists(paths.rootPath)) {
        ...
        if (status.status !== "current" && status.status !== "superseded") {
            throw new Error(`Evidence claim review identity exists but is ${status.status}; it was not overwritten.`);
        }
...
export async function showEvidenceClaimReview(workspace, claimId) {
    const reviews = await loadClaimReviewVersions(workspace, claimId);
    const effective = effectiveReviewFrom(reviews);
    if (!effective)
        throw new Error(`No effective evidence claim review exists for: ${claimId}`);
...
export async function getEvidenceClaimReviewStatus(workspace, claimId, requestedReviewId) {
    const versions = await discoverClaimReviewPaths(workspace, claimId);
    if (versions.length === 0)
        return missingStatus(claimId);
    ...
    const claim = foundation.claimById.get(claimId);
    if (!claim)
        return invalidStatus(base, ["Reviewed source claim no longer exists."]);
    let dependencies;
    try {
    ...
    const claimHashMatches = review.claimRecordSha256 === dependencies.claimRecordSha256 &&
        manifest.claimRecordSha256 === dependencies.claimRecordSha256 &&
        review.reviewedClaimSha256 === hashText(claim.claim);
    const evidenceHashMatches = review.evidenceInventorySha256 === dependencies.evidenceInventorySha256 &&
        manifest.evidenceInventorySha256 === dependencies.evidenceInventorySha256;
    const provenanceHashMatches = review.provenanceInventorySha256 === dependencies.provenanceInventorySha256 &&
        manifest.provenanceInventorySha256 === dependencies.provenanceInventorySha256;
    const policyMatches = review.policy.name === EVIDENCE_CLAIM_REVIEW_POLICY_NAME &&
    ...
    const identityMatches = review.id === base.reviewId &&
...
export async function loadEffectiveEvidenceClaimReviews(workspace) {
    const entries = await listEvidenceClaimReviews(workspace);
    const result = new Map();
    for (const entry of entries) {
        if (!entry.effectiveReviewId || entry.status !== "current")
            continue;
...
export function evidenceClaimReviewPaths(workspace, claimId, reviewId) {
    const safeClaimId = safeSegment(claimId);
    if (safeClaimId !== claimId)
...
export function formatEvidenceClaimReviewResult(result) {
    return [
...
export function formatEvidenceClaimReviewStatus(status) {
    return [
...
export function formatEvidenceClaimReviewList(entries) {
    const reviewed = entries.filter(({ effectiveReviewId }) => effectiveReviewId);
    if (reviewed.length === 0)
        return `Evidence claim reviews: none (${entries.length} unreviewed claims)`;
    return [
...
function validateReviewInput(input, claim, evidence, sources) {
    ...
    if (evidence.length === 0)
        throw new Error("Claim has no supporting evidence to review.");
    if (sources.length === 0 || evidence.some((item) => item.sourceIds.length === 0)) {
        throw new Error("Claim provenance is incomplete.");
    }
    const hardPrivate = evidence.some((item) => ["private", "do_not_use", "sensitive"].includes(item.visibility) ||
        item.sensitivityFlags.length > 0) ||
        sources.some((source) => ["private", "do_not_use", "sensitive"].includes(source.visibility));
    ...
    const resumeReadyAllowed = approved &&
        sufficientSupport &&
        defensibleScope &&
        input.publicSafety === "public-safe" &&
        !hasCriticalRisk;
    if (input.resumeReadiness === "resume-ready" && !resumeReadyAllowed) {
        throw new Error("Resume-ready requires approval, sufficient support, defensible scope, public safety, and no critical risk.");
    }
...
function validateCorrectedClaim(corrected, claim, evidence) {
    ...
    const unsupported = significantTokens(corrected).filter((token) => !sourceTokens.has(token) && !CORRECTION_GLUE_WORDS.has(token));
    if (unsupported.length > 0) {
        throw new Error(`Corrected claim introduces unsupported facts: ${[...new Set(unsupported)].sort().join(", ")}`);
    }
...
function validateClassification(input, claim, evidence) {
    const hasProject = Boolean(claim.parentProjectId) || evidence.some((item) => Boolean(item.parentProjectId) || item.category === "project");
    const hasEmployment = Boolean(claim.parentRoleId) || evidence.some((item) => Boolean(item.parentRoleId) || item.category === "role");
...
function validateMetricReview(input, claim, evidence) {
    ...
    if (metric.qualifiers.some((qualifier) => !normalized(metric.exactText).includes(normalized(qualifier)))) {
        throw new Error("Verified metric qualifiers must be preserved in exact wording.");
    }
    const directlySupported = evidence.some((item) => normalized(item.text).includes(normalized(metric.exactText)) ||
...
function reviewDependencies(claim, foundation) {
    const evidence = [...claim.supportingEvidenceIds].sort().map((id) => {
        const item = foundation.evidenceById.get(id);
        if (!item)
            throw new Error(`Claim references unknown evidence item: ${id}`);
        return item;
    });
    ...
    const sources = sourceIds.map((id) => {
        ...
        if (!source)
            throw new Error(`Evidence references unknown source: ${id}`);
        return source;
    });
...
async function loadFoundation(workspace) {
    const [rawClaims, rawEvidence, rawSources] = await Promise.all([
        readJson(resolveWithin(workspace, "kb/claims.json"), []),
        readJson(resolveWithin(workspace, "kb/evidence-items.json"), []),
        readJson(resolveWithin(workspace, "kb/sources.json"), []),
    ]);
...
async function discoverClaimReviewPaths(workspace, claimId) {
    ...
    return files.map((file) => evidenceClaimReviewPaths(workspace, claimId, path.basename(path.dirname(file)))).sort((left, right) => left.rootRelativePath.localeCompare(right.rootRelativePath));
}
async function loadClaimReviewVersions(workspace, claimId) {
...
async function loadReviewAt(paths) {
    return {
        review: EvidenceClaimReviewSchema.parse(await readJson(paths.reviewPath, null)),
        manifest: EvidenceClaimReviewManifestSchema.parse(await readJson(paths.manifestPath, null)),
    };
}
function effectiveReviewFrom(reviews) {
...
async function findSupersedingReview(workspace, claimId, reviewId) {
    const reviews = await loadClaimReviewVersions(workspace, claimId);
    return reviews.find(({ review }) => review.supersedesReviewId === reviewId)?.review.id;
}
function reviewIdentityInput(input, claim, dependencies) {
    return {
        schemaVersion: EVIDENCE_CLAIM_REVIEW_SCHEMA_VERSION,
        policy: {
            name: EVIDENCE_CLAIM_REVIEW_POLICY_NAME,
...
function reviewSemanticFromInput(input) {
    return {
        decision: input.decision,
...
function reviewSemanticContent(review) {
    return {
        decision: review.decision,
...
function resultFor(review, paths, result) {
    return {
        reviewId: review.id,
...
function missingStatus(claimId) {
    return {
        claimId,
        reviewExists: false,
...
function significantTokens(value) {
    return normalized(value).split(" ").filter((token) => token.length > 2 && !TOKEN_STOP_WORDS.has(token) && !/^\d/.test(token));
}
