...
import { getLatestChanges, getWorkspaceStatus } from "./update-impact.js";
export async function projectCareerTwin(workspace) {
    const [profile, sources, evidence, claims, publicProfile, latestRefresh, workspaceStatus, targets, outputManifest] = await Promise.all([
        ...
        readJson(path.join(workspace, "outputs/output-manifest.json"), {
            schemaVersion: 1,
    ...
    for (const status of reviewBatches.filter((entry) => entry.status === "current")) {
        ...
        const unresolved = batch.claims.filter((entry) => entry.selectedForControlledReview && !effectiveReviews.has(entry.claimId));
        if (unresolved.length > 0 && !advancedReviewBatchId)
            advancedReviewBatchId = batch.id;
    ...
    const approved = reviewed.filter(({ review }) => ["approved", "approved-with-qualifier"].includes(review.decision));
    const resumeReady = approved.filter(({ review }) => review.publicSafety === "public-safe" && review.resumeReadiness === "resume-ready");
    ...
    const readyForOutputs = resumeReady.length > 0 || currentOutputs.some((entry) => entry.publicationStatus === "final" || entry.publicationStatus === "export");
    const trustLabel = readyForOutputs
    ...
    return {
        ...
        questions: questions.sort((left, right) => priorityRank(left.priority) - priorityRank(right.priority) || left.title.localeCompare(right.title)),
        targets,
...
async function loadCareerProfile(workspace) {
    ...
    return CareerProfileSchema.parse(await readJson(profilePath, null));
}
async function loadArray(workspace, relativePath, schema) {
    return schema.array().parse(await readJson(path.join(workspace, relativePath), []));
}
function toOutputProjection(entry) {
...
function clarificationQuestion(claim) {
    if (claim.metricStatus === "needs_metric")
        return `Is the number in “${deterministicTitle(claim.claim)}” exact and safe to use?`;
    if (claim.type === "project_claim")
        return `Is “${deterministicTitle(claim.claim)}” project work, employment responsibility, or both?`;
    return `Can this statement be confirmed and used in a public resume: “${deterministicTitle(claim.claim)}”?`;
}
function humanSelectionReason(values) {
    const value = values[0];
    if (!value)
        return "This detail could materially affect a current output.";
    return value.replaceAll("_", " ").replace(/\.$/, "") + ".";
}
function deterministicTitle(value, limit = 92) {
