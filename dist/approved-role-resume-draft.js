...
import { ApprovedRoleResumeDraftManifestSchema, ApprovedRoleResumeDraftSchema, RoleResumeDraftProposalManifestSchema, RoleResumeDraftReviewManifestSchema, } from "./role-resume-draft-schemas.js";
import { buildClaimLedger, buildEvidenceUsage, getRoleResumeDraftProposalStatus, showRoleResumeDraftProposal, validateRoleResumeDraftPayload, } from "./role-resume-draft-proposal.js";
import { getRoleResumeDraftReviewStatus, mergeReviewedRoleResumeDraft, roleResumeDraftReviewPaths, showRoleResumeDraftReview, } from "./role-resume-draft-review.js";
...
import { evaluateRoleResumeDraftAgainstComposition, getRoleResumeCompositionStatus, } from "./role-resume-composition.js";
export async function approveRoleResumeDraftProposal(workspace, proposalId, options = {}) {
    ...
    if (scaffoldStatus.status !== "current")
        throw new Error("Role resume draft scaffold must be current before approval.");
    ...
    const proposalManifest = RoleResumeDraftProposalManifestSchema.parse(await readJson(proposalPaths.manifestPath, null));
    const reviewManifest = RoleResumeDraftReviewManifestSchema.parse(await readJson(reviewPaths.manifestPath, null));
    ...
    if (existingStatus.status === "current" && await pathExists(paths.manifestPath)) {
        ...
        if (manifest.proposalId === proposalId &&
            ...
            return approvalResult(await showApprovedRoleResumeDraft(workspace, proposal.targetId), review, proposalId, paths, "already-current");
        }
    }
    ...
    const validated = validateRoleResumeDraftPayload({ sections: reviewed.sections, warnings: proposal.warnings, ambiguities: reviewed.ambiguities }, proposal.id, scaffold, context, scaffoldSha256);
    const blocking = validated.validationIssues.filter((entry) => entry.severity === "critical" || entry.severity === "high");
    if (blocking.length)
        throw new Error(`Approved draft contains ${blocking.length} blocking validation issue(s).`);
    ...
    const completeness = deriveRoleResumeDraftCompleteness(scaffold, context.composition, sections, claimLedger, risks, validated.ambiguities);
    ...
    if (await pathExists(paths.draftPath)) {
        try {
            createdAt = ApprovedRoleResumeDraftSchema.parse(await readJson(paths.draftPath, null)).createdAt;
        }
        catch {
    ...
    return approvalResult(approved, review, proposalId, paths, existingStatus.status === "missing" ? "created" : "rebuilt");
}
export async function showApprovedRoleResumeDraft(workspace, targetId) {
    const paths = approvedRoleResumeDraftPaths(workspace, targetId);
    if (!(await pathExists(paths.draftPath)))
        throw new Error(`Approved role resume draft not found: ${targetId}`);
    return ApprovedRoleResumeDraftSchema.parse(await readJson(paths.draftPath, null));
}
export async function getApprovedRoleResumeDraftStatus(workspace, targetId) {
    const paths = approvedRoleResumeDraftPaths(workspace, targetId);
    ...
    if (!draftExists && !manifestExists)
        return emptyStatus(base, "missing", ["No approved role resume draft exists."]);
    if (!draftExists || !manifestExists)
        return emptyStatus(base, "invalid", ["Approved draft artifact set is incomplete."]);
    let draft;
    let manifest;
    try {
        draft = ApprovedRoleResumeDraftSchema.parse(await readJson(paths.draftPath, null));
        manifest = ApprovedRoleResumeDraftManifestSchema.parse(await readJson(paths.manifestPath, null));
...
export function deriveRoleResumeDraftCompleteness(scaffold, composition, sections, claimLedger, risks, ambiguities) {
    ...
    const status = compositionEvaluation.status === "blocked"
        ? "blocked"
...
export function assertApprovedRoleResumeDraft(draft) {
    ...
    if (draft.draftingPolicy.name !== ROLE_RESUME_DRAFTING_POLICY_NAME ||
...
export function approvedRoleResumeDraftPaths(workspace, targetId) {
...
export function formatApproveRoleResumeDraftResult(result) {
    return [
...
export function formatApprovedRoleResumeDraftStatus(status) {
    return [
...
function applyReviewTrust(item, proposal, review) {
    ...
    const edited = direct ?? review.decisions.find((entry) => {
        if (entry.itemType !== "draft-item" || entry.decision !== "edit" || !entry.editedValue)
            return false;
        const value = entry.editedValue;
    ...
    const trustState = edited.decision === "edit" ? "human-edited" : "human-approved";
    return {
        ...item,
        trustState,
        provenance: {
...
function deriveApprovedDraftRisks(sections, validationIssues, evidenceUsage) {
    const risks = validationIssues.map((entry) => ({
    ...
    for (const ids of duplicate.values())
        if (ids.length > 1) {
            risks.push({
...
function approvalResult(draft, review, proposalId, paths, result) {
    const items = draft.sections.flatMap((entry) => entry.items);
    return {
