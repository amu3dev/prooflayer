...
async function mapDependencyChecks(workspace, map, manifest, requirementInput, targetSha256, sourceSha256) {
    ...
    if (isSnapshotBacked) {
        try {
            ...
            if (pinned.manifest.pinSha256 !== manifest.evidencePinSha256 ||
                ...
                pinned.snapshot.snapshot.eligibleJobEvidenceSetSha256 !==
                    manifest.eligibleEvidenceSetSha256) {
                snapshotReasons.push("Pinned Evidence Snapshot dependencies changed.");
            }
        }
        catch (error) {
            snapshotReasons.push(`Pinned Evidence Snapshot is unavailable: ${errorMessage(error)}`);
        }
    }
...
async function validatePinnedEvidenceProvenance(workspace, map) {
    let pinned;
    try {
    ...
    for (const link of map.links) {
        ...
        if (evidenceRecord.contentSha256 !==
            link.evidenceProvenance.evidenceItemSha256 ||
        ...
        if (!evidenceRecord.eligibility.jobMapping ||
            ...
            !claim.publicSafe ||
            claim.needsConfirmation) {
            reasons.push(`Evidence link does not reference eligible pinned evidence: ${link.id}`);
        }
    }
    return uniqueSorted(reasons);
}
