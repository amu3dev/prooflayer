...
export function renderRoleResumeMarkdown(document) {
    const sections = document.sections.map((section) => renderMarkdownSection(section));
    return `${sections.filter(Boolean).join("\n\n").trim()}\n`;
}
export function renderRoleResumeHtml(document) {
...
export function canonicalVisibleSegments(document) {
    return document.sections.flatMap((section) => [
        ...(section.heading ? [section.heading] : []),
        ...section.blocks.map((block) => block.text),
    ]);
}
export function canonicalVisibleText(document) {
    return normalizeVisibleText(canonicalVisibleSegments(document).join("\n"));
}
export function extractVisibleTextFromMarkdown(markdown) {
    const lines = markdown.replace(/\r\n?/g, "\n").split("\n");
    return normalizeVisibleText(lines.map((line) => line
        .replace(/^#{1,6}\s+/, "")
        .replace(/^[-*+]\s+/, "")
        .replace(/\\([\\`*_[\]{}()#+.!>|-])/g, "$1")).join("\n"));
}
...
export function visibleTextEquivalent(document, extractedText) {
    return canonicalVisibleText(document) === normalizeVisibleText(extractedText);
}
export function firstAndLastMarkers(document) {
...
function renderHtmlSection(section) {
    const content = [];
    if (section.heading)
        content.push(`<h2>${escapeHtml(section.heading)}</h2>`);
    let bullets = [];
    const flushBullets = () => {
        if (!bullets.length)
            return;
...
function stableBlankLines(lines) {
    return lines
        .join("\n\n")
        .replace(/\n{3,}/g, "\n\n")
        .trim();
