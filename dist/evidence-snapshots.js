...
export async function buildEvidenceSnapshot(workspace, options = {}) {
    ...
    if (await pathExists(paths.rootPath)) {
        ...
        if (status.status !== "current") {
            throw new Error(`Evidence snapshot path already exists but is ${status.status}; immutable snapshots are never overwritten. ${status.reasons.join(" ")}`);
        }
        const loaded = await loadEvidenceSnapshot(workspace, calculated.id);
        if (stableJson(loaded.snapshot) !== stableJson(calculated)) {
            throw new Error(`Evidence snapshot identity collision at ${calculated.id}; stored content differs and was not overwritten.`);
        }
        return resultFor(loaded.snapshot, loaded.manifest, paths, "already-current");
    }
    ...
    const status = await getEvidenceSnapshotStatus(workspace, calculated.id);
    if (status.status !== "current") {
        throw new Error(`Created evidence snapshot failed validation: ${status.reasons.join(" ")}`);
    }
    return resultFor(calculated, manifest, paths, "created");
}
export async function calculateEvidenceFoundationSnapshot(workspace) {
    const sourceArtifacts = [
        { id: "sources", path: "kb/sources.json" },
        { id: "evidence-items", path: "kb/evidence-items.json" },
        { id: "claims", path: "kb/claims.json" },
    ];
    for (const artifact of sourceArtifacts) {
        if (!(await pathExists(resolveWithin(workspace, artifact.path)))) {
            throw new Error(`Required Evidence Foundation artifact is missing: ${artifact.path}`);
        }
    }
    ...
    const sourceInventorySha256 = hashText(stableJson(artifacts.map(({ id, sha256 }) => ({ id, sha256 }))));
    const sources = (await readJson(resolveWithin(workspace, "kb/sources.json"), [])).map((entry) => SourceSchema.parse(entry)).sort(byId);
    const evidenceItems = (await readJson(resolveWithin(workspace, "kb/evidence-items.json"), [])).map((entry) => EvidenceItemSchema.parse(entry)).sort(byId);
    const claims = (await readJson(resolveWithin(workspace, "kb/claims.json"), [])).map((entry) => ClaimSchema.parse(entry)).sort(byId);
...
export async function getEvidenceSnapshotStatus(workspace, snapshotId) {
    ...
    let rawManifest;
    try {
        rawSnapshot = await readJson(paths.snapshotPath, null);
        rawManifest = await readJson(paths.manifestPath, null);
    }
    catch (error) {
        return emptyStatus(base, "invalid", [
    ...
    if (schemaVersionOf(rawSnapshot) !== EVIDENCE_SNAPSHOT_SCHEMA_VERSION ||
    ...
    const manifestMatches = manifest.snapshotId === snapshot.id &&
        ...
        manifest.files.some((file) => file.path === paths.snapshotRelativePath && file.sha256 === contentHash);
    const reasons = [
        ...(!contentHashMatches
...
export async function loadEvidenceSnapshot(workspace, snapshotId) {
    const status = await getEvidenceSnapshotStatus(workspace, snapshotId);
    if (status.status !== "current") {
        throw new Error(`Evidence snapshot ${snapshotId} is ${status.status}: ${status.reasons.join(" ")}`);
    }
    const paths = evidenceSnapshotPaths(workspace, snapshotId);
    const snapshot = EvidenceFoundationSnapshotSchema.parse(await readJson(paths.snapshotPath, null));
    const manifest = EvidenceSnapshotManifestSchemaV1.parse(await readJson(paths.manifestPath, null));
    return {
...
export async function validateEvidenceSnapshot(workspace, snapshotId) {
    const status = await getEvidenceSnapshotStatus(workspace, snapshotId);
    if (status.status !== "current") {
        throw new Error(`Evidence snapshot validation failed: ${status.status}. ${status.reasons.join(" ")}`);
    }
    return status;
}
export async function listEvidenceSnapshots(workspace) {
    const root = resolveWithin(workspace, SNAPSHOT_ROOT);
    const files = (await walkFiles(root)).filter((file) => path.basename(file) === MANIFEST_FILE);
    ...
    return Promise.all(ids.map(async (snapshotId) => {
        const status = await getEvidenceSnapshotStatus(workspace, snapshotId);
        if (status.status !== "current")
            return { snapshotId, status: status.status };
...
export function evidenceSnapshotPaths(workspace, snapshotId) {
    assertSnapshotId(snapshotId);
...
export function formatEvidenceSnapshotBuild(result) {
    return [
...
export function formatEvidenceSnapshotList(entries) {
    if (entries.length === 0)
        return "Evidence snapshots: none";
    return [
        "Evidence snapshots:",
        ...entries.map((entry) => [
            entry.snapshotId,
...
export function formatEvidenceSnapshotStatus(status) {
...
function resultFor(snapshot, manifest, paths, result) {
    return {
...
function claimEligibilityReasons(claim) {
    return [
        ...
        ...(claim.outputReadiness !== "resume_ready"
            ...
            : []),
        ...(!claim.publicSafe ? ["claim-not-public-safe"] : []),
        ...(claim.needsConfirmation ? ["claim-needs-confirmation"] : []),
    ];
}
function snapshotWarning(code, message, recordIds) {
    return {
        id: `evidence-snapshot-warning_${hashText(stableJson({ code, recordIds })).slice(0, 12)}`,
        code,
        message,
        recordIds,
    };
}
function validateSnapshotContents(snapshot) {
    const hashReasons = [];
    const eligibilityReasons = [];
    const provenanceReasons = [];
    const evidenceIds = new Set();
    const claimIds = new Set();
    const sourceIds = new Set(snapshot.evidenceItems.flatMap((record) => record.sources.map(({ sourceId }) => sourceId)));
    for (const record of snapshot.evidenceItems) {
        ...
        if (record.sourceIds.some((sourceId) => !record.sources.some((source) => source.sourceId === sourceId))) {
    ...
    if (snapshot.eligibleRoleEvidenceSetSha256 !== eligibilitySetHash(snapshot.evidenceItems, snapshot.claims, eligibleRoleEvidence, eligibleRoleClaims))
        eligibilityReasons.push("Eligible Role evidence set hash is invalid.");
    if (snapshot.eligibleJobEvidenceSetSha256 !== eligibilitySetHash(snapshot.evidenceItems, snapshot.claims, eligibleJobEvidence, eligibleJobClaims))
