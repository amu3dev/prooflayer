...
import { JOB_FIT_PROOF_ASSESSMENT_ANALYZER_NAME, JOB_FIT_PROOF_ASSESSMENT_POLICY_NAME, buildJobFitProofAssessment, classifyJobRequirementAssessment, classifyOverallJobAssessment, getJobFitProofAssessmentStatus, jobFitProofAssessmentPaths, showJobFitProofAssessment, } from "../job-fit-proof-assessment.js";
...
describe("Slice 2.7D deterministic Job Fit and Proof Assessment", () => {
    ...
    it("builds one provenance-bearing assessment for every coverage entry", async () => {
        ...
        const result = await buildJobFitProofAssessment(fixture.workspace, fixture.targetId, { now: () => new Date(FIRST_TIME) });
        const assessment = await showJobFitProofAssessment(fixture.workspace, fixture.targetId);
        const paths = jobFitProofAssessmentPaths(fixture.workspace, fixture.targetId);
        const coveragePaths = jobCoveragePaths(fixture.workspace, fixture.targetId);
        expect(result).toMatchObject({
            result: "created",
            assessmentPath: `targets/jobs/${fixture.targetId}/assessment/deterministic/job-fit-proof-assessment.json`,
        });
        ...
        const coverage = JSON.parse(await readFile(coveragePaths.coveragePath, "utf8"));
        expect(assessment.requirementAssessments).toHaveLength(coverage.requirements.length);
        expect(new Set(assessment.requirementAssessments.map((entry) => entry.requirementId))).toEqual(new Set(coverage.requirements.map((entry) => entry.requirementId)));
        for (const entry of assessment.requirementAssessments) {
            const source = coverage.requirements.find((candidate) => candidate.requirementId === entry.requirementId);
        ...
        expect(serialized).not.toMatch(/"fitScore"|"coveragePercentage"|"hiringProbability"|"applicationRecommendation"/);
    ...
    it("surfaces mandatory gaps, limited proof, and preserved ambiguity safely", async () => {
        ...
        const assessment = await showJobFitProofAssessment(fixture.workspace, fixture.targetId);
        expect(assessment.risks.some((risk) => risk.code === "CRITICAL_MANDATORY_REQUIREMENT_UNSUPPORTED" ||
            risk.code === "MANDATORY_REQUIREMENT_PARTIAL")).toBe(true);
        expect(assessment.requirementAssessments.every((entry) => entry.assessmentStatement.length > 20)).toBe(true);
    ...
    it("does not rewrite unchanged current artifacts and preserves stable IDs", async () => {
        ...
        const result = await buildJobFitProofAssessment(fixture.workspace, fixture.targetId);
        const after = await showJobFitProofAssessment(fixture.workspace, fixture.targetId);
        expect(result.result).toBe("already-current");
        expect(await readFile(paths.assessmentPath, "utf8")).toBe(before.bytes);
        expect(await readFile(paths.manifestPath, "utf8")).toBe(before.manifestBytes);
        expect((await stat(paths.assessmentPath)).mtimeMs).toBe(before.assessmentStat.mtimeMs);
        ...
        expect(after.requirementAssessments.map((entry) => entry.id)).toEqual(before.value.requirementAssessments.map((entry) => entry.id));
    });
    it("reports missing, current, stale, and invalid lifecycle states", async () => {
        const fixture = await assessmentWorkspace();
        expect((await getJobFitProofAssessmentStatus(fixture.workspace, fixture.targetId)).status).toBe("missing");
        await buildJobFitProofAssessment(fixture.workspace, fixture.targetId);
        expect(await getJobFitProofAssessmentStatus(fixture.workspace, fixture.targetId)).toMatchObject({
            status: "current",
            assessmentHashMatches: true,
            coverageStatus: "current",
        });
        const coveragePaths = jobCoveragePaths(fixture.workspace, fixture.targetId);
        await writeFile(coveragePaths.coveragePath, `${await readFile(coveragePaths.coveragePath, "utf8")} `, "utf8");
        expect((await getJobFitProofAssessmentStatus(fixture.workspace, fixture.targetId)).status).toBe("stale");
        ...
        await writeFile(paths.assessmentPath, `${await readFile(paths.assessmentPath, "utf8")} `, "utf8");
        expect((await getJobFitProofAssessmentStatus(fresh.workspace, fresh.targetId)).status).toBe("invalid");
    });
    it("rejects Role Targets and missing or stale coverage", async () => {
        ...
        await expect(buildJobFitProofAssessment(workspace, role.target.id)).rejects.toThrow("rejects Role Target");
        ...
        await expect(buildJobFitProofAssessment(workspace, job.target.id)).rejects.toThrow("requires current Job Requirement Coverage");
        const fixture = await assessmentWorkspace();
        const coveragePaths = jobCoveragePaths(fixture.workspace, fixture.targetId);
        await writeFile(coveragePaths.coveragePath, `${await readFile(coveragePaths.coveragePath, "utf8")} `, "utf8");
        await expect(buildJobFitProofAssessment(fixture.workspace, fixture.targetId)).rejects.toThrow("requires current Job Requirement Coverage");
    });
});
...
function assessed(assessmentState, proofStrength, requirementId = "requirement") {
    return {
        requirementId,
        necessity: "mandatory",
        assessmentState,
