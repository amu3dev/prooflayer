...
export async function createExplicitFixtureReviews(workspace, now) {
    const [claims, evidence, sources] = await Promise.all([
        readJsonFile(path.join(workspace, "kb/claims.json")),
        readJsonFile(path.join(workspace, "kb/evidence-items.json")),
        readJsonFile(path.join(workspace, "kb/sources.json")),
    ]);
    ...
    for (const claim of claims) {
        ...
        if (supporting.some((entry) => !entry))
            continue;
        const sourceRecords = supporting.flatMap((entry) => entry.sourceIds)
            .map((id) => sourceById.get(id));
        if (supporting.some((entry) => ["private", "do_not_use", "sensitive"].includes(entry.visibility) ||
            entry.sensitivityFlags.length > 0) ||
            sourceRecords.some((entry) => !entry || ["private", "do_not_use", "sensitive"].includes(entry.visibility)))
            continue;
