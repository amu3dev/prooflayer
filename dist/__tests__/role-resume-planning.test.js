...
import { ROLE_RESUME_PLAN_PROMPT_TEMPLATE_ID, generateRoleResumePlanProposal, getRoleResumePlanProposalStatus, replayRoleResumePlanProposal, showRoleResumePlanProposal, } from "../role-resume-plan-proposal.js";
import { completeRoleResumePlanReview, getRoleResumePlanReviewStatus, initializeRoleResumePlanReview, setRoleResumePlanReviewDecision, } from "../role-resume-plan-review.js";
import { ROLE_RESUME_PLANNING_POLICY_NAME, ROLE_RESUME_PLANNING_POLICY_VERSION, buildRoleResumePlan, derivePositioningScope, getRoleResumePlanStatus, showRoleResumePlan, } from "../role-resume-planning.js";
...
describe("Slice 2.6A role resume content planning", () => {
    it("builds a deterministic plan from current approved role artifacts using conservative ordered rules", async () => {
        ...
        expect(plan.sections.filter((entry) => entry.status !== "exclude").map((entry) => entry.order)).toEqual(plan.sections.filter((entry) => entry.status !== "exclude").map((_, index) => index));
    ...
    it("maps all positioning scopes without turning scope into an employability judgment", async () => {
        ...
        if (assessment.summary.mode !== "role-positioning")
            throw new Error("Expected role summary.");
    ...
    it("allows quantified outcomes only when approved resume-ready evidence contains a verified metric", async () => {
        ...
        expect((await showRoleResumePlan(withoutMetric.workspace, withoutMetric.targetId)).claimBoundaries.every((entry) => !entry.allowedClaimTypes.includes("quantified-outcome"))).toBe(true);
        ...
        expect(plan.claimBoundaries.filter((entry) => entry.expectationId !== platformId).every((entry) => !entry.allowedClaimTypes.includes("quantified-outcome"))).toBe(true);
    });
    ...
    it.each([
        ["finished summary", (payload) => { payload.positioning.audience.notes = ["Results-driven Engineering Manager with platform expertise."]; }],
        ["resume bullet", (payload) => { payload.sections[0].emphasisNotes = ["Led cross-functional teams to deliver scalable platforms."]; }],
        ["invented metric", (payload) => { payload.sections[0].emphasisNotes = ["Plan around 73% growth."]; }],
        ["team size", (payload) => { payload.sections[0].emphasisNotes = ["Managed 42 engineers."]; }],
        ["ATS score", (payload) => { payload.sections[0].emphasisNotes = ["ATS score is suitable."]; }],
        ["application advice", (payload) => { payload.sections[0].emphasisNotes = ["Recommended application path."]; }],
        ["hiring probability", (payload) => { payload.sections[0].emphasisNotes = ["Hiring probability is high."]; }],
        ["job-specific tailoring", (payload) => { payload.sections[0].emphasisNotes = ["Tailor this plan to the job description."]; }],
        ["invented employer", (payload) => { payload.sections[0].emphasisNotes = ["Worked for ExampleCo on platform delivery."]; }],
        ["invented technology", (payload) => { payload.sections[0].emphasisNotes = ["Prioritize kubernetes expertise."]; }],
        ["invented scope", (payload) => { payload.sections[0].emphasisNotes = ["Claim enterprise-wide ownership."]; }],
        ["unknown expectation", (payload) => { payload.expectationSelections[0].expectationId = "unknown-expectation"; }],
        ["unknown assessment", (payload) => { payload.expectationSelections[0].assessmentId = "unknown-assessment"; }],
        ["unknown match", (payload) => { payload.expectationSelections[0].approvedMatchIds = ["unknown-match"]; }],
        ["unknown evidence", (payload) => { payload.evidenceSelections[0].evidenceId = "unknown-evidence"; }],
...
async function planningFixture(options = {}) {
...
async function writeEvidenceKb(workspace, count, verifiedMetricIndex) {
    const sources = [];
    const evidence = [];
    const claims = [];
...
async function writeMatchingPattern(workspace, targetId, includeNotAssessed) {
    ...
    context.eligibleExpectations.forEach((expectation, index) => {
        ...
        else if (expectation.statement.startsWith("Apply platform domain"))
            matches.push(matchFixture(context, expectation.id, evidence.evidenceId, "historical", "direct", "full", "medium", "historical", "medium"));
        else if (includeNotAssessed)
...
async function modelPayload(workspace, targetId) {
...
function decisionCounts(values) {
