...
import { approveEvidenceMatchProposal, } from "../approved-evidence-matching.js";
import { approveInterpretationProposal, } from "../approved-interpretation.js";
import { addManualEvidenceMatch, calculateEvidenceSnapshot, coverageId, deriveCoverage, expectationProvenance, getApprovedEvidenceMatchingStatus, listManualEvidenceMatches, loadMatchingContext, manualMatchId, matchingCompleteness, removeManualEvidenceMatch, showApprovedEvidenceMatching, } from "../evidence-matching.js";
import { generateEvidenceMatchProposal, getEvidenceMatchProposalStatus, matchProposalFileTimestamps, renderEvidenceMatchPrompt, replayEvidenceMatchProposal, showEvidenceMatchProposal, } from "../evidence-match-proposal.js";
import { completeEvidenceMatchReview, initializeEvidenceMatchReview, setEvidenceCoverageReviewDecision, setEvidenceMatchReviewDecision, showEvidenceMatchReview, } from "../evidence-match-review.js";
...
describe("Slice 2.4 evidence matching", () => {
    ...
    it("excludes unreviewed, inactive, private, and job-description evidence", async () => {
        ...
        const claims = JSON.parse(await readFile(path.join(fixture.workspace, "kb/claims.json"), "utf8"));
        sources.push(sourceFixture({ id: "src_private", path: "sources/markdown/private.md", visibility: "private" }), sourceFixture({ id: "src_inactive", path: "sources/markdown/inactive.md", status: "ignored" }), sourceFixture({ id: "src_job", path: "sources/jobs/job.md", type: "job_description" }));
        evidence.push(evidenceFixture({ id: "evi_unreviewed" }), evidenceFixture({ id: "evi_private", sourceIds: ["src_private"] }), evidenceFixture({ id: "evi_inactive", sourceIds: ["src_inactive"] }), evidenceFixture({ id: "evi_job", sourceIds: ["src_job"] }));
        claims.push(claimFixture({ id: "claim_private", supportingEvidenceIds: ["evi_private"] }), claimFixture({ id: "claim_inactive", supportingEvidenceIds: ["evi_inactive"] }), claimFixture({ id: "claim_job", supportingEvidenceIds: ["evi_job"] }));
    ...
    ])("adds a valid %s manual match with provenance and stable identity", async (matchType, coverage, limitations) => {
        const fixture = await roleMatchingFixture();
        const context = await loadMatchingContext(fixture.workspace, fixture.targetId);
    ...
    it.each([
        ["unknown expectation", (payload) => { payload.proposedMatches[0].expectationId = "expectation_unknown"; }, "UNKNOWN_EXPECTATION_ID"],
        ["unknown evidence", (payload) => { payload.proposedMatches[0].evidenceIds = ["evi_unknown"]; }, "UNKNOWN_EVIDENCE_ID"],
        ["invented metric", (payload) => { payload.proposedMatches[0].rationale = "Delivered 9000 users."; }, "INVENTED_METRIC"],
        ["fit output", (payload) => { payload.proposedMatches[0].rationale = "The candidate is a strong fit."; }, "FORBIDDEN_CONTENT"],
        ["resume output", (payload) => { payload.proposedMatches[0].rationale = "Use this resume bullet."; }, "FORBIDDEN_CONTENT"],
    ])("rejects invalid model proposal content: %s", async (_name, mutate, issueCode) => {
        const fixture = await roleMatchingFixture();
        const payload = await validMatchPayload(fixture.workspace, fixture.targetId);
        mutate(payload);
        const result = await generateEvidenceMatchProposal(fixture.workspace, fixture.targetId, { provider: new FakeInterpretationModelProvider(JSON.stringify(payload)) });
...
async function approveTargetInterpretation(workspace, targetId) {
    ...
    if (!source)
        throw new Error("Fixture lacks a deterministic expectation.");
    const payload = {
...
async function writeEvidenceKb(workspace, evidenceId) {
...
async function validMatchPayload(workspace, targetId, matchCount = 1) {
    ...
    if (!expectation || !evidence)
        throw new Error("Matching fixture lacks eligible input.");
    const matchTypes = ["direct", "supporting", "partial"];
    return {
...
async function matchProposalFixture(matchCount) {
    const fixture = await roleMatchingFixture();
...
function manualInput(expectationId, evidenceId) {
    return {
        expectationId,
...
function editedMatch(evidenceId) {
    return {
        evidenceIds: [evidenceId],
        matchType: "partial",
        coverage: "partial",
...
function matchFixture(targetId, expectationId, evidenceId, context, provenance, matchType, coverage) {
    return {
