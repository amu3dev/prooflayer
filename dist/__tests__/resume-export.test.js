...
import { AI_PRODUCT_EXPORT_DOCX, AI_PRODUCT_EXPORT_MARKDOWN, AI_PRODUCT_EXPORT_MANIFEST, AI_PRODUCT_EXPORT_PDF, AI_PRODUCT_FINAL_RESUME, TPM_EXPORT_DOCX, TPM_EXPORT_MARKDOWN, TPM_EXPORT_MANIFEST, TPM_EXPORT_PDF, TPM_FINAL_RESUME, exportFinalResume, formatResumeExportStatus, getResumeExportStatus } from "../resume-export.js";
describe("Slice 1.8 final resume export", () => {
    it("refuses unsupported roles and a missing final resume", async () => {
        ...
        await expect(exportFinalResume(workspace, "fullstack")).rejects.toThrow("Resume export currently supports --role ai-product or --role tpm.");
        await expect(exportFinalResume(workspace, "ai-product")).rejects.toThrow(`Final resume is missing: ${AI_PRODUCT_FINAL_RESUME}`);
    });
    it.each([
    ...
    ])("fails clearly when %s is unavailable", async (missingTool, expectedMessage) => {
    ...
    it("exports immutable source content, registers artifacts, and detects source-hash staleness", async () => {
        ...
        const tpmBefore = new Map(await Promise.all(tpmPaths.map(async (file) => [file, await readFile(path.join(workspace, file), "utf8")])));
        ...
        const persistedManifest = JSON.parse(await readFile(path.join(workspace, AI_PRODUCT_EXPORT_MANIFEST), "utf8"));
        const outputManifest = JSON.parse(await readFile(path.join(workspace, "outputs/output-manifest.json"), "utf8"));
        ...
        for (const file of tpmPaths)
            expect(await readFile(path.join(workspace, file), "utf8")).toBe(tpmBefore.get(file));
...
function fakeExportOptions() {
    return {
        ...
        resolveTool: async (tool) => `/tools/${tool}`,
