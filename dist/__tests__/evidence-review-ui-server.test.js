...
import { listEvidenceClaimReviews, loadEffectiveEvidenceClaimReviews, } from "../evidence-claim-review.js";
...
describe("Astro Local Evidence Review UI routes", () => {
    ...
    it("shows one Level 1 action and creates the projected canonical review in one click", async () => {
        ...
        expect(valid.status).toBe(303);
        expect(valid.headers.get("location")).toMatch(new RegExp(`/review/${fixture.batchId}/claim/claim_review_ui_(?!3)`));
        const reviews = await listEvidenceClaimReviews(fixture.workspace);
        expect(reviews.filter(({ status }) => status === "current")).toEqual([
            expect.objectContaining({ claimId: "claim_review_ui_3", decision: "approved" }),
        ]);
        expect((await loadEffectiveEvidenceClaimReviews(fixture.workspace)).get("claim_review_ui_3")?.review).toMatchObject({
            decision: "approved",
    ...
    it("submits a Level 2 answer immediately without a second submit action", async () => {
        ...
        expect(valid.status).toBe(303);
        expect(valid.headers.get("location")).toMatch(new RegExp(`/review/${fixture.batchId}/claim/claim_review_ui_(?!3)`));
        expect((await loadEffectiveEvidenceClaimReviews(fixture.workspace)).get("claim_review_ui_3")?.review.classification.workContext).toBe("project");
    }, 20_000);
    ...
    it.each(["127.0.0.1", "localhost"])("accepts a browser-like same-origin POST on %s without mixing loopback authorities", async (host) => {
        ...
        const claimHtml = await claimResponse.text();
        expect(claimResponse.status).toBe(200);
        expect(claimHtml).toContain(`<form class="recommendation-confirm-form" method="post" action="/review/${fixture.batchId}/claim/claim_review_ui_3">`);
    ...
    it("accepts an opaque browser origin only with loopback navigation metadata and a claim-bound token", async () => {
        ...
        const missingToken = await postBrowserForm(claimUrl, nullOriginHeaders, new URLSearchParams(stringFields(validApprovedFields())).toString());
        expect(missingToken.status).toBe(403);
        const invalidToken = await postBrowserForm(claimUrl, nullOriginHeaders, new URLSearchParams({ ...Object.fromEntries(validBody), csrfToken: "b".repeat(64) }).toString());
        expect(invalidToken.status).toBe(403);
        const crossSite = await postBrowserForm(claimUrl, { ...nullOriginHeaders, "sec-fetch-site": "cross-site" }, validBody.toString());
        expect(crossSite.status).toBe(403);
        const unsafeMode = await postBrowserForm(claimUrl, { ...nullOriginHeaders, "sec-fetch-mode": "cors" }, validBody.toString());
        expect(unsafeMode.status).toBe(403);
        const unsafeDestination = await postBrowserForm(claimUrl, { ...nullOriginHeaders, "sec-fetch-dest": "empty" }, validBody.toString());
        ...
        for (const forwardedHeader of ["x-forwarded-host", "x-forwarded-proto", "x-forwarded-port"]) {
            const forwarded = await postBrowserForm(claimUrl, { ...nullOriginHeaders, [forwardedHeader]: "forged.invalid" }, validBody.toString());
            expect(forwarded.status).toBe(400);
        }
        const crossBatch = await postBrowserForm(`${server.origin}/review/evidence-review-batch_00000000000000000000/claim/claim_review_ui_3`, nullOriginHeaders, validBody.toString());
        expect(crossBatch.status).toBe(403);
        const crossClaim = await postBrowserForm(`${server.origin}/review/${fixture.batchId}/claim/claim_review_ui_1`, nullOriginHeaders, validBody.toString());
...
async function startUiServer(workspace, batchId, readOnly, host = "127.0.0.1") {
    ...
    const csrfToken = evidenceReviewUiClaimCsrfToken(TEST_CSRF_SECRET, batchId, "claim_review_ui_3");
...
function formWithClass(html, className) {
    const match = html.match(new RegExp(`<form[^>]*class="${className}"[^>]*>[\\s\\S]*?</form>`));
    if (!match)
        throw new Error(`Expected form with class ${className}.`);
    return match[0];
}
