...
describe("Local Evidence Review UI domain adapter", () => {
    ...
    it("derives session-stable CSRF tokens bound to one batch and claim", () => {
        ...
        expect(token).toBe(evidenceReviewUiClaimCsrfToken(secret, batchId, "claim_review_ui_1"));
        expect(token).not.toBe(evidenceReviewUiClaimCsrfToken(secret, batchId, "claim_review_ui_2"));
        expect(token).not.toBe(evidenceReviewUiClaimCsrfToken("b".repeat(64), batchId, "claim_review_ui_1"));
