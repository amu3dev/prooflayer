...
import { buildEvidenceReviewBatch, evidenceReviewBatchPaths, } from "../evidence-review-batch.js";
import { evidenceReviewWorkspacePaths, getEvidenceReviewWorkspaceStatus, renderEvidenceReviewWorkspace, showEvidenceReviewWorkspace, } from "../evidence-review-workspace.js";
...
describe("deterministic Evidence Review Workspace rendering", () => {
    it("renders a self-contained read-only index and claim workspaces from canonical JSON", async () => {
        ...
        const result = await renderEvidenceReviewWorkspace(fixture.workspace, fixture.batchId, { now: () => new Date(TIME_1) });
        const index = await showEvidenceReviewWorkspace(fixture.workspace, fixture.batchId);
        const claimMarkdown = await Promise.all(result.claimWorkspacePaths.map((entry) => readFile(path.join(fixture.workspace, entry), "utf8")));
        ...
        expect(claimMarkdown.some((markdown) => markdown.includes("Supported platform extensibility."))).toBe(true);
        expect(claimMarkdown.some((markdown) => markdown.includes("Nice to have") || markdown.includes("preferred"))).toBe(true);
        const workspacePaths = evidenceReviewWorkspacePaths(fixture.workspace, fixture.batchId);
        const manifest = JSON.parse(await readFile(workspacePaths.indexManifestPath, "utf8"));
        ...
        const linkedIds = [...index.matchAll(/\]\(\.\/(claim_[^)]+)\.review\.md\)/g)]
            .map((match) => match[1]);
        const batch = JSON.parse(await readFile(evidenceReviewBatchPaths(fixture.workspace, fixture.batchId).batchPath, "utf8"));
        ...
        const tamperedManifest = JSON.parse(await readFile(workspacePaths.indexManifestPath, "utf8"));
    ...
    it("returns already-current and preserves IDs, hashes, bytes, timestamps, and mtimes", async () => {
        const fixture = await workspaceFixture();
        const first = await renderEvidenceReviewWorkspace(fixture.workspace, fixture.batchId, { now: () => new Date(TIME_1) });
        ...
        const second = await renderEvidenceReviewWorkspace(fixture.workspace, fixture.batchId, { rebuild: true, now: () => new Date(TIME_2) });
    ...
    it("marks an older Markdown renderer version stale without changing canonical JSON", async () => {
        ...
        const before = await canonicalBytes(fixture.workspace, fixture.batchId);
        const paths = evidenceReviewWorkspacePaths(fixture.workspace, fixture.batchId);
        const manifest = JSON.parse(await readFile(paths.indexManifestPath, "utf8"));
    ...
    it("reports missing, stale, rebuilt, and invalid lifecycle without consuming Markdown edits", async () => {
        ...
        const rebuilt = await renderEvidenceReviewWorkspace(fixture.workspace, fixture.batchId, { rebuild: true, now: () => new Date(TIME_2) });
        ...
        await writeFile(path.join(fixture.workspace, rebuilt.claimWorkspacePaths[0]), "# Human edit that ProofLayer must ignore\n", "utf8");
        expect(await getEvidenceReviewWorkspaceStatus(fixture.workspace, fixture.batchId))
...
async function workspaceFixture() {
    ...
    const evidence = themes.map((text, index) => ({
    ...
    const claims = themes.map((text, index) => ({
...
async function canonicalBytes(workspace, batchId) {
    const batchPaths = evidenceReviewBatchPaths(workspace, batchId);
    const batch = JSON.parse(await readFile(batchPaths.batchPath, "utf8"));
    ...
    return {
        ...
        template: await readFile(path.join(batchPaths.templateRootPath, `${firstClaim}.json`)),
    };
}
