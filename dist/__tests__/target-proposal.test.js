...
import { approveInterpretationProposal, getApprovedInterpretationStatus, showApprovedTargetInterpretation, } from "../approved-interpretation.js";
import { FakeInterpretationModelProvider, OpenAICompatibleInterpretationModelProvider, loadModelProviderConfiguration, } from "../model-provider.js";
...
import { PROPOSAL_POLICY_VERSION, PROPOSAL_PROMPT_TEMPLATE_ID, PROPOSAL_PROMPT_TEMPLATE_VERSION, generateInterpretationProposal, getInterpretationProposalStatus, proposalFileTimestamps, replayInterpretationProposal, showInterpretationProposal, } from "../target-proposal.js";
import { completeProposalReview, initializeProposalReview, setProposalReviewDecision, showProposalReview, } from "../target-proposal-review.js";
...
describe("Slice 2.3B model-assisted proposal and review", () => {
    ...
    it("supports Role proposals only when a reviewed Role Profile backs deterministic interpretation", async () => {
        ...
        const payload = await validPayload(fixture.workspace, fixture.targetId);
        const result = await generateInterpretationProposal(fixture.workspace, fixture.targetId, { provider: new FakeInterpretationModelProvider(JSON.stringify(payload)) });
        ...
        await expect(generateInterpretationProposal(missingProfile.workspace, missingProfile.targetId, { provider: new FakeInterpretationModelProvider(JSON.stringify(payload)) })).rejects.toThrow("requires a current deterministic interpretation backed by a Role Profile");
    });
    ...
    it("preserves malformed output as validation-failed and never marks it reviewable", async () => {
        const fixture = await jobFixture();
        const result = await generateInterpretationProposal(fixture.workspace, fixture.targetId, { provider: new FakeInterpretationModelProvider("{not-json") });
    ...
    it("rejects unsupported source IDs, missing provenance, invalid tags, and unsupported enums", async () => {
        ...
        const unsupported = await generateInterpretationProposal(fixture.workspace, fixture.targetId, { provider: new FakeInterpretationModelProvider(JSON.stringify(payload)) });
        ...
        const noProvenance = await validPayload(fixture.workspace, fixture.targetId);
        noProvenance.proposedExpectations[0].sourceReferences = [];
        const invalidTag = await validPayload(fixture.workspace, fixture.targetId);
        invalidTag.proposedExpectations[0].capabilityTags = ["Not Slug Safe"];
        const invalidEnum = await validPayload(fixture.workspace, fixture.targetId);
        invalidEnum.proposedExpectations[0].importance = "urgent";
        for (const raw of [noProvenance, invalidTag, invalidEnum]) {
            const generated = await generateInterpretationProposal(fixture.workspace, fixture.targetId, { provider: new FakeInterpretationModelProvider(JSON.stringify(raw)), refresh: true });
    ...
    it("supports accept, reject, and edit decisions with strict review validation", async () => {
        ...
        await expect(setProposalReviewDecision(fixture.workspace, fixture.proposalId, proposal.proposedExpectations[0].id, { decision: "accept" })).rejects.toThrow("already exists");
        await expect(setProposalReviewDecision(fixture.workspace, fixture.proposalId, "proposed_not-real", { decision: "accept" })).rejects.toThrow("Unknown proposed expectation ID");
        await expect(setProposalReviewDecision(fixture.workspace, fixture.proposalId, proposal.proposedExpectations[2].id, { decision: "edit" })).rejects.toThrow();
        await setProposalReviewDecision(fixture.workspace, fixture.proposalId, proposal.proposedExpectations[2].id, { decision: "edit", editedExpectation: editedExpectation("Reviewed platform delivery qualification.") });
    ...
    it("approves deterministic, accepted, and edited expectations while excluding rejected items", async () => {
        ...
        const deterministicPath = path.join(fixture.workspace, `targets/jobs/${fixture.targetId}/interpretation/target-interpretation.json`);
    ...
    it("approval fails for incomplete review, stale proposal, and invalid proposal", async () => {
        ...
        const malformed = await generateInterpretationProposal(invalid.workspace, invalid.targetId, { provider: new FakeInterpretationModelProvider("[]") });
...
async function roleFixture(withProfile) {
...
async function validPayload(workspace, targetId) {
    ...
    if (analysis.targetType === "role")
        return rolePayload(interpretation);
    ...
    if (!responsibility || !qualification)
        throw new Error("Test fixture lacks required items.");
    ...
    if (!responsibilityExpectation || !qualificationExpectation)
        throw new Error("Test fixture lacks deterministic expectations.");
    return {
...
function rolePayload(interpretation) {
    const source = interpretation.expectations[0];
    if (!source)
        throw new Error("Role fixture lacks a deterministic expectation.");
    return {
        proposedExpectations: [{
                operation: "enrich",
...
function editedExpectation(statement) {
    return {
        kind: "qualification",
        statement,
        necessity: "required",
