...
import { createEvidenceClaimReview, evidenceClaimReviewPaths, getEvidenceClaimReviewStatus, listEvidenceClaimReviews, showEvidenceClaimReview, } from "../evidence-claim-review.js";
import { buildEvidenceReviewBatch, evidenceReviewBatchPaths, getEvidenceReviewBatchStatus, showEvidenceReviewBatch, } from "../evidence-review-batch.js";
import { buildEvidenceSnapshot, loadEvidenceSnapshot, } from "../evidence-snapshots.js";
...
describe("human-controlled Evidence Foundation review and eligibility", () => {
    ...
    it("creates an approved review, preserves source bytes, and projects eligibility", async () => {
        ...
        const first = await createEvidenceClaimReview(fixture.workspace, "claim_platform", approvedInput(fixture.claims[0]), { now: () => new Date(TIME_1) });
        ...
        const second = await createEvidenceClaimReview(fixture.workspace, "claim_platform", approvedInput(fixture.claims[0]), { now: () => new Date(TIME_2) });
        const snapshotResult = await buildEvidenceSnapshot(fixture.workspace);
        const { snapshot } = await loadEvidenceSnapshot(fixture.workspace, snapshotResult.snapshotId);
    ...
    it("creates a new corrected projection without mutating the original claim", async () => {
        ...
        await expect(createEvidenceClaimReview((await reviewWorkspace()).workspace, "claim_platform", { ...input, correctedClaim: "Delivered 900 Salesforce deployments." })).rejects.toThrow(/unsupported numeric value|unsupported facts/);
    });
    it.each([
    ...
    ])("rejects inconsistent input: %s", async (_label, changes) => {
        const fixture = await reviewWorkspace();
        await expect(createEvidenceClaimReview(fixture.workspace, "claim_platform", { ...approvedInput(fixture.claims[0]), ...changes })).rejects.toThrow();
    });
    it("never promotes private source material to public-safe", async () => {
        const fixture = await reviewWorkspace({ privateEvidence: true });
        await expect(createEvidenceClaimReview(fixture.workspace, "claim_platform", approvedInput(fixture.claims[0]))).rejects.toThrow(/cannot be reviewed as public-safe/);
    });
    it("allows factual approval without resume readiness and projects Role and Job eligibility separately", async () => {
        ...
        await createEvidenceClaimReview(factualOnly.workspace, "claim_platform", approvedInput(factualOnly.claims[0], {
            resumeReadiness: "not-resume-ready",
            eligibleForRoleMatching: false,
            eligibleForJobMapping: false,
        }));
        const factualSnapshotResult = await buildEvidenceSnapshot(factualOnly.workspace);
        const factualSnapshot = (await loadEvidenceSnapshot(factualOnly.workspace, factualSnapshotResult.snapshotId)).snapshot;
        ...
        await createEvidenceClaimReview(roleOnly.workspace, "claim_platform", approvedInput(roleOnly.claims[0], {
            eligibleForRoleMatching: true,
            eligibleForJobMapping: false,
        }));
        const roleSnapshotResult = await buildEvidenceSnapshot(roleOnly.workspace);
        const roleSnapshot = (await loadEvidenceSnapshot(roleOnly.workspace, roleSnapshotResult.snapshotId)).snapshot;
    ...
    it("verifies only exact source-supported metric wording", async () => {
        ...
        await createEvidenceClaimReview(fixture.workspace, metricClaim.id, approvedInput(metricClaim, {
            metricReview: {
                state: "verified",
        ...
        await expect(createEvidenceClaimReview(invalid.workspace, invalidClaim.id, approvedInput(invalidClaim, {
            metricReview: {
                state: "verified",
    ...
    it("supersedes explicitly, keeps prior versions, and reports lifecycle", async () => {
        const fixture = await reviewWorkspace();
        const first = await createEvidenceClaimReview(fixture.workspace, "claim_platform", approvedInput(fixture.claims[0]), { now: () => new Date(TIME_1) });
        await expect(createEvidenceClaimReview(fixture.workspace, "claim_platform", deferredInput(fixture.claims[0]))).rejects.toThrow(/supersedesReviewId/);
        const secondInput = deferredInput(fixture.claims[0], first.reviewId);
        const second = await createEvidenceClaimReview(fixture.workspace, "claim_platform", secondInput, { now: () => new Date(TIME_2) });
        expect((await getEvidenceClaimReviewStatus(fixture.workspace, "claim_platform", first.reviewId))).toMatchObject({ status: "superseded", supersededByReviewId: second.reviewId });
        expect((await getEvidenceClaimReviewStatus(fixture.workspace, "claim_platform"))).toMatchObject({
            status: "current",
            reviewId: second.reviewId,
        });
        expect((await listEvidenceClaimReviews(fixture.workspace)).find(({ claimId }) => claimId === "claim_platform")).toMatchObject({ versionCount: 2, decision: "deferred", status: "current" });
        expect(await readFile(evidenceClaimReviewPaths(fixture.workspace, "claim_platform", first.reviewId).reviewPath)).toBeTruthy();
    });
    it("marks a review stale after source evidence changes and invalid after corruption", async () => {
        const fixture = await reviewWorkspace();
        const result = await createEvidenceClaimReview(fixture.workspace, "claim_platform", approvedInput(fixture.claims[0]));
        ...
        const corruptResult = await createEvidenceClaimReview(corrupt.workspace, "claim_platform", approvedInput(corrupt.claims[0]));
...
describe("deterministic evidence review batches", () => {
    it("builds an exact Natterbox-guided batch without creating decisions", async () => {
        ...
        expect(batch.claims.some(({ matchingTerms }) => matchingTerms.includes("artificial intelligence") || matchingTerms.includes("platform"))).toBe(true);
...
async function reviewWorkspace(options = {}) {
...
async function natterboxBatchWorkspace() {
    ...
    const fixture = await reviewWorkspace();
    const sources = JSON.parse(await readFile(path.join(fixture.workspace, "kb/sources.json"), "utf8"));
    const evidence = JSON.parse(await readFile(path.join(fixture.workspace, "kb/evidence-items.json"), "utf8"));
    const baseClaim = JSON.parse(await readFile(path.join(fixture.workspace, "kb/claims.json"), "utf8"))[0];
    const themes = [
    ...
    const claims = themes.map((theme, index) => ({
        ...baseClaim,
        id: `claim_batch_${String(index + 1).padStart(2, "0")}`,
        claim: `Supported ${theme}.`,
    }));
    const batchEvidence = claims.map((claim, index) => ({
        ...evidence[0],
...
function approvedInput(claim, overrides = {}) {
    return {
        schemaVersion: 1,
        claimId: claim.id,
        ...
        classification: { workContext: "employment", claimNature: "responsibility" },
        risks: [],
        warnings: [],
        ambiguities: [],
...
function deferredInput(claim, supersedesReviewId) {
    return {
        ...approvedInput(claim),
        decision: "deferred",
...
async function writeKnowledgeBase(workspace, sources, evidence, claims) {
