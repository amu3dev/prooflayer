...
import { EvidenceReviewUiSubmissionError, loadEvidenceReviewUiClaim, submitEvidenceReviewUiClaim, } from "../evidence-review-ui.js";
...
describe("simple Evidence Review intent projection", () => {
    it("projects a clear approval into a complete canonical review input", async () => {
        ...
        const created = await submitEvidenceReviewUiClaim(fixture.workspace, fixture.batchId, claim.id, { reviewMode: "simple", intent: "approve" });
    ...
    it("refuses simple approval for private evidence without inventing public safety", async () => {
        ...
        await expect(submitEvidenceReviewUiClaim(fixture.workspace, fixture.batchId, claim.id, { reviewMode: "simple", intent: "approve" })).rejects.toBeInstanceOf(EvidenceReviewUiSubmissionError);
    });
    it("asks only for ambiguous classifications and accepts explicit safe answers", async () => {
        const fixture = await createEvidenceReviewUiFixture();
        const loaded = await loadEvidenceReviewUiClaim(fixture.workspace, fixture.batchId, "claim_review_ui_3");
    ...
    it("blocks approval as written when source wording is flagged but allows validated narrowing", async () => {
        const fixture = await createEvidenceReviewUiFixture();
        const { claim } = await loadEvidenceReviewUiClaim(fixture.workspace, fixture.batchId, "claim_review_ui_1");
    ...
    it("requires narrowed wording and a qualifier for Edit and Approve", async () => {
        ...
        const result = await submitEvidenceReviewUiClaim(fixture.workspace, fixture.batchId, claim.id, {
            reviewMode: "simple",
            intent: "approve-with-edit",
            correctedClaim: "Supported AI product workflows.",
            requiredQualifiers: "Engineering collaboration is excluded.",
    ...
    it("keeps canonical corrected-claim validation authoritative", async () => {
        const fixture = await createEvidenceReviewUiFixture();
        await expect(submitEvidenceReviewUiClaim(fixture.workspace, fixture.batchId, "claim_review_ui_1", {
            reviewMode: "simple",
            intent: "approve-with-edit",
