...
describe("Slice 1.6.1 persisted pipeline integration", () => {
    it("runs a real refresh and atomically persists the complete knowledge state", async () => {
        ...
        const careerProfile = await readFile(path.join(workspace, "outputs/reports/career-profile.md"), "utf8");
        expect(careerProfile.indexOf("Technical Product Manager"))
            .toBeLessThan(careerProfile.indexOf("evi_"));
        const rebuildReport = await readFile(path.join(workspace, "outputs/changelogs/rebuild-changelog.md"), "utf8");
