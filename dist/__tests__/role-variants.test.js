...
import { generateRoleVariant, listVariantStatuses, rankClaimsForVariant, selectClaimsForVariant } from "../variant-generator.js";
describe("Slice 1.4 role variants", () => {
    ...
    ])("prioritizes %s-specific evidence", (roleKey, preferredText, secondaryText) => {
    ...
    it("generates warned one-column drafts and complete manifests without invented metrics", async () => {
        ...
        const persisted = JSON.parse(await readFile(path.join(variantRoot, "generation-manifest.json"), "utf8"));
...
async function createGenerationWorkspace() {
    ...
    const state = { sources, evidenceItems: evidence, claims, profile, privacyFindings: [] };
...
function profileFixture(evidence, claims) {
    return {
        id: "career_profile",
