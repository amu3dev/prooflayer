...
import { TARGET_INTERPRETATION_POLICY_VERSION, TARGET_INTERPRETER_NAME, TARGET_INTERPRETER_VERSION, formatInterpretTargetResult, formatTargetInterpretationStatus, getTargetInterpretationStatus, interpretTarget, loadRoleProfile, showTargetInterpretation, } from "../target-interpretation.js";
...
describe("Slice 2.3A semantic target interpretation contract", () => {
    ...
    it("keeps role targets first-class without inferring expectations when no profile exists", async () => {
        ...
        expect(interpretation.warnings.map((entry) => entry.code)).toEqual(expect.arrayContaining(["ROLE_PROFILE_NOT_CONFIGURED", "NO_EXPECTATIONS_PRODUCED"]));
    });
    ...
    it("rejects profile title mismatches, blank statements, duplicate IDs, and invalid tags", async () => {
        ...
        await expect(interpretTarget(mismatch.workspace, mismatch.target.id, { roleProfile: mismatchPath })).rejects.toThrow("do not exactly match target title");
        ...
        await expect(loadRoleProfile(duplicate.workspace, duplicatePath)).rejects.toThrow("Role profile expectation IDs must be unique");
    ...
    it("maps only explicitly supported job sections to conservative expectations", async () => {
        ...
        expect(interpretation.expectations.map((entry) => entry.statement)).not.toEqual(expect.arrayContaining([
            "ExampleCo builds software products.",
            "Flexible working arrangements.",
            "Work openly across disciplines.",
        ]));
        expect(interpretation.warnings.map((entry) => entry.code)).toEqual(expect.arrayContaining([
            "PARAGRAPH_NOT_INTERPRETED",
            "UNCLASSIFIED_ITEM_SKIPPED",
            "STRUCTURAL_ANALYSIS_CONTAINS_UNKNOWN_SECTIONS",
        ]));
        expect(interpretation.ambiguities.map((entry) => entry.code)).toContain("INSUFFICIENT_EXPLICIT_STRUCTURE");
    });
    it("keeps each job list item intact and preserves exact structural provenance", async () => {
        ...
        expect(expectation?.statement).toBe("Manage, mentor, and support a distributed team of engineers.");
    ...
    it("persists interpretation and manifest separately without modifying target or analysis", async () => {
        ...
        expect(result.interpretationPath).toBe(`targets/jobs/${fixture.targetId}/interpretation/target-interpretation.json`);
    ...
    it("marks changed role profiles stale and keeps profile identity explicit", async () => {
        ...
        await expect(interpretTarget(fixture.workspace, fixture.target.id)).rejects.toThrow("Re-run with --rebuild");
    });
    it("marks target and structural-analysis dependency changes stale", async () => {
        ...
        const targetJson = JSON.parse(await readFile(targetPath, "utf8"));
        ...
        const analysisManifest = JSON.parse(await readFile(analysisManifestPath, "utf8"));
        ...
        expect((await getTargetAnalysisStatus(analysisFixture.workspace, analysisFixture.target.id)).status)
            .toBe("current");
        const interpretationStatus = await getTargetInterpretationStatus(analysisFixture.workspace, analysisFixture.target.id);
    ...
    it("detects interpretation hash mismatch and malformed manifests without silent overwrite", async () => {
        ...
        await expect(interpretTarget(fixture.workspace, fixture.targetId)).rejects.toThrow("was not overwritten");
        await interpretTarget(fixture.workspace, fixture.targetId, {
            rebuild: true,
            now: () => new Date(SECOND_TIME),
        });
...
async function jobWorkspace() {
    ...
    return {
        ...
        analysis: () => import("../target-analysis.js").then((module) => module.showTargetAnalysis(workspace, created.target.id)),
    };
}
...
async function writeRoleProfile(workspace, overrides = {}) {
