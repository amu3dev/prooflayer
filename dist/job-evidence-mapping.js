...
import { getTargetEvidencePinStatus, loadTargetEvidencePin, } from "./target-evidence-pin.js";
...
export async function buildJobEvidenceMap(workspace, targetId, options = {}) {
    ...
    const input = await mappingInput(workspace, target, requirementInput, evidenceInput);
    ...
    const map = JobEvidenceMapSchema.parse({
        ...
        input: {
            ...
            evidenceSnapshotSchemaVersion: evidenceInput.snapshot.snapshot.schemaVersion,
            evidenceSnapshotContractName: evidenceInput.snapshot.snapshot.contract.name,
            evidenceSnapshotPolicyName: evidenceInput.snapshot.snapshot.policy.name,
            evidenceSnapshotPolicyVersion: evidenceInput.snapshot.snapshot.policy.version,
            sources: {
...
export async function getJobEvidenceMapStatus(workspace, targetId) {
    ...
    const requirementManifestHashMatches = requirementInput
        ...
        : false;
    const hasPinnedSnapshotDependencies = map.input.evidencePin !== undefined &&
    ...
    if (hasPinnedSnapshotDependencies && requirementInput) {
        try {
            ...
            evidencePinManifestHashMatches =
                await hashMatches(evidenceInput.paths.manifestPath, manifest.evidencePinManifestSha256) &&
            ...
            const candidates = loadEligibleCandidates(evidenceInput);
            const currentInput = await mappingInput(workspace, target, requirementInput, evidenceInput);
            ...
            dependencyReasons.push(`Pinned Evidence Snapshot dependencies are unavailable: ${errorMessage(error)}`);
        }
    }
    else if (!hasPinnedSnapshotDependencies) {
        dependencyReasons.push("Stored map uses legacy live-KB evidence dependencies; rebuild from an explicitly pinned Evidence Snapshot.");
    }
    const staleReasons = [
        ...dependencyReasons,
...
function mapRequirements(targetId, requirementInput, requirements, candidates) {
    const links = requirements.flatMap((requirement) => candidates.flatMap((candidate) => {
        ...
        return [{
                ...
                evidenceProvenance: {
                    evidenceId: candidate.evidence.id,
                    evidenceItemPath: `evidence-foundation/evidence-items/${candidate.evidence.id}`,
...
function loadEligibleCandidates(evidenceInput) {
    const snapshot = evidenceInput.snapshot.snapshot;
    if (evidenceInput.pin.targetType !== "job") {
        throw new Error(`Job evidence mapping requires a Job Target pin, received ${evidenceInput.pin.targetType}.`);
    }
    ...
    const evidenceById = new Map(snapshot.evidenceItems.map((entry) => [entry.id, entry]));
    ...
    for (const evidenceId of [...eligibleEvidenceIds].sort()) {
        ...
        if (!snapshotEntry ||
            ...
            throw new Error(`Pinned snapshot eligible evidence content is missing or invalid: ${evidenceId}`);
        }
        for (const claimId of snapshotEntry.supportingClaimIds) {
            ...
            if (!snapshotClaim ||
                ...
                throw new Error(`Pinned snapshot eligible claim provenance is invalid: ${claimId}/${evidence.id}`);
            }
...
async function mappingInput(workspace, target, requirementInput, evidenceInput) {
    const sourceArtifacts = new Map(evidenceInput.snapshot.snapshot.sourceFoundation.artifacts.map((artifact) => [artifact.id, artifact.sha256]));
    const sourcesSha256 = requiredArtifactHash(sourceArtifacts, "sources");
    const evidenceItemsSha256 = requiredArtifactHash(sourceArtifacts, "evidence-items");
    ...
    const normalizedInputSha256 = hashText(stableJson({
        ...
        evidencePinManifestSha256: await hashFile(evidenceInput.paths.manifestPath),
        evidenceSnapshotId: evidenceInput.snapshot.snapshot.id,
        evidenceSnapshotSha256: evidenceInput.snapshot.manifest.contentSha256,
        evidenceSnapshotManifestSha256: evidenceInput.snapshot.manifestSha256,
        evidenceSnapshotSchemaVersion: evidenceInput.snapshot.snapshot.schemaVersion,
        evidenceSnapshotContractName: evidenceInput.snapshot.snapshot.contract.name,
        evidenceSnapshotPolicyName: evidenceInput.snapshot.snapshot.policy.name,
        evidenceSnapshotPolicyVersion: evidenceInput.snapshot.snapshot.policy.version,
        ...
        evidenceItemsSha256,
        claimsSha256,
        eligibleEvidenceSetSha256: evidenceInput.snapshot.snapshot.eligibleJobEvidenceSetSha256,
        mapperName: JOB_EVIDENCE_MAPPER_NAME,
        mapperVersion: JOB_EVIDENCE_MAPPER_VERSION,
        policyName: JOB_EVIDENCE_MAPPING_POLICY_NAME,
        policyVersion: JOB_EVIDENCE_MAPPING_POLICY_VERSION,
...
function requiredArtifactHash(artifacts, id) {
    const value = artifacts.get(id);
    if (!value) {
        throw new Error(`Pinned Evidence Snapshot is missing source artifact hash: ${id}`);
    }
    return value;
}
...
function validateStoredMapIdentity(map, manifest, paths) {
    ...
    const commonDependenciesMatch = map.input.target.sha256 === manifest.targetSha256 &&
