...
import { FitAssessmentProposalManifestSchema, FitAssessmentProposalSchema, ModelFitAssessmentPayloadSchema, } from "./fit-assessment-schemas.js";
import { FIT_ASSESSMENT_POLICY_NAME, FIT_ASSESSMENT_POLICY_VERSION, getFitAssessmentStatus, loadAssessmentContext, showFitAssessment, } from "./fit-assessment.js";
...
export async function generateFitAssessmentProposal(workspace, targetId, options = {}) {
...
export async function showFitAssessmentProposal(workspace, proposalId) {
    const located = await locateProposal(workspace, proposalId);
    return FitAssessmentProposalSchema.parse(await readJson(located.location.proposalPath, null));
}
export async function listFitAssessmentProposals(workspace, targetId) {
    ...
    const proposals = await Promise.all(files.map(async (file) => FitAssessmentProposalSchema.parse(await readJson(file, null))));
    return proposals.sort((a, b) => a.createdAt.localeCompare(b.createdAt) || a.id.localeCompare(b.id));
}
export async function getFitAssessmentProposalStatus(workspace, proposalId) {
    const located = await locateProposal(workspace, proposalId);
    const { target, location } = located;
    const mode = target.type === "role" ? "role-positioning" : "job-specific";
    const base = { proposalId, targetId: target.id, targetType: target.type, mode, proposalPath: location.proposalRelativePath, manifestPath: location.manifestRelativePath, rawResponsePath: location.rawRelativePath };
    ...
    let manifest;
    try {
        proposal = FitAssessmentProposalSchema.parse(await readJson(location.proposalPath, null));
        manifest = FitAssessmentProposalManifestSchema.parse(await readJson(location.manifestPath, null));
    }
    catch (error) {
    ...
    if (proposal.id !== proposalId || manifest.proposalId !== proposalId ||
...
export async function replayFitAssessmentProposal(workspace, proposalId) {
    const status = await getFitAssessmentProposalStatus(workspace, proposalId);
    if (status.status !== "current")
        throw new Error(`Replay requires a current assessment proposal. Current status: ${status.status}`);
    const located = await locateProposal(workspace, proposalId);
    const original = await showFitAssessmentProposal(workspace, proposalId);
    const manifest = FitAssessmentProposalManifestSchema.parse(await readJson(located.location.manifestPath, null));
...
export function renderFitAssessmentPrompt(input, templateVersion = ASSESSMENT_PROPOSAL_PROMPT_TEMPLATE_VERSION, policyVersion = ASSESSMENT_PROPOSAL_POLICY_VERSION) {
    return [
...
export function formatFitAssessmentProposalResult(result) {
    return [
...
export function formatFitAssessmentProposalList(proposals) {
    if (!proposals.length)
        return "Assessment proposals: none";
...
export function formatFitAssessmentProposalStatus(status) {
...
export async function fitAssessmentProposalFileTimestamps(workspace, proposalId) {
    const located = await locateProposal(workspace, proposalId);
...
function normalizeRawProposal(input) {
    ...
    if (payload)
        issues.push(...validatePayload(payload, input.deterministic));
    const proposedExpectationAssessments = payload && issues.length === 0
    ...
    const warnings = payload && issues.length === 0
        ? [
            ...payload.warnings.map((entry, index) => ({ id: `assessment-warning_${hashText(`${input.proposalId}\u0000${index}\u0000${stableJson(entry)}`).slice(0, 12)}`, ...entry })),
            {
                ...
                code: "MODEL_ASSESSMENT_REQUIRES_HUMAN_REVIEW",
...
function validatePayload(payload, deterministic) {
    const issues = [];
    ...
    payload.proposedExpectationAssessments.forEach((entry, index) => {
        if (expectationIds.has(entry.expectationId))
            issues.push(issue("DUPLICATE_EXPECTATION_ASSESSMENT", "Only one proposal is allowed per expectation.", `proposedExpectationAssessments.${index}`));
        ...
        if (stableJson(entry.provenance) !== stableJson(source.provenance))
            issues.push(issue("INVALID_PROVENANCE", "Proposal provenance does not match the deterministic assessment.", `proposedExpectationAssessments.${index}.provenance`));
        if (stableJson(uniqueSorted(entry.approvedMatchIds)) !== stableJson(source.approvedMatchIds))
            issues.push(issue("UNKNOWN_MATCH_ID", "Proposal match IDs must exactly preserve approved matching.", `proposedExpectationAssessments.${index}.approvedMatchIds`));
        if (stableJson(uniqueSorted(entry.evidenceIds)) !== stableJson(source.evidenceIds))
            issues.push(issue("UNKNOWN_EVIDENCE_ID", "Proposal evidence IDs must exactly preserve reviewed evidence provenance.", `proposedExpectationAssessments.${index}.evidenceIds`));
        ...
        if (forbidden)
            issues.push(issue("FORBIDDEN_CONTENT", forbidden, `proposedExpectationAssessments.${index}`));
        const unsupportedNumbers = numericTokens(text).filter((token) => !numericTokens(stableJson(source)).includes(token));
        if (unsupportedNumbers.length)
            issues.push(issue("INVENTED_METRIC", `Proposal contains unsupported numeric token(s): ${unsupportedNumbers.join(", ")}`, `proposedExpectationAssessments.${index}`));
    });
    ...
    if (payload.proposedSummary.mode !== deterministic.mode)
        issues.push(issue("MODE_MISMATCH", "Proposed summary mode does not match the target."));
    const summaryForbidden = forbiddenContent(payload.proposedSummary.narrative);
    if (summaryForbidden)
        issues.push(issue("FORBIDDEN_CONTENT", summaryForbidden, "proposedSummary.narrative"));
    if (/\d+(?:\.\d+)?%\s*(?:fit|match|alignment)?/i.test(payload.proposedSummary.narrative))
        issues.push(issue("FIT_PERCENTAGE", "Fit percentages are forbidden.", "proposedSummary.narrative"));
    if (issues.length === 0) {
        try {
            validateProposedSummary(payload, deterministic);
        }
        catch (error) {
            issues.push(issue("SUMMARY_INCONSISTENT", errorMessage(error), "proposedSummary"));
        }
    }
    return deduplicateIssues(issues);
}
function validateAssessmentSemantics(proposed, source, issues, index) {
    ...
    if (proposed.supportStatus === "strongly-supported") {
        ...
        if (!canBeStrong)
            issues.push(issue("IMPOSSIBLE_SUPPORT_STATUS", "Strong support requires approved direct strong evidence.", `proposedExpectationAssessments.${index}.supportStatus`));
    }
    if (proposed.supportStatus === "unsupported" && proposed.approvedMatchIds.length)
        issues.push(issue("IMPOSSIBLE_SUPPORT_STATUS", "Unsupported cannot retain approved supporting matches.", `proposedExpectationAssessments.${index}.supportStatus`));
    if (proposed.supportStatus === "conflicting" && !source.provenance.deterministicInputs.matchTypes.includes("contradictory"))
        issues.push(issue("IMPOSSIBLE_SUPPORT_STATUS", "Conflict requires approved contradictory evidence.", `proposedExpectationAssessments.${index}.supportStatus`));
    if (proposed.supportStatus === "not-assessed" && coverage !== "not-assessed")
        issues.push(issue("IMPOSSIBLE_SUPPORT_STATUS", "Not-assessed requires not-assessed approved coverage.", `proposedExpectationAssessments.${index}.supportStatus`));
    if (proposed.gapType === "experience-gap-possible" && !/possible|unclear|may|cannot confirm/i.test(`${proposed.rationale} ${proposed.limitations.join(" ")}`))
        issues.push(issue("DEFINITE_EXPERIENCE_GAP", "Possible experience gaps must preserve uncertainty.", `proposedExpectationAssessments.${index}.gapType`));
}
function validateProposedSummary(payload, deterministic) {
    ...
    const merged = payload.proposedExpectationAssessments.map((entry) => {
        ...
        if (!source)
            throw new Error(`Unknown expectation: ${entry.expectationId}`);
    ...
    if (payload.proposedSummary.mode === "role-positioning") {
        ...
        if (payload.proposedSummary.stronglySupportedCount !== counts.stronglySupported ||
    ...
    for (const [summary, entries] of groups) {
        ...
        if (stableJson(summary) !== stableJson(expected))
            throw new Error("Job summary counts do not match proposed expectation assessments.");
    }
}
...
async function locateProposal(workspace, proposalId) {
    if (!/^assessment-proposal_[a-f0-9]+$/.test(proposalId))
        throw new Error(`Invalid assessment proposal ID: ${proposalId}`);
    ...
    if (!files.length)
        throw new Error(`Assessment proposal not found: ${proposalId}`);
    if (files.length > 1)
        throw new Error(`Assessment proposal ID is ambiguous: ${proposalId}`);
    const proposal = FitAssessmentProposalSchema.parse(await readJson(files[0], null));
...
function resolveWithin(workspace, relativePath) {
    ...
    if (relation === ".." || relation.startsWith(`..${path.sep}`) || path.isAbsolute(relation))
        throw new Error(`Assessment proposal path escapes workspace: ${relativePath}`);
    return resolved;
}
