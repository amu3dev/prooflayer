...
export function createRefreshBaseline(state, successfulRefreshAt = new Date().toISOString()) {
...
export function detectUpdateImpact(previous, current) {
...
export function countChanges(delta) {
    return {
...
export function sourceFamilyFor(source) {
    if (source.type === "cv")
        return "cv";
    if (source.type === "linkedin_export")
        return "linkedin_export";
    if (source.type === "recommendation")
...
function snapshotEvidence(item, parentContextById) {
...
function snapshotClaim(claim, evidenceById, parentContextById, sourceById) {
...
function snapshotProfileAreas(profile) {
    return {
...
function evidenceParentContext(item, contexts) {
    if (item.parentRoleId)
        return contexts.get(item.parentRoleId) ?? "role:unknown";
    if (item.parentProjectId)
        return contexts.get(item.parentProjectId) ?? "project:unknown";
    if (item.category === "role" || item.category === "project")
        return contexts.get(item.id) ?? `${item.category}:unknown`;
    return normalizeText(item.sourceSection ?? "unscoped");
}
function compareCollections(before, after) {
...
function collectClaimTransitions(changes) {
...
function collectPrivacyTransitions(sourceChanges, evidenceChanges, claimChanges) {
    return [
...
function transition(target, key, field, from, to) {
    return from === to ? null : { target, key, field, from, to };
}
function countTrust(claims) {
    return {
...
function countPrivacy(findings) {
    return {
