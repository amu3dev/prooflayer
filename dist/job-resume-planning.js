...
import { getJobCoverageStatus, jobCoveragePaths, showJobCoverage, } from "./job-coverage.js";
import { JobRequirementCoverageManifestSchema, } from "./job-coverage-schemas.js";
import { getJobFitProofAssessmentStatus, jobFitProofAssessmentPaths, showJobFitProofAssessment, } from "./job-fit-proof-assessment.js";
import { JobFitProofAssessmentManifestSchema, } from "./job-fit-proof-assessment-schemas.js";
import { jobEvidenceMapPaths, showJobEvidenceMap, } from "./job-evidence-mapping.js";
...
import { JobResumeContentPlanManifestSchema, JobResumeContentPlanSchema, } from "./job-resume-plan-schemas.js";
...
export const JOB_RESUME_PLANNING_POLICY_NAME = "job-resume-content-planning-policy";
...
export function jobResumePlanPaths(workspace, targetId) {
    const rootRelativePath = `targets/jobs/${targetId}/resume-planning/deterministic`;
...
export async function buildJobResumePlan(workspace, targetId, options = {}) {
    ...
    if (status.status === "current") {
        return resultFromPlan(await showJobResumePlan(workspace, targetId), paths, "already-current");
    }
    if ((status.status === "stale" || status.status === "invalid") &&
        !options.rebuild) {
        throw new Error(`Stored Job Resume Content Plan is ${status.status} and was not overwritten. Review dependencies, then use --rebuild. ${status.reasons.join(" ")}`);
    }
    ...
    const manifest = createManifest(plan, context, paths.planRelativePath, await hashFile(paths.planPath), createdAt, now);
    await writeJsonAtomic(paths.manifestPath, manifest);
    return resultFromPlan(plan, paths, status.status === "missing" ? "created" : "rebuilt");
}
export async function showJobResumePlan(workspace, targetId) {
    ...
    return JobResumeContentPlanSchema.parse(await readJson(paths.planPath, null));
}
export async function getJobResumePlanStatus(workspace, targetId) {
    await requireJobTarget(workspace, targetId);
    const paths = jobResumePlanPaths(workspace, targetId);
    ...
    let manifest;
    try {
        plan = JobResumeContentPlanSchema.parse(await readJson(paths.planPath, null));
        manifest = JobResumeContentPlanManifestSchema.parse(await readJson(paths.manifestPath, null));
    }
    catch (error) {
        return emptyStatus(base, "invalid", [
    ...
    if (!planHashMatches) {
        identityReasons.push("Job Resume Content Plan SHA-256 does not match its manifest.");
    }
    ...
    if (staleReasons.length > 0) {
        return statusWithChecks(base, planHashMatches, checks, "stale", staleReasons);
    }
    const expected = createJobResumePlan(context, plan.createdAt, plan.updatedAt);
    if (stableJson(planSemantics(plan)) !== stableJson(planSemantics(expected))) {
        return statusWithChecks(base, planHashMatches, checks, "invalid", [
            "Stored plan content does not match deterministic planning of the current Job assessment.",
        ]);
    }
    return statusWithChecks(base, planHashMatches, checks, "current", []);
}
export function deriveJobPositioningState(overall) {
    if (overall === "strong" || overall === "credible")
        return "direct";
    if (overall === "mixed")
        return "adjacent";
    if (overall === "limited")
...
export function deriveJobRequirementEmphasisDecision(assessment) {
    ...
    if (assessment.necessity === "mandatory")
        return "primary";
    if (assessment.necessity === "preferred")
        return "secondary";
    return "supporting";
}
export function formatBuildJobResumePlanResult(result) {
    return [
...
export function formatJobResumePlanStatus(status) {
...
function createJobResumePlan(context, createdAt, updatedAt) {
    const planId = deterministicPlanId(context);
    const requirementEmphasis = deriveRequirementEmphasis(context, planId);
    const claimBoundaries = deriveClaimBoundaries(context, planId, requirementEmphasis);
    const evidenceSelections = deriveEvidenceSelections(context, planId, requirementEmphasis, claimBoundaries);
    const metricPermissions = deriveMetricPermissions(context, planId, requirementEmphasis);
    const gapHandling = deriveGapHandling(context, planId, requirementEmphasis);
    const exclusions = deriveExclusions(context, planId, requirementEmphasis, evidenceSelections, metricPermissions);
    const positioning = derivePositioning(context, requirementEmphasis);
    const risks = deriveRisks(context, planId, requirementEmphasis, evidenceSelections, metricPermissions);
    const warnings = deriveWarnings(context, planId, requirementEmphasis, metricPermissions);
    const ambiguities = deriveAmbiguities(context, planId, requirementEmphasis, evidenceSelections);
    const sections = deriveSections(context, planId, requirementEmphasis, evidenceSelections, claimBoundaries, metricPermissions, exclusions, risks, warnings);
    const completeness = deriveCompleteness(context, positioning.state, requirementEmphasis, sections, claimBoundaries, gapHandling);
...
function deriveRequirementEmphasis(context, planId) {
    const linksByRequirement = groupBy(context.evidenceMap.links, (link) => link.requirementId);
    const coverageByRequirement = new Map(context.coverage.requirements.map((entry) => [entry.requirementId, entry]));
    return context.assessment.requirementAssessments
        .map((assessment) => {
        const coverage = coverageByRequirement.get(assessment.requirementId);
        if (!coverage) {
            throw new Error(`Assessment references missing coverage: ${assessment.requirementId}`);
        }
        ...
        return {
            ...
            selectedEvidenceIds: uniqueSorted(selectedLinks.map((link) => link.evidenceId)),
            selectedClaimIds: uniqueSorted(selectedLinks.map((link) => link.claimId)),
            ...
            provenance: elementProvenance(context, [assessment.requirementId], selectedLinks.map((link) => link.id)),
        };
    })
        .sort((left, right) => left.requirementId.localeCompare(right.requirementId));
}
function derivePositioning(context, emphasis) {
    const state = deriveJobPositioningState(context.assessment.overall.state);
    const selected = emphasis.filter((entry) => ["primary", "secondary", "supporting"].includes(entry.decision));
    ...
    return {
        ...
        provenance: elementProvenance(context, emphasis.map((entry) => entry.requirementId), uniqueSorted(emphasis.flatMap((entry) => entry.selectedLinkIds))),
    };
}
function deriveClaimBoundaries(context, planId, emphasis) {
    const boundaries = emphasis.map((entry) => {
        const state = entry.decision === "exclude"
            ? "prohibited"
        ...
        const allowed = state === "allowed" || state === "allowed-with-qualifier"
            ...
            : [];
        return {
            id: `job-claim-boundary_${hashText([planId, entry.requirementId, state, "requirement"].join("\0")).slice(0, 16)}`,
            ...
            prohibitedClaimTypes: allContentTypes().filter((type) => !allowed.includes(type)),
            requiredQualifierCodes: state === "allowed-with-qualifier"
            ...
            provenance: elementProvenance(context, [entry.requirementId], entry.selectedLinkIds),
        };
    });
    boundaries.push({
        id: `job-claim-boundary_${hashText([planId, context.target.id, "target-title"].join("\0")).slice(0, 16)}`,
        kind: "target-title",
        evidenceIds: [],
        claimIds: [],
        state: "allowed-with-qualifier",
        allowedClaimTypes: ["target-title"],
        prohibitedClaimTypes: allContentTypes().filter((type) => type !== "target-title"),
    ...
    const projectEvidence = selectedEvidence(context, emphasis).filter((evidence) => evidence.category === "project" || evidence.parentProjectId);
    for (const evidence of projectEvidence) {
        const links = context.evidenceMap.links.filter((link) => link.evidenceId === evidence.id);
        boundaries.push({
            id: `job-claim-boundary_${hashText([planId, evidence.id, "project-employment"].join("\0")).slice(0, 16)}`,
            ...
            provenance: elementProvenance(context, uniqueSorted(links.map((link) => link.requirementId)), links.map((link) => link.id)),
        });
    }
    return boundaries.sort((left, right) => left.id.localeCompare(right.id));
}
function deriveEvidenceSelections(context, planId, emphasis, boundaries) {
    const emphasisByRequirement = new Map(emphasis.map((entry) => [entry.requirementId, entry]));
    const linksByEvidence = groupBy(context.evidenceMap.links, (link) => link.evidenceId);
    return [...linksByEvidence.entries()]
        ...
        const sortedUses = links
            ...
            if (!requirement) {
                throw new Error(`Evidence link references unknown planning requirement: ${link.requirementId}`);
            }
            ...
            .sort((left, right) => requirementUseRank(left.decision) -
            requirementUseRank(right.decision) ||
            left.requirementId.localeCompare(right.requirementId));
        const selectedUses = sortedUses.filter((entry) => !["defer", "exclude"].includes(entry.decision));
        if (selectedUses.filter((entry) => entry.decision === "primary").length > 1) {
            let primarySeen = false;
            for (const use of sortedUses) {
                if (use.decision !== "primary")
                    continue;
                if (!primarySeen)
                    primarySeen = true;
                else {
        ...
        const decision = sortedUses
            ...
            .sort((left, right) => requirementUseRank(left) - requirementUseRank(right))[0];
        const intendedSections = uniqueSorted(sortedUses.flatMap((entry) => entry.intendedSections));
        ...
        const selectedRequirementIds = sortedUses
            ...
            .map((entry) => entry.requirementId);
        const reuseWarning = new Set(selectedRequirementIds).size > 1 ||
            intendedSections.length > 2;
        return {
            id: `job-evidence-selection_${hashText([planId, evidenceId, JOB_RESUME_PLANNING_POLICY_VERSION].join("\0")).slice(0, 16)}`,
            evidenceId,
            claimIds: uniqueSorted(links.map((link) => link.claimId)),
            decision,
            relationships: uniqueSorted(links.map((link) => link.relationship)),
            requirementUses: sortedUses,
            intendedSections,
            ...(decision === "primary"
                ? {
                    primaryRequirementId: sortedUses.find((entry) => entry.decision === "primary")?.requirementId,
                }
                : {}),
            reuseWarning,
            ...
            limitationCodes: uniqueSorted([
                ...
                ...(links.every((link) => link.relationship === "supporting")
                    ...
                    : []),
                ...(links.some((link) => link.evidenceStrength === "weak" ||
                ...
                ...(!hasVerifiedMetric(context, evidenceId)
                    ? ["no-verified-metric"]
                    : []),
                ...(reuseWarning ? ["multiple-section-reuse"] : []),
            ]),
            provenance: elementProvenance(context, uniqueSorted(links.map((link) => link.requirementId)), links.map((link) => link.id)),
        };
    })
...
function deriveMetricPermissions(context, planId, emphasis) {
    const selectedClaimIds = new Set(emphasis
        .filter((entry) => ["primary", "secondary", "supporting"].includes(entry.decision))
        .flatMap((entry) => entry.selectedClaimIds));
    const pairs = new Map();
    for (const link of context.evidenceMap.links) {
        if (!selectedClaimIds.has(link.claimId))
            continue;
    ...
    return [...pairs.values()]
        .map(({ evidence, claim }) => {
        const allowed = claim.metricStatus === "verified_metric" &&
            claim.approvalStatus === "approved" &&
            claim.outputReadiness === "resume_ready" &&
            claim.publicSafe &&
            !claim.needsConfirmation &&
            Boolean(claim.approvedWording);
        const links = context.evidenceMap.links.filter((link) => link.evidenceId === evidence.id && link.claimId === claim.id);
        return {
            id: `job-metric-permission_${hashText([planId, evidence.id, claim.id, allowed ? "allowed" : "prohibited"].join("\0")).slice(0, 16)}`,
            evidenceId: evidence.id,
            claimId: claim.id,
            state: allowed ? "allowed" : "prohibited",
            ...(allowed
            ...
            qualifierCodes: [
                ...(allowed ? ["use-exact-approved-text"] : []),
        ...
        .sort((left, right) => `${left.evidenceId}\0${left.claimId}`.localeCompare(`${right.evidenceId}\0${right.claimId}`));
}
function deriveGapHandling(context, planId, emphasis) {
    return emphasis
        .filter((entry) => entry.assessmentState === "partial" ||
        entry.assessmentState === "gap" ||
        entry.assessmentState === "contradiction" ||
        entry.assessmentState === "indeterminate")
        .map((entry) => ({
        id: `job-gap-handling_${hashText([planId, entry.requirementId, entry.assessmentState].join("\0")).slice(0, 16)}`,
        requirementId: entry.requirementId,
        assessmentState: entry.assessmentState,
        decision: entry.assessmentState === "partial"
            ? "drafting-caution"
            : entry.assessmentState === "indeterminate"
                ? "defer"
                : "exclude-positive-positioning",
        adjacentEvidenceIds: entry.assessmentState === "partial"
            ? entry.selectedEvidenceIds
            : [],
        adjacentClaimIds: entry.assessmentState === "partial"
...
function deriveSections(context, planId, emphasis, evidenceSelections, boundaries, metrics, exclusions, risks, warnings) {
    const selected = emphasis.filter((entry) => ["primary", "secondary", "supporting"].includes(entry.decision));
    const usableEvidence = evidenceSelections.filter((entry) => !["defer", "exclude"].includes(entry.decision));
    ...
    const hasImpact = metrics.some((entry) => entry.state === "allowed") ||
        usableEvidence.some((entry) => evidenceById.get(entry.evidenceId)?.category === "achievement");
    const hasTechnical = selected.some((entry) => entry.category === "technical-expectation");
    const hasLeadership = selected.some((entry) => entry.category === "leadership-expectation");
    const hasEducation = usableEvidence.some((entry) => evidenceById.get(entry.evidenceId)?.category === "education");
    const hasCertification = usableEvidence.some((entry) => evidenceById.get(entry.evidenceId)?.category === "certification");
    const specs = [
        {
            type: "headline",
            inclusion: "include",
    ...
    return specs.map((spec) => {
        const relevantRequirements = spec.inclusion === "exclude"
            ? []
            : selected.filter((entry) => entry.allowedSections.includes(spec.type));
        const evidenceIds = uniqueSorted(relevantRequirements.flatMap((entry) => entry.selectedEvidenceIds)).filter((evidenceId) => evidenceAllowedInSection(context, evidenceId, spec.type, metrics));
        const claimIds = uniqueSorted(context.evidenceMap.links
            .filter((link) => evidenceIds.includes(link.evidenceId))
            .map((link) => link.claimId));
        const boundaryIds = boundaries
            .filter((boundary) => boundary.kind === "target-title" && spec.type === "headline" ||
            boundary.evidenceIds.some((id) => evidenceIds.includes(id)) ||
            boundary.requirementId !== undefined &&
                relevantRequirements.some((entry) => entry.requirementId === boundary.requirementId))
            .map((entry) => entry.id)
            .sort();
        const exclusionIds = exclusions
            .filter((entry) => entry.sourceIds.some((id) => evidenceIds.includes(id) ||
            claimIds.includes(id) ||
            relevantRequirements.some((requirement) => requirement.requirementId === id)))
            .map((entry) => entry.id)
            .sort();
        return {
            id: `job-section-plan_${hashText([planId, spec.type, JOB_RESUME_PLANNING_POLICY_VERSION].join("\0")).slice(0, 16)}`,
            type: spec.type,
            inclusion: spec.inclusion,
            order: spec.inclusion === "exclude"
                ? excludedOrder++
                : includedOrder++,
            ...
            ...(spec.maximumItemCount
                ...
                : {}),
            riskCodes: uniqueSorted(risks
                .filter((risk) => risk.requirementIds.some((id) => relevantRequirements.some((entry) => entry.requirementId === id)) ||
                risk.evidenceIds.some((id) => evidenceIds.includes(id)))
                .map((risk) => risk.code)),
            warningCodes: uniqueSorted(warnings
                .filter((warning) => warning.requirementIds.length === 0 ||
                warning.requirementIds.some((id) => relevantRequirements.some((entry) => entry.requirementId === id)))
                .map((warning) => warning.code)),
            provenance: elementProvenance(context, relevantRequirements.map((entry) => entry.requirementId), uniqueSorted(context.evidenceMap.links
                .filter((link) => evidenceIds.includes(link.evidenceId))
                .map((link) => link.id))),
        };
    });
}
function deriveExclusions(context, planId, emphasis, evidenceSelections, metricPermissions) {
    const exclusions = [];
    for (const entry of emphasis.filter((candidate) => ["defer", "exclude"].includes(candidate.decision))) {
        const type = entry.assessmentState === "contradiction"
            ? "contradicted-requirement"
            : entry.assessmentState === "indeterminate"
                ? "indeterminate-requirement"
                : "unsupported-requirement";
        exclusions.push(exclusion(context, planId, type, "requirement", [entry.requirementId], entry.assessmentState === "contradiction"
            ? "explicit-contradiction"
            : entry.assessmentState === "indeterminate"
                ? "unresolved-ambiguity"
                : "no-reviewed-proof", entry.necessity === "mandatory" ? "high" : "medium", [entry.requirementId], []));
    }
    exclusions.push(exclusion(context, planId, "target-title-history", "target", [context.target.id], "target-title-is-not-history", "blocking", [], []));
    for (const selection of evidenceSelections) {
        ...
        if (evidence.category === "project" || evidence.parentProjectId) {
            exclusions.push(exclusion(context, planId, "project-as-employment", "evidence", [evidence.id], "project-scope-is-not-employment", "blocking", selection.requirementUses.map((entry) => entry.requirementId), selection.requirementUses.flatMap((entry) => entry.linkIds)));
        }
        if (selection.reuseWarning) {
            exclusions.push(exclusion(context, planId, "duplicate-evidence-use", "evidence", [evidence.id], "deduplicated-use", "low", selection.requirementUses.map((entry) => entry.requirementId), selection.requirementUses.flatMap((entry) => entry.linkIds)));
        }
    }
    for (const metric of metricPermissions.filter((entry) => entry.state === "prohibited")) {
        exclusions.push(exclusion(context, planId, "unverified-metric", "claim", [metric.claimId], "metric-not-verified", "blocking", metric.provenance.requirementIds, metric.provenance.evidenceLinkReferences.map((entry) => entry.linkId)));
    }
    return exclusions.sort((left, right) => left.id.localeCompare(right.id));
}
function deriveRisks(context, planId, emphasis, evidence, metrics) {
    const risks = [];
    const add = (code, severity, message, requirementIds = [], evidenceIds = [], claimIds = []) => risks.push({
    ...
    const mandatoryDeferred = emphasis.filter((entry) => entry.necessity === "mandatory" &&
        ["defer", "exclude"].includes(entry.decision));
    if (mandatoryDeferred.length)
        add("MANDATORY_REQUIREMENT_NOT_POSITIONABLE", "critical", "One or more mandatory requirements cannot be included in positive positioning.", mandatoryDeferred.map((entry) => entry.requirementId));
    const partial = emphasis.filter((entry) => entry.assessmentState === "partial");
    if (partial.length)
        add("PARTIAL_REQUIREMENT_OVERSTATEMENT_RISK", "high", "Partial requirement proof must remain qualified and cannot be presented as direct satisfaction.", partial.map((entry) => entry.requirementId), partial.flatMap((entry) => entry.selectedEvidenceIds), partial.flatMap((entry) => entry.selectedClaimIds));
    add("TARGET_TITLE_HISTORY_RISK", "critical", "The Job Target title is positioning metadata and must not become employment history.");
    ...
    if (projects.length)
        add("PROJECT_AS_EMPLOYMENT_RISK", "critical", "Project evidence must remain project-scoped unless employment provenance exists.", projects.flatMap((entry) => entry.requirementUses.map((use) => use.requirementId)), projects.map((entry) => entry.evidenceId), projects.flatMap((entry) => entry.claimIds));
    const responsibilities = evidence.filter((entry) => requiredEvidence(context, entry.evidenceId).category === "responsibility");
    if (responsibilities.length)
        add("RESPONSIBILITY_AS_ACHIEVEMENT_RISK", "high", "Responsibility evidence must not be rewritten as an achievement or outcome.", responsibilities.flatMap((entry) => entry.requirementUses.map((use) => use.requirementId)), responsibilities.map((entry) => entry.evidenceId), responsibilities.flatMap((entry) => entry.claimIds));
    if (metrics.some((entry) => entry.state === "prohibited"))
        add("UNSUPPORTED_METRIC_RISK", "critical", "Quantification is prohibited for selected claims without an approved resume-ready verified metric.", [], metrics.filter((entry) => entry.state === "prohibited").map((entry) => entry.evidenceId), metrics.filter((entry) => entry.state === "prohibited").map((entry) => entry.claimId));
    const reused = evidence.filter((entry) => entry.reuseWarning);
    if (reused.length)
        add("EVIDENCE_OVERUSE_RISK", "medium", "Repeated evidence use requires distinct section purpose and one primary allocation.", reused.flatMap((entry) => entry.requirementUses.map((use) => use.requirementId)), reused.map((entry) => entry.evidenceId), reused.flatMap((entry) => entry.claimIds));
    return risks.sort((left, right) => left.id.localeCompare(right.id));
}
function deriveWarnings(context, planId, emphasis, metrics) {
    const definitions = [
        [
    ...
    const materialGaps = emphasis.filter((entry) => ["critical", "material"].includes(entry.materiality) &&
        ["defer", "exclude"].includes(entry.decision));
...
function deriveAmbiguities(context, planId, emphasis, evidence) {
    const ambiguities = [];
    const add = (code, message, requirementIds = [], evidenceIds = [], claimIds = []) => ambiguities.push({
    ...
    if (deferred.length)
        add("REQUIREMENT_POSITIONING_DEFERRED", "Requirement positioning remains deferred rather than being strengthened without proof.", deferred.map((entry) => entry.requirementId));
    const partial = emphasis.filter((entry) => entry.assessmentState === "partial");
    if (partial.length)
        add("ADJACENT_EVIDENCE_NOT_DIRECT", "Partial evidence may support adjacent wording but not direct requirement satisfaction.", partial.map((entry) => entry.requirementId), partial.flatMap((entry) => entry.selectedEvidenceIds), partial.flatMap((entry) => entry.selectedClaimIds));
    const project = evidence.filter((entry) => {
        const item = requiredEvidence(context, entry.evidenceId);
        return item.category === "project" || Boolean(item.parentProjectId);
    });
    if (project.length)
        add("PROJECT_EMPLOYMENT_BOUNDARY", "Project evidence remains separate from employment history.", project.flatMap((entry) => entry.requirementUses.map((use) => use.requirementId)), project.map((entry) => entry.evidenceId), project.flatMap((entry) => entry.claimIds));
    const responsibility = evidence.filter((entry) => requiredEvidence(context, entry.evidenceId).category === "responsibility");
    if (responsibility.length)
        add("RESPONSIBILITY_ACHIEVEMENT_BOUNDARY", "Responsibility evidence remains distinct from achievement evidence.", responsibility.flatMap((entry) => entry.requirementUses.map((use) => use.requirementId)), responsibility.map((entry) => entry.evidenceId), responsibility.flatMap((entry) => entry.claimIds));
    const reused = evidence.filter((entry) => entry.reuseWarning);
    if (reused.length)
        add("EVIDENCE_REUSE_BOUNDARY", "Evidence reused for multiple requirements must retain one primary use and distinct secondary purposes.", reused.flatMap((entry) => entry.requirementUses.map((use) => use.requirementId)), reused.map((entry) => entry.evidenceId), reused.flatMap((entry) => entry.claimIds));
    for (const upstream of context.assessment.ambiguities)
        add("UPSTREAM_AMBIGUITY_PRESERVED", upstream.message, [upstream.requirementId], [], []);
    return ambiguities.sort((left, right) => left.id.localeCompare(right.id));
}
function deriveCompleteness(context, positioning, emphasis, sections, boundaries, gaps) {
    ...
    const boundedRequirements = new Set(boundaries
        .filter((entry) => entry.kind === "requirement-claim")
        .map((entry) => entry.requirementId)
        .filter((entry) => Boolean(entry)));
    const claimBoundariesComplete = boundaries.some((entry) => entry.kind === "target-title") &&
        [...requiredBoundaries].every((id) => boundedRequirements.has(id));
    const gapRequirements = emphasis
        .filter((entry) => ["partial", "gap", "contradiction", "indeterminate"].includes(entry.assessmentState))
        .map((entry) => entry.requirementId);
    const handledGaps = new Set(gaps.map((entry) => entry.requirementId));
    const criticalConstraintsRepresented = gapRequirements.every((id) => handledGaps.has(id));
    const provenanceComplete = emphasis.every((entry) => entry.provenance.requirementIds.includes(entry.requirementId) &&
    ...
    const status = requirementIds.length === 0
        ? "empty"
    ...
    return {
        ...
        provenanceComplete,
        criticalConstraintsRepresented,
        usableForDrafting: status === "complete" &&
            selectedRequirementIds.length > 0 &&
            positioning !== "insufficient-proof" &&
            positioning !== "indeterminate",
        blockingReasons,
...
export async function loadJobResumePlanningContext(workspace, targetId) {
    ...
    const assessmentStatus = await getJobFitProofAssessmentStatus(workspace, targetId);
    if (assessmentStatus.status !== "current") {
        throw new Error(`Job Resume Content Planning requires a current Job Fit and Proof Assessment. Current status: ${assessmentStatus.status}. ${assessmentStatus.reasons.join(" ")}`);
    }
    const coverageStatus = await getJobCoverageStatus(workspace, targetId);
    if (coverageStatus.status !== "current") {
        throw new Error(`Job Resume Content Planning requires current Job Requirement Coverage. Current status: ${coverageStatus.status}.`);
    }
    ...
    const [assessmentManifest, coverageManifest, mapManifest] = await Promise.all([
        readJson(assessmentPaths.manifestPath, null).then((value) => JobFitProofAssessmentManifestSchema.parse(value)),
        readJson(coveragePaths.manifestPath, null).then((value) => JobRequirementCoverageManifestSchema.parse(value)),
        readJson(mapPaths.manifestPath, null).then((value) => JobEvidenceMapManifestSchema.parse(value)),
    ]);
    if (assessment.targetId !== targetId ||
        ...
        throw new Error("Job Resume Content Planning rejects cross-target dependencies.");
    }
    const requirementInput = await loadRequirementInput(workspace, targetId, evidenceMap.input.requirementModelType);
    const [targetSha256, sourceSha256, evidenceMapSha256, evidenceMapManifestSha256, coverageSha256, coverageManifestSha256, assessmentSha256, assessmentManifestSha256,] = await Promise.all([
        hashFile(resolveWithin(workspace, `targets/jobs/${targetId}/target.json`)),
        hashFile(resolveWithin(workspace, `targets/jobs/${targetId}/job-description.md`)),
    ...
    if (dependencyProblems.length) {
        throw new Error(`Job Resume Content Planning dependency provenance is invalid. ${dependencyProblems.join(" ")}`);
    }
    validateReferencedCandidateEvidence(evidenceMap, sources, evidenceItems, claims);
    const selectedEvidence = uniqueSorted(evidenceMap.links.map((link) => link.evidenceId)).map((id) => {
        const evidence = evidenceItems.find((entry) => entry.id === id);
        if (!evidence)
            throw new Error(`Mapped evidence is missing: ${id}`);
        return evidence;
    });
    const selectedClaims = uniqueSorted(evidenceMap.links.map((link) => link.claimId)).map((id) => {
        const claim = claims.find((entry) => entry.id === id);
        if (!claim)
            throw new Error(`Mapped claim is missing: ${id}`);
        return claim;
    });
...
function validateDependencyAgreement(input) {
    const { targetId, targetSha256, sourceSha256, requirementInput, evidenceMap, evidenceMapSha256, evidenceMapManifestSha256, coverage, coverageSha256, coverageManifestSha256, assessment, assessmentSha256, assessmentManifestSha256, sourcesSha256, evidenceItemsSha256, claimsSha256, } = input;
    return uniqueSorted([
...
function validateReferencedCandidateEvidence(map, sources, evidenceItems, claims) {
    ...
    for (const link of map.links) {
        ...
        if (!evidence || !claim) {
            throw new Error(`Job Resume Content Planning found broken mapped evidence provenance: ${link.id}`);
        }
        if (evidence.visibility !== "public" ||
            ...
            throw new Error(`Job Resume Content Planning rejects ineligible evidence or claim: ${link.id}`);
        }
        if (hashText(stableJson(evidence)) !==
            ...
            throw new Error(`Job Resume Content Planning found changed mapped evidence provenance: ${link.id}`);
        }
        for (const sourceId of evidence.sourceIds) {
            ...
            if (!source ||
                ...
                throw new Error(`Job Resume Content Planning rejects private, inactive, or target-source evidence: ${link.id}`);
            }
        }
    }
}
...
function createManifest(plan, context, planPath, planSha256, createdAt, updatedAt) {
...
function deterministicPlanId(context) {
...
function deterministicManifestId(planId, planPath) {
    return `job-resume-content-plan-manifest_${hashText([planId, planPath, JOB_RESUME_PLANNING_POLICY_VERSION].join("\0")).slice(0, 16)}`;
}
...
function elementProvenance(context, requirementIds, linkIds) {
    const requirementSet = new Set(requirementIds);
    const linkSet = new Set(linkIds);
    const coverage = context.coverage.requirements.filter((entry) => requirementSet.has(entry.requirementId));
    const assessments = context.assessment.requirementAssessments.filter((entry) => requirementSet.has(entry.requirementId));
    const links = context.evidenceMap.links.filter((entry) => linkSet.has(entry.id));
    const requirementProvenance = new Map(coverage.map((entry) => [
        entry.requirementId,
    ...
    return {
        ...
        coverageReferences: coverage
            ...
            .sort((left, right) => left.coverageEntryId.localeCompare(right.coverageEntryId)),
        assessmentReferences: assessments
            .map((entry) => ({
            assessmentEntryId: entry.id,
            assessmentEntrySha256: hashText(stableJson(entry)),
        }))
            .sort((left, right) => left.assessmentEntryId.localeCompare(right.assessmentEntryId)),
        evidenceLinkReferences: links
            ...
            requirementProvenance: requirementProvenance.get(link.requirementId) ??
...
function emphasisRationale(assessment) {
    if (assessment.assessmentState === "contradiction") {
...
function emphasisCautions(assessment) {
    if (assessment.assessmentState === "partial") {
        return [
...
function positioningRationale(overall) {
    const values = {
...
function permittedTerminology(links) {
    return uniqueSorted(links.flatMap((link) => link.matchedSignals
        .filter((signal) => signal.type !== "exact-phrase")
        .map((signal) => signal.value)));
}
function allowedSectionsForRequirement(context, assessment, links) {
    const sections = [
        "professional-summary",
        "core-capabilities",
    ];
    if (assessment.assessmentState === "strength" &&
    ...
    for (const link of links) {
        ...
        if (evidence.category === "achievement")
            sections.push("selected-impact");
...
function contentTypesForRequirement(emphasis) {
    const byCategory = {
...
function evidenceUseDecision(requirementDecision, link) {
    if (requirementDecision === "defer" || requirementDecision === "exclude") {
        return requirementDecision;
    }
    if (requirementDecision === "primary" &&
...
function evidencePurpose(decision) {
    const values = {
...
function requirementUseRank(decision) {
    return {
        primary: 0,
        secondary: 1,
        supporting: 2,
...
function metricSections(evidence) {
    if (evidence.category === "project" || evidence.parentProjectId) {
...
function evidenceAllowedInSection(context, evidenceId, section, metrics) {
    const evidence = requiredEvidence(context, evidenceId);
    ...
    if (section === "education")
        return evidence.category === "education";
    if (section === "certifications")
        return evidence.category === "certification";
    if (section === "selected-impact") {
        return (evidence.category === "achievement" ||
            metrics.some((entry) => entry.state === "allowed" && entry.evidenceId === evidenceId));
    }
    return true;
}
function hasVerifiedMetric(context, evidenceId) {
    return context.evidenceMap.links
        .filter((link) => link.evidenceId === evidenceId)
        .some((link) => {
        const claim = requiredClaim(context, link.claimId);
        return (claim.metricStatus === "verified_metric" &&
            claim.approvalStatus === "approved" &&
            claim.outputReadiness === "resume_ready" &&
            claim.publicSafe &&
            !claim.needsConfirmation &&
...
function selectedEvidence(context, emphasis) {
    const ids = uniqueSorted(emphasis
        .filter((entry) => ["primary", "secondary", "supporting"].includes(entry.decision))
        .flatMap((entry) => entry.selectedEvidenceIds));
    return ids.map((id) => requiredEvidence(context, id));
}
function exclusion(context, planId, type, sourceType, sourceIds, reasonCode, severity, requirementIds, linkIds) {
    return {
        id: `job-plan-exclusion_${hashText([planId, type, sourceType, ...[...sourceIds].sort()].join("\0")).slice(0, 16)}`,
        type,
        sourceType,
...
function findingId(planId, kind, code, references) {
    return `job-plan-${kind}_${hashText([planId, code, ...uniqueSorted(references)].join("\0")).slice(0, 16)}`;
}
function requiredEvidence(context, evidenceId) {
    const evidence = context.evidenceItems.find((entry) => entry.id === evidenceId);
    if (!evidence)
        throw new Error(`Mapped evidence is missing: ${evidenceId}`);
    return evidence;
}
function requiredClaim(context, claimId) {
    const claim = context.claims.find((entry) => entry.id === claimId);
    if (!claim)
        throw new Error(`Mapped claim is missing: ${claimId}`);
    return claim;
}
...
function dependencyMatches(manifest, context) {
    return {
        targetHashMatches: manifest.targetSha256 === context.targetSha256,
        sourceHashMatches: manifest.sourceSha256 === context.sourceSha256,
        requirementModelHashMatches: manifest.requirementModelType === context.requirementInput.type &&
            manifest.requirementModelSha256 === context.requirementInput.modelSha256,
        requirementManifestHashMatches: manifest.requirementManifestSha256 ===
            context.requirementInput.manifestSha256,
        evidenceMapHashMatches: manifest.evidenceMapSha256 === context.evidenceMapSha256,
        evidenceMapManifestHashMatches: manifest.evidenceMapManifestSha256 ===
            context.evidenceMapManifestSha256,
        coverageHashMatches: manifest.coverageSha256 === context.coverageSha256,
        coverageManifestHashMatches: manifest.coverageManifestSha256 === context.coverageManifestSha256,
        assessmentHashMatches: manifest.assessmentSha256 === context.assessmentSha256,
        assessmentManifestHashMatches: manifest.assessmentManifestSha256 === context.assessmentManifestSha256,
        sourcesHashMatches: manifest.sourcesSha256 === context.sourcesSha256,
        evidenceItemsHashMatches: manifest.evidenceItemsSha256 === context.evidenceItemsSha256,
        claimsHashMatches: manifest.claimsSha256 === context.claimsSha256,
        selectedEvidenceSetHashMatches: manifest.selectedEvidenceSetSha256 ===
            context.selectedEvidenceSetSha256,
        selectedClaimSetHashMatches: manifest.selectedClaimSetSha256 === context.selectedClaimSetSha256,
        policyMatches: manifest.policyName === JOB_RESUME_PLANNING_POLICY_NAME &&
            manifest.policyVersion === JOB_RESUME_PLANNING_POLICY_VERSION,
        normalizedInputHashMatches: manifest.normalizedInputSha256 === context.normalizedInputSha256,
    };
}
function dependencyReason(name) {
    const values = {
        ...
        assessmentManifestHashMatches: "Job Fit and Proof Assessment manifest changed.",
...
function validateStoredIdentity(plan, manifest, paths) {
    const reasons = [];
    ...
    if (plan.input.target.sha256 !== manifest.targetSha256 ||
...
function resultFromPlan(plan, paths, result) {
    return {
        targetId: plan.targetId,
        result,
        planId: plan.id,
        ...
        selectedRequirementCount: plan.completeness.selectedRequirementIds.length,
...
function statusWithChecks(base, planHashMatches, checks, status, reasons) {
    return {
        ...base,
        planHashMatches,
...
function resolveWithin(workspace, relativePath) {
    ...
    if (relation === ".." ||
        ...
        throw new Error(`Job Resume Content Planning path escapes the workspace: ${relativePath}`);
    }
    return resolved;
}
