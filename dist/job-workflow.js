...
import { createJobTarget, showTarget, } from "./targets.js";
import { analyzeTarget, getTargetAnalysisStatus } from "./target-analysis.js";
import { buildJobRequirements, getJobRequirementModelStatus, } from "./job-requirements.js";
import { getApprovedJobRequirementsStatus } from "./approved-job-requirements.js";
import { buildEvidenceSnapshot, calculateEvidenceFoundationSnapshot, listEvidenceSnapshots, } from "./evidence-snapshots.js";
import { getTargetEvidencePinStatus, loadTargetEvidencePin, pinTargetEvidenceSnapshot, upgradeTargetEvidenceSnapshot, } from "./target-evidence-pin.js";
...
import { buildJobFitProofAssessment, getJobFitProofAssessmentStatus, } from "./job-fit-proof-assessment.js";
import { buildJobResumePlan, getJobResumePlanStatus, showJobResumePlan, } from "./job-resume-planning.js";
import { buildJobResumeDraftScaffold, getJobResumeDraftScaffoldStatus, } from "./job-resume-drafting.js";
import { generateJobResumeDraftProposal, getJobResumeDraftProposalStatus, listJobResumeDraftProposals, } from "./job-resume-draft-proposal.js";
import { getJobResumeDraftReviewStatus, initializeJobResumeDraftReview, } from "./job-resume-draft-review.js";
import { approveJobResumeDraft, getApprovedJobResumeDraftStatus, } from "./approved-job-resume-draft.js";
import { composeJobResumeRenderDocument, getJobResumeRenderDocumentStatus, } from "./job-resume-rendering.js";
import { exportJobResume, getJobResumeExportStatus, listJobResumeExports, } from "./job-resume-render-export.js";
import { normalizeRoleResumeRenderOptions } from "./role-resume-rendering.js";
import { buildEvidenceReviewBatch, listEvidenceReviewBatches, showEvidenceReviewBatch, } from "./evidence-review-batch.js";
import { getEvidenceReviewWorkspaceStatus, renderEvidenceReviewWorkspace, } from "./evidence-review-workspace.js";
import { listEvidenceClaimReviews } from "./evidence-claim-review.js";
import { stableJson } from "./target-proposal.js";
export const JOB_WORKFLOW_SCHEMA_VERSION = 1;
...
export async function createGuidedJob(workspace, input, options = {}) {
    return createJobTarget(workspace, input, options);
}
export async function inspectJobWorkflow(workspace, targetId, options = {}) {
    ...
    setLifecycleStage(stages, "analysis", analysis.status, analysis.reasons, "Structural analysis is current.");
    ...
    setLifecycleStage(stages, "requirements", requirements.status, requirements.reasons, `${capitalize(requirementSource)} requirement source is current.`);
    ...
    setLifecycleStage(stages, "evidence-pin", pinStatus.status, pinStatus.reasons, pinStatus.snapshotId
    ...
    setStage(stages, "evidence-snapshot", "current", `Pinned snapshot is current with ${eligibleJobEvidenceCount} eligible Job evidence item(s).`);
    ...
    if (reviewGate)
        base.reviewGate = reviewGate;
    if (eligibleJobEvidenceCount === 0) {
        setStage(stages, "evidence-review", "human-action-required", reviewGate?.status === "current"
    ...
    setStage(stages, "evidence-review", "current", "No Evidence Review gate blocks this snapshot; eligible Job evidence is available.");
    ...
    setLifecycleStage(stages, "evidence-mapping", mappingStatus, mappingReasons, "Evidence Map is current and uses only the pinned snapshot.");
    ...
    setLifecycleStage(stages, "coverage", coverage.status, coverage.reasons, "Requirement Coverage is current.");
    ...
    setLifecycleStage(stages, "assessment", assessment.status, assessment.reasons, "Fit and Proof Assessment is current and qualitative.");
    ...
    setLifecycleStage(stages, "planning", planning.status, planning.reasons, "Job Resume Content Plan is current.");
    ...
    if (!plan.completeness.usableForDrafting) {
        setStage(stages, "planning", "current", `Plan is ${plan.completeness.status} but is not usable for drafting.`);
        setStage(stages, "evidence-review", "human-action-required", reviewGate?.status === "current"
            ? `${reviewGate.pendingClaimCount} selected claim review(s) remain pending.`
    ...
    setLifecycleStage(stages, "scaffold", scaffold.status, scaffold.reasons, "Prose-free draft scaffold is current.");
    ...
    if (review.status === "in-progress") {
        setStage(stages, "draft-review", "human-action-required", `${review.unresolvedCount} draft review decision(s) remain unresolved.`);
        return finish(base, {
            currentStage: "draft-review",
            overallState: "paused",
    ...
    if (review.status !== "completed") {
        setStage(stages, "draft-review", review.status === "stale" ? "stale" : "invalid", "Draft review is not current.", review.reasons);
        return stopForLifecycle(base, "draft-review", review.status === "stale" ? "stale" : "invalid", review.reasons);
    }
    ...
    setLifecycleStage(stages, "approved-draft", approved.status, approved.reasons, approved.usableForRendering
    ...
    const composition = await getJobResumeRenderDocumentStatus(workspace, targetId, normalizeRoleResumeRenderOptions(renderOptions));
    setLifecycleStage(stages, "composition", composition.status, composition.reasons, "Canonical render document is current.");
    ...
    const exportStatuses = await Promise.all(exportManifests.map((entry) => getJobResumeExportStatus(workspace, entry.exportId)));
    ...
    const defaultExportsCurrent = DEFAULT_FINAL_FORMATS.every((format) => currentExportFormats.includes(format));
    setStage(stages, "export", defaultExportsCurrent ? "current" : "missing", defaultExportsCurrent
...
export async function runJobWorkflow(workspace, targetId, options = {}) {
    return orchestrateJobWorkflow(workspace, targetId, "run", options);
}
export async function continueJobWorkflow(workspace, targetId, options = {}) {
    return orchestrateJobWorkflow(workspace, targetId, "continue", options);
}
async function orchestrateJobWorkflow(workspace, targetId, mode, options) {
    ...
    if (options.upgradeSnapshotId) {
        ...
        if (pinStatus.status !== "current") {
            const status = await inspectJobWorkflow(workspace, targetId, { requirementSource });
            return resultForPause(mode, status, transitions, `Snapshot upgrade requires a current existing pin; current pin status is ${pinStatus.status}.`);
        }
        const upgraded = await upgradeTargetEvidenceSnapshot(workspace, targetId, options.upgradeSnapshotId);
        transitions.push({
            stage: "evidence-pin",
    ...
    for (let iteration = 0; iteration < JOB_WORKFLOW_STAGES.length * 2; iteration += 1) {
        ...
        if (status.currentStage === "evidence-review") {
            ...
            const review = await ensureReviewWorkspace(workspace, targetId, options.rebuildStale ?? false, transitions);
        ...
        if (status.currentStage === "draft-review") {
            ...
            if (review.status === "missing") {
                ...
                return completeRunResult(mode, await inspectJobWorkflow(workspace, targetId, { requirementSource }), transitions);
            }
            return completeRunResult(mode, status, transitions);
        }
...
export async function finalizeJobWorkflow(workspace, targetId, options = {}) {
    const renderOptions = {
...
export function formatGuidedJobCreation(result) {
    const target = result.target;
    if (target.type !== "job")
        throw new Error("Guided Job creation requires a Job Target result.");
    return [
        "Job Target created",
        "",
        target.title,
...
export function formatJobWorkflowStatus(status, options = {}) {
    const lines = [
    ...
    lines.push("", "Evidence Snapshot:", status.evidenceSnapshot.snapshotId ?? "Not pinned", "", "Eligible Job evidence:", String(status.evidenceSnapshot.eligibleJobEvidenceCount));
    if (status.reviewGate?.workspacePath) {
        lines.push("", "Review workspace:", status.reviewGate.workspacePath, "", "Pending claims:", String(status.reviewGate.pendingClaimCount));
    }
    if (status.reviewGate?.batchId && status.reviewGate.pendingClaimCount > 0) {
        lines.push("", "Open local review UI:", `prooflayer ui review ${status.reviewGate.batchId} --open`);
    }
    ...
    lines.push("", "Next action:", status.nextCommand, "", "Internal references:", `- Target ID: ${status.target.id}`, `- Job Description SHA-256: ${status.target.jobDescriptionSha256}`, `- Requirement source: ${status.requirementSource}`);
    return lines.join("\n");
}
export function formatJobWorkflowRunResult(result) {
    const heading = result.dryRun ? "ProofLayer Job Workflow dry run" : "ProofLayer Job Workflow result";
    const transitions = result.stageResults.filter((entry) => entry.result !== "reused" || result.dryRun);
    return [
        heading,
        "",
...
export function formatFinalizeJobWorkflowResult(result) {
    ...
    for (const format of ["markdown", "html", "docx", "pdf"]) {
    ...
    lines.push("Validation:", result.result === "paused" || result.dryRun
        ? "Not run"
        : result.failed.some((entry) => entry.format !== "pdf")
            ? "Failed"
            : "Passed for every created artifact", "", "Next:", result.status.nextCommand);
    return lines.join("\n");
}
export function formatJobWorkflowJson(value) {
...
function baseStatus(target, requirementSource, availableSnapshots, stages) {
    return {
...
function finish(status, update) {
    return { ...status, ...update };
}
function stopForLifecycle(status, stage, lifecycle, reasons) {
...
function setStage(stages, stage, status, detail, reasons = []) {
    const entry = stages.find((candidate) => candidate.stage === stage);
    if (!entry)
        throw new Error(`Unknown Job workflow stage: ${stage}`);
    entry.status = status;
    entry.detail = detail;
    entry.reasons = [...reasons];
}
function setLifecycleStage(stages, stage, status, reasons, currentDetail) {
...
function stageByName(status, stage) {
    const value = status.stages.find((entry) => entry.stage === stage);
    if (!value)
        throw new Error(`Workflow status omitted stage ${stage}.`);
    return value;
}
function canBuildLifecycle(stage, rebuildStale = false) {
    if (stage.status === "missing")
        return true;
    return rebuildStale && (stage.status === "stale" || stage.status === "invalid");
}
async function inspectReviewGate(workspace, targetId) {
    ...
    const pendingClaimCount = claimIds.filter((claimId) => effectiveByClaim.get(claimId)?.status !== "current").length;
...
async function ensureReviewWorkspace(workspace, targetId, rebuildStale, transitions) {
    ...
    if (!context?.batchId)
        throw new Error("Evidence Review batch creation did not produce an inspectable batch.");
...
async function currentProposal(workspace, targetId) {
...
function resolveGuidedProvider(options) {
    if (options.provider)
...
function dryRunResults(status, options) {
...
function stageBuildResult(stage, result, label) {
    return {
        stage,
...
function completeRunResult(mode, status, transitions) {
    return {
        mode,
        dryRun: false,
        status,
...
function resultForPause(mode, status, transitions, message) {
...
function summarizeStageResults(status, transitions) {
    ...
    return status.stages.flatMap((stage) => {
        ...
        if (explicit?.length)
            return explicit;
        if (stage.status === "current") {
            return [{ stage: stage.stage, result: "reused", detail: stage.detail }];
        }
        if (stage.status === "human-action-required") {
            return [{ stage: stage.stage, result: "paused", detail: stage.detail }];
        }
        if (["stale", "invalid", "incompatible"].includes(stage.status)) {
            return [{ stage: stage.stage, result: "paused", detail: stage.detail }];
        }
        return [{ stage: stage.stage, result: "skipped", detail: stage.detail }];
    });
}
...
function stageMarker(status) {
    if (status === "current")
        return "[x]";
    if (status === "human-action-required")
        return "[pause]";
    if (["stale", "invalid", "incompatible"].includes(status))
        return "[!]";
    if (status === "blocked")
        return "[-]";
    return "[ ]";
