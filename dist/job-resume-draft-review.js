...
import { JobResumeDraftAmbiguitySchema, JobResumeDraftClaimLedgerEntrySchema, JobResumeDraftItemSchema, JobResumeDraftReviewManifestSchema, JobResumeDraftReviewSchema, JobResumeDraftSectionSchema, } from "./job-resume-draft-schemas.js";
import { getJobResumeDraftProposalStatus, showJobResumeDraftProposal, validateJobResumeDraftPayload, } from "./job-resume-draft-proposal.js";
import { loadJobResumeDraftingContext, showJobResumeDraftScaffold, } from "./job-resume-drafting.js";
export async function initializeJobResumeDraftReview(workspace, proposalId, options = {}) {
    ...
    if (await pathExists(paths.reviewPath))
        return showJobResumeDraftReview(workspace, proposalId);
    ...
    const decisions = [
        ...
        ...proposal.sections.flatMap((entry) => entry.items.map((item) => pending(reviewId, "draft-item", item.id))),
...
export async function setJobResumeDraftReviewDecision(workspace, proposalId, itemType, itemId, input) {
    const review = await showJobResumeDraftReview(workspace, proposalId);
    if (review.status === "completed")
        throw new Error("Completed Job resume draft reviews are immutable.");
    const index = review.decisions.findIndex((entry) => entry.itemType === itemType && entry.itemId === itemId);
    if (index < 0)
        throw new Error(`Unknown Job resume draft review item: ${itemType}/${itemId}`);
...
export async function completeJobResumeDraftReview(workspace, proposalId, options = {}) {
    const review = await showJobResumeDraftReview(workspace, proposalId);
    if (review.status === "completed")
    ...
    const validated = validateJobResumeDraftPayload({ sections: merged.sections, warnings: proposal.warnings, ambiguities: merged.ambiguities }, proposal.id, scaffold, context, proposal.input.scaffoldSha256);
    ...
    if (validated.claimLedger.length !== items.length)
        throw new Error("Job draft claim ledger is incomplete.");
    review.status = "completed";
    review.updatedAt = (options.now ?? (() => new Date()))().toISOString();
    await writeReview(workspace, review, jobResumeDraftReviewPaths(workspace, review.targetId, proposalId));
    return getJobResumeDraftReviewStatus(workspace, review.id);
}
export async function showJobResumeDraftReview(workspace, reviewOrProposalId) {
    const location = await locateReview(workspace, reviewOrProposalId);
    if (!location)
        throw new Error(`Job resume draft review not found: ${reviewOrProposalId}`);
    return JobResumeDraftReviewSchema.parse(await readJson(location.reviewPath, null));
}
export async function getJobResumeDraftReviewStatus(workspace, reviewOrProposalId) {
    const location = await locateReview(workspace, reviewOrProposalId);
    if (!location)
        return emptyReviewStatus(reviewOrProposalId, "unknown", "unknown", "missing", ["Review not found."]);
    ...
    try {
        const review = JobResumeDraftReviewSchema.parse(await readJson(location.reviewPath, null));
        const manifest = JobResumeDraftReviewManifestSchema.parse(await readJson(location.manifestPath, null));
        ...
        const proposalPath = resolveWithin(workspace, `targets/jobs/${review.targetId}/resume-drafting/proposals/${review.proposalId}/proposal.json`);
        ...
        if (proposalStatus.status !== "current") {
            return emptyReviewStatus(review.id, review.proposalId, review.targetId, proposalStatus.status === "stale" ? "stale" : "invalid", ["Reviewed proposal is not current."], location);
        }
...
export async function mergeReviewedJobResumeDraft(workspace, proposalId, suppliedReview) {
    const proposal = await showJobResumeDraftProposal(workspace, proposalId);
    ...
    const context = await loadJobResumeDraftingContext(workspace, proposal.targetId);
    const scaffold = await showJobResumeDraftScaffold(workspace, proposal.targetId);
    const validated = validateJobResumeDraftPayload({ sections, warnings: proposal.warnings, ambiguities }, proposal.id, scaffold, context, proposal.input.scaffoldSha256);
    return {
...
export async function readJobResumeDraftReviewEdit(filePath) {
...
export function formatJobResumeDraftReviewStatus(status) {
    return [
...
async function validateReviewEdit(workspace, proposalId, itemType, itemId, value) {
    const proposal = await showJobResumeDraftProposal(workspace, proposalId);
    ...
    if (!original)
        throw new Error(`Unknown Job resume draft item: ${itemId}`);
...
async function validateSimulatedEdit(workspace, proposalId, itemType, itemId, value) {
    const proposal = await showJobResumeDraftProposal(workspace, proposalId);
    const sections = proposal.sections.map((section) => {
        if (itemType === "section" && section.id === itemId)
            return value;
    ...
    const scaffold = await showJobResumeDraftScaffold(workspace, proposal.targetId);
    const context = await loadJobResumeDraftingContext(workspace, proposal.targetId);
    const result = validateJobResumeDraftPayload({ sections, warnings: proposal.warnings, ambiguities: proposal.ambiguities }, proposal.id, scaffold, context, proposal.input.scaffoldSha256);
...
function pending(reviewId, itemType, itemId) {
    return {
...
export function jobResumeDraftReviewPaths(workspace, targetId, proposalId) {
...
async function locateReview(workspace, reviewOrProposalId) {
    ...
    for (const target of await readdir(jobsRoot, { withFileTypes: true })) {
        ...
        for (const proposal of await readdir(reviewsRoot, { withFileTypes: true })) {
            ...
            try {
                const review = JobResumeDraftReviewSchema.parse(await readJson(paths.reviewPath, null));
...
function emptyReviewStatus(reviewId, proposalId, targetId, status, reasons, paths) {
    return {
        reviewId,
        proposalId,
        targetId,
