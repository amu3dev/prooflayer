...
The Local Evidence Review UI is a thin, loopback-only Astro adapter over these same domain services. It locks one validated batch into the server process, renders human-readable batch and claim views, and submits decisions only through the canonical Evidence Claim Review service. Simple Review is the default human-intent layer: one explicit decision is deterministically projected into canonical fields from immutable claim and evidence metadata, with minimal follow-up only for unresolved classification or metric confirmation. Advanced Review preserves the complete canonical form for exceptional cases. Both paths pass through the unchanged canonical schema and authoritative immutable service; the intent adapter has no review, validation, eligibility, lifecycle, or persistence authority of its own. Read-only mode exposes no submission form and performs no writes. Completion reports the explicit guided-workflow command but never upgrades a snapshot, changes a target pin, or continues the pipeline automatically.
