...
const TargetIdSchema = z.string().regex(
  /^[a-z0-9]+(?:-[a-z0-9]+)*$/,
  "Target ID must contain only lowercase letters, numbers, and single hyphens."
);
...
export const EvidenceMatchReviewDecisionSchema = z.object({
  ...
  if (value.decision === "edit" && !value.editedMatch) context.addIssue({ code: z.ZodIssueCode.custom, message: "Edit requires editedMatch", path: ["editedMatch"] });
  if (value.decision !== "edit" && value.editedMatch) context.addIssue({ code: z.ZodIssueCode.custom, message: "Only edit may contain editedMatch", path: ["editedMatch"] });
});
...
export const ExpectationCoverageReviewDecisionSchema = z.object({
  ...
  if (value.decision === "edit" && !value.editedStatus) context.addIssue({ code: z.ZodIssueCode.custom, message: "Coverage edit requires editedStatus", path: ["editedStatus"] });
  if (value.decision !== "edit" && value.editedStatus) context.addIssue({ code: z.ZodIssueCode.custom, message: "Only coverage edit may contain editedStatus", path: ["editedStatus"] });
});
