...
export async function refreshWorkspace(workspace: string, options: RefreshOptions = {}): Promise<LatestRefresh> {
  const baselinePath = path.join(workspace, UPDATE_BASELINE_PATH);
  const previous = await readJson<RefreshBaseline | null>(baselinePath, null);
...
export async function getLatestChanges(workspace: string): Promise<LatestRefresh | null> {
  return readJson<LatestRefresh | null>(path.join(workspace, LATEST_REFRESH_PATH), null);
}
...
export async function getWorkspaceStatus(workspace: string): Promise<WorkspaceStatus> {
  ...
  const outputs = fingerprint
    ...
    : { status: "none_registered", registered: 0, current: 0, stale: 0, drafts: 0, finals: 0, exports: 0 } as OutputStaleness;
...
export function formatChangesSummary(latest: LatestRefresh | null): string {
  if (!latest) {
...
export function formatStatusSummary(status: WorkspaceStatus): string {
  return [
...
export async function inspectOutputStaleness(workspace: string, profileFingerprint: string): Promise<OutputStaleness> {
  ...
  const manifest = await readJson<OutputManifest>(manifestPath, { outputs: [] });
  const outputs = Array.isArray(manifest.outputs) ? manifest.outputs : [];
  if (outputs.length === 0) return { status: "none_registered", registered: 0, current: 0, stale: 0, drafts: 0, finals: 0, exports: 0 };
  const currentStates = await Promise.all(outputs.map(async (output) => {
    if (output.profileFingerprint !== profileFingerprint) return false;
    if (output.publicationStatus !== "export") return true;
    if (!output.sourceMarkdownPath || !output.sourceMarkdownHash) return false;
...
function buildLatestRefresh(
  previous: RefreshBaseline | null,
...
async function loadKnowledgeState(workspace: string): Promise<KnowledgeState> {
  const sources = await readJson<Source[]>(path.join(workspace, "kb/sources.json"), []);
  ...
  if (!profile) throw new Error("Refresh pipeline did not produce kb/career-profile.json.");
  const privacyFindings: PrivacyFinding[] = auditSourcesAndEvidence(sources, evidenceItems, claims);
  return { sources, evidenceItems, claims, profile, privacyFindings };
}

function buildWarnings(baseline: RefreshBaseline, outputs: OutputStaleness): string[] {
  const warnings: string[] = [];
  if (baseline.trustCounts.resumeReady === 0) warnings.push("No claims are currently resume-ready.");
  if (baseline.privacyCounts.high > 0) warnings.push("High-risk privacy findings require review.");
  if (baseline.privacyCounts.medium > 0) warnings.push("Medium-risk privacy findings require review before public output.");
  if (outputs.status === "stale") warnings.push(`${outputs.stale} registered output(s) are stale.`);
...
function renderUpdateImpactReport(latest: LatestRefresh): string {
  const areaLine = (label: string, counts: ChangeCounts): string =>
    `- ${label}: +${counts.added} added, -${counts.removed} removed, ~${counts.changed} changed, ${counts.unchanged} unchanged`;
...
function formatOutputStatus(outputs: OutputStaleness): string {
  if (outputs.status === "none_registered") return "none registered";
  const types = `draft ${outputs.drafts}, final ${outputs.finals}${outputs.exports > 0 ? `, export ${outputs.exports}` : ""}`;
  if (outputs.status === "stale") return `${outputs.stale} stale of ${outputs.registered} registered (${types})`;
