...
import {
  ...
  TargetFitAssessmentSchema,
  type AssessmentConfidence,
  type AssessmentMode,
  type AssessmentRisk,
  type Defensibility,
  type EvidenceActionRecommendation,
  type EvidenceSufficiency,
  type ExpectationFitAssessment,
  type FitAssessmentAmbiguity,
  type FitAssessmentCompleteness,
  type FitAssessmentManifest,
  type FitAssessmentSummary,
  type FitAssessmentWarning,
  type FreshnessRisk,
  type GapType,
  type JobFitAssessmentSummary,
  type Materiality,
  type ProofQuality,
  type RoleFitAssessmentSummary,
  type SupportStatus,
...
export async function loadAssessmentContext(
  ...
  const interpretationManifest = ApprovedTargetInterpretationManifestSchema.parse(
    await readJson<unknown>(resolveWithin(workspace, approvedInterpretationManifestPath), null),
  );
  const matchingManifest = MatchingManifestSchema.parse(
    await readJson<unknown>(resolveWithin(workspace, approvedMatchingManifestPath), null),
  );
  if (
    approvedMatching.approvedInterpretation.sha256 !== interpretationManifest.approvedInterpretationSha256 ||
...
export async function buildFitAssessment(
  workspace: string,
  targetId: string,
  options: BuildFitAssessmentOptions = {},
): Promise<BuildFitAssessmentResult> {
  ...
  if (await pathExists(paths.assessmentPath)) {
    try {
      createdAt = TargetFitAssessmentSchema.parse(await readJson<unknown>(paths.assessmentPath, null)).createdAt;
    } catch {
...
export async function promoteDeterministicRoleFitAssessment(
  workspace: string,
  targetId: string,
  options: BuildFitAssessmentOptions = {},
): Promise<BuildFitAssessmentResult> {
  const target = await showTarget(workspace, targetId);
  if (target.type !== "role") throw new Error("Deterministic Role assessment promotion accepts Role Targets only.");
  ...
  return buildResult(
    assessment,
    paths,
    approvedStatus.status === "missing" ? "created" : "rebuilt",
  );
}

export async function showFitAssessment(
  workspace: string,
  targetId: string,
  artifactType: "deterministic" | "approved" = "deterministic",
): Promise<TargetFitAssessment> {
  const target = await showTarget(workspace, targetId);
  const paths = assessmentPaths(workspace, target, artifactType);
  if (!(await pathExists(paths.assessmentPath))) throw new Error(`${artifactType} fit assessment not found: ${targetId}`);
  return TargetFitAssessmentSchema.parse(await readJson<unknown>(paths.assessmentPath, null));
}

export async function getFitAssessmentStatus(
  workspace: string,
  targetId: string,
  artifactType: "deterministic" | "approved" = "deterministic",
): Promise<FitAssessmentStatus> {
  const target = await showTarget(workspace, targetId);
  ...
  const base = { targetId, targetType: target.type, mode, artifactType, assessmentExists, manifestExists, assessmentPath: paths.assessmentRelativePath, manifestPath: paths.manifestRelativePath } as const;
  if (!assessmentExists && !manifestExists) return emptyStatus(base, "missing", [`No ${artifactType} fit assessment exists.`]);
  if (!assessmentExists || !manifestExists) return emptyStatus(base, "invalid", [`${artifactType} assessment artifact set is incomplete.`]);
  let assessment: TargetFitAssessment;
  let manifest: FitAssessmentManifest;
  try {
    assessment = TargetFitAssessmentSchema.parse(await readJson<unknown>(paths.assessmentPath, null));
    manifest = FitAssessmentManifestSchema.parse(await readJson<unknown>(paths.manifestPath, null));
  } catch (error) {
  ...
  if (!assessmentHashMatches) invalidReasons.push("Assessment SHA-256 does not match its manifest.");
  if (
  ...
  if (invalidReasons.length) return { ...emptyStatus(base, "invalid", invalidReasons), assessmentHashMatches };
...
export function deriveTargetFitAssessment(
  ...
  const assessments = context.approvedInterpretation.expectations.map((expectation) => {
    ...
    if (!coverage) throw new Error(`Approved matching lacks coverage for expectation: ${expectation.id}`);
...
export function deriveSupportStatus(
  coverage: ExpectationCoverageRecord["status"],
  matches: EvidenceMatch[],
): SupportStatus {
  if (coverage === "not-assessed") return "not-assessed";
  if (coverage === "unsupported") return "unsupported";
  if (coverage === "conflicting") return "conflicting";
  if (coverage === "partially-matched") return "partially-supported";
...
export function deriveMateriality(
  necessity: ApprovedTargetExpectation["necessity"],
  importance: ApprovedTargetExpectation["importance"],
): Materiality {
  if (necessity === "unknown" || importance === "unknown") return "unknown";
  if (necessity === "required") return importance;
  if (necessity === "preferred") return ["critical", "high"].includes(importance) ? "medium" : "low";
  if (necessity === "contextual") return ["critical", "high"].includes(importance) ? "medium" : "low";
  return "unknown";
}

export function deriveSummary(
  mode: AssessmentMode,
  assessments: ExpectationFitAssessment[],
  completeness: FitAssessmentCompleteness,
...
export function deriveRoleSummary(
  assessments: ExpectationFitAssessment[],
  completeness: FitAssessmentCompleteness,
): RoleFitAssessmentSummary {
  ...
  let overallPositioning: RoleFitAssessmentSummary["overallPositioning"];
  if (completeness.status !== "complete") overallPositioning = "incomplete";
  else if (criticalConflict) overallPositioning = "conflicting";
  else if (assessments.length > 0 && allMaterialSupported && counts.partiallySupported === 0 && counts.unsupported === 0 && counts.conflicting === 0) overallPositioning = "well-supported";
  else if (positive > 0) overallPositioning = "supported-with-gaps";
  else if (counts.partiallySupported > 0) overallPositioning = "partially-supported";
  else overallPositioning = "insufficient-evidence";
  return {
...
export function deriveJobSummary(
  ...
  let opportunityAlignment: JobFitAssessmentSummary["opportunityAlignment"];
  if (completeness.status !== "complete") opportunityAlignment = "incomplete";
  else if (materialConflict) opportunityAlignment = "material-conflict";
  else if (required.length > 0 && requiredCounts.unsupported === 0 && requiredCounts.conflicting === 0 && requiredCounts.partiallySupported === 0 && requiredCounts.notAssessed === 0) opportunityAlignment = "strong-alignment";
  else if (required.length > 0 && requiredCounts.unsupported === 0 && requiredCounts.conflicting === 0 && requiredPositive > 0) opportunityAlignment = "credible-alignment";
  else if (requiredPositive > 0) opportunityAlignment = "mixed-alignment";
  else opportunityAlignment = "weak-evidence-alignment";
  return {
    mode: "job-specific",
...
export function assertAssessmentConsistency(
  assessment: TargetFitAssessment,
  context: AssessmentContext,
): void {
  ...
  if (assessment.expectationAssessments.length !== expectations.size) throw new Error("Assessment does not cover every approved expectation.");
  for (const entry of assessment.expectationAssessments) {
    if (!expectations.has(entry.expectationId)) throw new Error(`Assessment references an unknown expectation: ${entry.expectationId}`);
    if (entry.approvedMatchIds.some((id) => !matches.has(id))) throw new Error(`Assessment references an unknown approved match: ${entry.expectationId}`);
    if (entry.evidenceIds.some((id) => !evidenceIds.has(id))) throw new Error(`Assessment references unknown reviewed evidence: ${entry.expectationId}`);
    ...
    if (entry.supportStatus === "unsupported" && entry.approvedMatchIds.length > 0) throw new Error("Unsupported assessment cannot contain approved support matches.");
    ...
    if (entry.supportStatus === "not-assessed" && assessment.completeness.status === "complete") throw new Error("A complete assessment cannot contain not-assessed expectations.");
  ...
  if (/\b\d+(?:\.\d+)?%\b/.test(assessment.summary.narrative)) throw new Error("Fit percentages are forbidden in assessment summaries.");
  if (/\b(should|recommend)\s+(apply|hire|interview)\b/i.test(assessment.summary.narrative)) throw new Error("Hiring and application recommendations are forbidden.");
}

export function createAssessmentManifest(
  assessment: TargetFitAssessment,
  context: AssessmentContext,
  assessmentPath: string,
  ...
  createdAt: string,
  updatedAt: string,
  reviewDependencies: { proposalId?: string; proposalSha256?: string; reviewSha256?: string } = {},
...
export function deterministicAssessmentId(context: AssessmentContext): string {
...
export function expectationAssessmentId(targetId: string, expectationId: string, approvedMatchingSha256: string): string {
...
export function formatBuildFitAssessmentResult(result: BuildFitAssessmentResult): string {
  return [
...
export function formatFitAssessmentStatus(status: FitAssessmentStatus): string {
...
export function assessmentPaths(workspace: string, target: Target, artifactType: "deterministic" | "approved") {
...
function deriveProofQuality(status: SupportStatus, matches: EvidenceMatch[]): ProofQuality {
  if (status === "strongly-supported") return "strong";
  if (status === "supported") return "adequate";
  if (status === "unsupported") return "none";
  if (status === "conflicting") return "conflicting";
  if (status === "not-assessed") return "unknown";
  if (matches.some((match) => ["strong", "medium"].includes(match.evidenceStrength))) return "limited";
  return matches.length ? "weak" : "none";
}

function deriveEvidenceSufficiency(status: SupportStatus): EvidenceSufficiency {
  if (["strongly-supported", "supported"].includes(status)) return "sufficient";
  if (status === "partially-supported") return "partially-sufficient";
  if (["unsupported", "conflicting"].includes(status)) return "insufficient";
  return "not-evaluated";
}

function deriveDefensibility(status: SupportStatus, quality: ProofQuality): Defensibility {
  if (status === "strongly-supported" && quality === "strong") return "high";
  if (status === "supported") return "medium";
  if (status === "partially-supported") return "low";
  if (["unsupported", "conflicting"].includes(status)) return "none";
  return "uncertain";
}

function deriveFreshnessRisk(matches: EvidenceMatch[]): FreshnessRisk {
  if (!matches.length) return "unknown";
  if (matches.every((match) => match.temporalRelevance === "historical")) return "high";
  if (matches.some((match) => match.temporalRelevance === "historical")) return "medium";
  if (matches.every((match) => match.temporalRelevance === "unknown")) return "unknown";
  if (matches.some((match) => match.temporalRelevance === "recent")) return "low";
  return "none";
}

function deriveGapType(status: SupportStatus, matches: EvidenceMatch[], freshness: FreshnessRisk): GapType {
  if (status === "not-assessed") return "not-assessed";
  if (status === "conflicting") return "contradiction";
  if (status === "unsupported") return "evidence-gap";
  if (freshness === "high") return "freshness-gap";
  if (status === "partially-supported") {
    if (matches.some((match) => match.matchType === "partial" || match.coverage === "partial")) return "coverage-gap";
    if (matches.length > 0 && matches.every((match) => match.evidenceProvenance.every((entry) => entry.evidenceType === "role"))) return "specificity-gap";
    return "specificity-gap";
  }
  return "none";
}

function deriveAssessmentConfidence(status: SupportStatus, matches: EvidenceMatch[]): AssessmentConfidence {
  if (status === "not-assessed") return "low";
  if (status === "unsupported") return "medium";
  if (!matches.length) return "low";
  if (matches.every((match) => match.matchConfidence === "high")) return "high";
  if (matches.some((match) => match.matchConfidence === "medium")) return "medium";
  return "low";
}

function deriveEvidenceActions(gapType: GapType, status: SupportStatus, evidenceIds: string[]): EvidenceActionRecommendation[] {
  const action = (type: EvidenceActionRecommendation["type"], priority: EvidenceActionRecommendation["priority"], rationale: string): EvidenceActionRecommendation => ({ type, priority, rationale, relatedEvidenceIds: evidenceIds });
  if (status === "strongly-supported") return [action("no-action", "low", "Current reviewed evidence is sufficiently specific and defensible for this expectation.")];
  if (gapType === "coverage-gap") return [action("separate-compound-claim", "high", "Separate the expectation or add evidence for the uncovered portion.")];
  if (gapType === "specificity-gap") return [action("add-specific-example", "high", "Add a reviewed example that demonstrates the expectation directly.")];
  if (gapType === "freshness-gap") return [action("add-recent-example", "high", "Add a recent reviewed example where current practice matters.")];
  if (gapType === "contradiction") return [action("resolve-contradiction", "high", "Resolve the approved contradictory evidence before relying on this expectation.")];
  if (gapType === "evidence-gap") return [action("review-unreviewed-source", "medium", "Review existing source material or add a specific example; missing evidence does not prove missing capability.")];
  if (gapType === "not-assessed") return [action("verify-claim", "high", "Complete reviewed evidence matching before using this expectation downstream.")];
  return [action("no-action", "low", "No immediate evidence action is required beyond preserving current limitations.")];
}

function deterministicRationale(status: SupportStatus, coverage: ExpectationCoverageRecord, matches: EvidenceMatch[]): string {
  const count = matches.length;
  if (status === "strongly-supported") return "Approved direct, strong, current or recent evidence fully supports this expectation with complete provenance.";
  if (status === "supported") return "Approved evidence materially and defensibly supports this expectation, with limitations below where applicable.";
  if (status === "partially-supported") return `Approved evidence supports only part of this expectation or is indirect; ${count} approved match(es) are retained with explicit limitations.`;
  if (status === "unsupported") return "The reviewed evidence base was assessed and does not currently prove this expectation; this does not prove the capability is absent.";
  if (status === "conflicting") return "Approved contradictory evidence materially conflicts with this expectation and must be resolved before reliance.";
  return coverage.blockingReasons[0] ?? "No completed approved matching decision exists for this expectation.";
}

export function deriveCompleteness(targetType: "role" | "job", assessments: ExpectationFitAssessment[], matchingStatus: "empty" | "partial" | "complete"): FitAssessmentCompleteness {
...
export function deriveRisks(targetId: string, assessments: ExpectationFitAssessment[], completeness: FitAssessmentCompleteness): AssessmentRisk[] {
  const risks: AssessmentRisk[] = [];
  const add = (code: AssessmentRisk["code"], severity: AssessmentRisk["severity"], message: string, entry?: ExpectationFitAssessment) => risks.push({
  ...
  if (completeness.status !== "complete") add("ASSESSMENT_INCOMPLETE", "high", "Approved matching is incomplete, so downstream construction is blocked.");
  for (const entry of assessments) {
    if (entry.supportStatus === "unsupported" && entry.materiality === "critical") add("CRITICAL_REQUIREMENT_UNSUPPORTED", "critical", "A critical expectation is unsupported by the reviewed evidence base.", entry);
    if (entry.supportStatus === "partially-supported" && entry.expectation.necessity === "required") add("REQUIRED_EXPECTATION_PARTIALLY_SUPPORTED", "high", "A required expectation has only partial reviewed support.", entry);
    if (entry.supportStatus === "conflicting") add("MATERIAL_CONTRADICTION", ["critical", "high"].includes(entry.materiality) ? "critical" : "high", "Approved evidence materially contradicts this expectation.", entry);
    if (entry.gapType === "specificity-gap") add("EVIDENCE_TOO_GENERAL", "medium", "Reviewed evidence is too general for the expectation.", entry);
    if (entry.gapType === "freshness-gap") add("EVIDENCE_TOO_OLD", "medium", "Reviewed evidence is historical where recency may matter.", entry);
    if (entry.proofQuality === "weak") add("EVIDENCE_TOO_WEAK", "medium", "Available reviewed evidence is weak.", entry);
    if (entry.gapType === "coverage-gap") add("COMPOUND_EXPECTATION_PARTIALLY_COVERED", "medium", "Only part of a compound expectation is covered.", entry);
  }
  return risks.sort((a, b) => a.id.localeCompare(b.id));
}

export function deriveWarnings(targetId: string, assessments: ExpectationFitAssessment[], completeness: FitAssessmentCompleteness): FitAssessmentWarning[] {
  const warnings: FitAssessmentWarning[] = [];
  const add = (code: FitAssessmentWarning["code"], message: string, entries: ExpectationFitAssessment[] = []) => warnings.push({
  ...
  if (completeness.status !== "complete") add("MATCHING_NOT_COMPLETE", "Approved matching is incomplete; this assessment is a draft and is unusable for downstream construction.");
  const supported = assessments.filter((entry) => ["strongly-supported", "supported", "partially-supported"].includes(entry.supportStatus));
  if (!supported.length) add("NO_SUPPORTED_EXPECTATIONS", "No expectation currently has approved reviewed support.");
  if (!assessments.some((entry) => entry.expectation.necessity === "required")) add("NO_REQUIRED_EXPECTATIONS_IDENTIFIED", "No required expectations were identified in the approved interpretation.");
  const unsupported = assessments.filter((entry) => entry.supportStatus === "unsupported");
  if (unsupported.length) add("UNSUPPORTED_EXPECTATION_DOES_NOT_PROVE_NO_EXPERIENCE", "Unsupported means the reviewed evidence base does not currently prove the expectation; it does not prove the capability is absent.", unsupported);
  const matchingEntries = supported.filter((entry) => entry.approvedMatchIds.length > 0);
  if (matchingEntries.length && matchingEntries.every((entry) => entry.provenance.deterministicInputs.matchTypes.every((type) => type !== "direct"))) add("ONLY_SUPPORTING_EVIDENCE_AVAILABLE", "Only supporting or partial evidence is available for supported expectations.", matchingEntries);
  if (matchingEntries.length && matchingEntries.every((entry) => entry.provenance.deterministicInputs.temporalRelevance.every((value) => value === "historical"))) add("ONLY_HISTORICAL_EVIDENCE_AVAILABLE", "Only historical evidence is available for supported expectations.", matchingEntries);
  return warnings.sort((a, b) => a.id.localeCompare(b.id));
}

export function deriveAmbiguities(targetId: string, assessments: ExpectationFitAssessment[]): FitAssessmentAmbiguity[] {
  const ambiguities: FitAssessmentAmbiguity[] = [];
  const add = (code: FitAssessmentAmbiguity["code"], message: string, entry: ExpectationFitAssessment) => ambiguities.push({
  ...
  for (const entry of assessments) {
    if (entry.gapType === "evidence-gap") add("EVIDENCE_GAP_VS_EXPERIENCE_GAP_UNCLEAR", "The current evidence gap cannot establish whether the capability itself is absent.", entry);
    if (entry.supportStatus === "supported" && entry.assessmentConfidence !== "high") add("SUPPORTED_VS_STRONGLY_SUPPORTED_UNCLEAR", "Evidence is supportable, but available quality or confidence does not justify strong support.", entry);
    if (entry.materiality === "unknown") add("MATERIALITY_UNCLEAR", "Approved necessity or importance is unknown.", entry);
    if (entry.freshnessRisk === "unknown" && entry.evidenceIds.length) add("FRESHNESS_RELEVANCE_UNCLEAR", "Evidence temporal relevance is unknown.", entry);
    if (entry.gapType === "coverage-gap") add("COMPOUND_EXPECTATION_BOUNDARY_UNCLEAR", "The boundary between covered and uncovered parts requires review.", entry);
    if (entry.supportStatus === "conflicting" && !["critical", "high"].includes(entry.materiality)) add("CONTRADICTION_MATERIALITY_UNCLEAR", "Contradictory evidence exists but its materiality is not high or critical.", entry);
  }
  return ambiguities.sort((a, b) => a.id.localeCompare(b.id));
}

function requirementCounts(entries: ExpectationFitAssessment[]) {
...
function supportCounts(entries: ExpectationFitAssessment[]) {
...
function assertSummaryCounts(summary: FitAssessmentSummary, assessments: ExpectationFitAssessment[]): void {
  if (summary.mode === "role-positioning") {
    const counts = supportCounts(assessments);
    if (
      summary.stronglySupportedCount !== counts.stronglySupported ||
  ...
  for (const [actual, entries] of groups) {
    if (stableJson(actual) !== stableJson(requirementCounts(entries))) throw new Error("Job assessment summary counts are inconsistent.");
  }
}

function roleNarrative(category: RoleFitAssessmentSummary["overallPositioning"]): string {
  const narratives: Record<typeof category, string> = {
...
function jobNarrative(category: JobFitAssessmentSummary["opportunityAlignment"]): string {
  const narratives: Record<typeof category, string> = {
...
function buildResult(assessment: TargetFitAssessment, paths: ReturnType<typeof assessmentPaths>, result: BuildFitAssessmentResult["result"]): BuildFitAssessmentResult {
  return {
...
function emptyStatus(
  base: Pick<FitAssessmentStatus, "targetId" | "targetType" | "mode" | "artifactType" | "assessmentExists" | "manifestExists" | "assessmentPath" | "manifestPath">,
  status: FitAssessmentStatus["status"],
  reasons: string[],
): FitAssessmentStatus {
  return {
    ...base,
...
async function dependencyHashMatches(workspace: string, target: Target, directory: "proposals" | "reviews", proposalId: string, fileName: "proposal.json" | "review.json", expectedSha256: string): Promise<boolean> {
