...
import {
  ...
  derivePlanAmbiguities,
  derivePlanCompleteness,
  derivePlanRisks,
  derivePlanWarnings,
...
export async function approveRoleResumePlanProposal(
  workspace: string,
  proposalId: string,
  options: ApproveRoleResumePlanOptions = {},
): Promise<ApproveRoleResumePlanResult> {
  const proposalStatus = await getRoleResumePlanProposalStatus(workspace, proposalId);
  if (proposalStatus.status !== "current" || !proposalStatus.readyForReview) throw new Error("Cannot approve a stale, invalid, or unreviewable role resume plan proposal.");
  const reviewStatus = await getRoleResumePlanReviewStatus(workspace, proposalId);
  if (reviewStatus.status !== "completed") throw new Error(`Role resume plan review must be completed before approval. Current status: ${reviewStatus.status}`);
  ...
  if (deterministicStatus.status !== "current") throw new Error("Deterministic role resume plan must be current before approval.");
  ...
  const proposalManifest = RoleResumePlanProposalManifestSchema.parse(await readJson<unknown>(proposalManifestPath, null));
  const reviewManifest = RoleResumePlanReviewManifestSchema.parse(await readJson<unknown>(reviewManifestPath, null));
  if (reviewManifest.proposalSha256 !== proposalManifest.proposalSha256) throw new Error("Review does not reference the current proposal hash.");
  ...
  if (existingStatus.status === "current" && await pathExists(paths.manifestPath)) {
    const manifest = await readJson<Record<string, unknown>>(paths.manifestPath, {});
  ...
  if (["stale", "invalid"].includes(existingStatus.status) && !options.rebuild) throw new Error(`Approved role resume plan is ${existingStatus.status}; use explicit --rebuild.`);
  ...
  const completeness = derivePlanCompleteness(
    reviewed.expectationSelections,
    reviewed.sections,
    reviewed.positioning,
    reviewed.claimBoundaries,
    context.approvedAssessment.completeness.status,
    false,
  );
  ...
  if (containsUnapprovedTrust(approved)) throw new Error("Approved plan contains proposed or rejected trust state.");
  await writeJsonAtomic(paths.planPath, approved);
  const manifest = createRoleResumePlanManifest(
    approved,
    context,
...
export function formatApproveRoleResumePlanResult(result: ApproveRoleResumePlanResult) {
  return [
...
function applyReviewTrust(
  payload: ModelRoleResumePlanPayload,
  review: RoleResumePlanReview,
  proposal: Awaited<ReturnType<typeof showRoleResumePlanProposal>>,
): ModelRoleResumePlanPayload {
  const decorate = <T extends { id: string; provenance: Record<string, unknown>; trustState: string }>(type: string, value: T): T => {
    const decision = review.decisions.find((entry) => entry.itemType === type && entry.itemId === value.id);
    if (!decision || decision.decision === "pending") throw new Error(`Missing completed review decision: ${type}/${value.id}`);
    if (decision.decision === "reject") return { ...value, trustState: "deterministic-approved" } as T;
    return {
      ...value,
  ...
  if (!positioningDecision || positioningDecision.decision === "pending") throw new Error("Positioning review is incomplete.");
  ...
  } as ModelRoleResumePlanPayload["positioning"];
  return {
    ...payload,
    positioning,
...
function containsUnapprovedTrust(plan: RoleResumeContentPlan) {
  return JSON.stringify(plan).includes('"trustState":"proposed"') || JSON.stringify(plan).includes('"trustState":"rejected"');
}
function approvalResult(plan: RoleResumeContentPlan, review: RoleResumePlanReview, paths: ReturnType<typeof roleResumePlanPaths>, proposalId: string, result: ApproveRoleResumePlanResult["result"]): ApproveRoleResumePlanResult {
