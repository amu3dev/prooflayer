...
import {
  RoleResumeCompositionManifestSchema,
  RoleResumeCompositionSchema,
  type RoleResumeComposition,
  type RoleResumeCompositionCompleteness,
  type RoleResumeCompositionSlot,
} from "./role-resume-composition-schemas.js";
import {
...
export async function loadRoleResumeCompositionContext(
  workspace: string,
  targetId: string,
): Promise<RoleResumeCompositionContext> {
  ...
  const planManifest = RoleResumePlanManifestSchema.parse(
    await readJson<unknown>(resolveWithin(workspace, planStatus.manifestPath), null),
  );
  ...
  const careerProfile = await pathExists(careerProfilePath)
    ? CareerProfileSchema.parse(await readJson<unknown>(careerProfilePath, null))
    : emptyCareerProfile();
...
export async function buildRoleResumeComposition(
  workspace: string,
  targetId: string,
  options: BuildRoleResumeCompositionOptions = {},
) {
  ...
  if (status.status === "current") return compositionResult(await showRoleResumeComposition(workspace, targetId), paths, "already-current");
  ...
  if (await pathExists(paths.compositionPath)) {
    try {
      createdAt = RoleResumeCompositionSchema.parse(await readJson<unknown>(paths.compositionPath, null)).createdAt;
    } catch {
...
export function deriveRoleResumeComposition(
  context: RoleResumeCompositionContext,
  createdAt: string,
  updatedAt: string,
): RoleResumeComposition {
  ...
  const experienceEntries = context.careerProfile.roles.map((entry, index) => {
    ...
    return {
      ...
      decision: included ? "include" as const : "exclude" as const,
      rationale: included
  ...
  const projectEntries = context.careerProfile.projects.map((entry, index) => {
    ...
    return {
      ...
      evidenceIds,
      selectedEvidenceIds: selected,
      decision: evidenceIds.length ? "include" as const : "exclude" as const,
  ...
  const identity = {
    ...
    source: context.publicProfile?.publicName ? "public-profile" as const : "unavailable" as const,
  };
  const slots: RoleResumeCompositionSlot[] = [];
  const addSlot = (input: Omit<RoleResumeCompositionSlot, "id" | "order">) => {
  ...
  if (identity.name) addSlot(fixedSlot("headline", "identity", identity.name, "Approved public identity", [], ["role-title"]));
  if (identity.contactItems.length) addSlot(fixedSlot("headline", "contact", identity.contactItems.join(" | "), "Approved public contact details", [], []));
  ...
  if (primaryTheme) {
    const slot = providerSlot(
      ...
      "headline",
      "headline",
      marketFacingLabel(primaryTheme.label),
      primaryTheme.evidenceIds,
      primaryTheme.sourceExpectationIds,
      ["role-title", "capability-theme"],
    );
    if (slot.evidenceIds.length && slot.claimTypes.length) addSlot(slot);
  }
  ...
  for (const theme of capabilityThemes.slice(0, 8)) {
    ...
    if (slot.claimTypes.includes("capability-theme")) addSlot(slot);
  }
  ...
  for (const evidenceId of impactEvidenceIds.slice(0, 4)) {
    ...
    const slot = providerSlot(
      ...
      "selected-impact",
      "impact",
      evidence?.normalizedSummary ?? evidenceId,
      [evidenceId],
      [],
      context.reviewedMetricEvidenceIds.has(evidenceId)
  ...
  for (const entry of experienceEntries.filter((candidate) => candidate.decision === "include")) {
    ...
    for (const evidenceId of entry.selectedEvidenceIds.slice(0, 5)) {
      ...
      if (slot.evidenceIds.length && slot.claimTypes.length) addSlot({ ...slot, careerEntryId: entry.id });
    }
  }
  ...
  for (const evidence of educationEvidence) addSlot(fixedSlot("education", "education", evidence.normalizedSummary, "Reviewed education", [evidence.id], ["education"]));
  for (const evidence of certificationEvidence) addSlot(fixedSlot("certifications", "certification", evidence.normalizedSummary, "Reviewed certification", [evidence.id], ["certification"]));
  ...
  const exclusions = [
    ...
    ...selectedEvidenceIds.filter((id) => !slots.some((slot) => slot.evidenceIds.includes(id))).map((id) => exclusion(
      compositionId,
      "evidence",
      id,
      evidenceById.get(id)?.normalizedSummary ?? id,
      "Selected evidence could not be allocated to a safe resume section and remains explicitly excluded.",
    )),
  ];
...
export function evaluateRoleResumeDraftAgainstComposition(
  composition: RoleResumeComposition,
  sections: Array<{ items: Array<{ compositionSlotId: string; text: string; evidenceIds: string[] }> }>,
  ...
  const evidenceBackedBulletSlotCount = composition.slots.filter((slot) =>
    ["experience-bullet", "project-bullet", "impact"].includes(slot.itemType) && itemBySlot.has(slot.id)).length;
  ...
  const status: RoleResumeCompositionCompleteness["status"] = !composition.completeness.usableForDrafting
...
export async function showRoleResumeComposition(workspace: string, targetId: string) {
  const paths = roleResumeCompositionPaths(workspace, targetId);
  if (!(await pathExists(paths.compositionPath))) throw new Error(`Role resume composition not found: ${targetId}`);
  return RoleResumeCompositionSchema.parse(await readJson<unknown>(paths.compositionPath, null));
}

export async function getRoleResumeCompositionStatus(
  workspace: string,
  targetId: string,
): Promise<RoleResumeCompositionStatus> {
  const paths = roleResumeCompositionPaths(workspace, targetId);
  ...
  if (!compositionExists && !manifestExists) return emptyStatus(base, "missing", ["No role resume composition exists."]);
  if (!compositionExists || !manifestExists) return emptyStatus(base, "invalid", ["Composition artifact set is incomplete."]);
  let composition: RoleResumeComposition;
  let manifest: ReturnType<typeof RoleResumeCompositionManifestSchema.parse>;
  try {
    composition = RoleResumeCompositionSchema.parse(await readJson<unknown>(paths.compositionPath, null));
    manifest = RoleResumeCompositionManifestSchema.parse(await readJson<unknown>(paths.manifestPath, null));
  } catch (error) {
  ...
  let context: RoleResumeCompositionContext;
  try {
...
export function roleResumeCompositionPaths(workspace: string, targetId: string) {
...
export function formatBuildRoleResumeCompositionResult(result: Awaited<ReturnType<typeof buildRoleResumeComposition>>) {
  return [
...
export function formatRoleResumeCompositionStatus(status: RoleResumeCompositionStatus) {
  return [
...
function deriveCompositionCompleteness(input: {
  ...
  slots: RoleResumeCompositionSlot[];
  experienceEntries: RoleResumeComposition["experienceEntries"];
  projectEntries: RoleResumeComposition["projectEntries"];
  skills: RoleResumeComposition["skills"];
  selectedEvidenceIds: string[];
  accountedEvidenceIds: string[];
  ...
  const status: RoleResumeCompositionCompleteness["status"] = hardBlocked
    ? "blocked"
    : !usable
      ? "incomplete"
      : complete
...
function fixedSlot(
  sectionType: RoleResumeSectionType,
  itemType: RoleResumeCompositionSlot["itemType"],
  exactText: string,
  sourceLabel: string,
  evidenceIds: string[],
  claimTypes: ResumeContentType[],
): Omit<RoleResumeCompositionSlot, "id" | "order"> {
  return {
    sectionType,
    itemType,
    mode: "fixed",
...
function providerSlot(
  ...
  fallbackClaimTypes: ResumeContentType[],
): Omit<RoleResumeCompositionSlot, "id" | "order"> {
  const candidateBoundaries = context.approvedPlan.claimBoundaries.filter((boundary) =>
    boundary.boundaryType !== "prohibited"
  ...
  const boundary = candidateBoundaries.find((entry) =>
    itemType === "headline" || fallbackClaimTypes.some((type) => entry.allowedClaimTypes.includes(type)))
    ?? candidateBoundaries[0];
  const boundaries = boundary ? [boundary] : [];
  const selectedExpectations = context.approvedPlan.expectationSelections.filter((selection) =>
    boundaries.some((boundary) => boundary.expectationId === selection.expectationId));
  ...
  return {
    ...
    claimTypes: uniqueSorted(allowedClaimTypes) as ResumeContentType[],
...
function uniqueThemes(context: RoleResumeCompositionContext) {
  ...
  return [
    ...
    if (seen.has(key) || !theme.evidenceIds.length) return false;
    seen.add(key);
    return true;
  });
}

function claimTypesForEvidence(evidence?: EvidenceItem): ResumeContentType[] {
  if (!evidence) return ["responsibility"];
  return ({
  ...
  } satisfies Record<EvidenceItem["category"], ResumeContentType[]>)[evidence.category];
}

function roleHeader(entry: CareerProfile["roles"][number]) {
  return [entry.title, entry.company, entry.dateRange].filter(Boolean).join(" | ");
}

function marketFacingLabel(value: string) {
  return value.replace(/\bAi\b/g, "AI").replace(/\bApi\b/g, "API").replace(/\bCto\b/g, "CTO");
}

function publicContactItems(profile: PublicProfile | null) {
  if (!profile) return [];
...
function exclusion(
  ...
  subjectType: "role" | "project" | "skill" | "evidence" | "section",
  subjectId: string,
  label: string,
  reason: string,
) {
...
function isAuditPlaceholder(text: string) {
...
function emptyCareerProfile(): CareerProfile {
...
function compositionResult(
  composition: RoleResumeComposition,
  paths: ReturnType<typeof roleResumeCompositionPaths>,
...
function emptyStatus(
  base: Pick<RoleResumeCompositionStatus, "targetId" | "compositionExists" | "manifestExists" | "compositionPath" | "manifestPath">,
  status: RoleResumeCompositionStatus["status"],
  reasons: string[],
): RoleResumeCompositionStatus {
  return {
    ...base,
