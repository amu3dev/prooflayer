...
import {
  ModelRoleResumePlanPayloadSchema,
  RoleResumeContentPlanSchema,
  RoleResumePlanProposalManifestSchema,
  RoleResumePlanProposalSchema,
  type ModelRoleResumePlanPayload,
  type RoleResumeContentPlan,
  type RoleResumePlanProposal,
  type RoleResumePlanProposalManifest,
...
export async function generateRoleResumePlanProposal(
  workspace: string,
  targetId: string,
  options: GenerateRoleResumePlanProposalOptions = {},
): Promise<GenerateRoleResumePlanProposalResult> {
  const deterministicStatus = await getRoleResumePlanStatus(workspace, targetId);
  if (deterministicStatus.status !== "current") throw new Error(`Deterministic role resume plan must be current before proposal generation. Current status: ${deterministicStatus.status}`);
...
export async function showRoleResumePlanProposal(workspace: string, proposalId: string): Promise<RoleResumePlanProposal> {
  const location = await locateProposal(workspace, proposalId);
  if (!location) throw new Error(`Role resume plan proposal not found: ${proposalId}`);
  return RoleResumePlanProposalSchema.parse(await readJson<unknown>(location.proposalPath, null));
}
export async function listRoleResumePlanProposals(workspace: string, targetId: string): Promise<RoleResumePlanProposal[]> {
  ...
  const proposals = await Promise.all(entries.filter((entry) => entry.isDirectory()).map(async (entry) => {
    try { return RoleResumePlanProposalSchema.parse(await readJson<unknown>(path.join(root, entry.name, "proposal.json"), null)); } catch { return null; }
  }));
  return proposals.filter((entry): entry is RoleResumePlanProposal => Boolean(entry)).sort((a, b) => a.createdAt.localeCompare(b.createdAt));
}
export async function getRoleResumePlanProposalStatus(workspace: string, proposalId: string): Promise<RoleResumePlanProposalStatus> {
  const location = await locateProposal(workspace, proposalId);
  if (!location) return { proposalId, targetId: "unknown", proposalExists: false, manifestExists: false, rawResponseExists: false, proposalHashMatches: null, rawResponseHashMatches: null, deterministicPlanHashMatches: null, dependenciesMatch: null, status: "missing", readyForReview: false, reasons: ["Proposal not found."] };
  const proposalExists = await pathExists(location.proposalPath);
  const manifestExists = await pathExists(location.manifestPath);
  if (!proposalExists || !manifestExists) return { proposalId, targetId: location.targetId, proposalExists, manifestExists, rawResponseExists: false, proposalHashMatches: null, rawResponseHashMatches: null, deterministicPlanHashMatches: null, dependenciesMatch: null, status: "invalid", readyForReview: false, reasons: ["Proposal artifact set is incomplete."] };
  let proposal: RoleResumePlanProposal;
  let manifest: RoleResumePlanProposalManifest;
  try {
    proposal = RoleResumePlanProposalSchema.parse(await readJson<unknown>(location.proposalPath, null));
    manifest = RoleResumePlanProposalManifestSchema.parse(await readJson<unknown>(location.manifestPath, null));
  } catch (error) {
...
export async function replayRoleResumePlanProposal(workspace: string, proposalId: string) {
  const status = await getRoleResumePlanProposalStatus(workspace, proposalId);
  if (status.status !== "current") throw new Error(`Replay requires a current role resume plan proposal. Current status: ${status.status}`);
...
export function renderRoleResumePlanPrompt(input: unknown): string {
  return [
...
export function formatRoleResumePlanProposalResult(result: GenerateRoleResumePlanProposalResult) {
...
export function formatRoleResumePlanProposalList(proposals: RoleResumePlanProposal[]) {
...
export function formatRoleResumePlanProposalStatus(status: RoleResumePlanProposalStatus) {
...
function normalizeResponse(raw: string, deterministic: RoleResumeContentPlan, context: Awaited<ReturnType<typeof loadRoleResumePlanningContext>>) {
  const issues: Array<{ code: string; message: string; path?: string }> = [];
  let payload: ModelRoleResumePlanPayload | undefined;
  try {
  ...
  for (const [pattern, code] of forbidden) if (pattern.test(output)) issues.push({ code, message: `Model output violates planning boundary: ${code}.` });
  ...
  if (numeric.some((value) => !allowedNumbers.has(value))) issues.push({ code: "INVENTED_METRIC", message: "Model output contains a numeric token absent from approved input." });
  try {
...
export function assertRoleResumePlanProposalAgainstDeterministic(payload: ModelRoleResumePlanPayload, deterministic: RoleResumeContentPlan, context: Awaited<ReturnType<typeof loadRoleResumePlanningContext>>) {
  assertPlanningVocabulary(payload, deterministic);
  if (payload.positioning.targetRoleTitle !== deterministic.positioning.targetRoleTitle) throw new Error("Target role title changed.");
  if (positioningRank(payload.positioning.positioningScope) > positioningRank(deterministic.positioning.positioningScope)) throw new Error("Positioning scope was strengthened beyond the approved assessment.");
  ...
  for (const proposed of payload.sections) {
    ...
    if (base.type !== proposed.type || (base.status === "exclude" && proposed.status !== "exclude")) throw new Error(`Section exceeds deterministic plan: ${proposed.id}`);
  }
  for (const proposed of payload.expectationSelections) {
    ...
    if (proposed.expectationId !== base.expectationId || proposed.assessmentId !== base.assessmentId) throw new Error(`Expectation references changed: ${proposed.id}`);
    if (!sameSet(proposed.approvedMatchIds, base.approvedMatchIds) || !sameSet(proposed.evidenceIds, base.evidenceIds)) throw new Error(`Expectation provenance changed: ${proposed.id}`);
    if (selectionRank(proposed.decision) > selectionRank(base.decision)) throw new Error(`Expectation selection was strengthened beyond deterministic boundary: ${proposed.id}`);
  }
  for (const proposed of payload.evidenceSelections) {
    ...
    if (proposed.evidenceId !== base.evidenceId || !sameSet(proposed.approvedMatchIds, base.approvedMatchIds) || !sameSet(proposed.expectationIds, base.expectationIds)) throw new Error(`Evidence provenance changed: ${proposed.id}`);
    if (evidenceRank(proposed.decision) > evidenceRank(base.decision)) throw new Error(`Evidence selection was strengthened beyond deterministic boundary: ${proposed.id}`);
  }
  for (const proposed of payload.claimBoundaries) {
    const base = deterministic.claimBoundaries.find((entry) => entry.id === proposed.id)!;
    if (proposed.expectationId !== base.expectationId || !sameSet(proposed.evidenceIds, base.evidenceIds)) throw new Error(`Claim boundary provenance changed: ${proposed.id}`);
    if (boundaryRank(proposed.boundaryType) > boundaryRank(base.boundaryType)) throw new Error(`Claim boundary was strengthened: ${proposed.id}`);
    if (proposed.allowedClaimTypes.some((type) => !base.allowedClaimTypes.includes(type))) throw new Error(`Claim type exceeds boundary: ${proposed.id}`);
  }
...
function modelInput(context: Awaited<ReturnType<typeof loadRoleResumePlanningContext>>, deterministic: RoleResumeContentPlan) {
  return {
...
function withProposedTrust(payload: ModelRoleResumePlanPayload): ModelRoleResumePlanPayload {
  const mark = <T extends { trustState: string }>(entry: T) => ({ ...entry, trustState: "proposed" as const });
  return {
    ...payload,
    positioning: mark({
      ...payload.positioning,
...
function sameIds(a: Array<{ id: string }>, b: Array<{ id: string }>, label: string) {
  if (!sameSet(a.map((entry) => entry.id), b.map((entry) => entry.id))) throw new Error(`Proposal ${label} IDs do not exactly match deterministic plan.`);
}
function sameSet(a: string[], b: string[]) { return a.length === b.length && [...a].sort().every((value, index) => value === [...b].sort()[index]); }
function selectionRank(value: string) { return ({ exclude: 0, defer: 1, supporting: 2, secondary: 3, primary: 4 })[value] ?? -1; }
function evidenceRank(value: string) { return ({ exclude: 0, "limited-use": 1, allowed: 2, preferred: 3 })[value] ?? -1; }
function boundaryRank(value: string) { return ({ prohibited: 0, "requires-review": 1, "allowed-with-caution": 2, allowed: 3 })[value] ?? -1; }
function positioningRank(value: string) { return ({ "insufficient-evidence": 0, "stretch-positioning": 1, "adjacent-role-positioning": 2, "direct-role-positioning": 3 })[value] ?? -1; }
function assertPlanningVocabulary(payload: ModelRoleResumePlanPayload, deterministic: RoleResumeContentPlan) {
  ...
  if (unknown.length) throw new Error(`Proposal introduces unsupported vocabulary: ${[...new Set(unknown)].slice(0, 8).join(", ")}.`);
}
function words(value: string) {
  return [...value.toLowerCase().matchAll(/[a-z][a-z0-9.+/#-]{2,}/g)].map((entry) => entry[0]);
}
function proposalResult(proposal: RoleResumePlanProposal, result: GenerateRoleResumePlanProposalResult["result"]): GenerateRoleResumePlanProposalResult {
...
function proposalPaths(workspace: string, targetId: string, proposalId: string, relativeOnly = false) {
...
async function locateProposal(workspace: string, proposalId: string) {
  ...
  for (const target of await readdir(rolesRoot, { withFileTypes: true })) {
    ...
    if (await pathExists(root.proposalPath) || await pathExists(root.manifestPath)) return { targetId: target.name, ...root };
  }
  return null;
}
