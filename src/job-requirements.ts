...
import {
  ...
  type JobRequirementModelManifest,
...
const TECHNOLOGY_PATTERNS = new Map<string, RegExp>([
...
export function jobRequirementPaths(workspace: string, targetId: string): RequirementPaths {
...
export async function buildJobRequirements(
  workspace: string,
  targetId: string,
  options: BuildOptions = {},
): Promise<BuildJobRequirementsResult> {
  ...
  if ((status.status === "stale" || status.status === "invalid") && !options.rebuild) {
    throw new Error(
      `Stored job requirement model is ${status.status} and was not overwritten. Review it, then use --rebuild. ${status.reasons.join(" ")}`,
    );
  }
  ...
  if (status.modelExists) {
    try {
      const previous = JobRequirementModelSchema.parse(
        await readJson<unknown>(input.paths.modelPath, null),
      );
...
export async function showJobRequirementModel(
  workspace: string,
  targetId: string,
): Promise<JobRequirementModel> {
  const target = await showTarget(workspace, targetId);
  if (target.type !== "job") throw new Error(`Job requirement modeling rejects Role Target: ${targetId}`);
  ...
  return JobRequirementModelSchema.parse(await readJson<unknown>(paths.modelPath, null));
}

export async function getJobRequirementModelStatus(
  workspace: string,
  targetId: string,
  options: Pick<BuildOptions, "policyName" | "policyVersion"> = {},
): Promise<JobRequirementModelStatus> {
  const target = await showTarget(workspace, targetId);
  if (target.type !== "job") throw new Error(`Job requirement modeling rejects Role Target: ${targetId}`);
  const paths = jobRequirementPaths(workspace, targetId);
  ...
  let model: JobRequirementModel;
  let manifest: JobRequirementModelManifest;
  try {
    model = JobRequirementModelSchema.parse(await readJson<unknown>(paths.modelPath, null));
    manifest = JobRequirementModelManifestSchema.parse(
  ...
  const structuralManifestHashMatches = await hashMatches(
    paths.analysisManifestPath,
    manifest.structuralAnalysisManifestSha256,
  );
  const policyMatches =
    manifest.policyName === (options.policyName ?? JOB_REQUIREMENT_POLICY_NAME) &&
  ...
  if (!modelHashMatches) invalidReasons.push("Requirement model SHA-256 does not match its manifest.");
  if (
  ...
  if (
    model.input.target.sha256 !== manifest.targetSha256 ||
...
export function formatBuildJobRequirementsResult(result: BuildJobRequirementsResult): string {
  return [
...
export function formatJobRequirementModelStatus(status: JobRequirementModelStatus): string {
...
async function loadCurrentInput(
  ...
  if (analysisStatus.status !== "current") {
    throw new Error(
      `Job Target structural analysis must be current before requirement modeling. Current status: ${analysisStatus.status}.`,
    );
  }
  ...
  const analysis = await showTargetAnalysis(workspace, targetId);
  if (analysis.targetType !== "job") throw new Error("Job Target structural analysis has the wrong type.");
...
> {
  ...
  const ambiguities = requirements.flatMap((requirement) => {
    ...
    if (requirement.category === "unknown") codes.push("AMBIGUOUS_CATEGORY");
    return codes.map((code) => ({
      id: `job-ambiguity_${hashText(`${requirement.id}\u0000${code}`).slice(0, 12)}`,
      code,
      message:
        code === "AMBIGUOUS_NECESSITY"
  ...
  const risks = [
    ...requirements
      ...
      .map((requirement) => ({
        ...
        code: "AMBIGUOUS_MANDATORY_STATUS" as const,
    ...
    ...requirements
      ...
      .map((requirement) => ({
        ...
        code: "UNCLASSIFIED_REQUIREMENT" as const,
    ...
    ...contradictions.map((contradiction) => ({
      ...
      code: "CONTRADICTORY_REQUIREMENT" as const,
  ...
  const warnings = [
    ...(analysis.items.some((item) => item.kind === "front-matter-field")
      ? [{
          ...
          code: "FRONT_MATTER_EXCLUDED" as const,
    ...
    ...requirements
      ...
      .map((requirement) => ({
        ...
        code: "AMBIGUOUS_ITEM_PRESERVED" as const,
    ...
    ...(requirements.length === 0
      ? [{
          ...
          code: "NO_REQUIREMENTS_FOUND" as const,
          message: "No explicit requirement statements were available in the structural analysis.",
          sourceAnalysisItemIds: [],
        }]
      : []),
  ];
  const modeledItemIds = new Set(
    requirements.map((requirement) => requirement.provenance.sourceAnalysisItemId),
  );
  ...
  return {
    ...
    completeness: {
      status:
        requirements.length === 0 ? "empty" : unmodeledItemIds.length > 0 ? "partial" : "complete",
...
function requirementFromStatement(
  ...
  const explicitness =
    ...
    item.category === "qualification"
      ...
      : "inferred";
  const confidence =
    explicitness === "explicit" && necessity !== "ambiguous"
      ? "high"
...
function classifyCategory(
  ...
  if (item.category === "responsibility") return "responsibility";
  if (necessity === "mandatory") return "required-capability";
...
function detectContradictions(requirements: JobRequirement[]) {
  const byLabel = new Map<string, JobRequirement[]>();
...
function resultFromModel(
  ...
  result: BuildJobRequirementsResult["result"],
): BuildJobRequirementsResult {
  return {
