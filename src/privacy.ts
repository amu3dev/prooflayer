...
export function detectSensitivity(text: string): string[] {
  const flags: string[] = [];
  if (EMAIL_RE.test(text)) flags.push("email");
  if (PHONE_RE.test(text)) flags.push("phone_number");
  if (ABSOLUTE_PATH_RE.test(text)) flags.push("absolute_local_path");
  if (SECRET_RE.test(text)) flags.push("secret_like_string");
  if (PRIVATE_URL_RE.test(text)) flags.push("private_url_or_internal_host");
  return [...new Set(flags)];
}

export function sourceVisibilityFromPath(relativePath: string): Visibility {
  ...
  if (normalized.includes("/github/") && !normalized.includes("private")) return "public";
  if (normalized.includes("/cvs/")) return "generic_only";
  if (normalized.includes("/recommendations/")) return "generic_only";
  if (normalized.includes("/certificates/")) return "public";
  if (normalized.includes("/jobs/")) return "private";
  if (normalized.includes("/pdf/") || normalized.includes("/docx/")) return "unknown";
  return "unknown";
}

export function auditSourcesAndEvidence(sources: Source[], evidenceItems: EvidenceItem[], claims: Claim[] = []): PrivacyFinding[] {
