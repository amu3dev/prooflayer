...
import {
  ...
  JobEvidenceMapSchema,
  type JobEvidenceLink,
  type JobEvidenceLinkConfidence,
  type JobEvidenceMap,
  type JobEvidenceMapManifest,
  type JobEvidenceRelationship,
  type JobEvidenceStrength,
  type JobRequirementInputType,
} from "./job-evidence-map-schemas.js";
import {
  ApprovedJobRequirementManifestSchema,
  type ApprovedJobRequirement,
...
export function jobEvidenceMapPaths(
  workspace: string,
  targetId: string,
): JobEvidenceMapPaths {
...
export async function buildJobEvidenceMap(
  workspace: string,
  targetId: string,
  options: BuildOptions = {},
): Promise<BuildJobEvidenceMapResult> {
  ...
  if (
    ...
    const currentManifest = JobEvidenceMapManifestSchema.parse(
      await readJson<unknown>(paths.manifestPath, null),
    );
    if (
      currentManifest.requirementModelType === requirementInput.type &&
      currentManifest.normalizedInputSha256 === input.normalizedInputSha256
    ) {
      return resultFromMap(
        await showJobEvidenceMap(workspace, targetId),
        requirementInput.type,
        paths,
        "already-current",
      );
    }
    if (!options.rebuild) {
      throw new Error(
        "A current job evidence map exists for a different requirement input. Use --rebuild to replace it explicitly.",
      );
    }
  }
  if ((status.status === "stale" || status.status === "invalid") && !options.rebuild) {
    throw new Error(
      `Stored job evidence map is ${status.status} and was not overwritten. Review dependencies, then use --rebuild. ${status.reasons.join(" ")}`,
    );
  }
  ...
  const mapped = mapRequirements(
    targetId,
    requirementInput,
    requirementInput.model.requirements,
    candidates,
  );
  const map = JobEvidenceMapSchema.parse({
    schemaVersion: 1,
    id: `job-evidence-map_${hashText(
      `${targetId}\u0000${input.normalizedInputSha256}\u0000${JOB_EVIDENCE_MAPPING_POLICY_VERSION}`,
    ).slice(0, 14)}`,
    targetId,
    targetType: "job",
    policy: {
...
export async function showJobEvidenceMap(
  workspace: string,
  targetId: string,
): Promise<JobEvidenceMap> {
  ...
  return JobEvidenceMapSchema.parse(await readJson<unknown>(paths.mapPath, null));
}

export async function getJobEvidenceMapStatus(
  workspace: string,
  targetId: string,
): Promise<JobEvidenceMapStatus> {
  const target = await requireJobTarget(workspace, targetId);
  const paths = jobEvidenceMapPaths(workspace, targetId);
  ...
  let map: JobEvidenceMap;
  let manifest: JobEvidenceMapManifest;
  try {
    map = JobEvidenceMapSchema.parse(await readJson<unknown>(paths.mapPath, null));
    manifest = JobEvidenceMapManifestSchema.parse(
      await readJson<unknown>(paths.manifestPath, null),
    );
  } catch (error) {
  ...
  let requirementInput: RequirementInput | undefined;
  let requirementModelStatus: JobEvidenceMapStatus["requirementModelStatus"] = null;
  const dependencyReasons: string[] = [];
  try {
    requirementInput = await loadRequirementInput(
      workspace,
      targetId,
      manifest.requirementModelType,
    );
    requirementModelStatus = "current";
  } catch (error) {
    requirementModelStatus = await requirementStatus(
      workspace,
      targetId,
      manifest.requirementModelType,
    );
  ...
  if (
    ...
    try {
      ...
      if (
        ...
        const semanticIssues = validateMapContents(
          map,
          requirementInput,
          candidates,
        );
...
export function formatBuildJobEvidenceMapResult(
  result: BuildJobEvidenceMapResult,
): string {
  return [
...
export function formatJobEvidenceMapStatus(status: JobEvidenceMapStatus): string {
...
function mapRequirements(
  ...
  JobEvidenceMap,
  "links" | "requirementMappings" | "warnings" | "completeness"
> {
  const links = requirements.flatMap((requirement) =>
    candidates.flatMap((candidate) => {
      const matched = matchRequirement(requirement, candidate);
      if (!matched) return [];
      const requirementProvenance = requirementProvenanceFor(
        requirementInput,
        requirement,
      );
      const matchedSignals = matched.signals.sort((a, b) =>
      ...
      return [{
        ...
        linkConfidence: linkConfidence(
          ...
          candidate.claim.factualConfidence,
        ),
        matchedSignals,
  ...
  const requirementMappings = requirements.map((requirement) => {
    ...
    return {
      id: `job-requirement-evidence_${hashText(
        `${targetId}\u0000${requirement.id}\u0000${JOB_EVIDENCE_MAPPING_POLICY_VERSION}`,
      ).slice(0, 14)}`,
      requirementId: requirement.id,
      status: requirementLinks.length > 0 ? "supported" as const : "unsupported" as const,
  ...
  const warnings = [
    ...(candidates.length === 0
      ? [{
          ...
          code: "NO_ELIGIBLE_EVIDENCE" as const,
    ...
    ...unsupported.map((entry) => ({
      ...
      code: "REQUIREMENT_UNSUPPORTED" as const,
  ...
  return {
    ...
    completeness: {
      ...
      supportedRequirementCount:
        requirementMappings.length - unsupported.length,
...
function matchRequirement(
  ...
  relationship: JobEvidenceRelationship;
  signals: JobEvidenceLink["matchedSignals"];
} | undefined {
  const requirementText = normalizeText(
    `${requirement.normalizedLabel} ${requirement.sourceText}`,
  );
  const claimText = normalizeText(
    `${candidate.claim.approvedWording ?? candidate.claim.claim} ${candidate.evidence.normalizedSummary}`,
  );
  ...
  const candidateTechnologies = uniqueNormalized([
    ...
    ...requirement.namedTechnologies.filter((technology) =>
      containsTerm(claimText, technology)),
  ]);
  const matchedTechnologies = requirementTechnologies.filter((technology) =>
    candidateTechnologies.includes(technology));
  const requirementDomains = uniqueNormalized(
    (candidate.evidence.domains ?? []).filter((domain) =>
      containsTerm(requirementText, domain)),
  );
  const candidateDomains = uniqueNormalized(candidate.evidence.domains ?? []);
  const matchedDomains = requirementDomains.filter((domain) =>
    candidateDomains.includes(domain));
  const requirementKeywords = uniqueNormalized(requirement.keywords);
  const matchedKeywords = requirementKeywords.filter((keyword) =>
    containsTerm(claimText, keyword));
  const exactPhrase = normalizeText(requirement.normalizedLabel);
  const hasExactPhrase =
    exactPhrase.length >= 12 && containsPhrase(claimText, exactPhrase);
  const signals: JobEvidenceLink["matchedSignals"] = [
    ...(hasExactPhrase
      ? [{ type: "exact-phrase" as const, value: requirement.normalizedLabel }]
      : []),
    ...matchedTechnologies.map((value) => ({ type: "technology" as const, value })),
    ...matchedDomains.map((value) => ({ type: "domain" as const, value })),
    ...matchedKeywords.map((value) => ({ type: "keyword" as const, value })),
  ];
  const deduplicatedSignals = [...new Map(
    signals.map((signal) => [`${signal.type}\u0000${signal.value}`, signal]),
  ).values()];
  if (deduplicatedSignals.length === 0) return undefined;
  ...
  if (
    requirement.category === "domain-expectation" &&
    requirementDomains.length > 0
  ) {
    return {
      relationship:
        matchedDomains.length === requirementDomains.length ? "direct" : "partial",
      signals: deduplicatedSignals,
    };
  }
  if (
    requirementKeywords.length > 1 &&
...
function compareLinks(a: JobEvidenceLink, b: JobEvidenceLink): number {
  return (
    strengthRank(b.evidenceStrength) - strengthRank(a.evidenceStrength) ||
...
function evidenceStrength(confidence: EvidenceItem["confidence"]): JobEvidenceStrength {
  return confidence === "high" ? "strong" : confidence === "medium" ? "medium" : "weak";
}

function linkConfidence(
  relationship: JobEvidenceRelationship,
  evidenceConfidence: EvidenceItem["confidence"],
  claimConfidence: Claim["factualConfidence"],
): JobEvidenceLinkConfidence {
...
async function mappingInput(
  ...
  const sourceSha256 = await hashFile(
    resolveWithin(workspace, `targets/jobs/${target.id}/job-description.md`),
  );
...
async function loadRequirementInput(
  ...
  if (type === "deterministic") {
    ...
    if (status.status !== "current") {
      throw new Error(
        `Deterministic Job Requirement Model must be current. Current status: ${status.status}.`,
      );
    }
  ...
  if (status.status !== "current") {
    throw new Error(
      `Approved Job Requirement Model must be current. Current status: ${status.status}.`,
    );
  }
...
function assertUsableRequirementModel(model: RequirementModel): void {
  const criticalRisks = model.risks.filter((risk) => risk.severity === "high");
  if (
    model.completeness.status !== "complete" ||
    model.requirements.length === 0 ||
    criticalRisks.length > 0 ||
    model.contradictions.length > 0
  ) {
    throw new Error(
      "Job Requirement Model is not usable for deterministic evidence mapping: it must be complete, non-empty, and free of unresolved critical ambiguity or contradiction.",
    );
  }
}

function requirementProvenanceFor(
...
function validateStoredMapIdentity(
  map: JobEvidenceMap,
  manifest: JobEvidenceMapManifest,
  paths: JobEvidenceMapPaths,
): string[] {
  const reasons: string[] = [];
  if (
    map.id !== manifest.mapId ||
  ...
  if (
    new Set(map.links.map((link) => link.id)).size !== map.links.length ||
...
function validateMapContents(
  ...
  const eligiblePairs = new Set(
    candidates.map((entry) => `${entry.evidence.id}\u0000${entry.claim.id}`),
  );
  const mappingIds = new Set(map.requirementMappings.map((entry) => entry.requirementId));
  if (
    mappingIds.size !== requirementIds.size ||
  ...
  const expected = mapRequirements(
    map.targetId,
    requirementInput,
    requirements,
    candidates,
...
async function requirementStatus(
  ...
  targetId: string,
  type: JobRequirementInputType,
): Promise<"current" | "missing" | "stale" | "invalid"> {
  try {
...
function resultFromMap(
  map: JobEvidenceMap,
  requirementSource: JobRequirementInputType,
  paths: JobEvidenceMapPaths,
  result: BuildJobEvidenceMapResult["result"],
): BuildJobEvidenceMapResult {
  return {
...
function emptyStatus(
  base: Pick<
    JobEvidenceMapStatus,
    "targetId" | "mapExists" | "manifestExists" | "mapPath" | "manifestPath"
  >,
...
function containsTerm(normalizedText: string, value: string): boolean {
...
function strengthRank(value: JobEvidenceStrength): number {
...
async function hashMatches(filePath: string, expected: string): Promise<boolean> {
