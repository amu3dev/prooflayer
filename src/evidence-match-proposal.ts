...
import {
  ...
  EvidenceMatchProposalSchema,
  ModelEvidenceMatchPayloadSchema,
  type Claim,
  type EvidenceMatchProposal,
  type EvidenceMatchProposalManifest,
  type EvidenceMatchValidationIssue,
  type EvidenceMatchingAmbiguity,
  type EvidenceMatchingWarning,
  type EvidenceSnapshotEntry,
  type ModelEvidenceMatchPayload,
  type ProposedEvidenceMatch,
  type ProposedExpectationCoverage,
  type Target,
} from "./schemas.js";
import {
...
interface EvidenceContent {
  ...
  evidenceType: string;
  summary: string;
  reviewedClaims: Array<{ id: string; claim: string }>;
  provenance: EvidenceSnapshotEntry["provenance"];
}

export async function generateEvidenceMatchProposal(
  workspace: string,
  targetId: string,
  options: MatchProposalGenerationOptions = {},
): Promise<MatchProposalGenerationResult> {
  const context = await loadMatchingContext(workspace, targetId, {
    persistSnapshot: true,
    rebuildSnapshot: options.refresh,
    now: options.now,
  });
  if (context.eligibleExpectations.length === 0) throw new Error("No approved expectations are eligible for matching.");
  if (context.snapshot.entries.length === 0) throw new Error("No reviewed active evidence is eligible for matching.");
...
export async function showEvidenceMatchProposal(workspace: string, proposalId: string): Promise<EvidenceMatchProposal> {
  const located = await locateProposal(workspace, proposalId);
  return EvidenceMatchProposalSchema.parse(await readJson<unknown>(located.location.proposalPath, null));
}

export async function listEvidenceMatchProposals(workspace: string, targetId: string): Promise<EvidenceMatchProposal[]> {
  ...
  const proposals = await Promise.all(files.map(async (file) => EvidenceMatchProposalSchema.parse(await readJson<unknown>(file, null))));
  return proposals.sort((a, b) => a.createdAt.localeCompare(b.createdAt) || a.id.localeCompare(b.id));
}

export async function getEvidenceMatchProposalStatus(workspace: string, proposalId: string): Promise<MatchProposalStatus> {
  const located = await locateProposal(workspace, proposalId);
  ...
  let proposal: EvidenceMatchProposal;
  let manifest: EvidenceMatchProposalManifest;
  try {
    proposal = EvidenceMatchProposalSchema.parse(await readJson<unknown>(location.proposalPath, null));
    manifest = EvidenceMatchProposalManifestSchema.parse(await readJson<unknown>(location.manifestPath, null));
  } catch (error) {
  ...
  if (proposal.id !== proposalId || manifest.proposalId !== proposalId || proposal.targetId !== target.id || manifest.targetId !== target.id) invalidReasons.push("Proposal identity is invalid.");
  if (manifest.proposalPath !== location.proposalRelativePath || manifest.rawResponsePath !== location.rawRelativePath) invalidReasons.push("Proposal paths disagree with the manifest.");
  ...
  try { context = await loadMatchingContext(workspace, target.id, { persistSnapshot: false }); }
  catch (error) {
    return {
      ...base,
      status: "stale",
...
export async function replayEvidenceMatchProposal(
  workspace: string,
  proposalId: string,
  ...
  if (status.status !== "current") throw new Error(`Replay requires a current proposal. Current status: ${status.status}`);
  const located = await locateProposal(workspace, proposalId);
  const original = await showEvidenceMatchProposal(workspace, proposalId);
  const manifest = EvidenceMatchProposalManifestSchema.parse(await readJson<unknown>(located.location.manifestPath, null));
...
export function renderEvidenceMatchPrompt(input: unknown, templateVersion = MATCH_PROPOSAL_PROMPT_TEMPLATE_VERSION, policyVersion = MATCH_PROPOSAL_POLICY_VERSION): string {
  return [
...
export function formatMatchProposalResult(result: MatchProposalGenerationResult): string {
  return [
...
export function formatMatchProposalList(proposals: EvidenceMatchProposal[]): string {
  if (!proposals.length) return "Match proposals: none";
  return ["Match proposals:", ...proposals.map((proposal) => `${proposal.id} | ${proposal.status} | ${proposal.model.model} | ${proposal.createdAt}`)].join("\n");
}

export function formatMatchProposalStatus(status: MatchProposalStatus): string {
...
function normalizeRawProposal(input: {
  ...
  const issues: EvidenceMatchValidationIssue[] = [];
  let payload: ModelEvidenceMatchPayload | undefined;
  try { payload = ModelEvidenceMatchPayloadSchema.parse(JSON.parse(input.rawText)); }
  catch (error) { issues.push(...issuesFromError(error)); }
  if (payload) issues.push(...validatePayload(payload, input.context, input.evidence));
  const proposedMatches: ProposedEvidenceMatch[] = payload && issues.length === 0
    ...
    : [];
  const proposedCoverage: ProposedExpectationCoverage[] = payload && issues.length === 0
    ...
    : [];
  const warnings: EvidenceMatchingWarning[] = payload && issues.length === 0
    ? payload.warnings.map((entry, index) => ({ id: `match-warning_${hashText(`${input.proposalId}\u0000${index}\u0000${stableJson(entry)}`).slice(0, 12)}`, ...entry }))
    : [];
  const ambiguities: EvidenceMatchingAmbiguity[] = payload && issues.length === 0
...
function validatePayload(payload: ModelEvidenceMatchPayload, context: MatchingContext, evidence: EvidenceContent[]): EvidenceMatchValidationIssue[] {
  const issues: EvidenceMatchValidationIssue[] = [];
  ...
  payload.proposedMatches.forEach((match, index) => {
    ...
    if (matchIdentities.has(matchIdentity)) issues.push(issue("DUPLICATE_PROPOSED_MATCH", "Exact duplicate proposed match.", `proposedMatches.${index}`));
    matchIdentities.add(matchIdentity);
    const expectation = expectationById.get(match.expectationId);
    if (!expectation) issues.push(issue("UNKNOWN_EXPECTATION_ID", `Unknown or ineligible expectation: ${match.expectationId}`, `proposedMatches.${index}`));
    const linked = match.evidenceIds.map((id) => evidenceById.get(id));
    for (const id of match.evidenceIds) if (!evidenceById.has(id)) issues.push(issue("UNKNOWN_EVIDENCE_ID", `Unknown or ineligible evidence: ${id}`, `proposedMatches.${index}`));
    ...
    const validLinked = linked.filter((entry): entry is EvidenceContent => Boolean(entry));
    if (validLinked.length === linked.length) {
      ...
      if (stableJson(expectedProvenance) !== stableJson(actualProvenance)) issues.push(issue("INVALID_EVIDENCE_PROVENANCE", "Evidence provenance does not match the eligible snapshot.", `proposedMatches.${index}.evidenceProvenance`));
    }
    if (["supporting", "partial", "contradictory"].includes(match.matchType) && match.limitations.length === 0) issues.push(issue("MISSING_LIMITATIONS", `${match.matchType} matches require explicit limitations.`, `proposedMatches.${index}.limitations`));
    if (match.matchType === "contradictory" && match.coverage !== "conflicting") issues.push(issue("INVALID_CONTRADICTION", "Contradictory matches require conflicting coverage.", `proposedMatches.${index}`));
    if (match.matchType !== "contradictory" && match.coverage === "conflicting") issues.push(issue("INVALID_CONTRADICTION", "Conflicting coverage requires contradictory evidence.", `proposedMatches.${index}`));
    if (match.matchType === "contradictory" && !/contradic|conflict|inconsisten|oppos/i.test(`${match.rationale} ${match.limitations.join(" ")}`)) issues.push(issue("ABSENCE_AS_CONTRADICTION", "Contradiction requires explicit conflict rationale, not absence of evidence.", `proposedMatches.${index}`));
    const evidenceTypes = validLinked.map((entry) => entry.evidenceType);
    if (match.matchType === "direct" && evidenceTypes.length > 0 && evidenceTypes.every((type) => type === "role")) issues.push(issue("TITLE_ONLY_DIRECT_MATCH", "Role titles alone cannot directly prove a complete expectation.", `proposedMatches.${index}`));
    if (match.matchType === "direct" && evidenceTypes.length > 0 && evidenceTypes.every((type) => ["skill", "tool"].includes(type))) issues.push(issue("TOOL_ONLY_DIRECT_MATCH", "Tool mentions alone cannot directly prove production expertise.", `proposedMatches.${index}`));
    if (match.matchType === "direct" && match.coverage === "full" && evidenceTypes.length > 0 && evidenceTypes.every((type) => type === "project")) issues.push(issue("PROJECT_EXISTENCE_AS_IMPACT", "Project existence alone cannot prove full business impact.", `proposedMatches.${index}`));
    const forbidden = forbiddenContent(match.rationale);
    if (forbidden) issues.push(issue("FORBIDDEN_CONTENT", forbidden, `proposedMatches.${index}.rationale`));
    ...
    if (unsupportedNumbers.length) issues.push(issue("INVENTED_METRIC", `Rationale contains unsupported numeric token(s): ${unsupportedNumbers.join(", ")}`, `proposedMatches.${index}.rationale`));
  });
  const coverageExpectationIds = new Set<string>();
  payload.proposedCoverage.forEach((coverage, index) => {
    if (coverageExpectationIds.has(coverage.expectationId)) issues.push(issue("DUPLICATE_PROPOSED_COVERAGE", "Only one proposed coverage record is allowed per expectation.", `proposedCoverage.${index}`));
    coverageExpectationIds.add(coverage.expectationId);
    if (!expectationById.has(coverage.expectationId)) issues.push(issue("UNKNOWN_EXPECTATION_ID", `Unknown coverage expectation: ${coverage.expectationId}`, `proposedCoverage.${index}`));
    if (coverage.status === "conflicting" && !/contradic|conflict|inconsisten|oppos/i.test(`${coverage.rationale} ${coverage.blockingReasons.join(" ")}`)) issues.push(issue("ABSENCE_AS_CONTRADICTION", "Conflicting coverage requires explicit contradictory evidence.", `proposedCoverage.${index}`));
    const forbidden = forbiddenContent(coverage.rationale);
    if (forbidden) issues.push(issue("FORBIDDEN_CONTENT", forbidden, `proposedCoverage.${index}.rationale`));
  });
...
async function loadEvidenceContent(workspace: string, context: MatchingContext): Promise<EvidenceContent[]> {
  ...
  return context.snapshot.entries.map((entry) => {
    ...
    if (!evidence) throw new Error(`Eligible evidence disappeared: ${entry.evidenceId}`);
    const reviewedClaims = entry.supportingClaimIds.map((id) => claimById.get(id)).filter((claim): claim is Claim => Boolean(claim));
    if (reviewedClaims.length !== entry.supportingClaimIds.length) throw new Error(`Eligible evidence review provenance is broken: ${entry.evidenceId}`);
    return {
...
async function findCached(workspace: string, targetId: string, fingerprint: string): Promise<{ proposal: EvidenceMatchProposal; location: ProposalLocation } | undefined> {
...
async function locateProposal(workspace: string, proposalId: string): Promise<{ target: Target; location: ProposalLocation }> {
  if (!/^match-proposal_[a-f0-9]+$/.test(proposalId)) throw new Error(`Invalid match proposal ID: ${proposalId}`);
  ...
  if (!files.length) throw new Error(`Match proposal not found: ${proposalId}`);
  if (files.length > 1) throw new Error(`Match proposal ID is ambiguous: ${proposalId}`);
  const proposal = EvidenceMatchProposalSchema.parse(await readJson<unknown>(files[0], null));
...
function resultFromProposal(proposal: EvidenceMatchProposal, location: ProposalLocation, result: MatchProposalGenerationResult["result"]): MatchProposalGenerationResult {
  return {
...
function invalidStatus(base: Pick<MatchProposalStatus, "proposalId" | "targetId" | "targetType" | "proposalPath" | "manifestPath" | "rawResponsePath">, reasons: string[], proposalHashMatches: boolean | null = null, rawResponseHashMatches: boolean | null = null): MatchProposalStatus {
...
function resolveWithin(workspace: string, relativePath: string): string {
  ...
  if (relation === ".." || relation.startsWith(`..${path.sep}`) || path.isAbsolute(relation)) throw new Error(`Match proposal path escapes workspace: ${relativePath}`);
  return resolved;
}
...
function issuesFromError(error: unknown): EvidenceMatchValidationIssue[] {
...
function issue(code: string, message: string, issuePath?: string): EvidenceMatchValidationIssue {
  return { code, message, ...(issuePath ? { path: issuePath } : {}) };
}

function deduplicateIssues(issues: EvidenceMatchValidationIssue[]): EvidenceMatchValidationIssue[] {
...
export async function matchProposalFileTimestamps(workspace: string, proposalId: string) {
  const located = await locateProposal(workspace, proposalId);
