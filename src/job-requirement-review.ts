...
export async function initializeJobRequirementReview(
  workspace: string,
  proposalId: string,
  options: ReviewOptions = {},
): Promise<JobRequirementReviewStatus> {
  const proposalStatus = await getJobRequirementProposalStatus(workspace, proposalId);
  if (!proposalStatus.readyForReview) {
    throw new Error(
      `Job requirement proposal is not ready for review. Current status: ${proposalStatus.status}`,
    );
  }
  ...
  const review = JobRequirementProposalReviewSchema.parse({
    ...
    decisions: [
      ...
      a.source.localeCompare(b.source) || a.requirementId.localeCompare(b.requirementId)),
    reviewer: {
      type: "human",
...
export async function setJobRequirementReviewDecision(
  workspace: string,
  proposalId: string,
  ...
  if (!existing) throw new Error(`Unknown reviewed requirement ID: ${requirementId}`);
  ...
  if (!source) throw new Error(`Reviewed requirement source is missing: ${requirementId}`);
  if (input.decision === "edit") {
    if (!input.editedRequirement) throw new Error("Edit requires edited requirement content.");
...
export async function completeJobRequirementReview(
  workspace: string,
  proposalId: string,
  options: Pick<ReviewOptions, "now"> = {},
): Promise<JobRequirementReviewStatus> {
  ...
  const deterministic = await showJobRequirementModel(workspace, proposal.targetId);
  const review = await showJobRequirementReview(workspace, proposalId);
  if (review.status === "completed") return getJobRequirementReviewStatus(workspace, proposalId);
  const pending = review.decisions.filter((entry) => entry.decision === "pending");
  if (pending.length > 0) {
    throw new Error(
      `Review cannot be completed: ${pending.length} requirement decision(s) remain pending.`,
    );
  }
  assertExactCoverage(
    review,
...
export async function showJobRequirementReview(
  workspace: string,
  proposalId: string,
): Promise<JobRequirementProposalReview> {
  const proposal = await showJobRequirementProposal(workspace, proposalId);
  const paths = reviewPaths(workspace, proposal.targetId, proposalId);
  if (!(await pathExists(paths.reviewPath))) {
    throw new Error(`Job requirement review not found for proposal: ${proposalId}`);
  }
  return JobRequirementProposalReviewSchema.parse(
    await readJson<unknown>(paths.reviewPath, null),
  );
}

export async function getJobRequirementReviewStatus(
  workspace: string,
  proposalId: string,
): Promise<JobRequirementReviewStatus> {
  const proposal = await showJobRequirementProposal(workspace, proposalId);
  ...
  try {
    ...
    const manifest = JobRequirementReviewManifestSchema.parse(
    ...
    const proposalHashMatches =
      proposalManifest.proposalSha256 === manifest.proposalSha256;
    const deterministicModelHashMatches =
      deterministicManifest.modelSha256 === manifest.deterministicModelSha256;
...
export async function readEditedJobRequirementFile(
  filePath: string,
): Promise<EditedJobRequirement> {
  try {
    return EditedJobRequirementSchema.parse(
...
export function formatJobRequirementReviewStatus(
  status: JobRequirementReviewStatus,
): string {
  return [
...
export function jobRequirementReviewPaths(
  workspace: string,
  targetId: string,
  proposalId: string,
) {
...
function validateEditedRequirement(
  edited: EditedJobRequirement,
  source: {
    sourceText: string;
  ...
  for (const technology of edited.namedTechnologies) {
    if (
      !source.namedTechnologies.includes(technology) &&
  ...
  for (const relationship of edited.relationships) {
    if (!knownRequirementIds.has(relationship.requirementId)) {
      throw new Error(
        `Edited requirement cannot reference an unknown requirement: ${relationship.requirementId}`,
      );
    }
  ...
  if (inventedLabelTokens.length > 0) {
    throw new Error(
      "Edited normalized wording must use terms present in the exact source statement.",
    );
  }
  const reviewedText = [edited.normalizedLabel, ...edited.notes].join(" ");
  if (
    /\b(candidate fit|candidate evidence|resume|cover letter|apply|hire|hiring probability|ATS score)\b/i.test(
      reviewedText,
    )
  ) {
...
function assertExactCoverage(
  ...
  if (new Set(actual).size !== actual.length || JSON.stringify(actual) !== JSON.stringify(expected)) {
    throw new Error(
      "Review decisions do not exactly cover deterministic and proposed requirements.",
    );
  }
}

function countDecisions(decisions: JobRequirementReviewDecision[]) {
