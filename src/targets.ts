...
export async function createJobTarget(
  ...
  const title = normalizeRequiredDisplayText(
    input.title !== undefined ? input.title : frontMatter.title,
    "Job target title"
  );
  const company = normalizeOptionalDisplayText(
    input.company !== undefined ? input.company : frontMatter.company
  );
  const location = normalizeOptionalDisplayText(
    input.location !== undefined ? input.location : frontMatter.location
  );
  const workingModel = normalizeOptionalLowerText(
    input.workingModel !== undefined ? input.workingModel : frontMatter.workingModel
  );
...
export async function listTargets(workspace: string): Promise<Target[]> {
  ...
  const targets = await Promise.all(files.map(async (file) => {
    try {
      return TargetSchema.parse(await readJson<unknown>(file, {}));
    } catch (error) {
...
export async function showTarget(workspace: string, targetId: string): Promise<Target> {
  ...
  if (existing.length === 0) throw new Error(`Target not found: ${targetId}`);
  if (existing.length > 1) throw new Error(`Target ID is ambiguous: ${targetId}`);
  return TargetSchema.parse(await readJson<unknown>(existing[0], {}));
}

export function formatTargetCreation(result: TargetCreationResult): string {
  const lines = [
...
export function formatTargetList(targets: Target[]): string {
  if (targets.length === 0) return "Targets: none";
  return [
    "Targets:",
    ...targets.map((target) => [
      target.id,
...
export function formatTargetJson(target: Target): string {
  return `${JSON.stringify(target, null, 2)}\n`;
}

async function prepareTargetWrite(
  workspace: string,
  type: Target["type"],
  id: string,
  ...
  if (!(await pathExists(targetDirectory))) return null;
  if (!replace) throw new Error(`Target already exists: ${id}. Use --replace to replace it explicitly.`);
  const existingPath = path.join(workspace, relativeTargetPath);
  if (!(await pathExists(existingPath))) return null;
  const existing = TargetSchema.parse(await readJson<unknown>(existingPath, {}));
  if (existing.type !== type) throw new Error(`Cannot replace ${id}: stored target type differs.`);
  return existing;
}

async function readJobMarkdown(workspace: string, inputPath: string): Promise<{
  bytes: Buffer;
  ...
  if (!(await pathExists(absolutePath))) throw new Error(`Job description file not found: ${inputPath}`);
  ...
  if (rawDescription.trim().length === 0) throw new Error("Job description Markdown must not be empty.");
...
function parseJobFrontMatter(rawDescription: string): JobFrontMatter {
  let data: Record<string, unknown>;
  try {
    data = matter(rawDescription).data as Record<string, unknown>;
  } catch (error) {
    throw new Error(`Invalid Markdown front matter: ${errorMessage(error)}`);
  }
  const result: JobFrontMatter = {};
  for (const field of ["title", "company", "location", "workingModel"] as const) {
    const value = data[field];
    if (value === undefined || value === null) continue;
    if (typeof value !== "string") throw new Error(`Front matter field "${field}" must be a string.`);
    result[field] = value;
  }
  return result;
}

function normalizeRequiredDisplayText(value: string | undefined, label: string): string {
  const normalized = normalizeOptionalDisplayText(value);
  if (!normalized) throw new Error(`${label} is required and must not be blank.`);
  return normalized;
}

function normalizeOptionalDisplayText(value: string | undefined): string | undefined {
  if (value === undefined) return undefined;
  const normalized = value.normalize("NFKC").replace(/\s+/g, " ").trim();
  return normalized || undefined;
}

function normalizeOptionalLowerText(value: string | undefined): string | undefined {
...
function targetJsonPath(type: Target["type"], id: string): string {
...
function assertTargetId(value: string): void {
  if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(value)) throw new Error(`Invalid target ID: ${value}`);
}
