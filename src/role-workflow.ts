...
export interface RoleWorkflowStatus {
  ...
  positioning: string;
  strongestThemes: Array<{ theme: string; evidence: string; claimId: string; evidenceId: string }>;
  weakerThemes: string[];
  materialGaps: string[];
  limitations: string[];
  ...
  draftPreview: {
    ...
    items: Array<{ text: string; claimId: string; evidenceId: string }>;
...
interface RoleTemplate {
  ...
  expectations: Array<Omit<z.input<typeof RoleExpectationSchema>, "id" | "trustState">>;
}
...
export async function createGuidedRole(
  workspace: string,
...
export async function runRoleWorkflow(
  workspace: string,
  targetId: string,
  options: RunRoleWorkflowOptions = {},
): Promise<RunRoleWorkflowResult> {
  return orchestrateRoleWorkflow(workspace, targetId, "run", options);
}

export async function continueRoleWorkflow(
  workspace: string,
  targetId: string,
  options: RunRoleWorkflowOptions = {},
): Promise<RunRoleWorkflowResult> {
...
async function orchestrateRoleWorkflow(
  workspace: string,
  targetId: string,
  mode: "run" | "continue",
  options: RunRoleWorkflowOptions,
  ...
  if (analysis.status === "missing") await analyzeTarget(workspace, target.id, { now: options.now });
  ...
  let result: RunRoleWorkflowResult["result"] = "already-current";
...
export async function inspectRoleWorkflow(workspace: string, targetId: string): Promise<RoleWorkflowStatus> {
  const target = await requireRoleTarget(workspace, targetId);
  ...
  const stages: RoleWorkflowStageState[] = [
    ...
    stage("role-understanding", understandingLifecycle.status,
      understanding ? `${understanding.expectations.length} generated expectations are available from ${sourceLabel(understanding)}.` : "No generated role understanding is available."),
    stage("evidence-selection", selected ? "current" : understanding ? "waiting" : "missing",
      selected ? `${projection.selectedEvidenceIds.length} eligible evidence item(s) support the conservative projection.` : "No eligible evidence can yet be selected conservatively."),
    ...
    stage("planning", planReady ? "current" : selected ? "current" : "waiting",
...
export async function finalizeRoleWorkflow(
  workspace: string,
  targetId: string,
  options: FinalizeRoleWorkflowOptions = {},
): Promise<FinalizeRoleWorkflowResult> {
  ...
  if (options.dryRun) return { dryRun: true, result: "paused", status, succeeded: [], failed: [] };
...
export async function showGeneratedRoleUnderstanding(workspace: string, targetId: string): Promise<GeneratedRoleUnderstanding> {
  const paths = understandingPaths(workspace, targetId);
  if (!(await pathExists(paths.understandingPath))) throw new Error(`Generated role understanding not found: ${targetId}`);
  return GeneratedRoleUnderstandingSchema.parse(await readJson<unknown>(paths.understandingPath, null));
}

export async function replayGeneratedRoleUnderstanding(
  workspace: string,
  targetId: string,
): Promise<{ targetId: string; originalSha256: string; replaySha256: string; matches: boolean }> {
  ...
  const target = await requireRoleTarget(workspace, targetId);
  const rawText = await import("node:fs/promises").then(({ readFile }) =>
    readFile(resolveWithin(workspace, source.rawResponsePath), "utf8"));
...
export async function getGeneratedRoleUnderstandingStatus(
  workspace: string,
  targetId: string,
): Promise<{ status: "missing" | "current" | "stale" | "invalid"; reasons: string[]; understandingPath: string; manifestPath: string }> {
  const paths = understandingPaths(workspace, targetId);
  ...
  if (!exists && !manifestExists) return { ...base, status: "missing", reasons: ["No generated role understanding exists."] };
  if (!exists || !manifestExists) return { ...base, status: "invalid", reasons: ["Generated role understanding artifact set is incomplete."] };
  let understanding: GeneratedRoleUnderstanding;
  let manifest: GeneratedRoleUnderstandingManifest;
  try {
    understanding = GeneratedRoleUnderstandingSchema.parse(await readJson<unknown>(paths.understandingPath, null));
    manifest = GeneratedRoleUnderstandingManifestSchema.parse(await readJson<unknown>(paths.manifestPath, null));
  } catch (error) {
  ...
  if (!contentMatches) return { ...base, status: "invalid", reasons: ["Generated role understanding hash does not match its manifest."] };
  let target: RoleTarget;
  try { target = await requireRoleTarget(workspace, targetId); }
  catch (error) { return { ...base, status: "invalid", reasons: [errorMessage(error)] }; }
...
async function generateRoleUnderstanding(
  workspace: string,
  target: RoleTarget,
  options: RunRoleWorkflowOptions,
): Promise<{ providerCallMade: boolean }> {
  ...
  if (await pathExists(paths.understandingPath)) {
    try { createdAt = (await showGeneratedRoleUnderstanding(workspace, target.id)).createdAt; } catch { /* explicit rebuild */ }
  }
  let understanding: GeneratedRoleUnderstanding;
  if (options.provider) {
...
async function providerUnderstanding(
  workspace: string,
  target: RoleTarget,
  targetSha256: string,
  provider: InterpretationModelProvider,
  paths: ReturnType<typeof understandingPaths>,
  createdAt: string,
  updatedAt: string,
): Promise<GeneratedRoleUnderstanding> {
  ...
  if (!response.rawText.length) throw new Error("Model provider returned an empty role-understanding response.");
  let payload: z.infer<typeof ModelRoleUnderstandingPayloadSchema>;
  try { payload = ModelRoleUnderstandingPayloadSchema.parse(JSON.parse(response.rawText)); }
  catch (error) { throw new Error(`Role-understanding proposal failed schema validation: ${errorMessage(error)}`); }
  const selected = payload.ambiguity?.conservativeDefaultId ?? "general";
  ...
  const source = {
    type: "model-proposal" as const,
    provider: provider.providerId,
    model: provider.identity.model,
    settings: provider.settings,
    promptId: ROLE_UNDERSTANDING_PROMPT_ID as typeof ROLE_UNDERSTANDING_PROMPT_ID,
    promptVersion: ROLE_UNDERSTANDING_PROMPT_VERSION as typeof ROLE_UNDERSTANDING_PROMPT_VERSION,
  ...
  return buildUnderstanding({
    ...
    ambiguity: payload.ambiguity ? {
      ...
      options: payload.ambiguity.options,
      selectedOptionId: selected,
      selectionSource: "conservative-default" as const,
    } : undefined,
    source,
    createdAt,
    updatedAt,
...
function builtInUnderstanding(
  target: RoleTarget,
  targetSha256: string,
  requestedSpecialization: string | undefined,
  createdAt: string,
  updatedAt: string,
): GeneratedRoleUnderstanding {
  ...
  const selectionSource = requestedSpecialization ? "user-choice" as const
    : selected ? "target-metadata" as const
...
function buildUnderstanding(input: {
  ...
  specializationSource: "conservative-default" | "target-metadata" | "user-choice";
  expectations: Array<Omit<z.input<typeof RoleExpectationSchema>, "id" | "trustState">>;
  ambiguity?: { question: string; options: Array<{ id: string; label: string }>; selectedOptionId: string; selectionSource: "conservative-default" | "target-metadata" | "user-choice" };
  source: z.input<typeof RoleUnderstandingSourceSchema>;
  createdAt: string;
  updatedAt: string;
}): GeneratedRoleUnderstanding {
...
async function deriveConservativeProjection(workspace: string, understanding: GeneratedRoleUnderstanding) {
  let snapshot: Awaited<ReturnType<typeof calculateEvidenceFoundationSnapshot>>;
  try { snapshot = await calculateEvidenceFoundationSnapshot(workspace); }
  catch { return emptyProjection(understanding.title, understanding.positioning, ["The current Evidence Foundation is unavailable."]); }
  ...
  const links: GuidedRoleEvidenceLink[] = [];
  const allocatedClaimIds = new Set<string>();
  const allocatedEvidenceIds = new Set<string>();
  for (const expectation of understanding.expectations) {
    const candidates = eligibleClaims.flatMap((claimRecord) => {
      ...
      return claimRecord.supportingEvidenceIds.flatMap((evidenceId) => {
        ...
        if (!evidenceRecord?.eligibility.roleMatching || !evidenceRecord.content) return [];
    ...
    const unallocated = candidates.find(({ claimRecord, evidenceRecord }) =>
      !allocatedClaimIds.has(claimRecord.id) && !allocatedEvidenceIds.has(evidenceRecord.id));
  ...
  const fit = snapshot.completeness.eligibleRoleEvidenceCount === 0 ? "insufficient evidence" as const
    : supportedRequired.length >= 3 && supportedRequired.length === required.length ? "strong" as const
      : supportedRequired.length >= 2 ? "credible" as const
        : supportedRequired.length === 1 ? "mixed" as const
          : "stretch" as const;
...
function emptyProjection(title: string, positioning = `Conservative positioning for ${title}`, limitations: string[] = []) {
  return {
    fit: "insufficient evidence" as const,
    positioning,
    strengths: [] as Array<{ theme: string; evidence: string; claimId: string; evidenceId: string }>,
    weaker: [] as string[],
    gaps: [] as string[],
    limitations,
    links: [] as GuidedRoleEvidenceLink[],
    selectedEvidenceIds: [] as string[],
    selectedClaimIds: [] as string[],
    eligibleEvidenceCount: 0,
    sectionPlan: [] as string[],
  };
}

function renderRoleUnderstandingPrompt(target: RoleTarget): string {
  return [
...
function templateFor(title: string): RoleTemplate {
...
function expectation(
  kind: z.input<typeof RoleExpectationSchema>["kind"],
  statement: string,
  necessity: z.input<typeof RoleExpectationSchema>["necessity"],
  importance: z.input<typeof RoleExpectationSchema>["importance"],
  capabilityTags: string[],
  group: z.input<typeof RoleExpectationSchema>["group"],
): Omit<z.input<typeof RoleExpectationSchema>, "id" | "trustState"> {
  return { kind, statement, necessity, importance, capabilityTags, group };
}

function understandingPaths(workspace: string, targetId: string) {
...
function targetPath(workspace: string, target: RoleTarget): string {
  return resolveWithin(workspace, `targets/roles/${target.id}/target.json`);
}

async function requireRoleTarget(workspace: string, targetId: string): Promise<RoleTarget> {
  const target = await showTarget(workspace, targetId);
  if (target.type !== "role") throw new Error("Guided Role workflow accepts Role Targets only.");
  return target;
}
...
function sourceLabel(understanding: GeneratedRoleUnderstanding): string {
...
function stage(stageName: RoleWorkflowStage, status: RoleWorkflowStageState["status"], detail: string): RoleWorkflowStageState {
  return { stage: stageName, status, detail };
}

async function safeStatus(load: () => Promise<{ status: string }>): Promise<{ status: string }> {
  try { return await load(); } catch { return { status: "missing" }; }
}

function pausedResult(mode: "run" | "continue", status: RoleWorkflowStatus, message: string): RunRoleWorkflowResult {
  return {
    mode,
    dryRun: false,
    result: "paused",
...
function slug(value: string): string {
  ...
  if (!result) throw new Error("A safe role value could not be derived.");
  return result;
}
...
function uniqueByClaim(links: GuidedRoleEvidenceLink[]): GuidedRoleEvidenceLink[] {
  const seen = new Set<string>();
  return links.filter((entry) => {
    if (seen.has(entry.claimId)) return false;
...
export function formatRoleWorkflowStatus(status: RoleWorkflowStatus, options: { verbose?: boolean } = {}): string {
  return [
...
export function formatRoleWorkflowJson(status: RoleWorkflowStatus): string {
  return `${JSON.stringify(status, null, 2)}\n`;
}

export function formatRoleWorkflowRunResult(result: RunRoleWorkflowResult): string {
  return [
...
export function formatFinalizeRoleWorkflowResult(result: FinalizeRoleWorkflowResult): string {
  return [
    formatRoleWorkflowStatus(result.status),
    "",
...
function humanStage(stageName: RoleWorkflowStage): string {
  const labels: Record<RoleWorkflowStage, string> = {
    target: "Target saved",
...
function humanCurrentState(status: RoleWorkflowStatus): string {
  if (status.understandingStatus === "missing") return "Role understanding needed";
  if (status.understandingStatus === "stale") return "Role understanding needs an explicit rebuild";
  if (status.understandingStatus === "invalid") return "Role understanding is invalid";
