...
import {
  ...
  JobRequirementCoverageModelSchema,
  type JobCoverageComponent,
  type JobRequirementCoverage,
  type JobRequirementCoverageManifest,
  type JobRequirementCoverageModel,
  type JobRequirementCoverageState,
  type JobRequirementEvidenceQuality,
} from "./job-coverage-schemas.js";
import {
  JOB_EVIDENCE_MAPPER_NAME,
  JOB_EVIDENCE_MAPPER_VERSION,
  JOB_EVIDENCE_MAPPING_POLICY_NAME,
  JOB_EVIDENCE_MAPPING_POLICY_VERSION,
  jobEvidenceMapPaths,
  showJobEvidenceMap,
...
export function jobCoveragePaths(
  workspace: string,
  targetId: string,
): JobCoveragePaths {
...
export async function buildJobCoverage(
  workspace: string,
  targetId: string,
  options: BuildOptions = {},
): Promise<BuildJobCoverageResult> {
  ...
  if ((status.status === "stale" || status.status === "invalid") && !options.rebuild) {
    throw new Error(
      `Stored Job Requirement Coverage is ${status.status} and was not overwritten. Review dependencies, then use --rebuild. ${status.reasons.join(" ")}`,
    );
  }
  ...
  const coverage = JobRequirementCoverageModelSchema.parse({
    schemaVersion: 1,
    id: `job-requirement-coverage_${hashText(
      `${targetId}\u0000${dependencies.normalizedInputSha256}\u0000${JOB_COVERAGE_POLICY_VERSION}`,
    ).slice(0, 14)}`,
    targetId,
    targetType: "job",
    analyzer: {
...
export async function showJobCoverage(
  workspace: string,
  targetId: string,
): Promise<JobRequirementCoverageModel> {
  ...
  return JobRequirementCoverageModelSchema.parse(
    await readJson<unknown>(paths.coveragePath, null),
  );
}

export async function getJobCoverageStatus(
  workspace: string,
  targetId: string,
): Promise<JobCoverageStatus> {
  await requireJobTarget(workspace, targetId);
  const paths = jobCoveragePaths(workspace, targetId);
  ...
  let coverage: JobRequirementCoverageModel;
  let manifest: JobRequirementCoverageManifest;
  try {
    coverage = JobRequirementCoverageModelSchema.parse(
      await readJson<unknown>(paths.coveragePath, null),
    );
    manifest = JobRequirementCoverageManifestSchema.parse(
  ...
  const coverageHashMatches =
    (await hashFile(paths.coveragePath)) === manifest.coverageSha256;
  ...
  const policyMatches =
    manifest.policyName === JOB_COVERAGE_POLICY_NAME &&
...
export function classifyJobRequirementCoverage(
  ...
  if (
    input.links.some(
      (link) =>
        link.relationship === "contradiction" &&
  ...
  const sufficientDirect = input.links.some(
    (link) =>
      link.relationship === "direct" &&
      link.evidenceStrength !== "weak" &&
      link.linkConfidence !== "low",
  );
  if (sufficientDirect) return { state: "supported", evidenceQuality };
  if (
    input.links.every(
      (link) =>
        link.relationship === "partial" || link.relationship === "supporting",
    )
  ) {
    return { state: "partially-supported", evidenceQuality };
  }
...
export function formatBuildJobCoverageResult(
  result: BuildJobCoverageResult,
): string {
  return [
...
export function formatJobCoverageStatus(status: JobCoverageStatus): string {
...
function analyzeCoverage(
  ...
  JobRequirementCoverageModel,
  "requirements" | "warnings" | "completeness"
> {
  ...
  const mappingByRequirement = new Map(
    map.requirementMappings.map((mapping) => [mapping.requirementId, mapping]),
  );
  const requirements = model.requirements.map((requirement) => {
    const mapping = mappingByRequirement.get(requirement.id);
    if (!mapping) {
      throw new Error(
        `Current Job Evidence Map is missing requirement mapping: ${requirement.id}`,
      );
    }
...
function coverageForRequirement(
  ...
  mapping: JobEvidenceMap["requirementMappings"][number],
  ...
  return JobRequirementCoverageModelSchema.shape.requirements.element.parse({
    id: `job-requirement-coverage-entry_${hashText(
      `${dependencies.target.id}\u0000${requirement.id}\u0000${JOB_COVERAGE_POLICY_VERSION}`,
    ).slice(0, 14)}`,
...
function identifyComponents(
  requirement: Requirement,
  links: JobEvidenceLink[],
): JobCoverageComponent[] {
  const labels =
    requirement.namedTechnologies.length > 1
      ? requirement.namedTechnologies
      : requirement.category === "language"
        ? LANGUAGE_NAMES.filter((language) =>
            new RegExp(`\\b${language}\\b`, "i").test(requirement.sourceText))
        : [];
  ...
  return uniqueSorted(labels).map((label) => {
    ...
    const mappedLinkIds = links
      .filter((link) =>
        link.matchedSignals.some(
          (signal) => normalizeSignal(signal.value) === normalizedLabel,
        ))
      .map((link) => link.id)
      .sort((left, right) => left.localeCompare(right));
    return {
      id: `job-coverage-component_${hashText(
        `${requirement.id}\u0000${normalizedLabel}`,
      ).slice(0, 12)}`,
      label,
      normalizedLabel,
      status: mappedLinkIds.length > 0 ? "supported" as const : "unsupported" as const,
      mappedLinkIds,
    };
  });
}
...
function requirementWarnings(
  state: JobRequirementCoverageState,
  quality: JobRequirementEvidenceQuality,
  components: JobCoverageComponent[],
): string[] {
...
function summarizeEvidenceQuality(
  ...
  const hasLimited = links.some(
    (link) => link.evidenceStrength === "weak" || link.linkConfidence === "low",
  );
  const hasReviewed = links.some(
    (link) =>
      link.evidenceStrength !== "weak" && link.linkConfidence !== "low",
  );
  if (hasLimited && hasReviewed) return "mixed";
  if (hasLimited) return "limited";
  const relationshipKinds = new Set(links.map((link) => link.relationship));
  if (relationshipKinds.size > 1) return "mixed";
  if (
    links.every(
      (link) =>
        link.relationship === "direct" &&
...
async function inspectDependencies(
  ...
  let evidenceMap: JobEvidenceMap;
  let evidenceMapManifest: JobEvidenceMapManifest;
  try {
    evidenceMap = await showJobEvidenceMap(workspace, targetId);
    evidenceMapManifest = JobEvidenceMapManifestSchema.parse(
      await readJson<unknown>(mapPaths.manifestPath, null),
    );
  } catch (error) {
    return {
      status: "invalid",
  ...
  const invalidReasons = validateMapIdentity(
    evidenceMap,
    evidenceMapManifest,
    mapPaths.mapRelativePath,
    evidenceMapSha256,
  );
  if (invalidReasons.length > 0) {
  ...
  let requirementInput: RequirementInput;
  try {
  ...
  const dependencyChecks = await mapDependencyChecks(
    workspace,
    evidenceMap,
    evidenceMapManifest,
    requirementInput,
  ...
  const semanticIssues = validateMapContentsWithoutRematching(
    evidenceMap,
    requirementInput,
  );
  const provenanceIssues = await validateReferencedEvidenceProvenance(
    workspace,
    evidenceMap,
  );
...
async function loadCurrentDependencies(
  workspace: string,
  targetId: string,
): Promise<CoverageDependencies> {
  const inspected = await inspectDependencies(workspace, targetId);
  if (inspected.status !== "current" || !inspected.dependencies) {
    throw new Error(
      `Job Requirement Coverage requires a current Job Evidence Map and Job Requirement Model. Current status: ${inspected.status}. ${inspected.reasons.join(" ")}`,
    );
  }
...
function validateMapIdentity(
  map: JobEvidenceMap,
  manifest: JobEvidenceMapManifest,
  expectedPath: string,
  actualMapSha256: string,
): string[] {
...
async function mapDependencyChecks(
  workspace: string,
  map: JobEvidenceMap,
  manifest: JobEvidenceMapManifest,
  requirementInput: RequirementInput,
...
function validateMapContentsWithoutRematching(
  map: JobEvidenceMap,
  input: RequirementInput,
): string[] {
  const reasons: string[] = [];
  const requirementsById = new Map(
    input.model.requirements.map((requirement) => [requirement.id, requirement]),
  );
  ...
  const linksById = new Map(map.links.map((link) => [link.id, link]));
  if (
    mappingsByRequirement.size !== requirementsById.size ||
  ...
  if (
    linksById.size !== map.links.length ||
  ...
  for (const link of map.links) {
    ...
    if (
      stableJson(link.requirementProvenance) !==
  ...
  if (
    map.completeness.requirementCount !== input.model.requirements.length ||
...
async function validateReferencedEvidenceProvenance(
  workspace: string,
  map: JobEvidenceMap,
): Promise<string[]> {
  let sources: Source[];
  ...
  try {
    ...
    ).map((entry) => SourceSchema.parse(entry));
    evidenceItems = (
...
function validateStoredIdentity(
  ...
  manifest: JobRequirementCoverageManifest,
  paths: JobCoveragePaths,
): string[] {
  const reasons: string[] = [];
  if (
    coverage.id !== manifest.coverageId ||
  ...
  if (
    coverage.input.target.sha256 !== manifest.targetSha256 ||
  ...
  if (
    new Set(coverage.requirements.map((entry) => entry.requirementId)).size !==
...
function resultFromCoverage(
  coverage: JobRequirementCoverageModel,
  paths: JobCoveragePaths,
  result: BuildJobCoverageResult["result"],
): BuildJobCoverageResult {
  return {
    ...
    readyForDownstreamAssessment:
      coverage.completeness.readyForDownstreamAssessment,
  };
}
...
async function safeHash(filePath: string): Promise<string | undefined> {
