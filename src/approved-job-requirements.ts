...
import {
  ...
  ApprovedJobRequirementModelSchema,
  JobRequirementModelManifestSchema,
  JobRequirementProposalManifestSchema,
  JobRequirementReviewManifestSchema,
  type ApprovedJobRequirement,
  type ApprovedJobRequirementManifest,
  type ApprovedJobRequirementModel,
  type JobRequirementProposalReview,
} from "./job-requirement-schemas.js";
import {
  JOB_REQUIREMENT_PROPOSAL_POLICY_VERSION,
  JOB_REQUIREMENT_PROPOSAL_PROMPT_ID,
  JOB_REQUIREMENT_PROPOSAL_PROMPT_VERSION,
  getJobRequirementProposalStatus,
  locateJobRequirementProposal,
  showJobRequirementProposal,
} from "./job-requirement-proposal.js";
import {
  getJobRequirementReviewStatus,
  jobRequirementReviewPaths,
  showJobRequirementReview,
} from "./job-requirement-review.js";
import {
  JOB_REQUIREMENT_POLICY_NAME,
  JOB_REQUIREMENT_POLICY_VERSION,
...
export const APPROVED_JOB_REQUIREMENT_POLICY_NAME =
  "approved-job-requirement-modeling-policy";
...
export async function approveJobRequirements(
...
): Promise<ApproveJobRequirementsResult> {
  ...
  if (deterministicStatus.status !== "current") {
    throw new Error(
      `A current deterministic job requirement model is required. Current status: ${deterministicStatus.status}.`,
    );
  }
  ...
  if (reviewStatus.status !== "completed") {
    throw new Error(
      `Job requirement review must be completed before approval. Current status: ${reviewStatus.status}`,
    );
  }
  ...
  if (existingStatus.status === "current") {
    ...
    if (manifest.proposalId === proposalId) {
      ...
      return resultFromApproved(
        approved,
        manifest,
        "already-current",
        await rejectedCount(workspace, proposalId),
      );
    }
  }
  if (
    (existingStatus.status === "stale" || existingStatus.status === "invalid") &&
    !options.rebuild
  ) {
    throw new Error(
      `Approved job requirement model is ${existingStatus.status} and was not overwritten. Review dependencies, then use --rebuild.`,
    );
  }
  ...
  const deterministicManifest = JobRequirementModelManifestSchema.parse(
    await readJson<unknown>(deterministicPaths.manifestPath, null),
  );
  const proposalLocation = await locateJobRequirementProposal(workspace, proposalId);
  const proposalManifest = JobRequirementProposalManifestSchema.parse(
    await readJson<unknown>(proposalLocation.manifestPath, null),
  );
  const reviewPaths = jobRequirementReviewPaths(workspace, targetId, proposalId);
  const reviewManifest = JobRequirementReviewManifestSchema.parse(
    await readJson<unknown>(reviewPaths.manifestPath, null),
  );
  const normalizedInputSha256 = approvedInputHash(
    deterministicManifest.modelSha256,
  ...
  const sourceItemIds = new Set(
    requirements.map((entry) => entry.provenance.sourceAnalysisItemId),
  );
  ...
  const approved = ApprovedJobRequirementModelSchema.parse({
    ...
    ambiguities: deterministic.ambiguities.filter((entry) =>
      entry.requirementIds.some((id) =>
        requirements.some((requirement) =>
          requirement.approvalProvenance.reviewedRequirementId === id))),
    contradictions: deterministic.contradictions.filter((entry) =>
      entry.requirementIds.every((id) =>
        requirements.some((requirement) =>
          requirement.approvalProvenance.reviewedRequirementId === id))),
    risks: deterministic.risks.filter((entry) =>
  ...
  return resultFromApproved(
    approved,
    manifest,
    existingStatus.status === "missing" ? "created" : "updated",
    review.decisions.filter((entry) => entry.decision === "reject").length,
  );
}

export async function showApprovedJobRequirements(
  workspace: string,
  targetId: string,
  ...
  return ApprovedJobRequirementModelSchema.parse(
    await readJson<unknown>(paths.approvedModelPath, null),
  );
}

export async function getApprovedJobRequirementsStatus(
  workspace: string,
  targetId: string,
  options: Pick<ApproveOptions, "policyName" | "policyVersion"> = {},
  ...
  let approved: ApprovedJobRequirementModel;
  let manifest: ApprovedJobRequirementManifest;
  try {
    approved = ApprovedJobRequirementModelSchema.parse(
      await readJson<unknown>(paths.approvedModelPath, null),
    );
    manifest = ApprovedJobRequirementManifestSchema.parse(
      await readJson<unknown>(paths.manifestPath, null),
    );
  } catch (error) {
  ...
  const deterministicPaths = jobRequirementPaths(workspace, targetId);
  const approvedModelHashMatches =
    (await hashFile(paths.approvedModelPath)) === manifest.approvedModelSha256;
  const targetHashMatches = await hashMatches(
    deterministicPaths.targetPath,
    manifest.targetSha256,
  );
  const sourceHashMatches = await hashMatches(
    deterministicPaths.sourcePath,
    manifest.sourceSha256,
  );
  const structuralAnalysisHashMatches = await hashMatches(
    deterministicPaths.analysisPath,
    manifest.structuralAnalysisSha256,
  );
  const deterministicModelHashMatches = await hashMatches(
    deterministicPaths.modelPath,
    manifest.deterministicModelSha256,
  );
  let proposalHashMatches = false;
  let reviewHashMatches = false;
  let proposalIsCurrent = false;
  let reviewIsCompleted = false;
  let dependencyLookupReason: string | undefined;
  try {
    const proposalLocation = await locateJobRequirementProposal(
      workspace,
      manifest.proposalId,
    );
    const reviewPaths = jobRequirementReviewPaths(
      workspace,
      targetId,
      manifest.proposalId,
    );
    proposalHashMatches = await hashMatches(
      proposalLocation.proposalPath,
      manifest.proposalSha256,
    );
    reviewHashMatches = await hashMatches(
  ...
  const promptMatches =
    manifest.promptTemplateId === JOB_REQUIREMENT_PROPOSAL_PROMPT_ID &&
    manifest.promptTemplateVersion === JOB_REQUIREMENT_PROPOSAL_PROMPT_VERSION &&
    proposalIsCurrent;
  const policyMatches =
    manifest.policyName === (options.policyName ?? APPROVED_JOB_REQUIREMENT_POLICY_NAME) &&
    manifest.policyVersion ===
      (options.policyVersion ?? APPROVED_JOB_REQUIREMENT_POLICY_VERSION);
  const normalizedInputHashMatches =
  ...
  if (
    approved.input.normalizedInputSha256 !== manifest.normalizedInputSha256 ||
...
export function formatApproveJobRequirementsResult(
  result: ApproveJobRequirementsResult,
): string {
  return [
...
export function formatApprovedJobRequirementsStatus(
  status: ApprovedJobRequirementsStatus,
): string {
...
function approvedRequirements(
  deterministic: Awaited<ReturnType<typeof showJobRequirementModel>>,
  proposal: Awaited<ReturnType<typeof showJobRequirementProposal>>,
  review: JobRequirementProposalReview,
  analysis: Awaited<ReturnType<typeof showTargetAnalysis>>,
): ApprovedJobRequirement[] {
  ...
  return review.decisions.flatMap((decision): ApprovedJobRequirement[] => {
    if (decision.decision === "pending" || decision.decision === "reject") return [];
    ...
    if (!sourceItemId) throw new Error(`Reviewed requirement has no source item: ${decision.requirementId}`);
    const sourceItem = analysisById.get(sourceItemId);
    if (!sourceItem) throw new Error(`Approved requirement source item is missing: ${sourceItemId}`);
    ...
    return [{
      id: `approved-job-requirement_${hashText(
        `${proposal.id}\u0000${decision.source}\u0000${decision.requirementId}\u0000${decision.decision}`,
      ).slice(0, 14)}`,
...
async function selectCompletedProposal(workspace: string, targetId: string): Promise<string> {
  ...
  const completed: Array<{ proposalId: string; updatedAt: string }> = [];
  for (const file of reviewFiles) {
    const review = await readJson<JobRequirementProposalReview | null>(file, null);
    if (review?.targetId !== targetId || review.status !== "completed") continue;
    try {
  ...
  if (completed.length === 0) {
    throw new Error(
      "No current completed job requirement review exists. Supply --proposal after review.",
    );
  }
...
function reviewedSourceAccepted(
  deterministicId: string,
  review: JobRequirementProposalReview,
  _acceptedIds: Set<string>,
): boolean {
  return review.decisions.some((entry) =>
    entry.source === "deterministic" &&
    entry.requirementId === deterministicId &&
    (entry.decision === "accept" || entry.decision === "edit"));
}

async function rejectedCount(workspace: string, proposalId: string): Promise<number> {
  const review = await showJobRequirementReview(workspace, proposalId);
  return review.decisions.filter((entry) => entry.decision === "reject").length;
}

function resultFromApproved(
  approved: ApprovedJobRequirementModel,
  manifest: ApprovedJobRequirementManifest,
  result: ApproveJobRequirementsResult["result"],
  rejected: number,
): ApproveJobRequirementsResult {
  return {
