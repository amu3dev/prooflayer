...
export const EVIDENCE_SNAPSHOT_SUPPORTED_POLICY_VERSIONS = ["1", "2"] as const;
export const EVIDENCE_SNAPSHOT_EXPORTER_NAME = "evidence-snapshot-exporter";
export const EVIDENCE_SNAPSHOT_EXPORTER_VERSION = "2";
export const EVIDENCE_SNAPSHOT_SUPPORTED_EXPORTER_VERSIONS = ["1", "2"] as const;

export const EvidenceSnapshotSha256Schema = z.string().regex(/^[a-f0-9]{64}$/);
export const EvidenceSnapshotIdSchema = z.string().regex(
  /^evidence-snapshot-[a-f0-9]{20}$/,
);
export const EvidenceSnapshotRelativePathSchema = z.string().min(1).refine(
  (value) =>
    !value.startsWith("/") &&
    !/^[A-Za-z]:[\\/]/.test(value) &&
...
export type EvidenceFoundationSnapshot = z.infer<
  typeof EvidenceFoundationSnapshotSchema
>;
export type EvidenceSnapshotManifestV1 = z.infer<
  typeof EvidenceSnapshotManifestSchemaV1
>;
export type EvidenceSnapshotEvidenceRecord = z.infer<
  typeof EvidenceSnapshotEvidenceRecordSchema
>;
export type EvidenceSnapshotClaimRecord = z.infer<
  typeof EvidenceSnapshotClaimRecordSchema
>;
export type EvidenceSnapshotSourceReference = z.infer<
  typeof EvidenceSnapshotSourceReferenceSchema
>;
