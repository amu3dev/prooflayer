...
export async function initWorkspace(workspace: string): Promise<void> {
...
export async function ingestSources(workspace: string): Promise<Source[]> {
...
export async function normalizeEvidence(workspace: string): Promise<EvidenceItem[]> {
  const sources = await readJson<Source[]>(path.join(workspace, "kb/sources.json"), []);
  const evidenceItems: EvidenceItem[] = [];
  const stats: NormalizationStats = { ignoredHeadings: 0, ignoredFragments: 0, warnings: [] };

  for (const source of sources) {
    if (!source.extractedTextPath || source.status !== "active") continue;
    const extractedPath = path.join(workspace, source.extractedTextPath);
    if (!(await pathExists(extractedPath))) continue;
...
export async function generateClaims(workspace: string): Promise<Claim[]> {
  const evidenceItems = await readJson<EvidenceItem[]>(path.join(workspace, "kb/evidence-items.json"), []);
  const existingClaims = await readJson<Claim[]>(path.join(workspace, "kb/claims.json"), []);
...
export function generateClaimsFromEvidence(evidenceItems: EvidenceItem[], existingClaims: Claim[] = []): Claim[] {
  const groupsByText = new Map<string, Map<string, EvidenceItem[]>>();
  ...
  for (const item of evidenceItems.filter(isClaimCandidate)) {
    ...
    const contextGroups = groupsByText.get(textKey) ?? new Map<string, EvidenceItem[]>();
  ...
  for (const [textKey, contextGroups] of groupsByText) {
    ...
    const manualClaims = existingClaims.filter((claim) =>
      claim.corroborationLevel === "manual_approved" && normalizeClaimIdentityText(claim.claim) === textKey
    );
...
function normalizeClaimIdentityText(value: string): string {
  return value.normalize("NFKC").toLowerCase().replace(/\s+/g, " ").trim();
}

function claimContextKey(item: EvidenceItem, parentContextById: Map<string, string>): string {
...
function buildClaimParentContextMap(items: EvidenceItem[]): Map<string, string> {
...
function selectLegacyClaimContext(
  entries: Array<[string, EvidenceItem[]]>,
  existingLegacyClaim?: Claim
): string {
  if (!existingLegacyClaim) return entries[0][0];
...
function manualClaimForContext(manualClaims: Claim[], items: EvidenceItem[], allowTextFallback: boolean): Claim | undefined {
  ...
  return manualClaims.find((claim) =>
    claim.parentRoleId === parentRoleId && claim.parentProjectId === parentProjectId
  ) ?? (allowTextFallback ? manualClaims[0] : undefined);
}

function sharedEvidenceValue(
  items: EvidenceItem[],
  selector: (item: EvidenceItem) => string | undefined
): string | undefined {
  const values = uniqueSorted(items.map(selector).filter((value): value is string => Boolean(value)));
  return values.length === 1 ? values[0] : undefined;
}

function buildClaim(items: EvidenceItem[], manualClaim: Claim | undefined, id: string): Claim {
  ...
  const extractionConfidence = items.reduce((current, item) => strongestConfidence(current, item.confidence), "low" as Confidence);
  ...
  const corroborationLevel: Claim["corroborationLevel"] = manualClaim
    ? "manual_approved"
    : sourceIds.length > 1
      ? "multi_source"
      : sourceIds.length === 1
  ...
  let approvalStatus: Claim["approvalStatus"] = "needs_confirmation";
  if (unsafe) {
...
export async function buildProfile(workspace: string): Promise<CareerProfile> {
  const evidenceItems = await readJson<EvidenceItem[]>(path.join(workspace, "kb/evidence-items.json"), []);
...
export async function auditPrivacy(workspace: string): Promise<ReturnType<typeof auditSourcesAndEvidence>> {
  const sources = await readJson<Source[]>(path.join(workspace, "kb/sources.json"), []);
  const evidenceItems = await readJson<EvidenceItem[]>(path.join(workspace, "kb/evidence-items.json"), []);
  const claims = await readJson<Claim[]>(path.join(workspace, "kb/claims.json"), []);
...
export async function rebuild(workspace: string): Promise<void> {
...
function detectSourceType(relativePath: string): Source["type"] {
  const normalized = relativePath.toLowerCase();
  const ext = path.extname(relativePath).toLowerCase();
  if (normalized.includes("/cvs/")) return "cv";
  if (normalized.includes("/linkedin/")) return "linkedin_export";
  if (normalized.includes("/github/")) return "github_summary";
  if (normalized.includes("/project-notes/")) return "project_note";
  if (normalized.includes("/recommendations/")) return "recommendation";
  if (normalized.includes("/certificates/")) return "certificate";
  if (normalized.includes("/jobs/")) return "job_description";
  if (normalized.includes("/pdf/") || ext === ".pdf") return "pdf";
  if (normalized.includes("/docx/") || ext === ".docx") return "docx";
...
export function extractEvidenceFromSource(source: Source, text: string): EvidenceItem[] {
  return extractEvidenceResult(source, text).items;
}

function extractEvidenceResult(source: Source, text: string): { items: EvidenceItem[]; stats: NormalizationStats } {
...
function extractResumeEvidence(source: Source, text: string): { items: EvidenceItem[]; stats: NormalizationStats } {
  ...
  for (const line of resume.educationLines) {
    const category: EvidenceItem["category"] = isCertificationLike(line, source) ? "certification" : "education";
...
function evidenceFromProject(source: Source, project: ParsedResumeEntry, confidence: Confidence): EvidenceItem[] {
  const items: EvidenceItem[] = [];
...
function evidenceFromExperience(source: Source, experience: ParsedResumeEntry, confidence: Confidence): EvidenceItem[] {
...
function extractGenericEvidence(source: Source, cleanText: string): { items: EvidenceItem[]; stats: NormalizationStats } {
  if (source.type === "project_note") return extractProjectNoteEvidence(source, cleanText);
...
function extractProjectNoteEvidence(source: Source, cleanText: string): { items: EvidenceItem[]; stats: NormalizationStats } {
  const items: EvidenceItem[] = [];
  const stats: NormalizationStats = { ignoredHeadings: 0, ignoredFragments: 0, warnings: [] };
  ...
  let projectEvidence: EvidenceItem | undefined;
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (index === narrativeIndex && projectEvidence) continue;
...
function detectProjectNoteName(source: Source, lines: string[]): string | undefined {
...
function confidenceFromVisibility(visibility: Visibility): Confidence {
  if (visibility === "public") return "medium";
  if (visibility === "generic_only") return "medium";
  return "low";
}

function structuredConfidence(visibility: Visibility): Confidence {
  return ["public", "generic_only"].includes(visibility) ? "high" : "medium";
}

function evidenceCategoryForBullet(text: string): EvidenceItem["category"] {
...
function isUsableEvidenceText(value: string): boolean {
  ...
  if (cleaned.length < 20 || cleaned.length > 500) return false;
  if (isResumeSectionHeading(cleaned) || isDateLocationLine(cleaned)) return false;
  if (isLikelyPersonName(cleaned)) return false;
  if (/^(?:evidence mentions|technologies\/tools found|domains found)\b/i.test(cleaned)) return false;
  if (/[,:;-]$/.test(cleaned) || /\.{3}$/.test(cleaned)) return false;
  return true;
}
...
function isRoleLike(line: string, nextLine: string, dateRange?: string): boolean {
  return Boolean(
    dateRange ||
...
function isProjectLike(line: string, source: Source): boolean {
...
function detectKnownValues(text: string, knownValues: string[]): string[] {
  ...
  if (!/\bReact\b/i.test(withoutReactNative)) removeValue(found, "React");

  const withoutGitHubProjects = text.replace(/\bGitHub Projects\b/gi, "");
  if (!/\bGitHub\b/i.test(withoutGitHubProjects)) removeValue(found, "GitHub");
...
function claimTextFromEvidence(item: EvidenceItem): string {
  return item.normalizedSummary;
}

export function isClaimCandidate(item: EvidenceItem): boolean {
  const text = stripMarkdown(item.text);
  if (isResumeSectionHeading(text) || isDateLocationLine(text)) return false;
  if (isLikelyPersonName(text) && !["role", "project", "education", "certification"].includes(item.category)) return false;
  if (/^(?:evidence mentions|technologies\/tools found|domains found)\b/i.test(text)) return false;
  if (/[,:;-]$/.test(text) || /\.{3}$/.test(text)) return false;
  if (text.length < 20 && !["role", "project", "skill", "tool", "education", "certification"].includes(item.category)) return false;
  if (text.split(/\s+/).length === 1 && !["skill", "tool", "project"].includes(item.category)) return false;
  return true;
}

function isGenericCompetencyEvidence(item: EvidenceItem): boolean {
  if (item.sourceSection === "Product & Technical Strengths") return true;
...
function claimTypeFromEvidence(category: EvidenceItem["category"]): Claim["type"] {
  const map: Record<EvidenceItem["category"], Claim["type"]> = {
    role: "role_claim",
...
function metricStatus(item: EvidenceItem): Claim["metricStatus"] {
  ...
  if (STRUCTURAL_METRIC_RE.test(item.text)) return "structural_metric";
...
function combinedMetricStatus(items: EvidenceItem[]): Claim["metricStatus"] {
  const order: Claim["metricStatus"][] = ["no_metric", "structural_metric", "verified_metric", "needs_metric"];
  return items.reduce((current, item) => {
    const next = metricStatus(item);
    return order.indexOf(next) > order.indexOf(current) ? next : current;
  }, "no_metric" as Claim["metricStatus"]);
}
...
function determineFactualConfidence(options: {
  ...
  if (options.corroborationLevel === "manual_approved") return "high";
  if (options.extractionConfidence === "low" || !options.factLike) return "low";
  if (options.competency) return "medium";
  if (options.corroborationLevel === "multi_source" && options.extractionConfidence === "high") return "high";
  if (options.hasPublicEvidence && options.extractionConfidence === "high") return "high";
  return "medium";
}

function determineOutputReadiness(items: EvidenceItem[], approvalStatus: Claim["approvalStatus"]): Claim["outputReadiness"] {
  if (items.some((item) => ["sensitive", "do_not_use"].includes(item.visibility))) return "do_not_use";
  if (items.some((item) => item.visibility === "private")) return "internal_only";
  if (approvalStatus === "approved") return "resume_ready";
  if (items.some((item) => item.visibility === "generic_only")) return "generic_only";
  return "internal_only";
}

function strongestConfidence(a: Confidence, b: Confidence): Confidence {
  const order: Confidence[] = ["low", "medium", "high"];
  return order.indexOf(a) >= order.indexOf(b) ? a : b;
}

function chronologicalValue(dateRange?: string): number {
  ...
  if (/\b(?:Present|Current|Now)\b/i.test(dateRange)) return 100_000_000 + start;
  return points.at(-1) ?? start;
}

function inferPositioningCandidates(items: EvidenceItem[], claims: Claim[]): string[] {
  const text = `${items.map((item) => item.text).join(" ")} ${claims.map((claim) => claim.claim).join(" ")}`;
  const candidates = new Set<string>();
  if (/\b(product|roadmap|discovery|stakeholder|platform)\b/i.test(text)) candidates.add("Technical Product Manager");
  if (/\b(CTO|Head of Technology|technology leadership|architecture)\b/i.test(text)) candidates.add("Product & Technology Leader");
  if (/\b(AI|LLM|workflow|automation)\b/i.test(text)) candidates.add("AI Product / Tooling Lead");
  if (/\b(engineering|developer|software|technical delivery)\b/i.test(text)) candidates.add("Engineering Leader with Product Depth");
  return [...candidates];
}

function inferSummaryThemes(items: EvidenceItem[]): string[] {
...
function buildRoles(items: EvidenceItem[]): CareerProfile["roles"] {
  return items
...
function buildProjects(items: EvidenceItem[]): CareerProfile["projects"] {
  ...
  return [...groups.entries()].slice(0, 50).map(([identity, projectGroup]) => {
    ...
    const supportingItems = items.filter((candidate) =>
      projectIds.has(candidate.id) || Boolean(candidate.parentProjectId && projectIds.has(candidate.parentProjectId))
    );
    return {
...
function buildSkills(items: EvidenceItem[]): CareerProfile["skills"] {
  const skills = new Map<string, Set<string>>();
  for (const item of items) {
    for (const tech of item.technologies ?? []) {
      if (!skills.has(tech)) skills.set(tech, new Set());
...
function renderPrivacyReport(
  findings: ReturnType<typeof auditSourcesAndEvidence>,
  sources: Source[],
  claims: Claim[]
): string {
...
async function writeNormalizationQualityReport(workspace: string): Promise<void> {
  const sources = await readJson<Source[]>(path.join(workspace, "kb/sources.json"), []);
  const evidenceItems = await readJson<EvidenceItem[]>(path.join(workspace, "kb/evidence-items.json"), []);
  const claims = await readJson<Claim[]>(path.join(workspace, "kb/claims.json"), []);
  const stats = await readJson<NormalizationStats>(path.join(workspace, "kb/normalization-stats.json"), {
    ignoredHeadings: 0,
    ignoredFragments: 0,
    warnings: []
  });
  ...
  if (roles === 0) warnings.push("No structured roles were detected.");
  if (projects === 0) warnings.push("No structured projects were detected.");
  if (approvedClaims === 0) warnings.push("No approved public-safe claims were generated.");
...
async function writeTrustModelReport(workspace: string): Promise<void> {
  const sources = await readJson<Source[]>(path.join(workspace, "kb/sources.json"), []);
  const claims = await readJson<Claim[]>(path.join(workspace, "kb/claims.json"), []);
  const count = (predicate: (claim: Claim) => boolean): number => claims.filter(predicate).length;
  ...
  if (sources.length === 1) warnings.push("All current claims rely on a single source; corroboration is not yet available.");
...
async function loadSnapshot(workspace: string): Promise<RebuildSnapshot> {
  return {
    sources: await readJson<Source[]>(path.join(workspace, "kb/sources.json"), []),
    evidenceItems: await readJson<EvidenceItem[]>(path.join(workspace, "kb/evidence-items.json"), []),
    claims: await readJson<Claim[]>(path.join(workspace, "kb/claims.json"), [])
  };
}

function renderRebuildChangelog(
