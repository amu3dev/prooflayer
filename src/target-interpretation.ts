...
import {
  RoleProfileSchema,
  TargetAnalysisManifestSchema,
  TargetInterpretationManifestSchema,
  TargetInterpretationSchema,
  type ExpectationGroup,
  type ExpectationGroupKind,
  type InterpretationAmbiguity,
  type InterpretationCompleteness,
  type InterpretationWarning,
  type RoleProfile,
  type Target,
  type TargetAnalysis,
  type TargetAnalysisItem,
  type TargetAnalysisSourceReference,
  type TargetExpectation,
  type TargetInterpretation,
  type TargetInterpretationManifest,
...
interface InterpretationPaths {
  targetRelativePath: string;
  targetPath: string;
  structuralAnalysisRelativePath: string;
  structuralAnalysisPath: string;
  structuralManifestRelativePath: string;
  structuralManifestPath: string;
  interpretationRelativePath: string;
...
interface CurrentDependencies {
  target: Target;
  paths: InterpretationPaths;
  targetSha256: string;
  structuralAnalysisStatus: Awaited<ReturnType<typeof getTargetAnalysisStatus>>;
  structuralAnalysisSha256?: string;
  structuralAnalysis?: TargetAnalysis;
  roleProfile?: LoadedRoleProfile;
  roleProfileExpectedPath?: string;
  roleProfileError?: string;
}
...
function interpretationPaths(workspace: string, target: Target): InterpretationPaths {
...
function roleProfileMatches(target: Target, profile: RoleProfile): boolean {
  if (target.type !== "role") return false;
...
export async function loadRoleProfile(
  ...
  if (!(await exists(absolutePath))) throw new Error(`Role profile not found: ${inputPath}`);
  ...
  if (target && !roleProfileMatches(target, profile)) {
    throw new Error(
      `Role profile title and aliases do not exactly match target title: ${target.title}`,
    );
  }
  return {
    profile,
...
async function findExactRoleProfile(
  workspace: string,
  target: Target,
): Promise<LoadedRoleProfile | undefined> {
  if (target.type !== "role") return undefined;
  ...
  const matches: LoadedRoleProfile[] = [];
  for (const candidate of candidates) {
    const loaded = await loadRoleProfile(workspace, candidate);
    if (roleProfileMatches(target, loaded.profile)) matches.push(loaded);
  }
  if (matches.length > 1) {
...
async function readManifestHint(paths: InterpretationPaths): Promise<TargetInterpretationManifest | undefined> {
  if (!(await exists(paths.manifestPath))) return undefined;
  try {
    return TargetInterpretationManifestSchema.parse(
...
): Promise<{ profile?: LoadedRoleProfile; expectedPath?: string; error?: string }> {
...
async function currentDependencies(
  ...
  const resolvedProfile = await resolveRoleProfile(
    workspace,
    target,
    roleProfilePath,
    roleProfilePath ? undefined : manifestHint?.roleProfilePath,
  );
  let structuralAnalysisSha256: string | undefined;
  let structuralAnalysis: TargetAnalysis | undefined;
...
function interpretationMetadata(options: InterpretOptions): {
  name: string;
  version: string;
  mode: "deterministic";
  policyVersion: string;
...
function roleProfileReference(
  profile: LoadedRoleProfile,
  expectation: RoleProfile["expectations"][number],
  index: number,
): TargetInterpretationSourceReference {
  return {
...
function uniqueReferences(
  references: TargetInterpretationSourceReference[],
): TargetInterpretationSourceReference[] {
...
function warning(
  targetId: string,
  policyVersion: string,
  code: InterpretationWarning["code"],
  context: string,
  message: string,
  itemIds: string[] = [],
  references: TargetInterpretationSourceReference[] = [],
...
function ambiguity(
  targetId: string,
  policyVersion: string,
  code: InterpretationAmbiguity["code"],
  context: string,
  message: string,
  itemIds: string[] = [],
  references: TargetInterpretationSourceReference[] = [],
...
function roleInterpretation(
  input: SemanticTargetInterpretationInput,
  metadata: ReturnType<typeof interpretationMetadata>,
): TargetInterpretation {
  const expectations: TargetExpectation[] = [];
  const ambiguities: InterpretationAmbiguity[] = [];
  ...
  if (input.roleProfile) {
    ...
    ambiguities.push(
      ambiguity(
        ...
        "role-profile",
        "No curated role profile is configured, so no role expectations were inferred from the title.",
      ),
    );
    warnings.push(
      warning(
        ...
        "role-profile",
        "Role interpretation contains no expectations because no exact-match role profile was supplied or found.",
      ),
    );
  }
  ...
  if (input.roleProfile) {
    const byGroup = new Map<ExpectationGroupKind, TargetExpectation[]>();
    for (const expectation of expectations) {
      const source = input.roleProfile.profile.expectations.find((entry) =>
        expectation.id === stableId("expectation", [
          input.target.id,
          input.roleProfile?.profile.id,
          entry.id,
          metadata.policyVersion,
  ...
  if (expectations.length === 0) {
    warnings.push(
      warning(
        ...
        "expectations",
        "No semantic expectations were produced for this target.",
      ),
    );
  }
...
function jobGroupKind(
  classification: TargetAnalysis["sections"][number]["classification"],
): ExpectationGroupKind | undefined {
  if (classification === "responsibilities") return "core-responsibilities";
  if (classification === "required") return "required-qualifications";
  if (classification === "preferred") return "preferred-qualifications";
  if (classification === "qualifications") return "context-dependent";
  return undefined;
}

function jobExpectationKind(
  classification: TargetAnalysis["sections"][number]["classification"],
): TargetExpectation["kind"] | undefined {
  if (classification === "responsibilities") return "responsibility";
  if (["required", "preferred", "qualifications"].includes(classification)) return "qualification";
  return undefined;
}

function jobInterpretation(
  input: SemanticTargetInterpretationInput,
  metadata: ReturnType<typeof interpretationMetadata>,
): TargetInterpretation {
  const analysis = input.structuralAnalysis;
  if (analysis.targetType !== "job") throw new Error("Job interpretation requires job structural analysis.");
  const sections = new Map(analysis.sections.map((section) => [section.id, section]));
  const expectations: TargetExpectation[] = [];
  const ambiguities: InterpretationAmbiguity[] = [];
  const warnings: InterpretationWarning[] = [];
  const expectationBySection = new Map<string, TargetExpectation[]>();
  const unknownWarnings = new Set<string>();

  for (const item of analysis.items) {
    if (item.kind === "front-matter-field") continue;
    ...
    if (!section) {
      warnings.push(
        warning(
          ...
          item.id,
          "An item without an explicit section was not interpreted as a candidate expectation.",
          [item.id],
    ...
    if (item.kind === "paragraph") {
      warnings.push(
        warning(
          ...
          item.id,
          "A paragraph was preserved structurally but not interpreted as a candidate expectation.",
          [item.id],
          item.sourceReferences,
        ),
      );
      ...
      if (section.classification === "unknown") {
        if (!unknownWarnings.has(section.id)) {
          ...
          warnings.push(
            warning(
              ...
              section.id,
              `Unknown section "${section.heading ?? "untitled"}" was not semantically interpreted.`,
        ...
        ambiguities.push(
          ambiguity(
            ...
            item.id,
            "The source section does not explicitly identify how this item should be interpreted.",
            [item.id],
            item.sourceReferences,
          ),
        );
        ...
        warnings.push(
          warning(
            ...
            "UNCLASSIFIED_ITEM_SKIPPED",
            item.id,
            `Items in ${section.classification} sections are not candidate expectations in this policy.`,
            [item.id],
            item.sourceReferences,
          ),
        );
  ...
  for (const section of analysis.sections) {
    ...
    if (!entries?.length || !kind) continue;
    groups.push({
  ...
  if (expectations.length === 0) {
    warnings.push(
      warning(
        ...
        "NO_EXPECTATIONS_PRODUCED",
        "expectations",
        "No semantic expectations were produced from explicit supported job-description structure.",
      ),
    );
  }
...
function deterministicCompleteness(
  targetType: "role" | "job",
  expectations: TargetExpectation[],
...
export class DeterministicSemanticTargetInterpreter implements SemanticTargetInterpreter {
  readonly name: string;
  readonly version: string;
  ...
  async interpret(input: SemanticTargetInterpretationInput): Promise<TargetInterpretation> {
    const metadata = {
      name: this.name,
...
async function validateJobSourceReferences(
  workspace: string,
  target: Target,
  analysis: TargetAnalysis,
): Promise<void> {
  if (target.type !== "job" || analysis.targetType !== "job") return;
  ...
  const references: TargetAnalysisSourceReference[] = [
  ...
  for (const reference of references) {
    ...
    if (
      reference.path !== analysis.input.sourcePath ||
...
async function storedInterpretationStatus(
  dependencies: CurrentDependencies,
  metadata: ReturnType<typeof interpretationMetadata>,
): Promise<TargetInterpretationStatus> {
  ...
  let interpretation: TargetInterpretation;
  let manifest: TargetInterpretationManifest;
  try {
    interpretation = TargetInterpretationSchema.parse(
      await readJson<unknown>(paths.interpretationPath, null),
    );
    manifest = TargetInterpretationManifestSchema.parse(
      await readJson<unknown>(paths.manifestPath, null),
    );
  } catch (error) {
    return {
  ...
  const interpretationHashMatches =
    (await hashFile(paths.interpretationPath)) === manifest.interpretationSha256;
  const targetHashMatches = dependencies.targetSha256 === manifest.targetSha256;
  const structuralAnalysisHashMatches =
    dependencies.structuralAnalysisStatus.status === "current" &&
    dependencies.structuralAnalysisSha256 === manifest.structuralAnalysisSha256;
  const interpreterNameMatches =
    metadata.name === manifest.interpreterName && metadata.name === interpretation.interpreter.name;
  const interpreterVersionMatches =
    metadata.version === manifest.interpreterVersion &&
    metadata.version === interpretation.interpreter.version;
  const policyVersionMatches =
    metadata.policyVersion === manifest.policyVersion &&
    metadata.policyVersion === interpretation.interpreter.policyVersion;

  let roleProfileHashMatches: boolean | null = null;
  if (target.type === "role") {
    ...
    if (storedHasProfile || dependencies.roleProfile || dependencies.roleProfileExpectedPath) {
      roleProfileHashMatches = Boolean(
  ...
  if (
    interpretation.input.targetPath !== paths.targetRelativePath ||
  ...
  if (
    interpretation.input.targetSha256 !== manifest.targetSha256 ||
  ...
  if (
    interpretation.interpreter.name !== manifest.interpreterName ||
  ...
  if (target.type === "role" && interpretation.targetType === "role") {
    if (
      interpretation.input.roleProfilePath !== manifest.roleProfilePath ||
  ...
  if (!targetHashMatches) staleReasons.push("Target input hash changed after interpretation.");
  ...
  if (roleProfileHashMatches === false) {
    staleReasons.push(
      dependencies.roleProfileError ?? "Role-profile path, identity, or hash changed.",
    );
  }
  if (!interpreterNameMatches) staleReasons.push("Interpreter name changed.");
  if (!interpreterVersionMatches) staleReasons.push("Interpreter version changed.");
  if (!policyVersionMatches) staleReasons.push("Interpretation policy version changed.");
  return {
    ...base,
    targetHashMatches,
    structuralAnalysisHashMatches,
...
export async function getTargetInterpretationStatus(
  workspace: string,
  targetId: string,
  options: Pick<
    InterpretOptions,
    "roleProfile" | "interpreterName" | "interpreterVersion" | "policyVersion"
  > = {},
): Promise<TargetInterpretationStatus> {
...
export async function interpretTarget(
  workspace: string,
  targetId: string,
  options: InterpretOptions = {},
): Promise<InterpretTargetResult> {
  const dependencies = await currentDependencies(workspace, targetId, options.roleProfile);
  const metadata = interpretationMetadata(options);
  if (dependencies.structuralAnalysisStatus.status !== "current" || !dependencies.structuralAnalysis) {
    throw new Error(
      `Structural target analysis must be current before interpretation. Current status: ${dependencies.structuralAnalysisStatus.status}`,
    );
  }
  ...
  await validateJobSourceReferences(
    workspace,
    dependencies.target,
    dependencies.structuralAnalysis,
  );

  const status = await storedInterpretationStatus(dependencies, metadata);
  if (status.status === "current" && !options.rebuild) {
    const current = TargetInterpretationSchema.parse(
      await readJson<unknown>(dependencies.paths.interpretationPath, null),
    );
    return resultFromInterpretation(
      current,
      dependencies.paths,
      "already-current",
      dependencies.roleProfile,
  ...
  if (["stale", "invalid"].includes(status.status) && !options.rebuild) {
    throw new Error(
      `Stored interpretation is ${status.status} and was not overwritten. Re-run with --rebuild after review. ${status.reasons.join(" ")}`,
    );
  }
  ...
  if (status.interpretationExists) {
    try {
      const stored = TargetInterpretationSchema.parse(
        await readJson<unknown>(dependencies.paths.interpretationPath, null),
      );
...
function resultFromInterpretation(
  interpretation: TargetInterpretation,
  paths: InterpretationPaths,
  result: InterpretTargetResult["result"],
  roleProfile?: LoadedRoleProfile,
): InterpretTargetResult {
  return {
...
export async function showTargetInterpretation(
  workspace: string,
  targetId: string,
): Promise<TargetInterpretation> {
  const target = await showTarget(workspace, targetId);
  const paths = interpretationPaths(workspace, target);
...
export function formatInterpretTargetResult(result: InterpretTargetResult): string {
  return [
...
export function formatTargetInterpretationStatus(status: TargetInterpretationStatus): string {
...
export async function interpretationFileTimestamps(
  workspace: string,
  targetId: string,
): Promise<{ interpretationMtimeMs: number; manifestMtimeMs: number }> {
  const target = await showTarget(workspace, targetId);
  const paths = interpretationPaths(workspace, target);
