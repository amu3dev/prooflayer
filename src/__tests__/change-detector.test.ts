...
describe("Slice 1.3 change detection", () => {
  ...
  it("updates the baseline only after a successful refresh", async () => {
    ...
    const persisted = JSON.parse(await readFile(baselinePath, "utf8")) as RefreshBaseline;
  ...
  it("reports project-note enrichment without role or duplicate-project additions", () => {
    ...
    const stableProfile: CareerProfile = {
...
type FixtureOptions = {
  ...
  evidenceText?: string;
  claimText?: string;
  evidenceVisibility?: EvidenceItem["visibility"];
  claimApproval?: Claim["approvalStatus"];
  claimReadiness?: Claim["outputReadiness"];
  evidenceItems?: EvidenceItem[];
  claims?: Claim[];
};

function fixtureState(options: FixtureOptions = {}): KnowledgeState {
...
function signalBoardSource(): Source {
...
function profileFixture(evidenceItems: EvidenceItem[], claims: Claim[]): CareerProfile {
...
async function writeState(workspace: string, state: KnowledgeState): Promise<void> {
...
function latestFixture(): LatestRefresh {
