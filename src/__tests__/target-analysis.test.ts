...
describe("Slice 2.2 target analysis foundation", () => {
  it("creates a minimal role analysis without inventing role requirements", async () => {
    ...
    const role = await createRoleTarget(
      workspace,
      { title: "Engineering Manager" },
  ...
  it("uses 1-based inclusive lines and exact byte-range excerpt hashes", async () => {
    ...
    expect(exactExcerpt.toString("utf8")).toBe(
      "- Coordinate product and engineering delivery as one intact statement.\n",
    );
  });
  ...
  it("does not rewrite unchanged analysis and preserves timestamps and stable IDs", async () => {
    ...
    expect(rebuilt.sections.map((section) => section.id)).toEqual(
      firstParsed.sections.map((section) => section.id),
    );
  ...
  it("marks changed source stale and rejects missing or mismatched persisted sources", async () => {
    ...
    await expect(analyzeTarget(fixture.workspace, fixture.targetId)).rejects.toThrow(
      "Persisted job description hash does not match",
    );

    await rm(sourcePath);
    await expect(analyzeTarget(fixture.workspace, fixture.targetId)).rejects.toThrow(
      "Persisted job description is missing",
    );
  });
  ...
  it("detects invalid manifests and analysis hash mismatches without silent overwrite", async () => {
    ...
    await expect(analyzeTarget(fixture.workspace, fixture.targetId)).rejects.toThrow(
      "Stored analysis is invalid and was not overwritten",
    );
  ...
  it("detects target hash changes while keeping target input separate from analysis", async () => {
    ...
    const targetBefore = JSON.parse(await readFile(targetPath, "utf8")) as Record<string, unknown>;
    ...
    expect(
      await readFile(path.join(fixture.workspace, "targets/jobs", fixture.targetId, "job-description.md")),
  ...
  it("persists a manifest that verifies the exact analysis, target, and source hashes", async () => {
    ...
    expect(result.analysisPath).toBe(
      `targets/jobs/${fixture.targetId}/analysis/target-analysis.json`,
    );
    expect(result.manifestPath).toBe(
      `targets/jobs/${fixture.targetId}/analysis/analysis-manifest.json`,
    );
...
async function structuredJobWorkspace(): Promise<{ workspace: string; targetId: string }> {
  const workspace = await temporaryWorkspace();
...
function targetArtifact(workspace: string, targetId: string, fileName: string): string {
