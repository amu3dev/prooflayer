...
describe("Slice 2.7F Job-specific Resume Draft Construction", () => {
  it("builds a stable prose-free scaffold from a current usable Job plan", async () => {
    ...
    expect(scaffold.sections).toHaveLength((await loadJobResumeDraftingContext(
      fixture.workspace,
      fixture.targetId,
    )).contentPlan.sections.length);
  ...
  it("generates only through an explicit provider call, caches, refreshes, and replays exactly", async () => {
    ...
    expect(
      first.result,
      JSON.stringify((await showJobResumeDraftProposal(fixture.workspace, first.proposalId)).validationIssues, null, 2),
    ).toBe("created");
  ...
  it("retains statement-level provenance and complete claim/evidence ledgers", async () => {
    ...
    expect(proposal.claimLedger.every((entry) =>
      entry.statementSha256 === hashText(entry.statementText) &&
    ...
    expect(items.every((entry) =>
      entry.provenance.targetId === fixture.targetId &&
      entry.provenance.planId.length > 0 &&
      entry.provenance.planSectionId.length > 0 &&
      entry.provenance.artifactHashes.contentPlanSha256.length === 64
    )).toBe(true);
  ...
  it("rejects hallucinated scope, target-title history, project employment, and unverified metrics", async () => {
    ...
    const scaffoldSha = await hashFile(path.join(
      fixture.workspace,
      "targets",
      "jobs",
      fixture.targetId,
    ...
    const mutations: Array<[string, (payload: ModelJobResumeDraftPayload) => void]> = [
    ...
    for (const [name, mutate] of mutations) {
      ...
      expect(
        result.validationIssues.some((entry) => entry.severity === "critical" || entry.severity === "high"),
        name,
      ).toBe(true);
    }
  });
  ...
  it("permits only an exact plan-authorized verified metric", async () => {
    ...
    const result = validateJobResumeDraftPayload(
      ...
      scaffold,
      context,
      await hashFile(path.join(fixture.workspace, "targets/jobs", fixture.targetId, "resume-drafting/scaffold/job-resume-draft-scaffold.json")),
    );
  ...
  it("requires review, validates human edits with the same rules, and never auto-approves model prose", async () => {
    ...
    await expect(setJobResumeDraftReviewDecision(
      fixture.workspace,
      fixture.proposalId,
      "draft-item",
      first.id,
      { decision: "edit", editedValue: "Managed 80 engineers." },
  ...
  it("supports accept, edit, and reject decisions while keeping proposal immutable", async () => {
    ...
    const proposal = await showJobResumeDraftProposal(fixture.workspace, fixture.proposalId);
    const scaffold = await showJobResumeDraftScaffold(fixture.workspace, fixture.targetId);
    const optionalSection = proposal.sections.find((section) =>
      scaffold.sections.find((entry) => entry.id === section.id)?.inclusion === "optional"
    );
    ...
    for (const decision of review.decisions) {
      ...
      await setJobResumeDraftReviewDecision(
        fixture.workspace,
        fixture.proposalId,
        decision.itemType,
        decision.itemId,
    ...
    if (optionalSection) expect(completed.counts.reject).toBeGreaterThan(0);
  ...
  it("detects duplicate statements, unsupported Job technology, and evidence overuse", async () => {
    ...
    const scaffoldSha = await hashFile(path.join(
      ...
      "targets/jobs",
      fixture.targetId,
      "resume-drafting/scaffold/job-resume-draft-scaffold.json",
    ));
    ...
    for (const section of repeatedSections) {
      if (section.items.length > 0) section.items = Array.from({ length: 20 }, () => sourceItem);
    }
...
async function draftingWorkspace(options: { noEvidence?: boolean } = {}) {
  const workspace = await temporaryWorkspace();
...
async function validDraftPayload(
  workspace: string,
  targetId: string,
): Promise<ModelJobResumeDraftPayload> {
  ...
  return {
    sections: scaffold.sections.map((guard) => {
      ...
      function createDraftItem(): JobResumeDraftItem {
        ...
        const link = (allowedMetricPermission
          ? eligibleLinks.find((entry) =>
            entry?.evidenceId === allowedMetricPermission.evidenceId &&
          ...
          ?? eligibleLinks.find((entry) => {
            ...
            return guard.allowedClaimBoundaryIds.map((id) => boundaryById.get(id)).some((boundary) =>
              boundary &&
              ...
              preferred.some((type) =>
                guard.allowedClaimTypes.includes(type) &&
                boundary.allowedClaimTypes.includes(type)
              )
            );
          })
          ?? eligibleLinks[0];
        if (!link) throw new Error(`No consistent planned link for ${guard.sectionType}`);
        ...
        const metricBoundary = guard.allowedMetricPermissionIds.length > 0
          ? guard.allowedClaimBoundaryIds.map((id) => boundaryById.get(id)).find((entry) =>
            entry?.kind === "metric" && entry.claimIds.includes(link.claimId)
          )
          : undefined;
        const claimBoundary = guard.allowedClaimBoundaryIds.map((id) => boundaryById.get(id)).find((entry) =>
          entry &&
          ...
          entry.requirementId === link.requirementId &&
            ...
            (allowedMetricPermission
              ? true
              : preferredClaimTypes(guard.sectionType, claim.type).some((type) =>
                guard.allowedClaimTypes.includes(type) &&
                entry.allowedClaimTypes.includes(type)
              ))
        );
        const boundary = titleBoundary ?? metricBoundary ?? claimBoundary;
        if (!boundary) throw new Error(`No allowed boundary for ${guard.sectionType}`);
        ...
        const claimType = guard.sectionType === "headline"
          ...
          : metricPermission
            ? "quantified-outcome" as const
          : preferredClaimTypes(guard.sectionType, claim.type).find((entry) =>
            boundary.allowedClaimTypes.includes(entry) &&
        ...
        const assessment = context.assessment.requirementAssessments.find((entry) =>
          entry.requirementId === link.requirementId
        )!;
        const coverage = context.coverage.requirements.find((entry) =>
          entry.requirementId === link.requirementId
        )!;
        return {
...
function itemTypeForSection(type: string): JobResumeDraftItem["itemType"] {
  return ({
    headline: "headline",
  ...
  } as Record<string, JobResumeDraftItem["itemType"]>)[type];
}
function draftText(sectionType: string, targetTitle: string, approvedWording: string) {
  ...
  } as Record<string, string>)[sectionType] ?? approvedWording;
}
function preferredClaimTypes(
  sectionType: string,
  claimType: Claim["type"] | undefined,
): JobResumeDraftItem["claimTypes"] {
  ...
  if (sectionType === "selected-projects") return ["project", "technology", "capability-theme"];
  if (sectionType === "technical-capabilities") return ["technology", "capability-theme"];
  if (sectionType === "leadership-capabilities") return ["leadership-behavior", "responsibility", "capability-theme"];
  if (sectionType === "selected-impact") return ["achievement", "delivery-outcome", "product-outcome", "business-outcome"];
  return ["capability-theme", "responsibility", "technology", "project", "achievement"];
}

async function acceptAllDecisions(
  workspace: string,
  proposalId: string,
  review: JobResumeDraftReview,
  options: { rejectSectionId?: string } = {},
) {
  for (const decision of review.decisions) {
...
function firstItem(payload: ModelJobResumeDraftPayload) {
  return payload.sections.flatMap((entry) => entry.items)[0];
}
function itemByType(payload: ModelJobResumeDraftPayload, itemType: JobResumeDraftItem["itemType"]) {
  const item = payload.sections.flatMap((entry) => entry.items).find((entry) => entry.itemType === itemType);
  if (!item) throw new Error(`No draft item of type ${itemType}`);
  return item;
}
...
async function upstreamArtifactHashes(workspace: string, targetId: string) {
...
async function writeEmptyKnowledgeBase(workspace: string) {
...
async function writeCandidateKnowledgeBase(workspace: string) {
