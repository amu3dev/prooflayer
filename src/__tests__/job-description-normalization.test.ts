...
import {
  NATTERBOX_JOB_DESCRIPTION,
  NATTERBOX_JOB_DESCRIPTION_SHA256,
} from "./fixtures/natterbox-job-description.js";
...
describe("plain-text Job Description structure normalization", () => {
  ...
  it("joins wrapped bullets and wrapped paragraphs into exact semantic source blocks", async () => {
    ...
    expect(shipped).toMatchObject({
      kind: "list-item",
      statement:
        "Shipped conversational-AI, LLM-based or agent products to market, as the person building them rather than a user of them.",
      necessity: "required",
    });
    ...
    expect(company).toMatchObject({
      kind: "paragraph",
      statement:
        "Natterbox builds AI communications that elevate human-led customer conversations, integrated with Salesforce and the wider CRM stack.",
    });
  ...
  it("models coherent mandatory, preferred, and contextual Natterbox requirements", async () => {
    ...
    const counts = Object.fromEntries(
      ["mandatory", "preferred", "contextual", "ambiguous"].map((necessity) => [
        necessity,
  ...
  it("keeps CRM mandatory and Salesforce strongly preferred with a stable relationship", async () => {
    ...
    expect(crm.provenance.sourceAnalysisItemId).toBe(
      salesforce.provenance.sourceAnalysisItemId,
    );
  ...
  it("keeps nice-to-have domain/platform items preferred and excludes benefits capabilities", async () => {
    ...
    expect(model.requirements.some((requirement) => /salary|bonus/i.test(requirement.normalizedLabel)))
      .toBe(false);
    expect(model.requirements.some((requirement) =>
      requirement.normalizedLabel.includes("shape a product line"))).toBe(false);
  ...
  it("retains exact full-source, line, byte-offset, and excerpt provenance", async () => {
    ...
    for (const requirement of model.requirements) {
      ...
      for (const reference of requirement.provenance.sourceReferences) {
        ...
        expect(excerpt.toString("utf8")).toContain(
          requirement.sourceText.split("\n")[0] ?? requirement.sourceText,
        );
      }
    }
...
function sectionFor(
  ...
  if (!section) throw new Error(`Fixture section not found: ${heading}`);
  return section;
}
...
function itemFor(
  ...
  if (!item) throw new Error(`Fixture analysis item not found: ${text}`);
  return item;
}
...
function requirementFor(
  ...
  const requirement = model.requirements.find((candidate) =>
    candidate.normalizedLabel.includes(text));
  if (!requirement) throw new Error(`Fixture requirement not found: ${text}`);
  return requirement;
}

function requirementArtifacts(workspace: string, targetId: string) {
