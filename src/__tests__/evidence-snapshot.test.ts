...
describe("Evidence Snapshot Contract v1", () => {
  it("exports deterministic immutable content with exact eligibility state", async () => {
    ...
    const loaded = await loadEvidenceSnapshot(
      fixture.workspace,
      first.snapshotId,
    );
  ...
  it("preserves exact verified metrics and never promotes unverified numbers", async () => {
    ...
    expect(snapshot.claims.find(
      ({ id }) => id === "claim_unverified_number",
    )).toMatchObject({ metricStatus: "needs_metric" });
  });
  ...
  it("uses identical identity for identical source bytes regardless of export timestamp", async () => {
    ...
    expect(await readFile(
      evidenceSnapshotPaths(left.workspace, first.snapshotId).snapshotPath,
    )).toEqual(await readFile(
      evidenceSnapshotPaths(right.workspace, second.snapshotId).snapshotPath,
    ));
  });
  ...
  it("exports no absolute private source paths or secrets", async () => {
    ...
    const bytes = await readFile(
      evidenceSnapshotPaths(fixture.workspace, result.snapshotId).snapshotPath,
      "utf8",
    );
  ...
  it("represents a valid zero-eligible snapshot honestly", async () => {
    ...
    expect(new Set(snapshot.warnings.map(({ code }) => code))).toEqual(
      new Set([
  ...
  it("validates manifest hashes, source inventory, and list metadata", async () => {
    const fixture = await evidenceWorkspace();
    const result = await buildEvidenceSnapshot(fixture.workspace);
    const status = await getEvidenceSnapshotStatus(
      fixture.workspace,
      result.snapshotId,
    );
  ...
  it("detects corrupted content, manifest, duplicate IDs, and future schemas", async () => {
    ...
    const corruptedPaths = evidenceSnapshotPaths(
      corrupted.workspace,
      corruptedResult.snapshotId,
    );
    await writeFile(corruptedPaths.snapshotPath, "{}", "utf8");
    expect((await getEvidenceSnapshotStatus(
      corrupted.workspace,
      corruptedResult.snapshotId,
    )).status).toBe("incompatible");

    const manifestFixture = await evidenceWorkspace();
    const manifestResult = await buildEvidenceSnapshot(
      manifestFixture.workspace,
    );
    const manifestPaths = evidenceSnapshotPaths(
      manifestFixture.workspace,
      manifestResult.snapshotId,
    );
    const manifest = JSON.parse(
      await readFile(manifestPaths.manifestPath, "utf8"),
    );
    ...
    const duplicateResult = await buildEvidenceSnapshot(
      duplicateFixture.workspace,
    );
    const duplicatePaths = evidenceSnapshotPaths(
      duplicateFixture.workspace,
      duplicateResult.snapshotId,
    );
    const duplicate = JSON.parse(
      await readFile(duplicatePaths.snapshotPath, "utf8"),
    );
    ...
    const futurePaths = evidenceSnapshotPaths(
      futureFixture.workspace,
      futureResult.snapshotId,
    );
  ...
  it("returns deeply frozen read-only consumer objects", async () => {
    ...
    expect(() => loaded.snapshot.evidenceItems.push(
      loaded.snapshot.evidenceItems[0],
    )).toThrow();
  });
  ...
  it("pins Job and Role targets explicitly with stable unchanged behavior", async () => {
    ...
    const jobPin = await pinTargetEvidenceSnapshot(
      fixture.workspace,
      fixture.jobTargetId,
      snapshot.snapshotId,
      { now: () => new Date(FIRST_TIME) },
    );
    const rolePin = await pinTargetEvidenceSnapshot(
      fixture.workspace,
      fixture.roleTargetId,
      snapshot.snapshotId,
      { now: () => new Date(FIRST_TIME) },
    );
    const jobPaths = (await loadTargetEvidencePin(
      fixture.workspace,
      fixture.jobTargetId,
    )).paths;
    const before = {
    ...
    const repeated = await pinTargetEvidenceSnapshot(
      fixture.workspace,
      fixture.jobTargetId,
      snapshot.snapshotId,
      { now: () => new Date(SECOND_TIME) },
    ...
    expect(await getTargetEvidencePinStatus(
      fixture.workspace,
      fixture.jobTargetId,
    )).toMatchObject({
      status: "current",
  ...
  it("rejects missing snapshots and refuses silent pin replacement", async () => {
    const fixture = await targetWorkspace();
    await expect(pinTargetEvidenceSnapshot(
      fixture.workspace,
      fixture.jobTargetId,
      `evidence-snapshot-${"0".repeat(20)}`,
    ...
    await expect(pinTargetEvidenceSnapshot(
      ...
      fixture.jobTargetId,
      second.snapshotId,
    )).rejects.toThrow("evidence-upgrade");
  });

  it("rejects invalid or incompatible snapshots and cross-target pin reuse", async () => {
    const invalidFixture = await targetWorkspace();
    const invalidSnapshot = await buildEvidenceSnapshot(
      invalidFixture.workspace,
    );
    const invalidPaths = evidenceSnapshotPaths(
      invalidFixture.workspace,
      invalidSnapshot.snapshotId,
    );
    const invalidManifest = JSON.parse(
      await readFile(invalidPaths.manifestPath, "utf8"),
    );
    invalidManifest.contentSha256 = "0".repeat(64);
    await writeJsonAtomic(invalidPaths.manifestPath, invalidManifest);
    await expect(pinTargetEvidenceSnapshot(
      invalidFixture.workspace,
      invalidFixture.jobTargetId,
      invalidSnapshot.snapshotId,
    )).rejects.toThrow(/invalid/i);

    const incompatibleFixture = await targetWorkspace();
    const incompatibleSnapshot = await buildEvidenceSnapshot(
      incompatibleFixture.workspace,
    );
    const incompatiblePaths = evidenceSnapshotPaths(
      incompatibleFixture.workspace,
      incompatibleSnapshot.snapshotId,
    );
    const incompatible = JSON.parse(
      await readFile(incompatiblePaths.snapshotPath, "utf8"),
    );
    incompatible.schemaVersion = 2;
    await writeJsonAtomic(incompatiblePaths.snapshotPath, incompatible);
    await expect(pinTargetEvidenceSnapshot(
      incompatibleFixture.workspace,
      incompatibleFixture.jobTargetId,
      incompatibleSnapshot.snapshotId,
    ...
    await pinTargetEvidenceSnapshot(
      crossFixture.workspace,
      crossFixture.jobTargetId,
      crossSnapshot.snapshotId,
    );
    const jobPaths = targetEvidencePinPaths(
      crossFixture.workspace,
      await showTarget(crossFixture.workspace, crossFixture.jobTargetId),
    );
    const rolePaths = targetEvidencePinPaths(
      crossFixture.workspace,
      await showTarget(crossFixture.workspace, crossFixture.roleTargetId),
    );
    await mkdir(rolePaths.rootPath, { recursive: true });
    await writeFile(rolePaths.pinPath, await readFile(jobPaths.pinPath));
    await writeFile(
      rolePaths.manifestPath,
      await readFile(jobPaths.manifestPath),
    );
    expect((await getTargetEvidencePinStatus(
      crossFixture.workspace,
      crossFixture.roleTargetId,
    )).status).toBe("invalid");
  });
  ...
  it("does not auto-upgrade and explicit upgrade stales but does not rebuild a Job map", async () => {
    ...
    const beforeMap = await showJobEvidenceMap(
      fixture.workspace,
      fixture.targetId,
    );
    await addApprovedEvidence(fixture.workspace);
    ...
    expect((await getTargetEvidencePinStatus(
      fixture.workspace,
      fixture.targetId,
    )).snapshotId).toBe(firstSnapshot.snapshotId);
    expect((await getJobEvidenceMapStatus(
      fixture.workspace,
      fixture.targetId,
    )).status).toBe("current");
    ...
    expect(await showJobEvidenceMap(
      fixture.workspace,
      fixture.targetId,
    )).toEqual(beforeMap);
  });
...
async function evidenceWorkspace(options: { reverse?: boolean } = {}) {
  const workspace = await temporaryWorkspace();
...
async function jobMappingWorkspace(options: { empty?: boolean } = {}) {
  const workspace = await temporaryWorkspace();
...
function knowledgeBase(): {
  ...
  const evidence: EvidenceItem[] = [
    evidenceFixture({
      ...
      normalizedSummary:
        "Delivered 3 reviewed platform releases using TypeScript.",
...
async function addApprovedEvidence(workspace: string): Promise<void> {
