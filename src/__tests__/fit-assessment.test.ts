...
import {
  ASSESSMENT_PROPOSAL_PROMPT_TEMPLATE_ID,
  fitAssessmentProposalFileTimestamps,
  generateFitAssessmentProposal,
  getFitAssessmentProposalStatus,
  renderFitAssessmentPrompt,
...
import type {
  EditedExpectationFitAssessment,
  ModelFitAssessmentPayload,
...
describe("Slice 2.5 fit and proof assessment", () => {
  ...
  it.each([
    ["unknown expectation", (payload: ModelFitAssessmentPayload) => { payload.proposedExpectationAssessments[0].expectationId = "expectation_unknown"; }, "UNKNOWN_EXPECTATION_ID"],
    ["unknown match", (payload: ModelFitAssessmentPayload) => { payload.proposedExpectationAssessments[0].approvedMatchIds = ["match_unknown"]; }, "UNKNOWN_MATCH_ID"],
    ["unknown evidence", (payload: ModelFitAssessmentPayload) => { payload.proposedExpectationAssessments[0].evidenceIds = ["evi_unknown"]; }, "UNKNOWN_EVIDENCE_ID"],
    ["invented metric", (payload: ModelFitAssessmentPayload) => { payload.proposedExpectationAssessments[0].rationale = "Improved adoption by 9000 percent."; }, "INVENTED_METRIC"],
    ["resume output", (payload: ModelFitAssessmentPayload) => { payload.proposedExpectationAssessments[0].rationale = "Use this as a resume bullet."; }, "FORBIDDEN_CONTENT"],
    ["apply recommendation", (payload: ModelFitAssessmentPayload) => { payload.proposedSummary.narrative = "Recommend apply for this role."; }, "FORBIDDEN_CONTENT"],
    ["fit percentage", (payload: ModelFitAssessmentPayload) => { payload.proposedSummary.narrative = "This is an 82% fit."; }, "FIT_PERCENTAGE"],
    ["definite experience gap", (payload: ModelFitAssessmentPayload) => { payload.proposedExpectationAssessments[3].rationale = "The candidate clearly lacks experience."; }, "FORBIDDEN_CONTENT"],
  ] as const)("rejects forbidden model output: %s", async (_name, mutate, issueCode) => {
    const fixture = await roleAssessmentFixture();
    await buildFitAssessment(fixture.workspace, fixture.targetId);
    const payload = await validAssessmentPayload(fixture.workspace, fixture.targetId);
    mutate(payload);
  ...
  it("approves reviewed assessment deterministically with explicit trust states and no provider call", async () => {
    ...
    expect(approved.expectationAssessments.every((entry) => entry.trustState !== ("proposed" as never))).toBe(true);
  ...
  it("preserves manifest hashes and complete provenance through deterministic, proposal, review, and approval artifacts", async () => {
    ...
    for (const relative of manifests) expect(JSON.parse(await readFile(path.join(fixture.workspace, relative), "utf8"))).toBeTruthy();
    const approved = await showFitAssessment(fixture.workspace, fixture.targetId, "approved");
...
async function roleAssessmentFixture(options: { includeNotAssessed?: boolean } = {}) {
  ...
  ] as Array<[string, string, "required" | "preferred" | "contextual" | "unknown", "critical" | "high" | "medium" | "low" | "unknown"]>;
  ...
  return { workspace, targetId: created.target.id, targetType: "role" as const };
}
...
async function jobAssessmentFixture() {
  ...
  return { workspace, targetId: created.target.id, targetType: "job" as const };
}
...
async function writeEvidenceKb(workspace: string, count: number): Promise<void> {
...
async function writeMatchingPattern(workspace: string, targetId: string, includeNotAssessed: boolean): Promise<void> {
  ...
  context.eligibleExpectations.forEach((expectation, index) => {
    ...
    if (index === 0) matches.push(matchFixture(context, expectation.id, evidence.evidenceId, "strong", "direct", "full", "strong", "current", "high"));
    else if (index === 1) matches.push(matchFixture(context, expectation.id, evidence.evidenceId, "adequate", "direct", "full", "medium", "recent", "medium"));
    else if (index === 2) matches.push(matchFixture(context, expectation.id, evidence.evidenceId, "partial", "partial", "partial", "medium", "current", "medium"));
    else if (index === 3) explicit.set(expectation.id, "unsupported");
    else if (index === 4) matches.push(matchFixture(context, expectation.id, evidence.evidenceId, "conflict", "contradictory", "conflicting", "strong", "current", "high"));
    else if (includeNotAssessed && index === context.eligibleExpectations.length - 1) explicit.set(expectation.id, "not-assessed");
...
async function validAssessmentPayload(workspace: string, targetId: string): Promise<ModelFitAssessmentPayload> {
...
async function completedAssessmentReviewFixture() {
  ...
  await setFitAssessmentReviewDecision(fixture.workspace, proposal.id, proposal.proposedExpectationAssessments[0].id, { decision: "accept" });
  await setFitAssessmentReviewDecision(fixture.workspace, proposal.id, proposal.proposedExpectationAssessments[1].id, { decision: "edit", editedAssessment: editedAssessment(proposal.proposedExpectationAssessments[1]) });
  for (const entry of proposal.proposedExpectationAssessments.slice(2)) await setFitAssessmentReviewDecision(fixture.workspace, proposal.id, entry.id, { decision: "reject" });
  await setFitAssessmentSummaryReviewDecision(fixture.workspace, proposal.id, { decision: "reject" });
  await completeFitAssessmentReview(fixture.workspace, proposal.id, { now: () => new Date(SECOND_TIME) });
  return fixture;
}

function editedAssessment(source: Awaited<ReturnType<typeof showFitAssessmentProposal>>["proposedExpectationAssessments"][number]): EditedExpectationFitAssessment {
  return {
...
async function directoryFileNames(root: string): Promise<string[]> {
