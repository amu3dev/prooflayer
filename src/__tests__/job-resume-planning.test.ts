...
import {
  JOB_RESUME_PLANNING_POLICY_NAME,
  JOB_RESUME_PLANNING_POLICY_VERSION,
  buildJobResumePlan,
  deriveJobPositioningState,
  deriveJobRequirementEmphasisDecision,
...
describe("Slice 2.7E deterministic Job-specific Resume Content Planning", () => {
  ...
  it("builds one evidence-grounded planning artifact and manifest", async () => {
    ...
    expect(result).toMatchObject({
      result: "created",
      planPath:
        `targets/jobs/${fixture.targetId}/resume-planning/deterministic/job-resume-content-plan.json`,
    });
    ...
    expect(plan.requirementEmphasis).toHaveLength(
      plan.completeness.requirementIds.length,
    );
    ...
    expect(manifest.manifestId).toMatch(
      /^job-resume-content-plan-manifest_[a-f0-9]{16}$/,
    );
    expect(manifest.planSha256).toBe(await hashFile(paths.planPath));
    expect(manifest.assessmentSha256).toBe(
      await hashFile(jobFitProofAssessmentPaths(
        fixture.workspace,
        fixture.targetId,
      ).assessmentPath),
    );
  ...
  it("plans requirements, evidence reuse, sections, and exact provenance without rematching", async () => {
    ...
    expect(plan.requirementEmphasis.some((entry) => entry.decision === "primary"))
      .toBe(true);
    expect(plan.requirementEmphasis.some((entry) =>
      entry.necessity === "mandatory" && entry.decision === "defer"
    )).toBe(true);
    ...
    expect(plan.evidenceSelections.every((entry) =>
      entry.requirementUses.filter((use) => use.decision === "primary").length <= 1
    )).toBe(true);
    ...
    for (const entry of plan.requirementEmphasis) {
      ...
      for (const reference of entry.provenance.evidenceLinkReferences) {
        expect(linkById.get(reference.linkId)?.requirementId).toBe(
          entry.requirementId,
        );
        expect(reference.linkSha256).toBe(
          hashText(stableJson(linkById.get(reference.linkId))),
        );
      }
    }
  ...
  it("enforces target-title and project-versus-employment boundaries", async () => {
    ...
    await buildJobResumePlan(fixture.workspace, fixture.targetId);
    const plan = await showJobResumePlan(fixture.workspace, fixture.targetId);
    const titleBoundary = plan.claimBoundaries.find(
      (entry) => entry.kind === "target-title",
    );
    const projectBoundary = plan.claimBoundaries.find(
      (entry) => entry.kind === "project-employment",
    );
    const experience = plan.sections.find(
      (entry) => entry.type === "professional-experience",
    )!;
    const projects = plan.sections.find(
      (entry) => entry.type === "selected-projects",
    )!;
    ...
    expect(plan.exclusions.some((entry) =>
      entry.type === "target-title-history" && entry.severity === "blocking"
    )).toBe(true);
    expect(plan.exclusions.some((entry) =>
      entry.type === "project-as-employment" &&
      entry.sourceIds.includes("evi_mobile_project")
    )).toBe(true);
  });
  ...
  it("permits only exact approved verified metrics and prohibits other quantification", async () => {
    ...
    await buildJobResumePlan(fixture.workspace, fixture.targetId);
    const plan = await showJobResumePlan(fixture.workspace, fixture.targetId);
    const allowed = plan.metricPermissions.find(
      (entry) => entry.claimId === "claim_verified_releases",
    );
    const prohibited = plan.metricPermissions.filter(
      (entry) => entry.claimId !== "claim_verified_releases",
    );
    const selectedImpact = plan.sections.find(
      (entry) => entry.type === "selected-impact",
    )!;
    ...
    expect(prohibited.every((entry) =>
      entry.state === "prohibited" && !entry.exactApprovedMetricText
    )).toBe(true);
  ...
  it("creates explicit no-metric behavior when no verified metric is selected", async () => {
    ...
    expect(plan.metricPermissions.every((entry) => entry.state === "prohibited"))
      .toBe(true);
    expect(plan.warnings.some((entry) =>
      entry.code === "NO_VERIFIED_METRIC_AVAILABLE"
    )).toBe(true);
    expect(plan.risks.some((entry) =>
      entry.code === "UNSUPPORTED_METRIC_RISK"
    )).toBe(true);
  });
  ...
  it("represents gaps as constraints while allowing an honest mixed plan", async () => {
    ...
    await buildJobResumePlan(fixture.workspace, fixture.targetId);
    const plan = await showJobResumePlan(fixture.workspace, fixture.targetId);
    const mandatoryGap = plan.requirementEmphasis.find((entry) =>
      entry.necessity === "mandatory" && entry.assessmentState === "gap"
    )!;
    const handling = plan.gapHandling.find(
      (entry) => entry.requirementId === mandatoryGap.requirementId,
    );
  ...
  it("does not write resume prose, application recommendations, or numeric scores", async () => {
    ...
    const serialized = await readFile(
      jobResumePlanPaths(fixture.workspace, fixture.targetId).planPath,
      "utf8",
    );

    expect(serialized).not.toMatch(
      /"headlineText"|"summaryText"|"resumeBullet"|"experienceProse"|"applicationRecommendation"|"atsScore"|"fitScore"|"hiringProbability"/,
    );
  ...
  it("preserves bytes, hashes, stable IDs, timestamps, and mtimes on unchanged rerun", async () => {
    ...
    expect(after.requirementEmphasis.map((entry) => entry.id)).toEqual(
      before.value.requirementEmphasis.map((entry) => entry.id),
    );
    expect(after.sections.map((entry) => entry.id)).toEqual(
      before.value.sections.map((entry) => entry.id),
    );
  ...
  it("reports missing, current, stale, and invalid lifecycle states", async () => {
    const fixture = await planningWorkspace();
    expect((await getJobResumePlanStatus(
      fixture.workspace,
      fixture.targetId,
    )).status).toBe("missing");
    await buildJobResumePlan(fixture.workspace, fixture.targetId);
    ...
    const assessmentPath = jobFitProofAssessmentPaths(
      fixture.workspace,
      fixture.targetId,
    ).assessmentPath;
    await writeFile(
    ...
    await expect(
      buildJobResumePlan(fresh.workspace, fresh.targetId),
    ).rejects.toThrow("use --rebuild");
  ...
  it("rejects Role Targets, stale dependencies, and cross-target artifacts", async () => {
    ...
    await expect(buildJobResumePlan(workspace, role.target.id)).rejects.toThrow(
      "rejects Role Target",
    );
    ...
    await expect(
      buildJobResumePlan(stale.workspace, stale.targetId),
    ).rejects.toThrow("requires a current Job Fit and Proof Assessment");

    const crossTarget = await planningWorkspace();
    const second = await createPlanningJob(
      crossTarget.workspace,
      "other-job.md",
      JOB_DESCRIPTION.replace("Example Systems", "Other Systems"),
    );
    const paths = jobFitProofAssessmentPaths(
      crossTarget.workspace,
      crossTarget.targetId,
    );
    const manifest = JSON.parse(await readFile(paths.manifestPath, "utf8")) as {
      targetId: string;
    };
    ...
    await expect(
      buildJobResumePlan(crossTarget.workspace, crossTarget.targetId),
    ).rejects.toThrow("requires a current Job Fit and Proof Assessment");
  });

  it("does not modify upstream target, Job Description, assessment, or candidate evidence", async () => {
    const fixture = await planningWorkspace();
    const assessmentPaths = jobFitProofAssessmentPaths(
      fixture.workspace,
      fixture.targetId,
    );
...
async function planningWorkspace(
  options: { verifiedMetric?: boolean; noEvidence?: boolean } = {},
) {
  const workspace = await temporaryWorkspace();
...
async function writeEmptyCandidateKnowledgeBase(workspace: string): Promise<void> {
...
async function writeCandidateKnowledgeBase(
  workspace: string,
  verifiedMetric: boolean,
): Promise<void> {
