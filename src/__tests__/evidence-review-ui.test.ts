...
describe("Local Evidence Review UI domain adapter", () => {
  it("builds a self-contained human view from one current locked batch", async () => {
    ...
    expect(batch.claims.every(({ title, text, selectionReason }) =>
      title.length > 0 && text.length > 0 && selectionReason.length > 0)).toBe(true);
  ...
  it("creates, deduplicates, and explicitly supersedes canonical reviews", async () => {
    ...
    const first = await submitEvidenceReviewUiClaim(
      fixture.workspace,
      fixture.batchId,
      claimId,
      fields,
    ...
    const duplicate = await submitEvidenceReviewUiClaim(
      fixture.workspace,
      fixture.batchId,
      claimId,
      fields,
    ...
    await expect(submitEvidenceReviewUiClaim(
      ...
      fixture.batchId,
      claimId,
      { ...fields, reviewerRationale: "A changed rationale creates a new immutable review." },
    )).rejects.toThrow(/explicitly set supersedesReviewId/);

    const superseding = await submitEvidenceReviewUiClaim(
      fixture.workspace,
      fixture.batchId,
      claimId,
      {
  ...
  it("supports qualified and rejected decisions while refusing invalid eligibility and metrics", async () => {
    const fixture = await createEvidenceReviewUiFixture();
    const qualified = await submitEvidenceReviewUiClaim(
      fixture.workspace,
      fixture.batchId,
      "claim_review_ui_1",
      validApprovedFields({
    ...
    const rejected = await submitEvidenceReviewUiClaim(
      fixture.workspace,
      fixture.batchId,
      "claim_review_ui_2",
      rejectedFields("employment", "responsibility"),
    );
    ...
    await expect(submitEvidenceReviewUiClaim(
      ...
      fixture.batchId,
      "claim_review_ui_4",
      rejectedFields("education", "credential", {
  ...
  it("keeps private evidence ineligible and withholds its exact excerpt", async () => {
    ...
    await expect(submitEvidenceReviewUiClaim(
      ...
      claim.id,
      validApprovedFields(),
    )).rejects.toThrow(/Private|public-safe/);
  });
  ...
  it("makes zero writes in read-only mode and rejects unknown or cross-batch identifiers", async () => {
    ...
    await expect(loadEvidenceReviewUiClaim(
      fixture.workspace,
      fixture.batchId,
      "claim_outside_batch",
    )).rejects.toThrow(/does not belong/);
    await expect(loadEvidenceReviewUiBatch(
      fixture.workspace,
      "../evidence-review-batch_escape",
    )).rejects.toThrow(/Invalid evidence review batch ID/);
  });
  ...
  it("updates progress and exposes only the explicit guided-workflow next command", async () => {
    ...
    const final = await submitEvidenceReviewUiClaim(
      fixture.workspace,
      fixture.batchId,
      "claim_review_ui_4",
      rejectedFields("education", "credential", { decision: "deferred" }),
    );
    const batch = await loadEvidenceReviewUiBatch(fixture.workspace, fixture.batchId);
