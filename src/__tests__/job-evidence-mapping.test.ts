...
describe("Slice 2.7B deterministic Job Evidence Mapping", () => {
  it("builds the primary deterministic map with only the required downstream artifact", async () => {
    ...
    expect(await readFile(paths.mapPath, "utf8")).not.toMatch(
      /fitScore|coveragePercentage|hiring|recommendation|resume bullet/i,
    );
    expect(paths.rootRelativePath).toBe(
      `targets/jobs/${fixture.targetId}/matching/deterministic`,
    );
  });
  ...
  it("uses explicit reviewed signals for direct, supporting, and partial links", async () => {
    ...
    expect(map.links.find((link) =>
      link.requirementId === technical.id &&
    ...
    expect(map.links.find((link) =>
      link.requirementId === roadmap.id &&
      link.evidenceId === "evi_platform_delivery")).toMatchObject({
      relationship: "supporting",
    });
    expect(map.links.find((link) =>
      link.requirementId === reactNative.id &&
      link.evidenceId === "evi_mobile")).toMatchObject({
      relationship: "direct",
      evidenceStrength: "medium",
      linkConfidence: "medium",
    });
    expect(new Set(map.links.map((link) => link.relationship))).toEqual(
      new Set(["direct", "supporting", "partial"]),
    );
  });
  ...
  it("marks absent evidence unsupported without creating a link or score", async () => {
    ...
    const mapping = map.requirementMappings.find(
      (entry) => entry.requirementId === language.id,
    );
  ...
  it("excludes private, draft, proposed, rejected, and non-resume-ready claims", async () => {
    ...
    expect(new Set(map.links.map((link) => link.evidenceId))).toEqual(
      new Set(["evi_mobile", "evi_platform_delivery"]),
    );
  ...
  it("preserves stable IDs, hashes, bytes, and timestamps on unchanged reruns", async () => {
    ...
    expect(after.parsed.links.map((entry) => entry.id)).toEqual(
      before.parsed.links.map((entry) => entry.id),
    );
  ...
  it("preserves exact requirement and reviewed evidence provenance", async () => {
    ...
    await buildJobEvidenceMap(fixture.workspace, fixture.targetId);
    const map = await showJobEvidenceMap(fixture.workspace, fixture.targetId);
    const link = map.links.find((entry) =>
      entry.evidenceId === "evi_platform_delivery" &&
      entry.matchedSignals.some((signal) => signal.value === "typescript"))!;
    const model = await showJobRequirementModel(fixture.workspace, fixture.targetId);
    const requirement = model.requirements.find(
      (entry) => entry.id === link.requirementId,
    )!;
  ...
  it("accepts an equivalent current approved reviewed requirement model explicitly", async () => {
    const fixture = await matchingWorkspace();
    const deterministic = await showJobRequirementModel(
      fixture.workspace,
      fixture.targetId,
    );
    const payload: ModelJobRequirementPayload = {
      proposedRequirements: [],
      warnings: [],
    };
    const generated = await generateJobRequirementProposal(
      fixture.workspace,
      fixture.targetId,
      {
    ...
    await initializeJobRequirementReview(
      fixture.workspace,
      generated.proposalId,
      { now: () => new Date(FIRST_TIME) },
    );
    const review = await showJobRequirementReview(
      fixture.workspace,
      generated.proposalId,
    );
    ...
    await completeJobRequirementReview(
      fixture.workspace,
      generated.proposalId,
      { now: () => new Date(FIRST_TIME) },
    );
    ...
    expect(map.links.every((entry) =>
      entry.requirementProvenance.requirementModelType === "approved")).toBe(true);
  });
...
async function writeCandidateKnowledgeBase(workspace: string): Promise<void> {
  ...
  const sources: Source[] = [publicSource, privateSource];
