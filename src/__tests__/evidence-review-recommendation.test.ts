...
import {
  ...
  projectEvidenceReviewRecommendationSubmission,
} from "../evidence-review-recommendation.js";
import {
...
describe("Evidence Review recommendation engine", () => {
  ...
  it("keeps multiple ambiguities in Level 3", async () => {
    const claim = await fixtureClaim();
    const ambiguous: EvidenceReviewUiClaim = {
...
async function fixtureClaim(): Promise<EvidenceReviewUiClaim> {
  const fixture = await createEvidenceReviewUiFixture();
  return (await loadEvidenceReviewUiClaim(
    fixture.workspace,
    fixture.batchId,
    "claim_review_ui_3",
  )).claim;
...
function withAmbiguousWorkContext(claim: EvidenceReviewUiClaim): EvidenceReviewUiClaim {
  return {
    ...claim,
