...
import {
  JOB_COVERAGE_ANALYZER_NAME,
  JOB_COVERAGE_ANALYZER_VERSION,
  JOB_COVERAGE_POLICY_NAME,
  JOB_COVERAGE_POLICY_VERSION,
  buildJobCoverage,
  classifyJobRequirementCoverage,
...
describe("Slice 2.7C deterministic Job Requirement Coverage", () => {
  ...
  it("builds one deterministic coverage entry for every mapped requirement", async () => {
    ...
    expect(result).toMatchObject({
      result: "created",
      coveragePath:
        `targets/jobs/${fixture.targetId}/coverage/deterministic/job-requirement-coverage.json`,
    });
    ...
    expect(new Set(coverage.requirements.map((entry) => entry.requirementId))).toEqual(
      new Set(model.requirements.map((entry) => entry.id)),
    );
    expect(await readFile(paths.coveragePath, "utf8")).not.toMatch(
      /fit score|coverage percentage|hiring probability|apply recommendation|resume plan/i,
    );
  });
  ...
  it("handles compound requirements conservatively without splitting artifacts", async () => {
    ...
    const entry = coverage.requirements.find(
      (candidate) => candidate.requirementId === technical.id,
    )!;
  ...
  it("does not mutate or regenerate requirement, map, or candidate evidence inputs", async () => {
    const fixture = await coverageWorkspace();
    const mapPaths = jobEvidenceMapPaths(fixture.workspace, fixture.targetId);
    const requirementModel = await showJobRequirementModel(
      fixture.workspace,
      fixture.targetId,
    );
    const protectedPaths = [
      ...
      path.isAbsolute(entry) ? entry : path.join(fixture.workspace, entry));
  ...
  it("preserves stable IDs, hashes, bytes, and timestamps on unchanged reruns", async () => {
    ...
    expect(after.parsed.requirements.map((entry) => entry.id)).toEqual(
      before.parsed.requirements.map((entry) => entry.id),
    );
  ...
  it("reports missing, current, stale, and invalid lifecycle states", async () => {
    ...
    await writeFile(
      paths.coveragePath,
      `${await readFile(paths.coveragePath, "utf8")} `,
      "utf8",
    );
  ...
  it("rejects Role Targets, missing maps, and broken map provenance", async () => {
    ...
    await expect(buildJobCoverage(workspace, role.target.id)).rejects.toThrow(
      "rejects Role Target",
    );
    ...
    await expect(buildJobCoverage(workspace, job.target.id)).rejects.toThrow(
      "requires a current Job Evidence Map",
    );
    ...
    const map = JSON.parse(
      await readFile(mapPaths.mapPath, "utf8"),
    ) as Record<string, unknown> & {
      links: Array<{
        evidenceProvenance: { claimId: string };
      }>;
    };
    map.links[0].evidenceProvenance.claimId = "claim_broken";
    await writeJsonAtomic(mapPaths.mapPath, map);
    const manifest = JSON.parse(
      await readFile(mapPaths.manifestPath, "utf8"),
    ) as Record<string, unknown>;
  ...
  it("rejects stale requirement models and stale evidence maps", async () => {
    ...
    const jobDescriptionPath = path.join(
      staleRequirement.workspace,
      "targets/jobs",
      staleRequirement.targetId,
      "job-description.md",
    );
    await writeFile(
      jobDescriptionPath,
      `${JOB_DESCRIPTION}\nChanged requirement source.`,
      "utf8",
    );
    await expect(
      buildJobCoverage(staleRequirement.workspace, staleRequirement.targetId),
    ).rejects.toThrow("Job Requirement Model is not current");
...
function link(
  relationship: "direct" | "supporting" | "partial" | "contradiction",
...
async function writeCandidateKnowledgeBase(workspace: string): Promise<void> {
  ...
  const evidence: EvidenceItem[] = [
    evidenceFixture({
      ...
      normalizedSummary:
        "Shaped roadmap decisions and led API platform delivery with TypeScript and Node.js.",
...
function requirementFor(
  ...
  const requirement = model.requirements.find((entry) =>
    entry.sourceText.includes(text));
  if (!requirement) throw new Error(`Requirement fixture not found: ${text}`);
  return requirement;
}

function entryFor(
  coverage: Awaited<ReturnType<typeof showJobCoverage>>,
  requirementId: string,
) {
  const entry = coverage.requirements.find(
    (candidate) => candidate.requirementId === requirementId,
  );
  if (!entry) throw new Error(`Coverage entry not found: ${requirementId}`);
  return entry;
}
