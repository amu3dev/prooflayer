...
describe("Slice 1.6.1 persisted pipeline integration", () => {
  it("runs a real refresh and atomically persists the complete knowledge state", async () => {
    ...
    const profile = JSON.parse(await readFile(path.join(workspace, "kb/career-profile.json"), "utf8")) as { id?: string };
    const baseline = JSON.parse(await readFile(path.join(workspace, UPDATE_BASELINE_PATH), "utf8")) as { successfulRefreshAt?: string };
    const changelog = JSON.parse(await readFile(path.join(workspace, LATEST_REFRESH_PATH), "utf8")) as { refreshId?: string };
...
async function findTemporaryFiles(root: string): Promise<string[]> {
  ...
  const nested = await Promise.all(entries.map(async (entry) => {
    ...
    if (entry.isDirectory()) return findTemporaryFiles(fullPath);
