...
import type { CareerProfile, Claim, EvidenceItem, Source, VariantReviewDecisions } from "../schemas.js";
import { listVariantStatuses, type OutputManifest, type VariantGenerationManifest } from "../variant-generator.js";
import { formatStatusSummary, getWorkspaceStatus } from "../update-impact.js";
import {
  finalizeVariant,
  getVariantReviewStatus,
  initializeVariantReview,
  type FinalVariantManifest
...
describe("Slice 1.5 output-specific variant review", () => {
  it("creates pending decisions without overwriting existing decisions", async () => {
    ...
    const existing: VariantReviewDecisions = {
      schemaVersion: 1,
      roleKey: "tpm",
    ...
    const persisted = JSON.parse(await readFile(reviewPath, "utf8")) as VariantReviewDecisions;
  ...
  it("finalizes only reviewed public wording and tracks final output separately", async () => {
    ...
    const reviewPath = path.join(workspace, "outputs/variants/tpm/review-decisions.json");
    const review = JSON.parse(await readFile(reviewPath, "utf8")) as VariantReviewDecisions;
    review.decisions = review.decisions.map((item) => {
      if (item.claimId === "claim_approve") return { ...item, decision: "approve" as const };
      if (item.claimId === "claim_revise") {
        return { ...item, decision: "revise" as const, approvedPublicWording: "Built a reviewed public project workflow." };
      }
      if (item.claimId === "claim_draft") return { ...item, decision: "draft_only" as const };
      if (item.claimId === "claim_exclude") return { ...item, decision: "exclude" as const };
      if (["claim_unknown", "claim_blocked", "claim_metric"].includes(item.claimId)) return { ...item, decision: "approve" as const };
      return item;
    });
    ...
    const persisted = JSON.parse(await readFile(path.join(root, "final-manifest.json"), "utf8")) as FinalVariantManifest;
  ...
  it("requires explicit public wording for unknown visibility", async () => {
    ...
    review.decisions = review.decisions.map((item) => item.claimId === "claim_unknown"
      ? { ...item, decision: "approve" as const, approvedPublicWording: "Validated a public-safe product workflow." }
      : item);
    await writeFile(reviewPath, JSON.stringify(review), "utf8");
  ...
  it("renders reviewed completeness claims and removes duplicate website bullets", async () => {
    ...
    review.decisions = review.decisions.map((item) => {
      if (item.claimId === "claim_summary") {
        return { ...item, decision: "revise" as const, approvedPublicWording: "Technical Product Manager connecting product strategy with platform delivery." };
      }
      if (item.claimId === "claim_a0d06ff53160") {
        return { ...item, decision: "revise" as const, approvedPublicWording: "Product and delivery tools: Jira, Linear, Notion, Figma, and GitHub Projects." };
      }
      if (item.claimId === "claim_8c7c3c3e8002") {
        return { ...item, decision: "revise" as const, approvedPublicWording: "Technical Product Manager Certification, Knowledge Officer." };
      }
      if (item.claimId === "claim_ffda3bd4dbfc") {
        return { ...item, decision: "revise" as const, approvedPublicWording: "Meta Product Management Scholarship, Product Manager track." };
      }
      if (item.claimId === "claim_871911b85bcf") {
        return { ...item, decision: "revise" as const, approvedPublicWording: "Bachelor’s Degree in Computer Science, Modern Academy Maadi." };
      }
      if (item.claimId === "claim_9379f09c8b8c") {
        return { ...item, decision: "revise" as const, approvedPublicWording: "Diploma in Project Management, Regional IT Institute." };
      }
      if (item.claimId === "claim_21082bbbbc33") return { ...item, decision: "approve" as const };
      if (["claim_approve", "claim_manifest_only"].includes(item.claimId)) return { ...item, decision: "approve" as const };
      if (item.claimId === "claim_draft") return { ...item, decision: "draft_only" as const };
      return item;
    });
    await writeFile(reviewPath, JSON.stringify(review), "utf8");
  ...
  it("applies public metadata and final wording overrides without changing drafts or evidence", async () => {
    ...
    review.decisions = review.decisions.map((item) => {
      ...
      if (item.claimId === "claim_summary") {
        return { ...item, decision: "revise" as const, approvedPublicWording: "Technical product leadership summary." };
      }
      if ([
        "claim_approve",
        "claim_manifest_only",
        ...
        "claim_9379f09c8b8c",
        "claim_ffda3bd4dbfc"
      ].includes(item.claimId)) return { ...item, decision: "approve" as const };
      return { ...item, decision: "exclude" as const };
    });
    await writeFile(reviewPath, JSON.stringify(review), "utf8");
  ...
  it("tracks TPM and AI Product final outputs independently and preserves TPM review", async () => {
    ...
    const tpmReview = JSON.parse(await readFile(tpmReviewPath, "utf8")) as VariantReviewDecisions;
    ...
    const baseline = JSON.parse(await readFile(baselinePath, "utf8")) as { profileFingerprint: string };
  ...
  it("prioritizes AI Product evaluation, traceability, and validation without changing TPM final output", async () => {
    ...
    const tpmBefore = new Map(await Promise.all(tpmFiles.map(async (file) => [file, await readFile(path.join(tpmRoot, file), "utf8")] as const)));
    ...
    const reviewedWording = new Map<string, string>([
    ...
    const aiReview = JSON.parse(await readFile(aiReviewPath, "utf8")) as VariantReviewDecisions;
    aiReview.decisions = aiReview.decisions.map((item) => {
      const wording = reviewedWording.get(item.claimId);
      if (wording) return { ...item, decision: "revise" as const, approvedPublicWording: wording };
      if (item.claimId === "claim_approve") return { ...item, decision: "approve" as const };
      if (item.claimId === "claim_draft") return { ...item, decision: "draft_only" as const };
      return { ...item, decision: "exclude" as const };
    });
    ...
    for (const file of tpmFiles) expect(await readFile(path.join(tpmRoot, file), "utf8")).toBe(tpmBefore.get(file));
    const registry = JSON.parse(await readFile(path.join(workspace, "outputs/output-manifest.json"), "utf8")) as OutputManifest;
  ...
  it("rejects unsupported output-review role keys clearly", async () => {
    const workspace = await createReviewWorkspace();
    await expect(initializeVariantReview(workspace, "fullstack")).rejects.toThrow(
      "Output-specific review supports --role tpm or --role ai-product."
    );
  });
});

async function createReviewWorkspace(): Promise<string> {
  ...
  const state: KnowledgeState = { sources: [source], evidenceItems: evidence, claims, profile, privacyFindings: [] };
...
async function addVariantFixture(
  workspace: string,
  roleKey: "tpm" | "ai-product",
  evidence?: EvidenceItem[]
): Promise<void> {
  const currentEvidence = evidence
    ?? JSON.parse(await readFile(path.join(workspace, "kb/evidence-items.json"), "utf8")) as EvidenceItem[];
  ...
  let outputManifest: OutputManifest = { schemaVersion: 1, updatedAt: generationManifest.generatedAt, outputs: [] };
  try {
    outputManifest = JSON.parse(await readFile(outputManifestPath, "utf8")) as OutputManifest;
  } catch {
...
async function addAiProductQualityEvidence(workspace: string): Promise<void> {
  const evidencePath = path.join(workspace, "kb/evidence-items.json");
  const claimsPath = path.join(workspace, "kb/claims.json");
  ...
  const profile = JSON.parse(await readFile(profilePath, "utf8")) as CareerProfile;
  ...
  evidence.push(
    evidenceFixture({ id: "evi_ai_summary", category: "responsibility", sourceSection: "Summary", visibility: "public" }),
    ...signalEvidenceIds.map((id) => evidenceFixture({ id, category: "responsibility", project: "SignalBoard", parentProjectId: "evi_project", visibility: "public" })),
    evidenceFixture({ id: "evi_ai_insight_project", category: "project", project: "InSightARLeans", visibility: "public" }),
    ...insightEvidenceIds.slice(1).map((id) => evidenceFixture({ id, category: "responsibility", project: "InSightARLeans", parentProjectId: "evi_ai_insight_project", visibility: "public" }))
  );
  claims.push(
    claimFixture({ id: "claim_ai_summary_quality", claim: "AI Product Manager and product-minded technology leader connecting product discovery, technical tradeoffs, and hands-on AI product validation.", supportingEvidenceIds: ["evi_ai_summary"], sourceSection: "Summary" }),
    claimFixture({ id: "claim_ai_workflow_quality", claim: "Built AI-assisted product workflows, prototypes, and technical evaluation loops.", type: "competency_claim", supportingEvidenceIds: ["evi_ai_summary"] }),
    claimFixture({ id: "claim_ai_signal_role_quality", claim: "SB (SignalBoard) - Product and Technical Lead (Sep 2025 - Present).", type: "project_claim", supportingEvidenceIds: ["evi_project"] }),
    claimFixture({ id: "claim_ai_signal_status_quality", claim: "SignalBoard is an AI-assisted market and competitive intelligence product experiment.", type: "project_claim", supportingEvidenceIds: ["evi_project"] }),
    claimFixture({ id: "claim_ai_market_quality", claim: "Led an AI-assisted market-signal intelligence initiative spanning signal extraction and repeatable decision-support workflows.", supportingEvidenceIds: ["evi_ai_market"], parentProjectId: "evi_project" }),
    claimFixture({ id: "claim_ai_provenance_quality", claim: "Built stricter action-provenance checks for AI-generated recommendations to improve traceability and evidence support.", supportingEvidenceIds: ["evi_ai_provenance"], parentProjectId: "evi_project" }),
    claimFixture({ id: "claim_ai_evaluation_quality", claim: "Added evaluation scenarios to test whether generated briefings were supported by real source signals.", supportingEvidenceIds: ["evi_ai_evaluation"], parentProjectId: "evi_project" }),
    claimFixture({ id: "claim_ai_traceability_quality", claim: "Improved product thinking around evidence-backed insights, traceability, and decision-support workflows.", type: "impact_claim", supportingEvidenceIds: ["evi_ai_traceability"], parentProjectId: "evi_project" }),
    claimFixture({ id: "claim_ai_backend_quality", claim: "Designed AI-assisted workflows connecting app experience, backend/data configuration, technical evaluation, and product validation.", supportingEvidenceIds: ["evi_ai_backend"], parentProjectId: "evi_project" }),
    claimFixture({ id: "claim_ai_insight_role_quality", claim: "InSightARLeans - Product and Technical Lead (Mar 2025 - Present).", type: "project_claim", supportingEvidenceIds: ["evi_ai_insight_project"] }),
    claimFixture({ id: "claim_ai_prototype_quality", claim: "Built and iterated a React Native and Expo computer-vision prototype exploring object detection and contextual overlays using TensorFlow.js.", supportingEvidenceIds: ["evi_ai_prototype"], parentProjectId: "evi_ai_insight_project" }),
    ...
    claimFixture({ id: "claim_ai_tradeoff_quality", claim: "Worked through product and technical tradeoffs across camera flows, overlay UX, responsiveness, and mobile interaction design.", supportingEvidenceIds: ["evi_ai_tradeoff"], parentProjectId: "evi_ai_insight_project" })
  );
  const signalProject = profile.projects.find((project) => project.name.includes("SignalBoard"));
  if (signalProject) signalProject.evidenceIds.push(...signalEvidenceIds);
