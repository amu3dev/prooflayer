...
async function createRoleReadyFixture() {
  ...
  const roleByEvidenceId: Record<string, string> = {
  ...
  const projectByEvidenceId: Record<string, { id: string; name: string }> = {
    ai: { id: "project_guided_ai", name: "AI Product Evaluation" },
    "hands-on": { id: "project_guided_platform", name: "Platform Transformation" },
  };
  const evidence: EvidenceItem[] = definitions.map(([id, text, tags]) => {
