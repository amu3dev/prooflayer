...
describe("Slice 2.1 target modeling foundation", () => {
  ...
  it("rejects an empty role title", async () => {
    ...
    await expect(createRoleTarget(workspace, { title: "   " })).rejects.toThrow(
      "Role target title is required and must not be blank."
    );
  });
  ...
  it("creates a job target from front matter with exact bytes and SHA-256", async () => {
    ...
    if (target.type !== "job") throw new Error("Expected job target fixture.");
    ...
    expect(result.persistedSourcePath).toBe(
      `targets/jobs/${target.id}/${JOB_DESCRIPTION_FILE_NAME}`
    );
  ...
  it("protects duplicate targets and replaces only when explicitly requested", async () => {
    ...
    await expect(createJobTarget(workspace, { file: sourcePath })).rejects.toThrow(
      "Target already exists: job-exampleco-engineering-manager. Use --replace"
    );
  ...
  it("lists role and job targets and shows complete stable JSON", async () => {
    ...
    expect(formatTargetList(targets)).toContain(
      "job-exampleco-platform-product-manager | job | Platform Product Manager | ExampleCo"
    );
...
async function writeMarkdown(workspace: string, fileName: string, contents: string): Promise<string> {
