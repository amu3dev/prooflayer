...
import {
  createExplicitFixtureReviews,
  pinCurrentEvidenceSnapshot,
} from "./evidence-snapshot-fixture.js";
...
describe("guided Job workflow", () => {
  it("creates an exact Job Target and does not run downstream stages", async () => {
    ...
    expect(result.target.type === "job" && result.target.source.sha256).toBe(
      createHash("sha256").update(bytes).digest("hex"),
    );
    expect(output.indexOf("Technical Product Manager")).toBeLessThan(
      output.indexOf(result.target.id),
    );
  ...
  it("pins only an explicitly supplied snapshot and creates a review workspace without approving evidence", async () => {
    ...
    expect((await getEvidenceReviewWorkspaceStatus(
      fixture.workspace,
      batch!.batchId,
    )).status).toBe("current");
    expect(result.stageResults.some((entry) =>
      entry.stage === "evidence-review" && entry.result === "built")).toBe(true);
    expect(result.status.evidenceSnapshot.eligibleJobEvidenceCount).toBe(0);
    expect(formatJobWorkflowStatus(result.status)).toContain(
      `prooflayer ui review ${batch!.batchId} --open`,
    );
  });
  ...
  it("builds a new immutable snapshot after completed evidence reviews but does not auto-upgrade", async () => {
    ...
    await createExplicitFixtureReviews(
      fixture.workspace,
  ...
  it("pauses cleanly when the pinned snapshot contains no claims", async () => {
    ...
    expect((await listEvidenceReviewBatches(workspace)).filter((entry) =>
      entry.targetId === target.target.id)).toHaveLength(0);
  });
  ...
  it("continues through eligible deterministic stages but pauses before proposal without a provider", async () => {
    ...
    expect((await getJobResumeDraftScaffoldStatus(
      fixture.workspace,
      fixture.targetId,
    )).status).toBe("current");
    expect((await getApprovedJobResumeDraftStatus(
      fixture.workspace,
      fixture.targetId,
    )).status).toBe("missing");
  });
  ...
  it("generates an untrusted proposal explicitly, initializes review, and never auto-approves", async () => {
    ...
    expect((await getJobResumeDraftReviewStatus(
      fixture.workspace,
      result.status.currentProposalId!,
  ...
  it("does not mutate incomplete review and approves only after completed human decisions", async () => {
    ...
    for (const decision of review.decisions) {
      await setJobResumeDraftReviewDecision(
        ...
        decision.itemType,
        decision.itemId,
        { decision: "accept", now: () => new Date(SECOND_TIME) },
      );
    }
    ...
    expect((await getApprovedJobResumeDraftStatus(
      fixture.workspace,
      fixture.targetId,
    ))).toMatchObject({ status: "current", usableForRendering: true });
  }, 15_000);
  ...
  it("renders stable human and JSON status with meaning before identifiers", async () => {
    ...
    const secondJson = formatJobWorkflowJson(
      await inspectJobWorkflow(fixture.workspace, fixture.targetId),
    );

    expect(human.indexOf("Technical Product Manager")).toBeLessThan(
      human.indexOf(fixture.targetId),
    );
  ...
  it("rejects Role Targets at the guided boundary", async () => {
    ...
    await expect(inspectJobWorkflow(workspace, role.target.id)).rejects.toThrow(
      "Guided Job workflow requires a Job Target",
    );
  });
});
...
async function validDraftPayload(
  ...
  const scaffoldSha256 = await hashFile(path.join(
    workspace,
    "targets/jobs",
    targetId,
  ...
  return {
    sections: scaffold.sections.map((guard) => {
      ...
      return {
        ...
        order: guard.order,
        status: guard.inclusion === "exclude" ? "excluded" as const
          : items.length ? "drafted" as const : "empty" as const,
      ...
      function createItem(): JobResumeDraftItem {
        ...
        const link = (allowedMetricPermission
          ...
          ?? eligibleLinks.find((entry) => {
            ...
            return guard.allowedClaimBoundaryIds.map((id) => boundaries.get(id)).some((boundary) =>
              boundary
        ...
        const claimType = guard.sectionType === "headline"
          ...
          : metricPermission
            ...
            : preferredClaimTypes(guard.sectionType, claim.type).find((entry) =>
              ...
              ?? boundary.allowedClaimTypes.find((entry) =>
                guard.allowedClaimTypes.includes(entry))
...
function draftText(sectionType: string, targetTitle: string, approvedWording: string): string {
...
function fakeToolchain(
  ...
  return {
    ...
    async createPdf({ outputPath }) {
      if (options.failPdf) throw new Error("LibreOffice test adapter unavailable");
...
async function workspaceInventory(workspace: string): Promise<Array<{
  path: string;
  sha256: string;
  mtimeMs: number;
}>> {
