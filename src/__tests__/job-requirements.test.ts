...
describe("Slice 2.7A deterministic Job Requirement Modeling", () => {
  ...
  it("preserves preferred, technical, domain, leadership, and operating context signals", async () => {
    ...
    expect(model.namedTechnologies).toEqual(
      expect.arrayContaining(["API", "CI/CD", "Docker", "Node.js", "React Native", "SaaS", "TypeScript"]),
    );
  ...
  it("keeps stable IDs, hashes, bytes, and timestamps on unchanged reruns", async () => {
    ...
    const manifestPath = deterministicArtifact(
      fixture.workspace,
      fixture.targetId,
    ...
    expect(secondParsed.requirements.map((entry) => entry.id)).toEqual(
      firstParsed.requirements.map((entry) => entry.id),
    );
  });
  ...
  it("reports missing/current/invalid lifecycle and requires explicit rebuild", async () => {
    ...
    expect((await getJobRequirementModelStatus(fixture.workspace, fixture.targetId)).status)
      .toBe("invalid");
    await expect(buildJobRequirements(fixture.workspace, fixture.targetId)).rejects.toThrow(
      "was not overwritten",
    );
  ...
  it("rejects Role Targets and stale or missing Job Description artifacts", async () => {
    ...
    await expect(buildJobRequirements(workspace, role.target.id)).rejects.toThrow(
      "rejects Role Target",
    );
  ...
  it("persists deterministic model and manifest separately from immutable target input", async () => {
    ...
    expect(manifest).toMatchObject({
      targetId: fixture.targetId,
      targetType: "job",
      modelSha256: await hashFile(
        deterministicArtifact(fixture.workspace, fixture.targetId, "job-requirement-model.json"),
      ),
  ...
  it("rejects unsupported model proposals rather than inventing source provenance", async () => {
    ...
    expect(proposal.validationIssues).toEqual(
  ...
  it("reviews deterministic and proposed requirements without a model call", async () => {
    ...
    let status = await initializeJobRequirementReview(
      fixture.workspace,
      fixture.proposalId,
      { reviewerName: "Reviewer", now: () => new Date(FIRST_TIME) },
    );
    ...
    for (const decision of review.decisions) {
      status = await setJobRequirementReviewDecision(
        fixture.workspace,
        fixture.proposalId,
        decision.requirementId,
        {
          decision:
            ...
            requirementTextForDecision(decision.requirementId, fixture.model, proposal).includes(
    ...
    status = await completeJobRequirementReview(
      fixture.workspace,
      fixture.proposalId,
      { now: () => new Date(SECOND_TIME) },
    );
  ...
  it("approves reviewed requirements without a model call and keeps exact provenance", async () => {
    ...
    const providerCalls = fixture.provider.callCount;
    const result = await approveJobRequirements(
      fixture.workspace,
      fixture.targetId,
      {
    ...
    const status = await getApprovedJobRequirementsStatus(
      fixture.workspace,
      fixture.targetId,
    );
    ...
    expect(approved.requirements.every((entry) =>
      ["human-approved", "human-edited"].includes(entry.trustState))).toBe(true);
  ...
  it("allows human classification edits while keeping source text and provenance immutable", async () => {
    ...
    const review = await showJobRequirementReview(fixture.workspace, fixture.proposalId);
    const proposal = await showJobRequirementProposal(fixture.workspace, fixture.proposalId);
    const deterministicVague = review.decisions.find((decision) =>
      decision.source === "deterministic" &&
      requirementTextForDecision(
        decision.requirementId,
        fixture.model,
        proposal,
      ).includes("Collaboration across"));
    ...
    const approved = await showApprovedJobRequirements(fixture.workspace, fixture.targetId);
    const edited = approved.requirements.find((entry) =>
      entry.approvalProvenance.reviewedRequirementId === deterministicVague?.requirementId);
    ...
    expect(edited?.provenance.sourceReferences).toEqual(
      requirementFor(fixture.model, "Collaboration across").provenance.sourceReferences,
    );
    expect(fixture.provider.callCount).toBe(providerCalls);
  });
  ...
  it("marks policy changes stale and never silently replaces approved artifacts", async () => {
    ...
    const deterministicPath = deterministicArtifact(
      fixture.workspace,
      fixture.targetId,
    ...
    await writeFile(
      deterministicPath,
      `${await readFile(deterministicPath, "utf8")} `,
      "utf8",
    );
    expect((await getApprovedJobRequirementsStatus(fixture.workspace, fixture.targetId)).status)
      .toBe("stale");
    await expect(
      approveJobRequirements(fixture.workspace, fixture.targetId, {
        proposalId: fixture.proposalId,
      }),
    ).rejects.toThrow();
  });
...
async function preparedJobWorkspace(): Promise<{ workspace: string; targetId: string }> {
  const workspace = await temporaryWorkspace();
...
async function builtJobWorkspace(): Promise<{ workspace: string; targetId: string }> {
  const fixture = await preparedJobWorkspace();
  await buildJobRequirements(fixture.workspace, fixture.targetId, {
    now: () => new Date(FIRST_TIME),
  });
...
async function completedReviewWorkspace() {
  ...
  const review = await showJobRequirementReview(fixture.workspace, fixture.proposalId);
  for (const decision of review.decisions) {
    const text = requirementTextForDecision(
      decision.requirementId,
      fixture.model,
      proposal,
    );
    await setJobRequirementReviewDecision(
      ...
      {
        decision:
          decision.source === "deterministic" && text.includes("Collaboration across")
            ? "reject"
            : "accept",
      },
      { now: () => new Date(SECOND_TIME) },
...
async function validProposalPayload(workspace: string, targetId: string) {
  ...
  return {
    proposedRequirements: [{
      ...
      category: "operating-context" as const,
      normalizedLabel: "Collaboration across changing priorities",
      sourceText: source.sourceText,
      necessity: "contextual" as const,
...
function requirementFor(
  model: Awaited<ReturnType<typeof showJobRequirementModel>>,
  text: string,
) {
  const found = model.requirements.find((entry) => entry.normalizedLabel.includes(text));
  if (!found) throw new Error(`Fixture requirement not found: ${text}`);
  return found;
}

function requirementTextForDecision(
...
function deterministicArtifact(workspace: string, targetId: string, name: string): string {
  return path.join(
    workspace,
    "targets",
    "jobs",
