...
import type { JobResumeSectionType } from "./job-resume-plan-schemas.js";
import {
  JobResumeRenderDocumentManifestSchema,
  JobResumeRenderDocumentSchema,
  type JobResumeRenderBlock,
  type JobResumeRenderDocument,
  type JobResumeRenderSection,
  type JobResumeRenderSourceMapEntry,
} from "./job-resume-render-schemas.js";
import {
  normalizeRoleResumeRenderOptions,
  resolveRoleResumeRenderProfile,
  ROLE_RESUME_RENDER_PROFILE_VERSION,
  type NormalizedRoleResumeRenderOptions,
...
const SECTION_HEADINGS: Record<JobResumeSectionType, string | null> = {
  headline: null,
...
const ITEM_BLOCK_TYPES: Record<JobResumeDraftItem["itemType"], JobResumeRenderBlock["type"]> = {
  headline: "headline",
  summary: "paragraph",
...
export async function composeJobResumeRenderDocument(
  workspace: string,
  targetId: string,
  options: JobResumeRenderOptions = {},
): Promise<ComposeJobResumeResult> {
  ...
  if (status.status === "current") {
    return composeResult(
      await showJobResumeRenderDocument(workspace, targetId),
      paths,
      "already-current",
    );
  }
  ...
  if (await pathExists(paths.documentPath)) {
    try {
      createdAt = JobResumeRenderDocumentSchema.parse(
        await readJson<unknown>(paths.documentPath, null),
      ).createdAt;
    } catch {
      // Explicit rebuild may replace an invalid canonical artifact.
    }
  }
  const profile = resolveRoleResumeRenderProfile(
  ...
  const validation = validateCanonicalComposition(
    context.approvedDraft,
    sections,
    sourceMap,
    warnings,
...
export async function showJobResumeRenderDocument(
  workspace: string,
  targetId: string,
): Promise<JobResumeRenderDocument> {
  const paths = jobResumeRenderDocumentPaths(workspace, targetId);
  if (!(await pathExists(paths.documentPath))) {
    throw new Error(`Canonical Job resume render document not found: ${targetId}`);
  }
  return JobResumeRenderDocumentSchema.parse(await readJson<unknown>(paths.documentPath, null));
}

export async function getJobResumeRenderDocumentStatus(
  workspace: string,
  targetId: string,
  requestedOptions?: NormalizedRoleResumeRenderOptions,
): Promise<JobResumeRenderDocumentStatus> {
  const paths = jobResumeRenderDocumentPaths(workspace, targetId);
  ...
  let document: JobResumeRenderDocument;
  let manifest: ReturnType<typeof JobResumeRenderDocumentManifestSchema.parse>;
  try {
    document = JobResumeRenderDocumentSchema.parse(
      await readJson<unknown>(paths.documentPath, null),
    );
    manifest = JobResumeRenderDocumentManifestSchema.parse(
  ...
  const policyVersionMatches =
    manifest.policyName === JOB_RESUME_RENDERING_POLICY_NAME
...
export function jobResumeRenderDocumentPaths(workspace: string, targetId: string) {
...
export function formatComposeJobResumeResult(result: ComposeJobResumeResult): string {
  return [
...
export function formatJobResumeRenderDocumentStatus(
  status: JobResumeRenderDocumentStatus,
): string {
  return [
...
function buildRenderSourceMap(
  ...
  documentId: string,
  sections: JobResumeRenderSection[],
): JobResumeRenderSourceMapEntry[] {
  const itemById = new Map(
    draft.sections.flatMap((section) => section.items).map((item) => [item.id, item]),
  );
  const ledgerByItemId = new Map(
    draft.claimLedger.map((entry) => [entry.draftItemId, entry]),
  );
...
function validateCanonicalComposition(
  draft: ApprovedJobResumeDraft,
  sections: JobResumeRenderSection[],
  sourceMap: JobResumeRenderSourceMapEntry[],
  ...
  const itemOrderPreserved = expectedItems.every((item, index) =>
    renderedItems[index]?.draftItemId === item.id);
  ...
  const privateMetadataAbsent =
    !/(?:\/Users\/|\/home\/|[A-Za-z]:\\Users\\|evidence_[a-z0-9]|claim_[a-z0-9]|sha256)/i.test(visible);
...
async function loadJobResumeRenderContext(workspace: string, targetId: string) {
...
function assertApprovedTrustState(
  trustState: JobResumeDraftItem["trustState"],
  itemId: string,
): asserts trustState is "human-approved" | "human-edited" {
...
function pageBreakRule(type: JobResumeRenderBlock["type"]) {
...
function composeResult(
  ...
  paths: ReturnType<typeof jobResumeRenderDocumentPaths>,
  result: ComposeJobResumeResult["result"],
): ComposeJobResumeResult {
  return {
    ...
    blockCount: document.sections.reduce(
...
function emptyRenderStatus(
  base: Pick<
    JobResumeRenderDocumentStatus,
    "targetId" | "documentExists" | "manifestExists" | "documentPath" | "manifestPath"
  >,
  status: JobResumeRenderDocumentStatus["status"],
  reasons: string[],
): JobResumeRenderDocumentStatus {
  return {
    ...base,
