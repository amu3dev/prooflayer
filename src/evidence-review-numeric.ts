...
export type EvidenceReviewNumericCategory =
  typeof EVIDENCE_REVIEW_NUMERIC_CATEGORIES[number];
...
export function classifyEvidenceReviewNumericClaim(
  text: string,
): EvidenceReviewNumericClassification {
...
export function isEvidenceReviewMetricCandidate(
  text: string,
  sourceMetricStatus: string,
): boolean {
  const classification = classifyEvidenceReviewNumericClaim(text);
  if (classification.requiresMetricVerification) return true;
