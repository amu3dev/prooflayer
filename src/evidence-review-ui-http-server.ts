...
import {
  ...
  resolveProofLayerUiRequestScope,
  type ProductWorkflowActionName,
  type ProofLayerUiRequestScope,
...
async function prepareNullOriginSubmission(
  request: IncomingMessage,
  runtime: EvidenceReviewUiHttpRuntime,
  scope: Exclude<ProofLayerUiRequestScope, { scope: "read-only" | "unregistered" }>,
  ...
  if (scope.scope === "evidence-review") {
    ...
    try {
      expectedToken = proofLayerUiActionCsrfToken(runtime.csrfSecret, {
        actionName: actionName as ProductWorkflowActionName,
...
function isSupportedFormContentType(contentType: string | undefined): boolean {
...
function singleTextField(fields: FormData, name: string): string | undefined {
