...
type ExportableRoleKey = Extract<RoleKey, "ai-product" | "tpm">;

type ResumeExportConfig = {
  roleKey: ExportableRoleKey;
...
const RESUME_EXPORT_CONFIGS: Record<ExportableRoleKey, ResumeExportConfig> = {
  "ai-product": {
    roleKey: "ai-product",
...
export type ResumeExportOptions = {
  ...
  runTool?: (executable: string, args: string[], options: { cwd: string; env?: NodeJS.ProcessEnv }) => Promise<void>;
};

export async function exportFinalResume(
  workspace: string,
  roleKeyInput: string,
  options: ResumeExportOptions = {}
): Promise<ResumeExportManifest> {
  ...
  if (!(await pathExists(sourcePath))) throw new Error(`Final resume is missing: ${config.sourceMarkdownPath}`);
  if (!(await pathExists(finalManifestPath))) throw new Error(`Final manifest is missing: ${config.finalManifestPath}`);
  if (!(await pathExists(readinessReviewPath))) throw new Error(`Export readiness review is missing: ${config.readinessReviewPath}`);

  const finalManifest = await readJson<FinalManifestForExport | null>(finalManifestPath, null);
  if (!finalManifest?.profileFingerprint) throw new Error("Final manifest does not contain a profile fingerprint.");
  if (finalManifest.roleKey !== roleKey) throw new Error(`Final manifest role mismatch: expected ${roleKey}.`);
  if (finalManifest.finalizationReadiness !== "ready") throw new Error("Final resume is not marked ready for export.");

  const baseline = await readJson<RefreshBaseline | null>(path.join(workspace, UPDATE_BASELINE_PATH), null);
  ...
  if (!pandoc) throw new Error('Required export tool "pandoc" was not found in PATH.');
  const soffice = await resolveTool("soffice");
  if (!soffice) throw new Error('Required export tool "soffice" (LibreOffice) was not found in PATH.');
  ...
  try {
    ...
    const manifest: ResumeExportManifest = {
      schemaVersion: 2,
...
export async function getResumeExportStatus(workspace: string, roleKeyInput: string): Promise<ResumeExportStatus> {
  return inspectConfiguredResumeExportStatus(workspace, requireSupportedExportRole(roleKeyInput));
}

export async function inspectResumeExportStatus(workspace: string, roleKey: RoleKey): Promise<ResumeExportStatus> {
  const config = exportConfigForRole(roleKey);
  const defaultStatus: ResumeExportStatus = {
    roleKey,
    generated: false,
...
async function inspectConfiguredResumeExportStatus(
  workspace: string,
  config: ResumeExportConfig
): Promise<ResumeExportStatus> {
  const defaultStatus: ResumeExportStatus = {
  ...
  if (!(await pathExists(manifestPath))) return defaultStatus;
  const manifest = await readJson<Partial<ResumeExportManifest> | null>(manifestPath, null);
  if (!manifest) return { ...defaultStatus, generated: true, freshness: "stale", reasons: ["Export manifest is invalid."] };
  ...
  if (!markdownPresent) reasons.push("Exported Markdown is missing.");
  if (!docxPresent) reasons.push("DOCX export is missing.");
  if (!pdfPresent) reasons.push("PDF export is missing.");
  ...
  if (!(await pathExists(sourcePath))) {
    ...
    if (!sourceHashMatches) reasons.push("Source final Markdown hash changed.");
    if (markdownPresent) {
      ...
      if (!sourceExportMarkdownHashMatch) reasons.push("Exported Markdown hash does not match the source.");
  ...
  if (manifest.exportedMarkdownPath !== config.markdownPath) reasons.push("Export manifest does not register the Markdown artifact.");
  if (manifest.verification?.exportedMarkdownMatchesSource !== true) reasons.push("Export manifest lacks Markdown hash verification.");

  const baseline = await readJson<RefreshBaseline | null>(path.join(workspace, UPDATE_BASELINE_PATH), null);
  const profileMatches = Boolean(baseline && baseline.profileFingerprint === manifest.profileFingerprint);
  if (!profileMatches) reasons.push("Career profile fingerprint changed.");
  const finalManifestPath = manifest.finalManifestPath ?? config.finalManifestPath;
  const finalManifest = await readJson<FinalManifestForExport | null>(path.join(workspace, finalManifestPath), null);
  const finalManifestMatches = Boolean(finalManifest && finalManifest.profileFingerprint === manifest.profileFingerprint);
  if (!finalManifestMatches) reasons.push("Final resume manifest fingerprint changed.");
...
export function formatResumeExportStatus(status: ResumeExportStatus): string {
  return [
...
function formatArtifactStatus(present: boolean, current: boolean): string {
  if (!present) return "missing/stale";
  return current ? "present/current" : "present/stale";
}

function requireSupportedExportRole(value: string): ResumeExportConfig {
  const config = exportConfigForRole(value);
  if (!config) throw new Error("Resume export currently supports --role ai-product or --role tpm.");
  return config;
}

function exportConfigForRole(value: string): ResumeExportConfig | undefined {
  return value === "ai-product" || value === "tpm" ? RESUME_EXPORT_CONFIGS[value] : undefined;
}

async function resolveExecutable(tool: ExportToolName): Promise<string | null> {
...
async function assertNonEmptyFile(filePath: string, label: string): Promise<void> {
  if (!(await pathExists(filePath))) throw new Error(`${label} export was not created.`);
  const fileStat = await stat(filePath);
  if (!fileStat.isFile() || fileStat.size === 0) throw new Error(`${label} export is empty or invalid.`);
}

async function registerExportOutput(
  workspace: string,
  manifest: ResumeExportManifest,
  finalManifest: FinalManifestForExport,
  exportManifestPath: string
): Promise<void> {
  const registryPath = path.join(workspace, OUTPUT_MANIFEST_PATH);
  const registry = await readJson<OutputManifest>(registryPath, {
    schemaVersion: 1,
  ...
  await writeJsonAtomic(registryPath, { schemaVersion: 1, updatedAt: manifest.exportedAt, outputs } satisfies OutputManifest);
}
