...
import {
  ...
  ApprovedTargetInterpretationSchema,
  ...
  type ApprovedTargetInterpretationManifest,
...
export async function approveInterpretationProposal(
  workspace: string,
  proposalId: string,
  ...
  const proposalManifest = InterpretationProposalManifestSchema.parse(
    await readJson<unknown>(proposalManifestPath(workspace, target, proposalId), null),
  );
  const reviewManifest = InterpretationProposalReviewManifestSchema.parse(
    await readJson<unknown>(reviewManifestPath(workspace, target, proposalId), null),
  );
  const deterministicManifest = TargetInterpretationManifestSchema.parse(
    await readJson<unknown>(deterministicManifestPath(workspace, target), null),
  );
  ...
  const deterministicExpectations: ApprovedTargetExpectation[] = deterministic.expectations.map((expectation) => ({
    ...expectation,
  ...
  const approvedByProposalId = new Map<string, ApprovedTargetExpectation>();
  for (const proposed of proposal.proposedExpectations) {
    ...
    if (decision.decision === "edit" && !edited) throw new Error(`Edit decision is missing edited content: ${proposed.id}`);
    const trustState = decision.decision === "edit" ? "human-edited" : "human-approved";
    const approved: ApprovedTargetExpectation = {
  ...
  const modelGroups: ExpectationGroup[] = proposal.proposedGroups.flatMap((group) => {
    const expectationIds = group.expectationIds
      .map((id) => approvedByProposalId.get(id)?.id)
      .filter((id): id is string => Boolean(id));
    if (expectationIds.length === 0) return [];
    return [{
  ...
  const completeness = approvedCompleteness(
    expectations,
    analysis.items.filter((item) => item.kind !== "front-matter-field").map((item) => item.id),
  );
...
export async function showApprovedTargetInterpretation(
  workspace: string,
  targetId: string,
): Promise<ApprovedTargetInterpretation> {
  const target = await showTarget(workspace, targetId);
  const paths = approvedPaths(workspace, target);
  if (!(await pathExists(paths.interpretationPath))) throw new Error(`Approved interpretation not found for target: ${targetId}`);
  return ApprovedTargetInterpretationSchema.parse(await readJson<unknown>(paths.interpretationPath, null));
}

export async function getApprovedInterpretationStatus(
  workspace: string,
  targetId: string,
  options: Pick<ApprovalOptions, "policyVersion"> = {},
): Promise<ApprovedInterpretationStatus> {
  const target = await showTarget(workspace, targetId);
  const paths = approvedPaths(workspace, target);
  ...
  if (!interpretationExists && !manifestExists) return emptyStatus(base, "missing", ["No approved interpretation exists."]);
  if (!interpretationExists || !manifestExists) return emptyStatus(base, "invalid", ["Approved interpretation artifact set is incomplete."]);
  let approved: ApprovedTargetInterpretation;
  let manifest: ApprovedTargetInterpretationManifest;
  try {
    approved = ApprovedTargetInterpretationSchema.parse(await readJson<unknown>(paths.interpretationPath, null));
    manifest = ApprovedTargetInterpretationManifestSchema.parse(await readJson<unknown>(paths.manifestPath, null));
  } catch (error) {
  ...
  const deterministicInterpretationHashMatches = await currentHash(
    workspace,
    approved.input.deterministicInterpretationPath,
    manifest.deterministicInterpretationSha256,
  );
  ...
  const proposalHashMatches =
    await currentHash(workspace, approved.input.proposalPath, manifest.proposalSha256) &&
    proposalLifecycle.status === "current";
  const reviewHashMatches =
    await currentHash(workspace, approved.input.reviewPath, manifest.reviewSha256) &&
  ...
  if (!interpretationHashMatches) invalidReasons.push("Approved interpretation SHA-256 does not match its manifest.");
  if (
    approved.targetId !== target.id || manifest.targetId !== target.id ||
...
export function formatApprovedInterpretationStatus(status: ApprovedInterpretationStatus): string {
...
function approvedCompleteness(
  ...
  if (relevant.length === 0) return { status: "complete", usableForEvidenceMatching: true, blockingReasons: [] };
...
function resultFromApproved(
  approved: ApprovedTargetInterpretation,
  manifest: ApprovedTargetInterpretationManifest,
...
function approvedPaths(workspace: string, target: Target) {
...
function deterministicPath(target: Target): string {
  return `${targetRoot(target)}/interpretation/target-interpretation.json`;
}

function deterministicManifestPath(workspace: string, target: Target): string {
  return resolveWithin(workspace, `${targetRoot(target)}/interpretation/interpretation-manifest.json`);
}

function proposalManifestPath(workspace: string, target: Target, proposalId: string): string {
  return resolveWithin(workspace, `${targetRoot(target)}/interpretation/proposals/${proposalId}/proposal-manifest.json`);
}

function reviewManifestPath(workspace: string, target: Target, proposalId: string): string {
  return resolveWithin(workspace, `${targetRoot(target)}/interpretation/reviews/${proposalId}/review-manifest.json`);
}

async function currentHash(workspace: string, relativePath: string, expected: string): Promise<boolean> {
...
function emptyStatus(
  base: Pick<ApprovedInterpretationStatus, "targetId" | "targetType" | "interpretationExists" | "manifestExists" | "interpretationPath" | "manifestPath">,
  status: "missing" | "invalid",
  reasons: string[],
): ApprovedInterpretationStatus {
  return {
    ...base,
