...
import {
  ...
  type TargetAnalysisManifest,
  type TargetAnalysisSection,
...
const HEADING_CLASSIFICATIONS = new Map<string, TargetAnalysisSection["classification"]>([
...
function normalizedRelative(relativePath: string): string {
...
function targetPaths(workspace: string, target: Target): AnalysisPaths {
...
function sourceLines(buffer: Buffer): SourceLine[] {
...
function stableId(prefix: string, values: Array<string | number | null>): string {
...
function sectionSemantics(classification: TargetAnalysisSection["classification"]): {
  necessity: TargetAnalysisItem["necessity"];
  category: TargetAnalysisItem["category"];
} {
...
function sourceReference(
  ...
  startIndex: number,
  endIndex: number,
): TargetAnalysisSourceReference {
  const start = lines[startIndex];
  const end = lines[endIndex];
  if (!start || !end) throw new Error("Cannot create a source reference outside the source document");
...
function frontMatterRange(lines: SourceLine[]): { start: number; end: number } | null {
  if (lines[0]?.content.trim() !== "---") return null;
  for (let index = 1; index < lines.length; index += 1) {
    if (lines[index]?.content.trim() === "---") return { start: 0, end: index };
  }
  return null;
}

function parseHeading(content: string): { heading: string; level: number } | null {
...
function parseSectionHeading(content: string): { heading: string; level: number } | null {
  const markdownHeading = parseHeading(content);
  if (markdownHeading) return markdownHeading;

  const heading = content.trim();
  if (!heading || content !== heading) return null;
  const normalized = normalizeHeading(heading);
  if (!HEADING_CLASSIFICATIONS.has(normalized)) return null;
  return {
    heading,
...
function listStatement(content: string): string | null {
...
function makeSection(
  ...
  startIndex: number,
  endIndex: number,
  heading: { heading: string; level: number } | null,
...
function parseJobMarkdown(
  ...
  if (frontMatter) {
    for (let index = frontMatter.start + 1; index < frontMatter.end; index += 1) {
      ...
      if (!line || !match?.[1] || !match[2]) continue;
      const normalizedKey = match[1].toLowerCase().replace(/[-_]/g, "");
      if (!FRONT_MATTER_FIELDS.has(normalizedKey)) continue;
  ...
  for (let index = bodyStart; index < lines.length; index += 1) {
    if (parseSectionHeading(lines[index]?.content ?? "")) headingIndexes.push(index);
  }

  const firstContent = lines.findIndex(
    (line, index) => index >= bodyStart && line.content.trim().length > 0,
  );
  if (firstContent >= 0 && (headingIndexes[0] === undefined || firstContent < headingIndexes[0])) {
    let endIndex = (headingIndexes[0] ?? lines.length) - 1;
    while (endIndex > firstContent && !lines[endIndex]?.content.trim()) endIndex -= 1;
    sections.push(
      makeSection(
        targetId,
        buffer,
  ...
  for (let headingPosition = 0; headingPosition < headingIndexes.length; headingPosition += 1) {
    ...
    while (endIndex > startIndex && !lines[endIndex]?.content.trim()) endIndex -= 1;
    sections.push(
      makeSection(
        targetId,
        buffer,
        ...
        startIndex,
        endIndex,
        parseSectionHeading(lines[startIndex]?.content ?? ""),
      ),
    );
  }

  const sectionStack: TargetAnalysisSection[] = [];
  for (const section of sections) {
    if (section.headingLevel === null) continue;
    while (
    ...
    while (index <= endIndex) {
      ...
      if (listItem) {
        ...
        items.push({
          ...
          extractionMethod:
            section.classificationBasis === "explicit-heading"
              ? "explicit-heading"
              : "markdown-structure",
          sourceReferences: [reference],
        });
...
async function loadValidatedInput(workspace: string, targetId: string): Promise<{
  target: Target;
  paths: AnalysisPaths;
  targetSha256: string;
  sourceBuffer?: Buffer;
  ...
  if (target.type === "role") return { target, paths, targetSha256 };

  const jobTarget = JobTargetSchema.parse(target);
  if (!paths.sourcePath) throw new Error(`Persisted job description path is missing for ${target.id}`);
  ...
  if (sourceBuffer.length === 0) throw new Error(`Persisted job description is empty: ${paths.sourceRelativePath}`);
...
async function storedStatus(
  ...
  let manifest: TargetAnalysisManifest;
  try {
    analysis = TargetAnalysisSchema.parse(await readJson<unknown>(paths.analysisPath, null));
    manifest = TargetAnalysisManifestSchema.parse(await readJson<unknown>(paths.manifestPath, null));
  } catch (error) {
    return {
      ...base,
      targetHashMatches: null,
      ...
      analysisHashMatches: null,
      status: "invalid",
      reasons: [`Stored analysis data is invalid: ${(error as Error).message}`],
    };
  }
  ...
  const analyzerVersionMatches =
    manifest.analyzerName === analyzerName &&
    manifest.analyzerVersion === analyzerVersion &&
    analysis.analyzer.name === analyzerName &&
    analysis.analyzer.version === analyzerVersion;
  let sourceHashMatches: boolean | null = null;
  if (target.type === "job") {
    try {
      if (!paths.sourcePath) throw new Error("missing path");
      if (analysis.targetType !== "job") throw new Error("analysis type mismatch");
  ...
  if (
    analysis.analyzer.name !== manifest.analyzerName ||
  ...
  if (!analysisHashMatches) agreementErrors.push("Stored analysis SHA-256 does not match the manifest.");
  ...
  if (!targetHashMatches) staleReasons.push("Target input hash changed after analysis.");
  if (sourceHashMatches === false) staleReasons.push("Job-description source hash or raw content changed.");
  if (!analyzerVersionMatches) staleReasons.push("Analyzer name or version changed.");
  return {
    ...base,
    targetHashMatches,
    sourceHashMatches,
...
export async function getTargetAnalysisStatus(
  workspace: string,
  targetId: string,
  options: Pick<AnalyzeOptions, "analyzerName" | "analyzerVersion"> = {},
): Promise<TargetAnalysisStatus> {
  const target = await showTarget(workspace, targetId);
  const paths = targetPaths(workspace, target);
  const targetSha256 = await hashFile(paths.targetPath);
  return storedStatus(
    ...
    paths,
    targetSha256,
    options.analyzerName ?? TARGET_ANALYZER_NAME,
    options.analyzerVersion ?? TARGET_ANALYZER_VERSION,
  );
}
...
export async function analyzeTarget(
  ...
  if (status.status === "current" && !options.rebuild) {
    const analysis = TargetAnalysisSchema.parse(await readJson<unknown>(input.paths.analysisPath, null));
    return {
  ...
  if (status.status === "invalid" && !options.rebuild) {
    throw new Error(
      `Stored analysis is invalid and was not overwritten. Re-run with --rebuild after review. ${status.reasons.join(" ")}`,
    );
  }
  ...
  if (status.analysisExists) {
    try {
      const stored = TargetAnalysisSchema.parse(await readJson<unknown>(input.paths.analysisPath, null));
  ...
  let sections: TargetAnalysisSection[] = [];
  let items: TargetAnalysisItem[] = [];
  let warnings: TargetAnalysis["warnings"] = [];
  if (input.target.type === "role") {
    ...
    ({ sections, items, warnings } = parseJobMarkdown(
      input.target.id,
      input.sourceBuffer,
      input.paths.sourceRelativePath,
      input.sourceSha256,
    ));
  }
...
export async function showTargetAnalysis(
  workspace: string,
  targetId: string,
): Promise<TargetAnalysis> {
  const target = await showTarget(workspace, targetId);
  const paths = targetPaths(workspace, target);
  if (!(await exists(paths.analysisPath))) throw new Error(`Analysis not found for target: ${targetId}`);
  return TargetAnalysisSchema.parse(await readJson<unknown>(paths.analysisPath, null));
}

export function formatAnalyzeTargetResult(result: AnalyzeTargetResult): string {
  return [
...
export function formatTargetAnalysisStatus(status: TargetAnalysisStatus): string {
...
export async function analysisFileTimestamps(
  workspace: string,
  targetId: string,
): Promise<{ analysisMtimeMs: number; manifestMtimeMs: number }> {
  const target = await showTarget(workspace, targetId);
  const paths = targetPaths(workspace, target);
