...
import {
  RoleResumeDraftScaffoldManifestSchema,
  RoleResumeDraftScaffoldSchema,
  type RoleResumeDraftScaffold,
  type RoleResumeDraftScaffoldManifest,
...
export async function loadRoleResumeDraftingContext(
  workspace: string,
  targetId: string,
...
export async function buildRoleResumeDraftScaffold(
  workspace: string,
  targetId: string,
  options: BuildRoleResumeDraftScaffoldOptions = {},
): Promise<BuildRoleResumeDraftScaffoldResult> {
  ...
  if (await pathExists(paths.scaffoldPath)) {
    try {
      createdAt = RoleResumeDraftScaffoldSchema.parse(
  ...
  const manifest = createRoleResumeDraftScaffoldManifest(
    scaffold,
    context,
...
export function deriveRoleResumeDraftScaffold(
  context: RoleResumeDraftingContext,
  createdAt: string,
  updatedAt: string,
...
export async function showRoleResumeDraftScaffold(
  workspace: string,
  targetId: string,
): Promise<RoleResumeDraftScaffold> {
  const paths = roleResumeDraftScaffoldPaths(workspace, targetId);
  if (!(await pathExists(paths.scaffoldPath))) throw new Error(`Role resume draft scaffold not found: ${targetId}`);
  return RoleResumeDraftScaffoldSchema.parse(await readJson<unknown>(paths.scaffoldPath, null));
}

export async function getRoleResumeDraftScaffoldStatus(
  workspace: string,
  targetId: string,
): Promise<RoleResumeDraftScaffoldStatus> {
  const paths = roleResumeDraftScaffoldPaths(workspace, targetId);
  ...
  if (!scaffoldExists && !manifestExists) return emptyScaffoldStatus(base, "missing", ["No role resume draft scaffold exists."]);
  if (!scaffoldExists || !manifestExists) return emptyScaffoldStatus(base, "invalid", ["Scaffold artifact set is incomplete."]);
  let scaffold: RoleResumeDraftScaffold;
  let manifest: RoleResumeDraftScaffoldManifest;
  try {
    scaffold = RoleResumeDraftScaffoldSchema.parse(await readJson<unknown>(paths.scaffoldPath, null));
    manifest = RoleResumeDraftScaffoldManifestSchema.parse(await readJson<unknown>(paths.manifestPath, null));
  } catch (error) {
  ...
  let context: RoleResumeDraftingContext;
  try {
...
export function assertRoleResumeDraftScaffoldConsistency(
  ...
  if (scaffold.roleTitle !== context.target.title) throw new Error("Scaffold role title differs from the Role Target.");
  if (
    scaffold.draftingPolicy.name !== ROLE_RESUME_DRAFTING_POLICY_NAME ||
...
export function deterministicRoleResumeDraftScaffoldId(context: RoleResumeDraftingContext): string {
...
export function roleResumeDraftScaffoldPaths(workspace: string, targetId: string) {
...
export function createRoleResumeDraftScaffoldManifest(
  scaffold: RoleResumeDraftScaffold,
  context: RoleResumeDraftingContext,
  scaffoldPath: string,
  ...
  createdAt: string,
  updatedAt: string,
): RoleResumeDraftScaffoldManifest {
...
export function formatBuildRoleResumeDraftScaffoldResult(result: BuildRoleResumeDraftScaffoldResult): string {
  return [
...
export function formatRoleResumeDraftScaffoldStatus(status: RoleResumeDraftScaffoldStatus): string {
  return [
...
function scaffoldProvenance(context: RoleResumeDraftingContext) {
  return {
    targetSha256: context.targetSha256,
    approvedInterpretationSha256: context.approvedInterpretationSha256,
    approvedInterpretationManifestSha256: context.approvedInterpretationManifestSha256,
...
function constraint(
  ...
  code: string,
  description: string,
  sections: RoleResumeDraftScaffoldSection[],
  blocking: boolean,
) {
  return {
...
function defaultMaximumItems(type: RoleResumeSectionType): number {
  return ({
    headline: 1,
...
function maximumSentences(type: RoleResumeSectionType): number | undefined {
...
function scaffoldResult(
  scaffold: RoleResumeDraftScaffold,
  paths: ReturnType<typeof roleResumeDraftScaffoldPaths>,
  result: BuildRoleResumeDraftScaffoldResult["result"],
): BuildRoleResumeDraftScaffoldResult {
  return {
    targetId: scaffold.targetId,
    result,
    scaffoldId: scaffold.id,
...
function emptyScaffoldStatus(
  base: Pick<RoleResumeDraftScaffoldStatus, "targetId" | "scaffoldExists" | "manifestExists" | "scaffoldPath" | "manifestPath">,
  status: RoleResumeDraftScaffoldStatus["status"],
  reasons: string[],
): RoleResumeDraftScaffoldStatus {
  return {
    ...base,
