...
import {
  EditedExpectationFitAssessmentSchema,
  FitAssessmentProposalManifestSchema,
  FitAssessmentProposalReviewSchema,
  FitAssessmentReviewManifestSchema,
  FitAssessmentSummarySchema,
  type EditedExpectationFitAssessment,
  type FitAssessmentProposalReview,
  type FitAssessmentReviewDecision,
...
export async function initializeFitAssessmentReview(
  workspace: string,
  proposalId: string,
  options: FitAssessmentReviewOptions = {},
): Promise<FitAssessmentReviewStatus> {
  const proposalStatus = await getFitAssessmentProposalStatus(workspace, proposalId);
  if (!proposalStatus.readyForReview) throw new Error(`Assessment proposal is not ready for review. Current status: ${proposalStatus.status}`);
  ...
  if (await pathExists(paths.reviewPath)) return getFitAssessmentReviewStatus(workspace, proposalId);
...
export async function setFitAssessmentReviewDecision(
  workspace: string,
  proposalId: string,
  ...
  options: Pick<FitAssessmentReviewOptions, "now"> = {},
): Promise<FitAssessmentReviewStatus> {
  ...
  if (!proposed) throw new Error(`Unknown proposed expectation assessment ID: ${proposedAssessmentId}`);
  ...
  if (!existing) throw new Error(`Review is missing proposed assessment: ${proposedAssessmentId}`);
  if (existing.decision !== "pending") throw new Error(`A decision already exists for proposed assessment: ${proposedAssessmentId}`);
  if (input.decision === "edit") {
    if (!input.editedAssessment) throw new Error("Edit requires edited assessment content.");
...
export async function setFitAssessmentSummaryReviewDecision(
  workspace: string,
  proposalId: string,
  input: {
    decision: "accept" | "reject" | "edit";
  ...
  const review = await showFitAssessmentReview(workspace, proposalId);
  assertMutable(review);
  if (review.summaryDecision.decision !== "pending") throw new Error("A summary review decision already exists.");
  if (input.decision === "edit") {
    if (!input.editedSummary) throw new Error("Summary edit requires edited summary content.");
    if (input.editedSummary.mode !== proposal.mode) throw new Error("Edited summary mode must match the target mode.");
...
export async function completeFitAssessmentReview(
  workspace: string,
  proposalId: string,
  options: Pick<FitAssessmentReviewOptions, "now"> = {},
): Promise<FitAssessmentReviewStatus> {
  ...
  const proposal = await showFitAssessmentProposal(workspace, proposalId);
  const review = await showFitAssessmentReview(workspace, proposalId);
  if (review.status === "completed") return getFitAssessmentReviewStatus(workspace, proposalId);
...
export async function showFitAssessmentReview(workspace: string, proposalId: string): Promise<FitAssessmentProposalReview> {
  const proposal = await showFitAssessmentProposal(workspace, proposalId);
  const paths = pathsForProposal(workspace, proposal);
  if (!(await pathExists(paths.reviewPath))) throw new Error(`Assessment review not found for proposal: ${proposalId}`);
  return FitAssessmentProposalReviewSchema.parse(await readJson<unknown>(paths.reviewPath, null));
}

export async function getFitAssessmentReviewStatus(workspace: string, proposalId: string): Promise<FitAssessmentReviewStatus> {
  const proposal = await showFitAssessmentProposal(workspace, proposalId);
  const paths = pathsForProposal(workspace, proposal);
  ...
  const base = { proposalId, targetId: proposal.targetId, reviewExists, manifestExists, expectationCounts: { ...emptyCounts }, summaryDecision: "pending" as const, reviewPath: paths.reviewRelativePath, manifestPath: paths.manifestRelativePath };
  if (!reviewExists && !manifestExists) return { ...base, reviewHashMatches: null, proposalHashMatches: null, status: "missing", reasons: ["No assessment review exists."] };
  if (!reviewExists || !manifestExists) return { ...base, reviewHashMatches: null, proposalHashMatches: null, status: "invalid", reasons: ["Assessment review artifact set is incomplete."] };
  try {
    const review = FitAssessmentProposalReviewSchema.parse(await readJson<unknown>(paths.reviewPath, null));
    const manifest = FitAssessmentReviewManifestSchema.parse(await readJson<unknown>(paths.manifestPath, null));
    const proposalManifest = FitAssessmentProposalManifestSchema.parse(await readJson<unknown>(proposalManifestPath(workspace, proposal.targetType, proposal.targetId, proposalId), null));
    ...
    if (review.proposalId !== proposalId || review.targetId !== proposal.targetId || manifest.proposalId !== proposalId || manifest.targetId !== proposal.targetId || manifest.reviewPath !== paths.reviewRelativePath) reasons.push("Review identity or paths are invalid.");
    try {
      assertExactCoverage(review, proposal.proposedExpectationAssessments.map((entry) => entry.id));
      if (review.status === "completed") await validateCompletedReview(workspace, review, proposal);
    } catch (error) {
...
export async function readEditedFitAssessmentFile(filePath: string): Promise<EditedExpectationFitAssessment> {
  try {
...
export async function readEditedFitAssessmentSummaryFile(filePath: string): Promise<FitAssessmentSummary> {
  try {
...
export function formatFitAssessmentReviewStatus(status: FitAssessmentReviewStatus): string {
  return [
...
async function validateCompletedReview(
  workspace: string,
  review: FitAssessmentProposalReview,
  proposal: Awaited<ReturnType<typeof showFitAssessmentProposal>>,
): Promise<void> {
  if (!proposal.proposedSummary) throw new Error("Assessment proposal has no valid summary.");
  ...
  const merged = review.expectationDecisions.map((decision) => {
    ...
    if (!proposed) throw new Error(`Unknown reviewed assessment: ${decision.proposedAssessmentId}`);
    const source = deterministicByExpectation.get(proposed.expectationId);
    if (!source) throw new Error(`Deterministic fallback is missing: ${proposed.expectationId}`);
    if (decision.decision === "reject") return source;
    if (decision.decision === "edit") {
      if (!decision.editedAssessment) throw new Error(`Edited assessment content is missing: ${decision.proposedAssessmentId}`);
      validateEditedAssessment(decision.editedAssessment, proposed);
      return { ...source, ...decision.editedAssessment };
    }
    if (decision.decision === "accept") return { ...source, ...proposed, id: source.id, expectation: source.expectation, trustState: source.trustState };
  ...
  const summary = review.summaryDecision.decision === "accept"
    ...
    : review.summaryDecision.decision === "edit"
      ...
      : review.summaryDecision.decision === "reject"
        ...
        : undefined;
  if (!summary) throw new Error("Completed review lacks a valid summary decision.");
  validateSummaryCounts(summary, merged);
  validateForbiddenText(summary.narrative);
}

function validateEditedAssessment(
  edited: EditedExpectationFitAssessment,
  proposed: Awaited<ReturnType<typeof showFitAssessmentProposal>>["proposedExpectationAssessments"][number],
): void {
  if (!sameSet(edited.approvedMatchIds, proposed.approvedMatchIds)) throw new Error("Edited assessment cannot change approved match IDs.");
  if (!sameSet(edited.evidenceIds, proposed.evidenceIds)) throw new Error("Edited assessment cannot change reviewed evidence IDs.");
  const coverage = proposed.provenance.deterministicInputs.coverageStatus;
  if (edited.supportStatus === "strongly-supported" && !(coverage === "matched" && proposed.provenance.deterministicInputs.matchTypes.includes("direct") && proposed.provenance.deterministicInputs.evidenceStrengths.includes("strong"))) throw new Error("Strong support requires approved direct strong evidence.");
  if (edited.supportStatus === "unsupported" && edited.approvedMatchIds.length) throw new Error("Unsupported assessment cannot contain approved supporting matches.");
  if (edited.supportStatus === "conflicting" && !proposed.provenance.deterministicInputs.matchTypes.includes("contradictory")) throw new Error("Conflicting assessment requires approved contradictory evidence.");
  if (edited.supportStatus === "not-assessed" && coverage !== "not-assessed") throw new Error("Not-assessed requires approved not-assessed coverage.");
  if (edited.gapType === "experience-gap-possible" && !/possible|unclear|may|cannot confirm/i.test(`${edited.rationale} ${edited.limitations.join(" ")}`)) throw new Error("Possible experience gaps must preserve uncertainty.");
...
function validateSummaryCounts(summary: FitAssessmentSummary, entries: Array<{ supportStatus: string; expectation: { necessity: string } }>): void {
  if (summary.mode === "role-positioning") {
    const counts = statusCounts(entries);
    if (summary.stronglySupportedCount !== counts.stronglySupported || summary.supportedCount !== counts.supported || summary.partiallySupportedCount !== counts.partiallySupported || summary.unsupportedCount !== counts.unsupported || summary.conflictingCount !== counts.conflicting || summary.notAssessedCount !== counts.notAssessed) throw new Error("Edited role summary counts are inconsistent with reviewed assessment decisions.");
    return;
  }
  const groups = [
  ...
  for (const [actual, grouped] of groups) {
    ...
    if (actual.total !== grouped.length || actual.stronglySupported !== counts.stronglySupported || actual.supported !== counts.supported || actual.partiallySupported !== counts.partiallySupported || actual.unsupported !== counts.unsupported || actual.conflicting !== counts.conflicting || actual.notAssessed !== counts.notAssessed) throw new Error("Edited job summary counts are inconsistent with reviewed assessment decisions.");
  }
}

function validateForbiddenText(text: string): void {
  const checks = [
  ...
  if (checks.some((pattern) => pattern.test(text))) throw new Error("Reviewed assessment contains forbidden fit, application, resume, prediction, or definite experience-gap language.");
}
...
async function assertProposalReviewable(workspace: string, proposalId: string): Promise<void> {
  ...
  if (!status.readyForReview) throw new Error(`Only a current ready-for-review assessment proposal may be reviewed. Current status: ${status.status}`);
}

function assertMutable(review: FitAssessmentProposalReview): void {
  if (review.status === "completed") throw new Error("Completed assessment reviews cannot be modified.");
}

function assertExactCoverage(review: FitAssessmentProposalReview, proposalIds: string[]): void {
  const reviewed = review.expectationDecisions.map((entry) => entry.proposedAssessmentId);
  if (!sameSet(reviewed, proposalIds)) throw new Error("Review decisions do not exactly cover proposed expectation assessments.");
}
...
async function persistReview(workspace: string, review: FitAssessmentProposalReview, paths: ReturnType<typeof reviewPaths>): Promise<void> {
  const proposal = await showFitAssessmentProposal(workspace, review.proposalId);
  const manifest = FitAssessmentProposalManifestSchema.parse(await readJson<unknown>(proposalManifestPath(workspace, proposal.targetType, proposal.targetId, proposal.id), null));
...
function resolveWithin(workspace: string, relativePath: string): string {
  ...
  if (relation === ".." || relation.startsWith(`..${path.sep}`) || path.isAbsolute(relation)) throw new Error(`Assessment review path escapes workspace: ${relativePath}`);
  return resolved;
}
