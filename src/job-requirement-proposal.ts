...
import {
  ...
  JobRequirementProposalSchema,
  ModelJobRequirementPayloadSchema,
  type JobRequirementModel,
  type JobRequirementProposal,
  type JobRequirementProposalManifest,
  type ModelJobRequirementPayload,
  type ProposedJobRequirement,
...
export const JOB_REQUIREMENT_PROPOSAL_PROMPT_ID =
  "target-job-requirement-model-proposal";
...
export async function generateJobRequirementProposal(
  workspace: string,
  targetId: string,
  options: ProposalOptions = {},
): Promise<GenerateJobRequirementProposalResult> {
  const target = await showTarget(workspace, targetId);
  if (target.type !== "job") throw new Error(`Job requirement proposals reject Role Target: ${targetId}`);
  const modelStatus = await getJobRequirementModelStatus(workspace, targetId);
  if (modelStatus.status !== "current") {
    throw new Error(
      `A current deterministic job requirement model is required. Current status: ${modelStatus.status}.`,
    );
  }
  ...
  const promptTemplateId =
    options.promptTemplateId ?? JOB_REQUIREMENT_PROPOSAL_PROMPT_ID;
  const promptTemplateVersion =
    options.promptTemplateVersion ?? JOB_REQUIREMENT_PROPOSAL_PROMPT_VERSION;
  const policyVersion =
    options.policyVersion ?? JOB_REQUIREMENT_PROPOSAL_POLICY_VERSION;
  ...
  const proposalId = `job-requirements-proposal_${hashText(
    `${requestFingerprint}\u0000${now}\u0000${options.refresh ? "refresh" : "initial"}`,
  ).slice(0, 16)}`;
  ...
  const proposal = JobRequirementProposalSchema.parse({
    ...
    targetType: "job",
    status:
      parsed.validationIssues.length === 0 ? "ready-for-review" : "validation-failed",
    model: {
      ...provider.identity,
      settings: provider.settings,
    },
    ...
    warnings: [
      {
        ...
        sourceAnalysisItemIds: uniqueSorted(
          parsed.proposedRequirements.flatMap((entry) => entry.sourceAnalysisItemIds),
        ),
      },
    ],
...
export async function showJobRequirementProposal(
  workspace: string,
  proposalId: string,
): Promise<JobRequirementProposal> {
  const location = await locateProposal(workspace, proposalId);
  return JobRequirementProposalSchema.parse(
    await readJson<unknown>(location.proposalPath, null),
  );
}

export async function listJobRequirementProposals(
  workspace: string,
  targetId: string,
): Promise<JobRequirementProposal[]> {
  const target = await showTarget(workspace, targetId);
  ...
  const proposals = await Promise.all(files.map(async (file) =>
    JobRequirementProposalSchema.parse(await readJson<unknown>(file, null))));
  return proposals.sort((a, b) => b.createdAt.localeCompare(a.createdAt) || a.id.localeCompare(b.id));
}

export async function getJobRequirementProposalStatus(
  workspace: string,
  proposalId: string,
): Promise<JobRequirementProposalStatus> {
  const location = await locateProposal(workspace, proposalId);
  ...
  let proposal: JobRequirementProposal;
  let manifest: JobRequirementProposalManifest;
  try {
    proposal = JobRequirementProposalSchema.parse(
      await readJson<unknown>(location.proposalPath, null),
    );
    manifest = JobRequirementProposalManifestSchema.parse(
  ...
  const rawResponseHashMatches =
    (await hashFile(location.rawResponsePath)) === manifest.rawResponseSha256 &&
  ...
  if (!rawResponseHashMatches) invalidReasons.push("Raw response SHA-256 does not match.");
  if (
...
export async function replayJobRequirementProposal(
  workspace: string,
  proposalId: string,
...
export function formatJobRequirementProposalResult(
  result: GenerateJobRequirementProposalResult,
): string {
  return [
...
export function formatJobRequirementProposalList(proposals: JobRequirementProposal[]): string {
  if (proposals.length === 0) return "Job requirement proposals: none";
  return [
    "Job requirement proposals:",
    ...proposals.map((proposal) =>
      `${proposal.id} | ${proposal.status} | ${proposal.model.provider}/${proposal.model.model} | ${proposal.updatedAt}`),
  ].join("\n");
}

export function formatJobRequirementProposalStatus(status: JobRequirementProposalStatus): string {
...
function parseAndValidatePayload(
  ...
  proposedRequirements: ProposedJobRequirement[];
  validationIssues: Array<{ code: string; message: string; path?: string }>;
} {
  let payload: ModelJobRequirementPayload | undefined;
  const validationIssues: Array<{ code: string; message: string; path?: string }> = [];
  try {
  ...
  if (!payload) return { proposedRequirements: [], validationIssues };
  ...
  const proposedRequirements: ProposedJobRequirement[] = [];
  for (const [index, proposed] of payload.proposedRequirements.entries()) {
    ...
    if (
      proposed.sourceReferences.some((reference) =>
        !allowedReferences.some((allowed) => JSON.stringify(allowed) === JSON.stringify(reference)))
    ) {
...
function normalizeModelInput(model: JobRequirementModel): string {
...
function renderPrompt(model: JobRequirementModel, normalizedInput: string): string {
  return [
...
async function findCurrentCachedProposal(
  workspace: string,
  targetId: string,
  requestFingerprint: string,
): Promise<JobRequirementProposal | undefined> {
  const candidates = await listJobRequirementProposals(workspace, targetId);
  for (const candidate of candidates) {
    if (candidate.requestFingerprint !== requestFingerprint) continue;
    const status = await getJobRequirementProposalStatus(workspace, candidate.id);
    if (status.status === "current") return candidate;
  }
  return undefined;
}

export async function locateJobRequirementProposal(
  workspace: string,
  proposalId: string,
): Promise<ProposalPaths & { targetId: string }> {
...
async function locateProposal(
  ...
  if (!/^[a-z0-9_-]+$/.test(proposalId)) throw new Error(`Invalid proposal ID: ${proposalId}`);
  ...
  if (files.length === 0) throw new Error(`Job requirement proposal not found: ${proposalId}`);
  if (files.length > 1) throw new Error(`Job requirement proposal ID is ambiguous: ${proposalId}`);
  ...
  if (!match?.[1] || match[2] !== proposalId) throw new Error("Proposal path is invalid.");
  return { targetId: match[1], ...proposalPaths(workspace, match[1], proposalId) };
}

function proposalPaths(workspace: string, targetId: string, proposalId: string): ProposalPaths {
