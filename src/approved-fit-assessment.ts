...
export async function approveFitAssessmentProposal(
  workspace: string,
  proposalId: string,
  options: ApproveFitAssessmentOptions = {},
): Promise<ApproveFitAssessmentResult> {
  ...
  if (reviewStatus.status !== "completed") throw new Error(`Assessment proposal review must be completed before approval. Current status: ${reviewStatus.status}`);
  ...
  if (deterministicStatus.status !== "current") throw new Error(`Deterministic assessment must be current before approval. Current status: ${deterministicStatus.status}`);
  ...
  const proposalManifest = FitAssessmentProposalManifestSchema.parse(await readJson<unknown>(proposalManifestPath, null));
  const reviewManifest = FitAssessmentReviewManifestSchema.parse(await readJson<unknown>(reviewManifestPath, null));
  if (reviewManifest.proposalSha256 !== proposalManifest.proposalSha256) throw new Error("Assessment review does not reference the current proposal hash.");
  ...
  if (existingStatus.status === "current" && await pathExists(paths.manifestPath)) {
    const currentManifest = (await readJson<Record<string, unknown>>(paths.manifestPath, {}));
  ...
  const finalAssessments: ExpectationFitAssessment[] = review.expectationDecisions.map((decision) => {
    const proposed = proposedById.get(decision.proposedAssessmentId);
    if (!proposed) throw new Error(`Unknown reviewed proposed assessment: ${decision.proposedAssessmentId}`);
    const fallback = deterministicByExpectation.get(proposed.expectationId);
    if (!fallback) throw new Error(`Deterministic fallback is missing: ${proposed.expectationId}`);
    if (decision.decision === "reject") return fallback;
    if (decision.decision === "pending") throw new Error(`Pending assessment decision remains: ${decision.proposedAssessmentId}`);
    const content = decision.decision === "edit" ? decision.editedAssessment : proposed;
    if (!content) throw new Error(`Reviewed assessment content is missing: ${decision.proposedAssessmentId}`);
    return {
      ...fallback,
      ...
      trustState: decision.decision === "edit" ? "human-edited" as const : "human-approved" as const,
    };
  ...
  if (await pathExists(paths.assessmentPath)) {
    try { createdAt = TargetFitAssessmentSchema.parse(await readJson<unknown>(paths.assessmentPath, null)).createdAt; }
  ...
  const manifest = createAssessmentManifest(
    ...
    createdAt,
    now,
    { proposalId, proposalSha256: proposalManifest.proposalSha256, reviewSha256: reviewManifest.reviewSha256 },
  );
  await writeJsonAtomic(paths.manifestPath, manifest);
  return approvalResult(approved, review, paths, proposalId, existingStatus.status === "missing" ? "created" : "rebuilt");
}

function reviewedAssessmentContent(content: NonNullable<Awaited<ReturnType<typeof showFitAssessmentReview>>["expectationDecisions"][number]["editedAssessment"]> | Awaited<ReturnType<typeof showFitAssessmentProposal>>["proposedExpectationAssessments"][number]) {
  return {
...
export function formatApproveFitAssessmentResult(result: ApproveFitAssessmentResult): string {
  return [
...
function reviewedSummary(
  decision: Awaited<ReturnType<typeof showFitAssessmentReview>>["summaryDecision"],
  proposed: FitAssessmentSummary | undefined,
  deterministic: FitAssessmentSummary,
): FitAssessmentSummary {
  if (decision.decision === "reject") return deterministic;
  if (decision.decision === "accept") {
    if (!proposed) throw new Error("Accepted assessment proposal has no summary.");
    return proposed;
  }
  if (decision.decision === "edit") {
    if (!decision.editedSummary) throw new Error("Edited assessment summary is missing.");
...
function approvalResult(
  ...
  review: Awaited<ReturnType<typeof showFitAssessmentReview>>,
  paths: ReturnType<typeof assessmentPaths>,
  proposalId: string,
  result: ApproveFitAssessmentResult["result"],
): ApproveFitAssessmentResult {
...
function resolveWithin(workspace: string, relativePath: string): string {
  ...
  if (relation === ".." || relation.startsWith(`..${path.sep}`) || path.isAbsolute(relation)) throw new Error(`Approved assessment path escapes workspace: ${relativePath}`);
  return resolved;
}
