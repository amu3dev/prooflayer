...
import {
  VariantReviewDecisionsSchema,
  type CareerProfile,
  type Claim,
  type EvidenceItem,
  type PublicProfile,
  type VariantReviewDecision,
  type VariantReviewDecisionValue,
  type VariantReviewDecisions,
  type Visibility
} from "./schemas.js";
import {
  OUTPUT_MANIFEST_FILE,
  VARIANTS_ROOT,
  generalizeClaimForDraft,
...
const REVIEW_ROLE_KEYS = ["tpm", "ai-product"] as const satisfies readonly RoleKey[];
...
type ReviewContext = {
  ...
  review: VariantReviewDecisions;
  profile: CareerProfile;
  claims: Claim[];
  evidence: EvidenceItem[];
  evidenceById: Map<string, EvidenceItem>;
  publicProfile: PublicProfile | null;
  baseline: RefreshBaseline | null;
  reviewScopeIds: string[];
};

type PublicClaim = {
  claim: Claim;
  decision: VariantReviewDecision;
  wording: string;
};

type PublicSelection = {
  accepted: PublicClaim[];
  warnings: string[];
  publicProfile: PublicProfileUsage;
  missingSections: string[];
...
export async function initializeVariantReview(
  workspace: string,
  roleKeyInput: string,
  now = new Date()
): Promise<VariantReviewStatus> {
  ...
  if (existing && existing.roleKey !== roleKey) throw new Error(`Review file role mismatch: expected ${roleKey}.`);
  ...
  for (const claimId of additions) existingById.set(claimId, { claimId, decision: "pending" });
  ...
  const review: VariantReviewDecisions = {
    schemaVersion: 1,
    roleKey,
...
export async function getVariantReviewStatus(workspace: string, roleKeyInput: string): Promise<VariantReviewStatus> {
  const roleKey = reviewRoleKey(roleKeyInput);
  const root = variantRoot(roleKey);
  const reviewPath = path.join(workspace, root, REVIEW_DECISIONS_FILE);
  const manifest = await readJson<VariantGenerationManifest | null>(path.join(workspace, root, "generation-manifest.json"), null);
  ...
  const review = VariantReviewDecisionsSchema.parse(await readJson<unknown>(reviewPath, {}));
...
export function formatVariantReviewStatus(status: VariantReviewStatus): string {
  return [
...
export async function finalizeVariant(
  ...
  roleKeyInput: string,
  now = new Date()
): Promise<FinalVariantManifest> {
  const roleKey = reviewRoleKey(roleKeyInput);
  const root = variantRoot(roleKey);
  const reviewPath = path.join(workspace, root, REVIEW_DECISIONS_FILE);
  if (!(await pathExists(reviewPath))) throw new Error(`Run review variant --role ${roleKey} before finalization.`);
  const manifest = await readJson<VariantGenerationManifest | null>(path.join(workspace, root, "generation-manifest.json"), null);
  if (!manifest) throw new Error(`Generate the ${roleKey} draft before finalization.`);
  const review = VariantReviewDecisionsSchema.parse(await readJson<unknown>(reviewPath, {}));
  const baseline = await readJson<RefreshBaseline | null>(path.join(workspace, "kb/update-baseline.json"), null);
  ...
  const finalManifest: FinalVariantManifest = {
    schemaVersion: 1,
...
function reviewRoleKey(value: string): RoleKey {
  if (!isRoleKey(value) || !(REVIEW_ROLE_KEYS as readonly string[]).includes(value)) {
    throw new Error("Output-specific review supports --role tpm or --role ai-product.");
  }
  return value;
}

function variantRoot(roleKey: RoleKey): string {
...
export function buildReviewScopeIds(
  ...
  const includeUnresolved = (claimId: string): boolean => {
...
function selectTpmCompletenessClaimIds(claims: Claim[]): string[] {
  const explicitIds = new Set<string>(TPM_COMPLETENESS_CLAIM_IDS);
  return claims.filter((claim) =>
    explicitIds.has(claim.id)
    ...
    || (claim.sourceSection === "Summary" && ["responsibility_claim", "competency_claim", "leadership_claim"].includes(claim.type))
  ).map((claim) => claim.id);
}

function selectAiProductCompletenessClaimIds(claims: Claim[]): string[] {
...
function currentReviewDecisions(context: ReviewContext): VariantReviewDecision[] {
...
function buildPublicSelection(context: ReviewContext, decisions: VariantReviewDecision[]): PublicSelection {
  const claimById = new Map(context.claims.map((claim) => [claim.id, claim]));
  const variant = getRoleVariant(context.roleKey);
  const accepted: PublicClaim[] = [];
  const warnings: string[] = [];

  for (const decision of decisions) {
    if (decision.decision !== "approve" && decision.decision !== "revise") continue;
...
function publicProfileWordingOverride(claim: Claim, profile: PublicProfile | null): string | undefined {
  if (!profile) return undefined;
  if (claim.type === "education_claim") return profile.educationWordingOverrides?.[claim.id]?.trim();
  if (claim.type === "certification_claim") return profile.certificationWordingOverrides?.[claim.id]?.trim();
  return undefined;
}

function buildPublicProfileUsage(profile: PublicProfile | null, roleKey: RoleKey, accepted: PublicClaim[]): PublicProfileUsage {
  ...
  const missingWordingOverrideClaimIds = accepted.filter((item) => {
    if (item.claim.type !== "education_claim" && item.claim.type !== "certification_claim") return false;
    if (publicProfileWordingOverride(item.claim, profile)) return false;
    if (item.decision.approvedPublicWording?.trim() || item.claim.approvedWording?.trim()) return false;
  ...
  const contactFieldsUsed = ["location", "email", "website", "linkedin", "github"]
    .filter((field) => Boolean(profile?.[field as keyof PublicProfile]))
    .sort();
  const warnings = [
...
function publicWordingForApprovedClaim(
  claim: Claim,
  evidence: EvidenceItem[],
  variant: ReturnType<typeof getRoleVariant>,
  visibilities: Set<Visibility>
): string {
  if (claim.approvedWording) return claim.approvedWording;
  if (claim.outputReadiness === "generic_only" || claim.outputReadiness === "internal_only" || visibilities.has("generic_only")) {
    if (variant.roleKey === "ai-product") {
      ...
      if (aiProductWording) return aiProductWording;
    }
...
function aiProductFoundationWording(claim: Claim): string | undefined {
...
function publicCoverage(profile: CareerProfile, accepted: PublicClaim[]): { roles: number; projects: number; skills: number; education: number } {
  return {
...
function renderFinalResume(context: ReviewContext, selection: PublicSelection, counts: ReviewDecisionCounts): string {
  const variant = getRoleVariant(context.roleKey);
...
function renderFinalWebsite(context: ReviewContext, selection: PublicSelection, counts: ReviewDecisionCounts): string {
  const variant = getRoleVariant(context.roleKey);
  const publicName = context.publicProfile?.publicName ?? "[Name]";
  const headline = publicHeadlineOverride(context.publicProfile, context.roleKey) ?? variant.headline;
  ...
  const general = selectFinalGeneralClaims(
    context.roleKey,
    publicClaims.filter((item) => !projectClaimIds.has(item.claim.id) && !roleClaimIds.has(item.claim.id))
  );
...
function renderPublicContactItems(profile: PublicProfile | null): string[] {
  if (!profile) return [];
  return [
    profile.location,
...
function publicHeadlineOverride(profile: PublicProfile | null, roleKey: RoleKey): string | undefined {
  return profile?.headlineOverrides?.[roleKey]?.trim() ?? profile?.headlineOverride?.trim();
}

function groupProjectClaims(
  profile: CareerProfile,
  accepted: PublicClaim[],
  roleKey: RoleKey
): FinalProjectGroup[] {
  return profile.projects.map((project) => {
    ...
    return {
      name: project.name,
      claims: rankProjectClaimsForFinal(
        roleKey,
        project.name,
        matching.filter((item) => !metadataIds.has(item.claim.id))
      ).slice(0, 3),
      roleAndTimeline,
      status
    };
...
function aiProductProjectRoleAndTimeline(claims: PublicClaim[]): string | undefined {
...
function aiProductProjectStatus(projectName: string, claims: PublicClaim[]): string | undefined {
...
function isProjectRoleClaim(item: PublicClaim): boolean {
  return /\bproduct and technical lead\b/i.test(`${item.claim.claim} ${item.wording}`);
}

function isProjectStatusClaim(item: PublicClaim): boolean {
  return /\bproduct experiment\b/i.test(`${item.claim.claim} ${item.wording}`);
}

function rankProjectClaimsForFinal(roleKey: RoleKey, projectName: string, claims: PublicClaim[]): PublicClaim[] {
  if (roleKey !== "ai-product") return claims;
  return [...claims].sort((a, b) =>
    aiProductProjectPriority(projectName, b) - aiProductProjectPriority(projectName, a)
    || a.claim.id.localeCompare(b.claim.id)
  );
}

function aiProductProjectPriority(projectName: string, item: PublicClaim): number {
  const project = projectName.toLowerCase();
  const text = `${item.claim.claim} ${item.wording}`.toLowerCase();
  if (project.includes("signalboard")) {
    if (/market-signal intelligence|signal extraction/.test(text)) return 1000;
    if (/action-?provenance/.test(text)) return 950;
    if (/evaluation scenarios?/.test(text) && /briefings?|source signals?/.test(text)) return 900;
    if (/evidence-backed insights?|traceability|decision-support/.test(text)) return 850;
    if (/app(?:-layer)? experience/.test(text) && /backend\/data configuration/.test(text)) return 800;
    if (/market and competitive intelligence product experiment/.test(text)) return 750;
    if (/problem framing/.test(text) && /experiment design/.test(text)) return 700;
  }
  if (project.includes("insightarleans")) {
    if (/computer-vision prototype/.test(text) && /object detection/.test(text)) return 1000;
    if (/iterative testing/.test(text) && /qualitative feedback/.test(text)) return 950;
    if (/camera flows?/.test(text) && /overlay ux/.test(text)) return 900;
  }
  return 0;
}

function selectFinalGeneralClaims(roleKey: RoleKey, claims: PublicClaim[]): PublicClaim[] {
  if (roleKey !== "ai-product") return claims;
  const ranked = [...claims]
...
function isRepetitiveAiProductGeneralClaim(item: PublicClaim): boolean {
  const text = `${item.claim.claim} ${item.wording}`.toLowerCase();
...
function aiProductGeneralPriority(item: PublicClaim): number {
  const text = `${item.claim.claim} ${item.wording}`.toLowerCase();
  if (/ai product manager and product-minded technology leader/.test(text)) return 1000;
  if (/built ai-assisted product workflows, prototypes, and technical evaluation loops/.test(text)) return 950;
  if (/app(?:-layer)? experience/.test(text) && /backend\/data configuration/.test(text)) return 900;
  if (/evidence-backed insights?/.test(text) && /traceability/.test(text)) return 850;
  if (/evidence-backed decision support/.test(text)) return 825;
  if (/currently building and validating ai-assisted product workflows/.test(text)) return 800;
  if (/problem framing/.test(text) && /roadmap definition/.test(text)) return 750;
  return 0;
}

function aiProductGeneralTheme(item: PublicClaim): string | undefined {
  const text = `${item.claim.claim} ${item.wording}`.toLowerCase();
  if (/evidence-backed/.test(text) && /(?:decision support|decision-support|traceability)/.test(text)) return "evidence-backed-decision-support";
  return undefined;
}

function groupRoleClaims(
  profile: CareerProfile,
  accepted: PublicClaim[],
  roleKey: RoleKey
): Array<{ title?: string; company?: string; dateRange?: string; claims: PublicClaim[] }> {
  ...
  if (roleKey !== "ai-product") return groups;
  const seen = new Set<string>();
...
function claimMatchesEvidence(claim: Claim, evidenceIds: string[]): boolean {
...
function renderProjectGroup(group: FinalProjectGroup): string {
  const metadata = [
...
function renderWebsiteProjectGroup(group: FinalProjectGroup, limit = 2): string {
  const metadata = [
    group.roleAndTimeline ? `**Role and timeline:** ${group.roleAndTimeline}` : undefined,
    group.status ? `**Status:** ${group.status}` : undefined
  ].filter((item): item is string => Boolean(item));
...
function renderRoleGroup(group: { title?: string; company?: string; dateRange?: string; claims: PublicClaim[] }): string {
...
function renderWebsiteRoleGroup(group: { title?: string; company?: string; dateRange?: string; claims: PublicClaim[] }): string {
...
function uniquePublicClaimsByWording(claims: PublicClaim[]): PublicClaim[] {
  const seen = new Set<string>();
...
function normalizedPublicWording(wording: string): string {
  return wording.normalize("NFKC").toLowerCase().replace(/\s+/g, " ").trim();
}

function emptyDecisionCounts(): ReviewDecisionCounts {
  return { pending: 0, approve: 0, revise: 0, draft_only: 0, exclude: 0 };
}

function countDecisions(decisions: VariantReviewDecision[]): ReviewDecisionCounts {
  const counts = emptyDecisionCounts();
  for (const decision of decisions) counts[decision.decision] += 1;
  return counts;
}

async function registerFinalOutput(workspace: string, manifest: FinalVariantManifest): Promise<void> {
  const outputManifestPath = path.join(workspace, OUTPUT_MANIFEST_FILE);
  const registry = await readJson<OutputManifest>(outputManifestPath, {
    schemaVersion: 1,
  ...
  await writeJsonAtomic(outputManifestPath, { schemaVersion: 1, updatedAt: manifest.finalizedAt, outputs } satisfies OutputManifest);
}
