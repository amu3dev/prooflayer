...
import {
  JobResumeDraftScaffoldManifestSchema,
  JobResumeDraftScaffoldSchema,
  type JobResumeDraftScaffold,
  type JobResumeDraftScaffoldManifest,
  type JobResumeDraftScaffoldSection,
...
import {
  ...
  loadJobResumePlanningContext,
  showJobResumePlan,
  type JobResumePlanningContext,
...
export async function loadJobResumeDraftingContext(
  workspace: string,
  targetId: string,
): Promise<JobResumeDraftingContext> {
  ...
  const manifest = JobResumeContentPlanManifestSchema.parse(
    await readJson<unknown>(resolveWithin(workspace, contentPlanManifestPath), null),
  );
  ...
  if (
    contentPlan.input.requirementModel.sha256 !== base.requirementInput.modelSha256 ||
...
export async function buildJobResumeDraftScaffold(
  workspace: string,
  targetId: string,
  options: BuildJobResumeDraftScaffoldOptions = {},
): Promise<BuildJobResumeDraftScaffoldResult> {
  ...
  if (await pathExists(paths.scaffoldPath)) {
    try {
      createdAt = JobResumeDraftScaffoldSchema.parse(
        await readJson<unknown>(paths.scaffoldPath, null),
      ).createdAt;
    } catch {
  ...
  const manifest = createJobResumeDraftScaffoldManifest(
    scaffold,
    context,
    paths.scaffoldRelativePath,
    await hashFile(paths.scaffoldPath),
    createdAt,
    now,
  );
...
export function deriveJobResumeDraftScaffold(
  context: JobResumeDraftingContext,
  createdAt: string,
  updatedAt: string,
): JobResumeDraftScaffold {
  ...
  const sections = plan.sections
    ...
    .map<JobResumeDraftScaffoldSection>((section) => {
      ...
      const metricPermissions = plan.metricPermissions.filter((entry) =>
        entry.allowedSections.includes(section.type) &&
      ...
      return {
        ...
        placeholderIds: section.inclusion === "exclude"
          ...
          : Array.from({ length: maximumItemCount }, (_, index) =>
            `job-resume-draft-slot_${hashText([scaffoldId, section.id, String(index)].join("\0")).slice(0, 16)}`),
      };
    });
...
export async function showJobResumeDraftScaffold(
  workspace: string,
  targetId: string,
): Promise<JobResumeDraftScaffold> {
  const paths = jobResumeDraftScaffoldPaths(workspace, targetId);
  if (!(await pathExists(paths.scaffoldPath))) {
    throw new Error(`Job resume draft scaffold not found: ${targetId}`);
  }
  return JobResumeDraftScaffoldSchema.parse(await readJson<unknown>(paths.scaffoldPath, null));
}

export async function getJobResumeDraftScaffoldStatus(
  workspace: string,
  targetId: string,
): Promise<JobResumeDraftScaffoldStatus> {
  const paths = jobResumeDraftScaffoldPaths(workspace, targetId);
  ...
  let scaffold: JobResumeDraftScaffold;
  let manifest: JobResumeDraftScaffoldManifest;
  try {
    scaffold = JobResumeDraftScaffoldSchema.parse(await readJson<unknown>(paths.scaffoldPath, null));
    manifest = JobResumeDraftScaffoldManifestSchema.parse(await readJson<unknown>(paths.manifestPath, null));
  } catch (error) {
  ...
  let context: JobResumeDraftingContext;
  try {
...
export function assertJobResumeDraftScaffoldConsistency(
  ...
  if (
    scaffold.draftingPolicy.name !== JOB_RESUME_DRAFTING_POLICY_NAME ||
  ...
  for (let index = 0; index < planned.length; index += 1) {
    ...
    if (
      section.planSectionId !== source.id ||
...
export function deterministicJobResumeDraftScaffoldId(context: JobResumeDraftingContext): string {
...
export function jobResumeDraftScaffoldPaths(workspace: string, targetId: string) {
...
export function createJobResumeDraftScaffoldManifest(
  scaffold: JobResumeDraftScaffold,
  context: JobResumeDraftingContext,
  scaffoldPath: string,
  ...
  createdAt: string,
  updatedAt: string,
): JobResumeDraftScaffoldManifest {
...
export function formatBuildJobResumeDraftScaffoldResult(result: BuildJobResumeDraftScaffoldResult): string {
  return [
...
export function formatJobResumeDraftScaffoldStatus(status: JobResumeDraftScaffoldStatus): string {
  return [
...
function scaffoldProvenance(context: JobResumeDraftingContext) {
  return {
...
function constraint(
  ...
  code: string,
  description: string,
  sections: JobResumeDraftScaffoldSection[],
) {
  return {
...
function defaultMaximumItems(type: JobResumeSectionType): number {
  return ({
    headline: 1,
...
function maximumSentences(type: JobResumeSectionType): number | undefined {
...
function scaffoldResult(
  scaffold: JobResumeDraftScaffold,
  paths: ReturnType<typeof jobResumeDraftScaffoldPaths>,
  result: BuildJobResumeDraftScaffoldResult["result"],
): BuildJobResumeDraftScaffoldResult {
  return {
    targetId: scaffold.targetId,
    result,
    scaffoldId: scaffold.id,
...
function emptyStatus(
  base: Pick<JobResumeDraftScaffoldStatus, "targetId" | "scaffoldExists" | "manifestExists" | "scaffoldPath" | "manifestPath">,
  status: JobResumeDraftScaffoldStatus["status"],
  reasons: string[],
): JobResumeDraftScaffoldStatus {
  return {
    ...base,
