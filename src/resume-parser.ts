...
const SECTION_BY_KEY = new Map(
  RESUME_SECTION_NAMES.map((name) => [normalizeHeading(name), name])
);
...
export function isMarkdownResume(text: string): boolean {
...
export function isDateLocationLine(value: string): boolean {
...
export function parseMarkdownResume(text: string): ParsedResume {
  ...
  let section: ResumeSectionName | undefined;
  let entry: ParsedResumeEntry | undefined;
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (headingMatch) {
      ...
      if (level === 2) {
        ...
        if (!matchedSection) parsed.warnings.push(`Unrecognized resume section: ${heading}`);
        continue;
      }
    ...
    if (section === "Summary" || section === "Career Through-Line") {
      ...
      else parsed.ignoredFragments += 1;
      continue;
    }

    if (section === "Product & Technical Strengths") {
      const value = stripListMarker(raw);
      if (isUsableNarrative(value)) parsed.strengthLines.push(value);
      else parsed.ignoredFragments += 1;
      continue;
    }
    ...
    if (section === "Technical Fluency") {
      const value = stripListMarker(raw);
      if (isUsableNarrative(value)) parsed.skillLines.push(value);
      else parsed.ignoredFragments += 1;
      continue;
    }
    ...
    if (section === "Education & Certifications") {
      const value = stripListMarker(raw);
      if (isUsableEducation(value)) parsed.educationLines.push(value);
      else parsed.ignoredFragments += 1;
      continue;
    }
    ...
    if (section === "Additional Information") {
      const value = stripListMarker(raw);
      if (isUsableNarrative(value)) parsed.additionalLines.push(value);
      else parsed.ignoredFragments += 1;
      continue;
    }

    if (!entry) {
      if (!isNarrativeIntro(raw)) parsed.ignoredFragments += 1;
      continue;
    }
    ...
    if (boldLine) {
      ...
      if (section === "Enterprise Engineering Foundation") {
        ...
        if (section === "Professional Experience") entry.company = entry.name;
      }
      if (boldLine[2]) applyDateAndLocation(entry, boldLine[2]);
      continue;
    }
    ...
    if (/^[-*]\s+/.test(raw)) {
      ...
      if (isUsableNarrative(bullet)) entry.bullets.push(bullet);
      else parsed.ignoredFragments += 1;
      continue;
    }
  ...
  parsed.projects = parsed.projects.filter((project) => {
    ...
    if (!valid) parsed.ignoredFragments += 1;
    return valid;
  });
...
function applyDateAndLocation(entry: ParsedResumeEntry, value: string): void {
...
function isUsableEducation(value: string): boolean {
...
function isNarrativeIntro(value: string): boolean {
  const cleaned = stripInlineMarkdown(value);
