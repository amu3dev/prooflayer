...
import {
  ...
  RoleResumeRenderDocumentManifestSchema,
  RoleResumeRenderDocumentSchema,
  RoleResumeRenderProfileNameSchema,
  RoleResumeRenderProfileSchema,
  type RoleResumeDateFormat,
  type RoleResumePageSize,
  type RoleResumeRenderBlock,
  type RoleResumeRenderDocument,
  type RoleResumeRenderProfile,
  type RoleResumeRenderProfileName,
  type RoleResumeRenderSection,
  type RoleResumeRenderSourceMapEntry,
} from "./role-resume-render-schemas.js";
import type { RoleResumeSectionType } from "./role-resume-plan-schemas.js";
...
const SECTION_HEADINGS: Record<RoleResumeSectionType, string | null> = {
  headline: null,
...
const ITEM_BLOCK_TYPES: Record<RoleResumeDraftItem["itemType"], RoleResumeRenderBlock["type"]> = {
  identity: "headline",
  contact: "paragraph",
  headline: "paragraph",
  summary: "paragraph",
...
export function normalizeRoleResumeRenderOptions(options: RoleResumeRenderOptions = {}): NormalizedRoleResumeRenderOptions {
  return {
...
export function resolveRoleResumeRenderProfile(
  profileName: RoleResumeRenderProfileName,
  pageSize: RoleResumePageSize,
): RoleResumeRenderProfile {
...
export async function composeRoleResumeRenderDocument(
  workspace: string,
  targetId: string,
  options: RoleResumeRenderOptions = {},
): Promise<ComposeRoleResumeResult> {
  ...
  if (await pathExists(paths.documentPath)) {
    try {
      createdAt = RoleResumeRenderDocumentSchema.parse(await readJson<unknown>(paths.documentPath, null)).createdAt;
    } catch {
  ...
  const sourceMap = buildRenderSourceMap(
    context.approvedDraft,
    context.approvedDraftSha256,
    documentId,
    sections,
  );
...
export async function showRoleResumeRenderDocument(
  workspace: string,
  targetId: string,
): Promise<RoleResumeRenderDocument> {
  const paths = roleResumeRenderDocumentPaths(workspace, targetId);
  if (!(await pathExists(paths.documentPath))) throw new Error(`Canonical role resume render document not found: ${targetId}`);
  return RoleResumeRenderDocumentSchema.parse(await readJson<unknown>(paths.documentPath, null));
}

export async function getRoleResumeRenderDocumentStatus(
  workspace: string,
  targetId: string,
  requestedOptions?: NormalizedRoleResumeRenderOptions,
): Promise<RoleResumeRenderDocumentStatus> {
  const paths = roleResumeRenderDocumentPaths(workspace, targetId);
  ...
  if (!documentExists && !manifestExists) return emptyRenderStatus(base, "missing", ["Canonical render document is missing."]);
  if (!documentExists || !manifestExists) return emptyRenderStatus(base, "invalid", ["Canonical render artifact set is incomplete."]);
  let document: RoleResumeRenderDocument;
  let manifest: ReturnType<typeof RoleResumeRenderDocumentManifestSchema.parse>;
  try {
    document = RoleResumeRenderDocumentSchema.parse(await readJson<unknown>(paths.documentPath, null));
    manifest = RoleResumeRenderDocumentManifestSchema.parse(await readJson<unknown>(paths.manifestPath, null));
  } catch (error) {
...
export function roleResumeRenderDocumentPaths(workspace: string, targetId: string) {
...
export function formatComposeRoleResumeResult(result: ComposeRoleResumeResult): string {
  return [
...
export function formatRoleResumeRenderDocumentStatus(status: RoleResumeRenderDocumentStatus): string {
  return [
...
function assertApprovedTrustState(
  trustState: RoleResumeDraftItem["trustState"],
  itemId: string,
): asserts trustState is "deterministic-approved" | "human-approved" | "human-edited" {
...
function buildRenderSourceMap(
  draft: ApprovedRoleResumeDraft,
  approvedDraftSha256: string,
  documentId: string,
  sections: RoleResumeRenderSection[],
): RoleResumeRenderSourceMapEntry[] {
  ...
  return sections.flatMap((section) => section.blocks.map((block) => {
    ...
    if (!item) throw new Error(`Render block references unknown approved draft item: ${block.draftItemId}`);
    return {
...
function validateCanonicalComposition(
  draft: ApprovedRoleResumeDraft,
  sections: RoleResumeRenderSection[],
  sourceMap: RoleResumeRenderSourceMapEntry[],
  warnings: ReturnType<typeof renderNotice>[],
  ambiguities: Array<{
    id: string;
    code: string;
    message: string;
  ...
  const addRisk = (code: string, message: string, sectionIds: string[] = [], draftItemIds: string[] = []) => {
    risks.push({
  ...
  if (!exactTextPreserved) addRisk("VISIBLE_TEXT_MISMATCH", "Canonical blocks do not preserve exact approved item text.");
  const sectionOrderPreserved = expectedSections.every((section, index) =>
    sections[index]?.draftSectionId === section.id && sections[index]?.order === section.order);
  if (!sectionOrderPreserved) addRisk("SECTION_ORDER_MISMATCH", "Canonical section order differs from the approved draft.");
  const itemOrderPreserved = expectedItems.every((item, index) => renderedItems[index]?.draftItemId === item.id);
  if (!itemOrderPreserved) addRisk("ITEM_ORDER_MISMATCH", "Canonical item order differs from the approved draft.");
  const sourceMapComplete = sourceMap.length === renderedItems.length
    && renderedItems.every((block) => sourceMap.some((entry) =>
      entry.documentBlockId === block.id
      && entry.draftItemId === block.draftItemId
      && entry.statementTextSha256 === hashText(block.text)));
  if (!sourceMapComplete) addRisk("SOURCE_MAP_INCOMPLETE", "One or more visible blocks lack exact source-map provenance.");
  ...
  if (!privateMetadataAbsent) addRisk("PRIVATE_METADATA_EXPOSED", "Visible canonical text exposes internal paths, IDs, or hashes.");
  return {
    status: risks.length ? "invalid" as const : "valid" as const,
    exactTextPreserved,
...
async function loadRoleResumeRenderContext(workspace: string, targetId: string) {
  const target = await showTarget(workspace, targetId);
  if (target.type !== "role") throw new Error("Role resume rendering supports Role Targets only.");
  ...
  if (!status.usableForRendering) throw new Error("Approved Role Resume Draft is not complete or usable for rendering.");
...
function pageBreakRule(type: RoleResumeRenderBlock["type"]) {
...
function renderNotice(documentId: string, code: string, message: string) {
  return {
...
function composeResult(
  ...
  paths: ReturnType<typeof roleResumeRenderDocumentPaths>,
  result: ComposeRoleResumeResult["result"],
): ComposeRoleResumeResult {
  return {
...
function emptyRenderStatus(
  base: Pick<RoleResumeRenderDocumentStatus, "targetId" | "documentExists" | "manifestExists" | "documentPath" | "manifestPath">,
  status: RoleResumeRenderDocumentStatus["status"],
  reasons: string[],
): RoleResumeRenderDocumentStatus {
  return {
    ...base,
...
function resolveWithin(workspace: string, relativePath: string) {
  ...
  if (absolute !== root && !absolute.startsWith(`${root}${path.sep}`)) throw new Error("Path escapes the ProofLayer workspace.");
  return absolute;
}
